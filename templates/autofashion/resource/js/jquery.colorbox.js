...
(function($, document, window) {
    ...
    function launch(element) {
        ...
        if (!closing) {
            ...
            if (!open) {
                ...
                settings.w = (maxWidth !== false
                    ? Math.min(initialWidth, setSize(maxWidth, 'x'))
                    : initialWidth) - loadedWidth - interfaceWidth;
                settings.h = (maxHeight !== false
                    ? Math.min(initialHeight, setSize(maxHeight, 'y'))
                    : initialHeight) - loadedHeight - interfaceHeight;
            ...
            if (settings.get('closeButton')) {
                $close.html('<i class="fa fa-times"></i>').attr('title', settings.get('close')).appendTo($wrap);
            } else {
    ...
    publicMethod.prep = function(object) {
        ...
        var callback,
            speed = settings.get('transition') === "none"
            ? 0
            : settings.get('speed');
        ...
        function getWidth() {
            ...
            if (settings.get('minWidth') && settings.w < settings.get('minWidth'))
                settings.w = settings.get('minWidth');
            return settings.w;
        }
        function getHeight() {
            ...
            if (settings.get('minHeight') && settings.h < settings.get('minHeight'))
                settings.w = settings.get('minHeight');
            return settings.h;
        }
