    ...
    cart: function(method) {
        ...
        var methods = {
            ...
            add: function(url, offer, multioffers, concomitants, amount, noShowCartDialog) {
                ...
                if (multioffers)
                    params = $.extend(params, multioffers); // Многомерных комплектаций
                if (concomitants)
                    params = $.extend(params, concomitants);  // Сопутствующие товары
            ...
            showCart: function(url, params) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        $('<div id="' + data.options.popupCartId + '" />').hide().append(response.html).appendTo('.cartBlock.fixedCart').fadeIn();
        ...
        var addToCart = function() {
            ...
            var concomitantValues = (concomitants) ? concomitants
                : false;
            ...
            initCart = function(serverData) {
                ...
                if ($cartBlock.length) {
                    ...
                    $('html, body').on('click.cart', function() {
                        closeDlg();
                    });
