...
<div class="container">
    <div class="product{if !$product->isAvailable()} notAvaliable{/if}{if $product->canBeReserved()} canBeReserved{/if}{if $product.reservation == 'forced'} forcedReserve{/if}" data-id="{$product.id}">
        ...
        <div class="row prodnav">
            <div class="col-md-24">
                {if $product->getSibling('prev', $back_url) !== false}
                    <a href="{$product->getSibling('prev', $back_url)}" class="siblings prevProd"><i class="fa fa-angle-left"></i>Предыдущий</a>
                    {if $product->getSibling('next', $back_url) !== false}
                        |
                    {/if}
                {/if}
                {if $product->getSibling('next', $back_url) !== false}
                    <a href="{$product->getSibling('next', $back_url)}" class="siblings nextProd">Следующий<i class="fa fa-angle-right"></i></a>
                {/if}
                    ...
                    <div class="wrap">
                        <ul>
                            {foreach $images as $key => $image}
                                <li data-id="{$image.id}" class="{if $offer_images && !in_array($image.id, $offer_images)}hidden {elseif !$first++}first active{/if}"><a href="{$image->getUrl(800,600,'xy')}" class="preview" data-n="{$key}"><img src="{$image->getUrl(90,130)}" alt="Фотография товара"></a></li>
                            {/foreach}
                        </ul>
                    </div>
                    {if count($images)>8}
            ...
            {if count($images)>1}
                <div class="gallery col-md-6" style="display: none;">
                    <div class="wrap">
                        <ul>
                            {foreach $images as $key => $image}
                                <li data-id="{$image.id}" class="{if $offer_images && !in_array($image.id, $offer_images)}hidden{elseif !$first++} first active{/if}"><a href="{$image->getUrl(800,600,'xy')}" class="preview" data-n="{$key}"><img src="{$image->getUrl(50, 75)}" alt=""></a></li>
                            {/foreach}
                        </ul>
                    </div>
                    {if count($images)>8}
                ...
                {if $shop_config}
                    <div class="addToCartWrap">
                        <a data-href="{$router->getUrl('shop-front-cartpage', ["add" => $product.id])}" class="button addToCart" data-add-text="Добавлено" title="Добавить в корзину">
                            <img src="{$THEME_IMG}/cart.png" alt="Add to Cart" width="32" height="32">
                ...
                {if !$shop_config || (!$product->shouldReserve() && (!$check_quantity || $product.num>0))}
                    {if $catalog_config.buyinoneclick }
                        <div class="buyOneClickWrap">
                            <a data-href="{$router->getUrl('catalog-front-oneclick',["product_id"=>$product.id])}" title="Купить в 1 клик" class="button buyOneClick inDialog">Быстрая покупка</a>
                        </div>
                    {/if}
                {/if}
                <div class="wishWrap">
                        ...
                        <!-- <noindex> -->
                            {foreach $materials.value as $material}
                                <div class="materialItem col-md-6">
                                    <div class="title">
                                        <span title="{$material}" {* data-content="{$materialsdata.materials[$material].shortdescription}" *}>
                            ...
                            <!-- <noindex> -->
                                {foreach $options.value as $option}
                                    <div class="optionItem col-md-6">
                                        <button tabindex="0" role="button" data-placement="top" data-html="true" data-toggle="popover" data-trigger="focus" data-content="<img src='{$optionsdata.optlists[$option]->__icon->getLink()}' align='left' width='85' height='85' style='margin-right: 15px; margin-bottom: 11px;'>{$optionsdata.optlists[$option].shortdescription}" title="{$option}" >
...
{*
									<div class="optionItem col-md-6">
                                        <a tabindex="0" role="button" data-toggle="popover" data-trigger="focus" title="{$option}" data-content="<img src='{$optionsdata.optlists[$option]->__icon->getLink()}' align='left' style='width: 85px; margin-right: 15px; margin-bottom: 11px;'>{$optionsdata.optlists[$option].shortdescription}">
                                            <i class="fa fa-check-square-o"></i>
                                            {$option}
                                        </a>
                                    </div>
...
<div class="container taWrap">
    <div class="row">
        ...
        <div class="col-md-8">
            <a href="{$router->getUrl('menu.item_46')}" data-toggle="tooltip" title="Если не понравился товар - вернуть легко!" target="_blank" rel="nofollow"><img src="{$THEME_IMG}/product-change.png" alt="Если не понравился товар - вернуть легко!" height="85" width="85">Возврат товара</a>
        </div>
    </div>
</div>
