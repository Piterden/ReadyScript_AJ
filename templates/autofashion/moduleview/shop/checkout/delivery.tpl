    ...
    <ul class="vertItems">
        ...
        <li class="deliveryItem row{if $item@first} first{/if}{if $something_wrong} collapse{/if}" id="delivery_{$item.id}" data-delivery-id="{$item.id}" role="tabpanel">
            <div class="radio col-sm-8">
                ...
                {/if}
                <input type="radio" name="delivery" value="{$item.id}" id="dlv_{$item.id}" {if ($order.use_addr<=0 && $item.id==7) || ($order.use_addr>0 && $item.id==3)} checked="checked"{/if}{if $something_wrong} disabled="disabled"{/if}>
            ...
            <div class="info col-sm-10">
                ...
                {if ($delivery_type->getOption('address_type') * 1)
                < 0 && !$something_wrong} <div class="addressBlock">
                    <div class="h4">Выберите пункт выдачи заказа</div>
                    <div class="sdInfoWrap">
                        {if $order->getExtraKeyPair('update') == true}
                        <span class="text-capitalize">{$order->getExtraKeyPair('region_id_to')|lower}</span>
                        <br> {$order->getExtraKeyPair('sd_info')|replace:'&lt;':'<'|replace:'&gt;':'>'|replace:'&quot;':'"'} {/if}
                    </div>
            </div>
            ...
            <div class="priceWrap col-sm-3">
                <div id="scost_{$item.id}" class="price">
                    {if $something_wrong}
                    <span class="text-danger">{$something_wrong}</span> {else}
                    <span class="cost-info text-muted">{$order->getDeliveryExtraText($item)}</span>
                    <span class="total-cost text-success">{$item->getDeliveryCost($order)}</span>
