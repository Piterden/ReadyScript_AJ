...
<form method="POST" class="formStyle checkoutBox">
    ...
    {if $this_controller->getModuleConfig()->require_license_agree}
        <div class="col-xs-24 text-center">
            <input type="checkbox" name="iagree" value="1" id="iagree"> <label for="iagree">{t}Я согласен с <a href="{$router->getUrl('menu.item_37')}" class="licAgreement" target="_blank">условиями предоставления услуг</a>{/t}</label>
        </div>
        <script type="text/javascript">
            $(function() {
                $('.formSave').click(function() {
                    if (!$('#iagree').prop('checked')) {
                    	showError('Подтвердите согласие с условиями предоставления услуг');
                        return false;
                    }
                });
            });
