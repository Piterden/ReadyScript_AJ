    ...
    if (!PIE) {
                ...
                pixels: function(el, pct100) {
                    ...
                    switch (unit) {
                        case "px":
                            return num;
                        case "%":
                            return num * (typeof pct100 === 'function'
                                ? pct100() : pct100) / 100;
                        case "em":
            ...
            Angle.prototype = {
                ...
                degrees: function() {
                    ...
                    if (deg === undefined) {
                        ...
                        deg = this._deg = (u === 'deg' ? n : u === 'rad'
                            ? n / Math.PI * 180 : u === 'grad' ? n / 400 * 360
                            : u === 'turn' ? n * 360 : 0);
                    }
                    return deg;
                }
        ...
        PIE.Color = (function() {
            ...
            Color.prototype = {
                ...
                colorValue: function(el) {
                    this.parse();
                    return this._color === 'currentColor'
                        ? el.currentStyle.color : this._color;
                },
                /**
                    ...
                    if (match = css.match(this.ident)) {
                        ...
                        if (css.charAt(val.length) === '(') {
                            ...
                            if (val.toLowerCase() in this.colorFunctions) {
                                ...
                                if ((val.charAt(0) === 'r' ? isNumOrPct(next())
                                    : isNum(next())) &&
                                    isValue(next(), ',') &&
                                    isNumOrPct(next()) &&
                                    isValue(next(), ',') &&
        ...
        (function() {
            ...
            function cacheWhenLocked(fn) {
                ...
                return function() {
                    if (this._locked) {
                        ...
                        return (uid in cache) ? cache[ uid ]
                            : (cache[ uid ] = fn.call(this));
                    } else {
                        return fn.call(this);
                    }
                }
            ...
            PIE.StyleInfoBase = {
                ...
                getProps: function() {
                    ...
                    return css ? (css in cache ? cache[ css ]
                        : (cache[ css ] = this.parseCss(css))) : null;
                },
                /**
            ...
            parseCss: function(css) {
                ...
                this.withActualBorder(function() {
                    ...
                    for (; i < 4; i++) {
                        ...
                        width = w[ ltr ] = PIE.getLength(s[ ltr ] === 'none'
                            ? '0' : (this.namedWidths[ width ] || width));
            ...
            parseCss: function(css) {
                ...
                if (css) {
                    ...
                    p.slice = distributeSides(slices, function(tok) {
                        return PIE.getLength((tok.tokenType & NUMBER)
                            ? tok.tokenValue + 'px' : tok.tokenValue);
                    });

                    if (widths && widths[0]) {
                        p.widths = distributeSides(widths, function(tok) {
                            return tok.isLengthOrPercent()
                                ? PIE.getLength(tok.tokenValue)
                                : tok.tokenValue;
                        });
                    }
        ...
        PIE.BoxShadowStyleInfo = PIE.StyleInfoBase.newStyleInfo({
            ...
            parseCss: function(css) {
                    ...
                    function parseItem() {
                        ...
                        if (len > 1 && len < 5) {
                            (inset ? props.inset : props.outset).push({
                                ...
                                blur: getLength(lengths[2]
                                    ? lengths[2].tokenValue : '0'),
                                spread: getLength(lengths[3]
                                    ? lengths[3].tokenValue : '0'),
                ...
                return props && (props.inset.length || props.outset.length)
                    ? props : null;
            }
        });
        /**
        ...
        PIE.RendererBase = {
            ...
            addLayer: function(index, el) {
                ...
                for (var layers = this._layers || (this._layers = []),
                    i = index + 1,
                    len = layers.length,
                    layer; i < len; i++) {
                    layer = layers[i];
                ...
                if ((isIE6 && (tag in PIE.childlessElements || tag === 'FIELDSET')) ||
                    ...
                    for (i = sides.length; i--; ) {
                        ...
                        rs[ 'padding' + side ] = (PIE.getLength(cs[ 'padding' + side ])).pixels(el) +
                            ...
                            (PIE.ieVersion !== 8 && i % 2 ? 1
                                : 0); //needs an extra horizontal pixel to counteract the extra "inner border" going away
                    }
                    rs.borderWidth = 0;
                } else if (isIE6) {
        ...
        PIE.RootRenderer = PIE.RendererBase.newRenderer({
            ...
            updateVisibility: function() {
                ...
                this.getBox().style.display = (vis.visible && vis.displayed)
                    ? '' : 'none';
            },
            updateProps: function() {
                if (this.isActive()) {
        ...
        PIE.BackgroundRenderer = PIE.RendererBase.newRenderer({
                ...
                PIE.Util.withImageSize(shape.fill.src, function(size) {
                    ...
                    if (elW && elH) {
                        var fill = shape.fill,
                            ...
                            bgPos = bg.bgPosition
                            ? bg.bgPosition.coords(el, elW - size.w - bwL - bwR, elH - size.h - bwT - bwB)
                            : {
                                x: 0,
                                y: 0
                            },
                        repeat = bg.imgRepeat,
                            ...
                            clipAdjust = PIE.ieVersion === 8 ? 0
                            : 1; //prior to IE8 requires 1 extra pixel in the image clip region
            ...
            addLinearGradient: function(shape, info) {
                ...
                vmlAngle = (angle % 90)
                    ? Math.atan2(deltaX * w / h, deltaY) / PI * 180
                    : (angle + 90);
                ...
                for (i = 0; i < stopCount; i++) {
                    stopPx.push(stops[i].offset
                        ? stops[i].offset.pixels(el, lineLength) :
            ...
            draw: function() {
                ...
                if (props) {
                    ...
                    for (i = 0, len = segments.length; i < len; i++) {
                        seg = segments[i];
                        shape = this.getShape('borderPiece' + i, seg.stroke
                            ? 'stroke' : 'fill', this.getBox());
                        shape.coordsize = w * 2 + ',' + h * 2;
                        shape.coordorigin = '1,1';
                        shape.path = seg.path;
                        s = shape.style;
                        ...
                        if (seg.stroke) {
                            ...
                            stroke['dashstyle'] = seg.stroke === 'dashed'
                                ? '2 2' : seg.stroke === 'dotted' ? '1 1'
                                : 'solid';
                            stroke['linestyle'] = seg.stroke === 'double' && seg.weight > 2
                                ? 'ThinThin'
                                : 'Single';
                        } else {
                    ...
                    if (borderProps.widthsSame && borderProps.stylesSame && borderProps.colorsSame) {
                        ...
                        function curve(corner, shrinkX, shrinkY, startAngle, ccw, doMove) {
                            ...
                            return (rx > 0 && ry > 0) ?
                                (doMove ? 'al' : 'ae') +
                                (isRight ? ceil(elW - rx)
                                    : floor(rx)) * mult + ',' + // center x
                                (isBottom ? ceil(elH - ry)
                                    : floor(ry)) * mult + ',' + // center y
            ...
            draw: function() {
                ...
                PIE.Util.withImageSize(props.src, function(imgSize) {
                    var elW = bounds.w,
                        ...
                        widths = props.widths || (borderProps
                            ? borderProps.widths : {
                                't': zero,
                                'r': zero,
                                'b': zero,
        ...
        PIE.BoxShadowOutsetRenderer = PIE.RendererBase.newRenderer({
            ...
            draw: function() {
                var me = this,
                    ...
                    w = bounds.w,
                    h = bounds.h,
                    clipAdjust = PIE.ieVersion === 8 ? 1
                    : 0, //workaround for IE8 bug where VML leaks out top/left of clip region by 1px
                    corners = ['tl', 'tr', 'br', 'bl'],
                    corner,
                    shadowInfo,
                    shape,
                    ...
                    if (blur) {
                        ...
                        if (blur - spread > w / 2 || blur - spread > h / 2) {
                            ...
                            for (j = 4; j--; ) {
                                ...
                                fill = shape.fill;
                                fill['focusposition'] = (isRight ? 1 - focusX
                                    : focusX) + ',' +
                                ...
                                shape.style.clip = 'rect(' + ((isBottom
                                    ? totalH / 2 : 0) + clipAdjust) + 'px,' +
        ...
        PIE.IE9BackgroundRenderer = PIE.RendererBase.newRenderer({
            ...
            getGradientSvg: function(info, bgWidth, bgHeight) {
                ...
                stopPx = [];
                for (i = 0; i < stopCount; i++) {
                    stopPx.push(stopsInfo[i].offset
                        ? stopsInfo[i].offset.pixels(el, lineLength) :
                        i === 0 ? 0 : i === stopCount - 1 ? lineLength : null);
                }
                // Fill in gaps with evenly-spaced offsets
                for (i = 1; i < stopCount; i++) {
            ...
            draw: function() {
                ...
                PIE.Util.withImageSize(props.src, function(imgSize) {
                    var elW = bounds.w,
                        ...
                        tileSizeT = repeatH === STRETCH ? centerW
                        : imgCenterW * widthT / sliceT,
                        tileSizeR = repeatV === STRETCH ? middleH
                        : imgMiddleH * widthR / sliceR,
                        tileSizeB = repeatH === STRETCH ? centerW
                        : imgCenterW * widthB / sliceB,
                        tileSizeL = repeatV === STRETCH ? middleH
                        : imgMiddleH * widthL / sliceL,
                        svg,
                        patterns = [],
                        rects = [],
                        i = 0;
                    ...
                    function addImage(x, y, w, h, cropX, cropY, cropW, cropH, tileW, tileH) {
                        patterns.push(
                            ...
                            'x="' + (repeatH === REPEAT ? x + w / 2 - tileW / 2
                                : x) + '" ' +
                            'y="' + (repeatV === REPEAT ? y + h / 2 - tileH / 2
                                : y) + '" ' +
