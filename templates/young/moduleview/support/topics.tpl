<div class="supportTopics">
    ...
    <div class="writeBlock mwHalf {if $error}open{/if}">
        <a href="#" class="colorButton handler" onclick="$(this).parent().addClass('open');
                return false;">Написать сообщение</a>
        <div class="caption">
            Написать сообщение в поддержку
            <a class="closeButton" href="#" onclick="$(this).closest('.writeBlock').removeClass('open');
                    return false;"></a>
        </div>
        {if $error}
            ...
            <tbody>  
                {foreach $list as $item}
                    <tr data-id="{$item.id}">
                        ...
                        <td class="remove">
                            <a href="{$router->getUrl('support-front-support', ["Act" => "delTopic", "id" => $item.id])}" class="closeButton" title="Удалить переписку по этой теме"></a>
                        </td>
                    </tr>
                {/foreach}
            </tbody>
...
<script>
    $(function() {
        ...
        $('.supportTable .closeButton').click(function() {
            if (!confirm('Вы действительно хотите удалить переписку по теме?'))
                return false;
