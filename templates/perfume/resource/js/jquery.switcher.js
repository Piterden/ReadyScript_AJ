...
$.fn.switcher = function(method) {
    ...
    return this.each(function() {
        ...
        var methods = {
            init: function(initoptions) {
                ...
                if ($this.data('cookieId') && $.cookie) {
                    $.cookie('switcher-' + $this.data('cookieId')) == 1
                        ? methods.switchOn() : methods.switchOff();
                }
            },
            toggle: function() {
        ...
        var getSwitchElement = function() {
            return (data.options.parentSelector)
                ? $this.parents(data.options.parentSelector) : $this;
        },
            setCookie = function() {
                if ($this.data('cookieId') && $.cookie) {
                    $.cookie('switcher-' + $this.data('cookieId'), (+getSwitchElement().hasClass(data.options.onClass)), {
                        path: '/'
                    });
                }
            };
