...
class MyOrderView extends \RS\Controller\AuthorizedFront
{
	...
	function actionIndex()
	{
		...
		$order = $this->api->setFilter ( array('order_num' => $order_id,'user_id' => $this->user['id'] 
		) )->getFirst ();
		
		if (! $order) return $this->e404 ( t ( 'Заказ не найден' ) );
		...
		$this->app->breadcrumbs->addBreadCrumb ( t ( 'Мои заказы' ), $this->router->getUrl ( 'shop-front-myorders' ) )->addBreadCrumb ( t ( 'Заказ №%0', array($order['order_num'] 
		) ) );
