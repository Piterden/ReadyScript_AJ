...
{
	protected static $controller_title = 'Сопутствующие товары', $controller_description = 'Отображает товары, отмеченные как сопутствующие';
	protected $default_params = array('indexTemplate' => 'blocks/concomitant/concomitant.tpl' 
	);
	function actionIndex()
	{
		...
		if ($route->getId () == 'catalog-front-product' || $route->getId () == 'shop-front-multioffers') {
			if (isset ( $route->product )) {
				...
				$this->view->assign ( array('shop_config' => $shop_config,'current_product' => $route->product,'list' => $route->product->getConcomitant () 
				) );
			}
		}
