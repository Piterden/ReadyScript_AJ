...
class PaymentCtrl extends \RS\Controller\Admin\Crud
{
	...
	function helperIndex()
	{
		...
		), array('add' => t ( 'Добавить способ оплаты' ) 
		) ) );
		...
		) ),new TableType\String ( 'title', t ( 'Название' ), array('Sortable' => SORTABLE_BOTH,'href' => $this->router->getAdminPattern ( 'edit', array(':id' => '@id' 
		) ),'LinkAttr' => array('class' => 'crud-edit' 
		) 
		) ),new TableType\String ( 'description', t ( 'Описание' ), array('Hidden' => true 
		) ),new TableType\String ( 'class', t ( 'Тип рассчета' ) ),new TableType\String ( 'user_type', t ( 'Доступен для' ) ),new TableType\StrYesno ( 'default_payment', t ( 'По умолчанию' ) ),new TableType\Yesno ( 'public', 'Видим.', array('Sortable' => SORTABLE_BOTH,'toggleUrl' => $this->router->getAdminPattern ( 'ajaxTogglePublic', array(':id' => '@id' 
		) ) 
		...
		) ),new TableType\Action\DropDown ( array(array('title' => t ( 'Клонировать способ оплаты' ),'attr' => array('class' => 'crud-add','@href' => $this->router->getAdminPattern ( 'clone', array(':id' => '~field~' 
		) ) 
		) 
		) 
		) ) 
	...
	function actionUserAct()
	{
		...
		if ($act && $payment_obj) {
			...
			return $this->result->setSuccess ( false )->addEMessage ( 'Не установлен метод или объект доставки' );
		}
	}
