...
class UserStatusCtrl extends \RS\Controller\Admin\Crud
{
	...
	function helperIndex()
	{
		...
		), array('add' => t ( 'Добавить статус заказа' ) 
		) ) );
		$helper->setTopTitle ( t ( 'Статусы заказа' ) );
		$helper->setListFunction ( 'tableData' );
		$helper->setTable ( new Table\Element ( array('Columns' => array(new TableType\Checkbox ( 'id', array('cellAttrParam' => 'checkbox_attribute' 
		...
		) ),new TableType\Usertpl ( 'bgcolor', t ( 'Цвет' ), '%shop%/color_cell.tpl' ),new TableType\String ( 'id', 'ID', array('Sortable' => SORTABLE_BOTH,'CurrentSort' => SORTABLE_ASC,'TdAttr' => array('class' => 'cell-sgray' 
		) 
		) ),new TableType\String ( 'type', t ( 'Тип' ), array('Sortable' => SORTABLE_BOTH,'TdAttr' => array('class' => 'cell-sgray' 
		) 
		) ),
