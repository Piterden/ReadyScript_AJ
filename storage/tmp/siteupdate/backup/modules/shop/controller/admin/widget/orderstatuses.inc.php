...
{
	protected $orderapi, $info_title = 'Диаграмма статусов заказов', $info_description = 'Позволяет отслеживать статистику по выполнению заказов';
	function actionIndex()
	{
		...
		foreach($counts as $id => $count) {
			$json_data[] = array('label' => (isset ( $statuses[$id] ) ? $statuses[$id]['title'] : t ( 'статус удален' )) . "($count)",'data' => ( int ) $count,'color' => isset ( $statuses[$id] ) ? $statuses[$id]['bgcolor'] : '' 
			);
		...
		$this->view->assign ( array('total' => $total,'counts' => $counts,'statuses' => $statuses,'inwork' => $inwork,'finished' => $finished,'json_data' => json_encode ( $json_data ) 
		) );
