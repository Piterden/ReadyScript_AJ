...
{
	private $opt = array();
	protected $post_params = array(), // Параметры для POST запроса
$order, $transaction;
	...
	function getFormHtml()
	{
		if ($params = $this->getFormObject ()) {
			...
			return $params->getForm ( array('payment_type' => $this 
			), null, false, null, '%system%/coreobject/tr_form.tpl', $tpl_folder );
		}
	}
	...
	function getDocUrl($doc_key = null, $absolute = false)
	{
		...
		if ($this->order) {
			return \RS\Router\Manager::obj ()->getUrl ( 'shop-front-documents', array('doc_key' => $doc_key,'order' => $this->order['hash'] 
			), $absolute );
		}
		
		// Если это транзакция для пополнения лицевого счета
		if ($this->transaction) {
			return \RS\Router\Manager::obj ()->getUrl ( 'shop-front-documents', array('doc_key' => $doc_key,'transaction' => $this->transaction['sign'] 
			), $absolute );
		}
