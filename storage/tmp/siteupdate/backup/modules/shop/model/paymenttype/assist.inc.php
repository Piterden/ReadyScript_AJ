...
class Assist extends AbstractType
{
	...
	{
		$this->setOption ( array('payurl' => 'https://test.paysecure.ru/pay/order.cfm' 
		) );
	}
	...
	{
		$properties = new \RS\Orm\PropertyIterator ( array('payurl' => new Type\String ( array('description' => t ( 'URL оплаты' ) 
		) ),'merchant_id' => new Type\String ( array('description' => t ( 'Merchant ID' ) 
		) ),'secret_word' => new Type\String ( array('description' => t ( 'Секретное слово' ) 
		) ),'__help__' => new Type\Mixed ( array('description' => t ( '' ),'visible' => true,'template' => '%shop%/form/payment/assist/help.tpl' 
		) ) 
		) );
	...
	function onResult(\Shop\Model\Orm\Transaction $transaction, \RS\Http\Request $request)
	{
		...
		$fail_response = '<?xml version="1.0" encoding="UTF-8"?>' . '<pushpaymentresult firstcode="1" secondcode="0">' . '</pushpaymentresult>';
		...
		return '<?xml version="1.0" encoding="UTF-8"?>' . '<pushpaymentresult firstcode="0" secondcode="0">' . '<order>' . '<billnumber>' . $transaction->id . '</billnumber>' . '<packetdate>' . date ( "d.m.Y H:i:s" ) . '</packetdate>' . '</order>' . '</pushpaymentresult>';
	}
