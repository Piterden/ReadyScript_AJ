...
class DeliveryApi extends \RS\Module\AbstractModel\EntityList
{
	...
	{
		parent::__construct ( new \Shop\Model\Orm\Delivery (), array('nameField' => 'title','multisite' => true,'defaultOrder' => 'sortn','sortField' => 'sortn' 
		) );
	}
	...
	{
		$zone = array_merge ( array(0 
		), ( array ) $zone );
		$q = $this->queryObj ()->join ( new Orm\DeliveryXZone (), 'DZ.delivery_id = A.id AND DZ.zone_id IN (' . implode ( ',', $zone ) . ')', 'DZ' );
	}
}
