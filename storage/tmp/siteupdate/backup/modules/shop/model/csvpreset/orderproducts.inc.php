...
class OrderProducts extends \RS\Csv\Preset\AbstractPreset
{
	...
	protected $delimiter = ';', $id_field, $link_preset_id, $link_id_field, $orm_object;
	...
	function getColumnsData($n)
	{
		...
		if (! empty ( $products )) {
			...
			foreach($products as $uniq_id => $product) {
				...
				$data[] = "Артикул:" . $product['barcode'];
				$data[] = "Кол-во:" . $product['amount'];
				$data[] = "Сумма:" . $product['price'];
				if (! empty ( $product['discount'] )) {
					$data[] = "Скидка:" . $product['discount'] . "";
				}
				$lines[] = implode ( ' | ', $data );
			}
			$line = implode ( $this->delimiter . "\n", $lines );
		}
		return array($this->id . '-products' => $line 
		);
	}
	...
	function getColumns()
	{
		return array($this->id . '-products' => array('key' => 'products','title' => t ( 'Товары' ) 
		) 
		);
	}
}