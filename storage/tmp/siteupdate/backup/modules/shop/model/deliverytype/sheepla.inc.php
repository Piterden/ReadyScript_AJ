...
{
	const API_URL = "http://api.sheepla.com/", // URL API Sheepla
...
REQUEST_TIMEOUT = 10; // 10 сек. таймаут запроса к api
	protected $templates = null, // Шаблоны доставки
...
Использовать правила расчета стоимости доставки&quot; и настроить правила используя автоматический(динамический) подсчёт в настройках аккаунта на Sheepla 
	...
	{
		$properties = new \RS\Orm\PropertyIterator ( array('max_weight' => new Type\String ( array('description' => t ( 'Максимальный вес, грамм' ) 
		) ),'width' => new Type\Integer ( array('description' => t ( 'Свойство со значением ширины изделия' ),'list' => array(array('\Catalog\Model\PropertyApi','staticSelectList' 
		),true 
		) 
		) ),'height' => new Type\Integer ( array('description' => t ( 'Свойство со значением высоты изделия' ),'list' => array(array('\Catalog\Model\PropertyApi','staticSelectList' 
		),true 
		) 
		) ),'length' => new Type\Integer ( array('description' => t ( 'Свойство со значением длинны изделия' ),'list' => array(array('\Catalog\Model\PropertyApi','staticSelectList' 
		),true 
		) 
		) ),'language' => new Type\Integer ( array('description' => t ( 'Язык интефейса для виджетов' ),'ListFromArray' => array($this->getLanguages () 
		) 
		) ),'admin_api' => new Type\String ( array('description' => t ( 'Ключ API администратора' ),'hint' => t ( 'Получается из панели Sheepla' ),'maxLength' => 255,'template' => '%shop%/form/delivery/sheepla/sheepla_admin_api.tpl' 
		) ),'public_api' => new Type\String ( array('description' => t ( 'Ключ API публичный' ),'hint' => t ( 'Получается из панели Sheepla' ),'maxLength' => 255 
		) ),'template_id' => new Type\Integer ( array('description' => t ( 'Шаблон доставки' ),'list' => array(array('\Shop\Model\DeliveryType\Sheepla','staticGetTemplates' 
		),$this 
		),'template' => '%shop%/form/delivery/sheepla/sheepla_template.tpl' 
		) ),'timeout' => new Type\Integer ( array('description' => t ( 'Время ожидания ответа Sheepla, сек' ),'hint' => t ( 'Иногда запросы к Sheepla идут очень долго,<br/> чтобы не дожидатся ответа используется это значение.<br/>Рекоммендуемое значение 10 сек.' ),'default' => 10 
		) ) 
		) );
	...
	private function getCurrencyByName($name)
	{
		if ($this->cache_currencies === null) {
			...
			$this->cache_currencies = \RS\Orm\Request::make ()->from ( new \Catalog\Model\Orm\Currency () )->where ( array('public' => 1 
	...
	function getDeliveryCostText(\Shop\Model\Orm\Order $order, \Shop\Model\Orm\Address $address = null)
	{
		...
		return ($cost) ? \RS\Helper\CustomView::cost ( $cost ) . ' ' . $this->delivery_currency : 'бесплатно';
	}
	...
	private function applyRule(\Shop\Model\Orm\Order $order, \Shop\Model\Orm\Address $address, $rule, $last_delivery_cost)
	{
		...
		$formula_vars = array('S' => $order_price_without_delivery,'W' => $order->getWeight () 
		);
	...
	private function apiRequest($xml_request, $method)
	{
		try {
			$result = false;
			$context = stream_context_create ( array('http' => array('method' => "POST",'header' => "Content-Type: text/xml; charset=utf-8",'content' => $xml_request,'content-length' => mb_strlen ( $xml_request ),'timeout' => $this->getOption ( 'timeout', 10 ) ? $this->getOption ( 'timeout', 10 ) : 10 
			) 
			) );
	...
	private function checkOrderErrorsAnswer($response, $method)
	{
		if (isset ( $response->orders->order->errors->error )) { // Если секция с ошибками при оформлении заказа найдена
			...
			throw new \RS\Exception ( "При отправке заказа с доставкой возникли следующие 
            ошибки: \"" . implode ( ",", $this->getErrors () ) . "\" Метод " . $method, 100 );
			exit ();
		}
		return true;
	}
	...
	private function requestGetShipmentTemplates()
	{
		if ($this->templates === null) {
			...
			if (! $response_xml) { // Если запрос не удался
				throw new \RS\Exception ( 'Не удалось отправить запрос в Sheepla. Метод ' . $method, 101 );
			}
			...
			foreach($response_xml->templates->template as $template) {
				...
				$templates[$template_id] = array('id' => $template_id,'name' => ( string ) $template['name'],'carrierName' => ( string ) $template['carrierName'],'isCod' => ( boolean ) $template['isCod'],'baseServiceCode' => ( int ) $template['baseServiceCode'] 
				);
			}
		...
		if ($this->carries_accounts === null) {
			...
			if (isset ( $response_xml->carrierAccounts->carrierAccount )) {
				foreach($response_xml->carrierAccounts->carrierAccount as $carrierAccount) {
					$arr[( string ) $carrierAccount->$field_key] = array('carrier' => ( string ) $carrierAccount->carrier,'name' => ( string ) $carrierAccount->name,'carrierAccountId' => ( string ) $carrierAccount->carrierAccountId 
					);
				}
			}
		...
		if ($this->carries === null) {
			...
			if (isset ( $response_xml->shipments->carrier )) {
				foreach($response_xml->shipments->carrier as $carrier) {
					$arr[( string ) $carrier->carrierId] = array('id' => ( string ) $carrier->carrierId,'name' => ( string ) $carrier->name,'country' => ( string ) $carrier->country 
					);
				}
			}
			$this->carries = $arr;
	...
	private function requestGetDeliveryCost(\Shop\Model\Orm\Order $order, \Shop\Model\Orm\Address $address = null)
	{
		...
		foreach($products as $n => $item) {
			...
			$sxml->products->product[$i]->attribute[0]['value'] = $product->getWeight ();
	...
	private function requestCreateOrder(\Shop\Model\Orm\Order $order, \Shop\Model\Orm\Address $address = null)
	{
		...
		$sxml->orders->order->deliveryPrice = $delivery->getDeliveryCost ( $order ); // Стоимость доставки
		$sxml->orders->order->deliveryPriceCurrency = $currency['title']; // Валюта
		$sxml->orders->order->orderWeight = $order->getWeight (); // Вес
		...
		foreach($products as $n => $item) {
			...
			$sxml->orders->order->orderItems->orderItem[$i]->name = $offer_title ? $offer_title : $product['title'];
			...
			$sxml->orders->order->orderItems->orderItem[$i]->weight = $product->getWeight ();
			
			if ($product->getPropertyValueById ( $this->getOption ( 'width' ) )) { // Если ширина есть
				$sxml->orders->order->orderItems->orderItem[$i]->width = $product->getPropertyValueById ( $this->getOption ( 'width' ) );
			}
			
			if ($product->getPropertyValueById ( $this->getOption ( 'height' ) )) { // Если высота есть
				$sxml->orders->order->orderItems->orderItem[$i]->height = $product->getPropertyValueById ( $this->getOption ( 'height' ) );
			}
			
			if ($product->getPropertyValueById ( $this->getOption ( 'length' ) )) { // Если длинна есть
				$sxml->orders->order->orderItems->orderItem[$i]->length = $product->getPropertyValueById ( $this->getOption ( 'length' ) );
			}
	...
	function getAddittionalHtml(\Shop\Model\Orm\Delivery $delivery, \Shop\Model\Orm\Order $order = null)
	{
		...
		$view->assign ( array('public_api_js_url' => self::PUBLIC_API_URL_JS,'public_api_css_url' => self::PUBLIC_API_URL_CSS,'public_api_key' => trim ( $this->getOption ( 'public_api' ) ), // Публичный API
...
'user' => \RS\Application\Auth::getCurrentUser () 
 ...
 + \RS\Module\Item::getResourceFolders ( $this ) );
	...
	function getAdminAddittionalHtml(\Shop\Model\Orm\Order $order = null)
	{
		...
		$view = new \RS\View\Engine ();
		$view->assign ( array('errors' => $this->getErrors (),'order' => $order,'cost' => $cost,'extra_info' => $order->getExtraKeyPair (),'delivery' => $delivery,'template_id' => $template_id,'sheepla' => $this,'public_api_js_url' => self::PUBLIC_API_URL_JS,'public_api_css_url' => self::PUBLIC_API_URL_CSS,'public_api_key' => $this->getOption ( 'public_api' ),'culture_id' => $this->getOption ( 'language' ) 
		) + \RS\Module\Item::getResourceFolders ( $this ) );
	...
	function onOrderCreate(\Shop\Model\Orm\Order $order, \Shop\Model\Orm\Address $address = null)
	{
		...
		if (! isset ( $extra['sheepla_order_id'] )) { // Если заказ в sheepla ещё не создан
			...
			if ($created_order) {
				...
				$order->addExtraInfoLine ( 'id заказа в sheepla', '<a href="http://panel.sheepla.ru/mOrder/EditOrder/' . $sheepla_order_id . '" target="_blank">' . $sheepla_order_id . '</a>', array('orderId' => $sheepla_order_id 
				), 'sheepla_order_id' );
			} else { // Иначе
				$order->addExtraInfoLine ( 'Sheepla', '<a href="http://panel.sheepla.ru/mOrder/All" target="_blank">Ссылка в список заказов в sheepla</a>', 'sheepla_order_id' );
			}
			$extra = $order->getExtraInfo ();
		}
	...
	{
		$view = new \RS\View\Engine ();
		$view->assign ( array('public_api_js_url' => self::PUBLIC_API_URL_JS,'public_api_css_url' => self::PUBLIC_API_URL_CSS,'api_key' => $this->getOption ( 'admin_api', 0 ),'cultureId' => $this->getOption ( 'language', self::DEFAULT_LANGUAGE_ID ),'type' => 'short','title' => 'Краткие сведения заказа','order' => $order,'delivery_type' => $this 
		) );
