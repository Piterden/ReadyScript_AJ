...
use \Shop\Model\UserStatusApi, \Shop\Model\Orm\UserStatus;
...
{
	const ORDER_FILTER_ALL = 'all', ORDER_FILTER_SUCCESS = 'success', 

	ORDER_SHOW_TYPE_NUM = 'num', ORDER_SHOW_TYPE_SUMM = 'summ';
	function __construct()
	{
		parent::__construct ( new \Shop\Model\Orm\Order (), array('multisite' => true,'defaultOrder' => 'id DESC' 
		) );
	}
	...
	{
		$query = \RS\Orm\Request::make ()->select ( 'COUNT(*) cnt' )->from ( $this->obj_instance );
		
		$queries = array();
		$queries['total_orders'] = $query;
		$queries['open_orders'] = clone $query->where ( "status != '#status'", array('status' => UserStatusApi::getIdByType ( UserStatus::STATUS_NEW ) 
		) );
		$queries['closed_orders'] = clone $query->where ( array('status' => UserStatusApi::getIdByType ( UserStatus::STATUS_NEW ) 
		) );
		$queries['last_order_date'] = \RS\Orm\Request::make ()->select ( 'dateof cnt' )->from ( $this->obj_instance )->where ( array('site_id' => \RS\Site\Manager::getSiteId () 
	...
	/**
	 * Возвращает даты заказов, сгруппированные по годам.
	 * Для видета статистики
	 * 
	...
	function ordersByYears($order_filter = self::ORDER_FILTER_ALL, $show_type = self::ORDER_SHOW_TYPE_NUM, $lastrange = 5, $cache = true)
	{
		...
		if ($cache) {
			$result = \RS\Cache\Manager::obj ()->request ( array($this,'ordersByYears' 
			), $order_filter, $show_type, $lastrange, false, $site_id );
		} else {
			$q = \RS\Orm\Request::make ()->select ( 'dateof, COUNT(*) cnt, SUM(totalcost) total_cost' )->from ( $this->obj_instance )->where ( 'dateof >= NOW()-INTERVAL #lastrange YEAR', array('lastrange' => $lastrange 
			) )->where ( array('site_id' => $site_id 
			) )->groupby ( 'YEAR(dateof), MONTH(dateof)' )->orderby ( 'dateof' );
			...
			while ( $row = $res->fetchRow () ) {
				...
				$result[$year]['data'][date ( 'n', $date ) - 1] = array('x' => mktime ( 4, 0, 0, date ( 'n', $date ), 1 ) * 1000,'y' => $show_type == self::ORDER_SHOW_TYPE_NUM ? $row['cnt'] : $row['total_cost'],'pointDate' => $date * 1000,'total_cost' => $row['total_cost'],'count' => $row['cnt'] 
				);
			}
			...
			foreach($result as $year => $val) {
				...
				for($month = 1; $month <= 12; $month ++) {
					$month_list[$month - 1] = isset ( $result[$year]['data'][$month - 1] ) ? $result[$year]['data'][$month - 1] : array('x' => mktime ( 4, 0, 0, $month, 1 ) * 1000,'y' => 0,'pointDate' => mktime ( 4, 0, 0, $month, 1, $year ) * 1000,'total_cost' => 0,'count' => 0 
					);
				}
	...
	function ordersByMonth($order_filter = self::ORDER_FILTER_ALL, $show_type = self::ORDER_SHOW_TYPE_NUM, $lastrange = 1, $cache = true)
	{
		...
		if ($cache) {
			$result = \RS\Cache\Manager::obj ()->request ( array($this,'ordersByMonth' 
			), $order_filter, $show_type, $lastrange, false, $site_id );
		} else {
			$currency = \Catalog\Model\CurrencyApi::getBaseCurrency ()->stitle;
			$start_time = strtotime ( '-1 month' );
			
			$q = \RS\Orm\Request::make ()->select ( 'dateof, COUNT(*) cnt, SUM(totalcost) total_cost' )->from ( $this->obj_instance )->where ( "dateof >= '#starttime'", array('starttime' => date ( 'Y-m-d', $start_time ) 
			) )->where ( array('site_id' => $site_id 
			) )->groupby ( 'DATE(dateof)' )->orderby ( 'dateof' );
			...
			while ( $row = $res->fetchRow () ) {
				...
				$ymd = date ( 'Ymd', $date );
				$result[0][$ymd] = array('x' => $date * 1000,'y' => $show_type == self::ORDER_SHOW_TYPE_NUM ? $row['cnt'] : $row['total_cost'],'total_cost' => $row['total_cost'],'count' => $row['cnt'] 
				);
			}
			...
			while ( ($time = strtotime ( "+$i day", $start_time )) && $time <= $today ) {
				...
				if (! isset ( $result[0][$ymd] )) {
					$result[0][$ymd] = array('x' => $time * 1000,'y' => 0,'total_cost' => 0,'count' => 0 
					);
				}
				$i ++;
			}
	...
	function generateOrderNum($order)
	{
		...
		$found_order = \RS\Orm\Request::make ()->from ( new \Shop\Model\Orm\Order () )->where ( array('order_num' => $code,'site_id' => $order['site_id'] 
		) )->object ();
	...
	{
		return \RS\Orm\Request::make ()->from ( new Orm\Order () )->where ( array('user_id' => $user_id,'site_id' => \RS\Site\Manager::getSiteId () 
		) )->count ();
	}
	...
	function getReport(\RS\Orm\Request $request)
	{
		...
		$request->select = "";
		$list['payment'] = $request->select ( '
                payment,
                COUNT(id) as orderscount,
                SUM(totalcost) as totalcost,
                SUM(user_delivery_cost) as user_delivery_cost,
                SUM(deliverycost) as single_deliverycost
            ' )->groupby ( 'payment' )->exec ()->fetchSelected ( 'payment' );
		...
		$request->select = "";
		$request->groupby = "";
		$list['delivery'] = $request->select ( '
                delivery,
                COUNT(id) as orderscount,
                SUM(totalcost) as totalcost,
                SUM(user_delivery_cost) as user_delivery_cost,
                SUM(deliverycost) as single_deliverycost
            ' )->groupby ( 'delivery' )->exec ()->fetchSelected ( 'delivery' );
