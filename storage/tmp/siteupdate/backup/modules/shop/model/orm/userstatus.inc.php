...
{
	const STATUS_NEW = 'new', STATUS_WAITFORPAY = 'waitforpay', STATUS_INPROGRESS = 'inprogress', STATUS_SUCCESS = 'success', STATUS_CANCELLED = 'cancelled', 

	STATUS_USER = 'other';
	protected static $table = 'order_userstatus', $sort = array(self::STATUS_NEW,self::STATUS_WAITFORPAY,self::STATUS_INPROGRESS,self::STATUS_USER,self::STATUS_SUCCESS,self::STATUS_CANCELLED 
	);
	function _init()
	{
		parent::_init ()->append ( array('site_id' => new Type\CurrentSite (),'title' => new Type\String ( array('description' => t ( 'Статус' ) 
		) ),'bgcolor' => new Type\Color ( array('maxLength' => 7,'description' => t ( 'Цвет фона' ) 
		) ),'type' => new Type\String ( array('maxLength' => 20,'description' => t ( 'Идентификатор(Англ.яз)' ) 
		) ) 
		) );
		$this->addIndex ( array('site_id','type' 
		), self::INDEX_UNIQUE );
	}
	...
	{
		return array(self::STATUS_NEW => array('title' => t ( 'Новый' ),'bgcolor' => '#83b7b3' 
		),self::STATUS_WAITFORPAY => array('title' => t ( 'Ожидает оплату' ),'bgcolor' => '#687482' 
		),self::STATUS_INPROGRESS => array('title' => t ( 'В обработке' ),'bgcolor' => '#f2aa17' 
		),self::STATUS_SUCCESS => array('title' => t ( 'Выполнен и закрыт' ),'bgcolor' => '#5f8456' 
		),self::STATUS_CANCELLED => array('title' => t ( 'Отменен' ),'bgcolor' => '#ef533a' 
		) 
		);
	}
	...
	{
		$count_by_status = \RS\Orm\Request::make ()->select ( 'status, COUNT(*) as cnt' )->from ( new \Shop\Model\Orm\Order () )->where ( array('site_id' => \RS\Site\Manager::getSiteId () 
		) )->groupby ( 'status' )->exec ()->fetchSelected ( 'status', 'cnt' );
