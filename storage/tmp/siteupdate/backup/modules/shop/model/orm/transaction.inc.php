...
class Transaction extends \RS\Orm\OrmObject
{
	...
	function _init()
	{
		parent::_init ()->append ( array('site_id' => new Type\CurrentSite (),'dateof' => new Type\Datetime ( array('description' => t ( 'Дата транзакции' ) 
		) ),'user_id' => new Type\Integer ( array('maxLength' => '11','description' => t ( 'ID пользователя' ) 
		) ),'order_id' => new Type\Integer ( array('maxLength' => '11','description' => t ( 'ID заказа' ) 
		) ),'personal_account' => new Type\Integer ( array('maxLength' => '1','description' => t ( 'Транзакция изменяющая баланс лицевого счета' ) 
		) ),'cost' => new Type\Decimal ( array('maxLength' => '15','decimal' => 2,'description' => t ( 'Сумма' ) 
		) ),'payment' => new Type\Integer ( array('description' => t ( 'Тип оплаты' ) 
		) ),'reason' => new Type\Text ( array('description' => t ( 'Назначение платежа' ) 
		) ),'error' => new Type\String ( array('description' => t ( 'Ошибка' ) 
		) ),'status' => new Type\Enum ( array(self::STATUS_NEW,self::STATUS_SUCCESS,self::STATUS_FAIL 
		), array('allowEmpty' => false,'description' => t ( 'Статус транзакции' ),'listFromArray' => array(array(self::STATUS_NEW => 'Платеж инициирован',self::STATUS_SUCCESS => 'Платеж успешно завершен',self::STATUS_FAIL => 'Платеж завершен с ошибкой' 
		) 
		) 
		) ),'sign' => new Type\String ( array('description' => t ( 'Подпись транзакции' ) 
		) ),'entity' => new Type\String ( array('description' => t ( 'Сущность к которой привязана транзакция' ),'maxLength' => 50 
		) ),'entity_id' => new Type\Bigint ( array('description' => t ( 'ID сущности, к которой привязана транзакция' ) 
		) ),'extra' => new Type\String ( array('maxLength' => '4096','description' => t ( 'Дополнительное поле для данных' ),'visible' => false 
		) ),'extra_arr' => new Type\ArrayList ( array('visible' => false 
		) ) 
		)
		 );
		
		$this->addIndex ( array('entity','entity_id' 
		), self::INDEX_KEY );
	}
	...
	public function afterWrite($save_flag)
	{
		...
		if ($user->getBalance () != $real_balance) {
			...
			$old_balance = $transApi->getBalance ( $user->id, array($this->id 
			) ); // Получаем сумму на счету без учета этой транзакции
		...
		if ($save_flag == self::INSERT_FLAG && (! $this['status'] || $this['status'] == self::STATUS_NEW) && $this['cost'] > 0) { // если новая и баланс на пополнение
