(function( $ ){
   $.CDEKWidjetCreator = function( method ) {
        ...
        var defaults = {            
           cdekDiv        : '.cdekWidjet',         //Контейнер куда будет вставлятся информация СДЕК
           deliveryExtra  : '.cdekDeliveryExtra',  //Скрытое поле с экстра информацией
           select         : '.cdekSelect',         //Выпадающий список с забора СДЕК
           additionalInfo : '.cdekAdditionalInfo', //Дополнительная информация СДЕК
        ...
        var methods = {
            ...
            */
            init: function(initoptions) 
            {
                ...
                //Разблокируем доставку СДЕК если она уже выбрана
                ...
                $('body').on('new-content',function(){
                    ...
                    //Навесим события на переключатели
                    presetCDEKRadioEvents();
                    //Отработаем установку выборки сдек
            ...
            /**
            * Открывает доступность СДЕК для выбора
            * 
            * @param checkBoxObj  - объект с функциями СДЕК
            */
            ...
            /**
            * Убирает доступность СДЕК для выбора из всех вариантов
            * и скрывает поля
            * 
            */
            clearCDEK : function ()
        ...
        * Если у этой доставки есть признак что если есть признак, что это СДЕК,
        ...
        */
        var setOrClearCDEKInfo = function (changeEvent, item) 
        {
            ...
            if ( typeof(dataCDEKDiv) != 'undefined' ) { //Если это радиокнопка СДЕК
        ...
        /**
        * Проходится по контейнерам СДЕК помечая радиокнопки маркером СДЕК
        * 
        */
        ...
        */
        insertAdditionalData = function (divObj, info)
        {
            ...
            <span class='row'><span class='key'>Тел.:</span><span class='val'>"+info.phone+"</span></span>\
            "); 
            //Если есть ограничение оплаты налиными
            if ( typeof(info.cashOnDelivery) != "undefined" ) {
                divObj.append("\
            <span class='row'><span class='key'>Ограничение оплаты наличными при получении:</span><span class='val'>"+info.city+"</span></span>");
            }
            if ( info.note.length>0 ){
               divObj.append("<span class='key'>Заметка:</span><span class='val'>"+info.note+"</span>"); 
            }
        },
        ...
        */ 
           ...
           ymaps.ready(function(){
               ...
               $(addresses).each(function(i){
                   ...
                   var myPlacemark = new ymaps.Placemark([adress.coordY, adress.coordX], { 
                      hintContent: "Выберите пункт: "+adress.adress,
        ...
        /**
        * Проверяет выбрана ли доставка СДЕК и отрабатывает выбор адреса доставки
        */
        ...
        */
        changeCDEKAdress = function(event, item)
        {
            if (event === null){ //Если сработало событие
               ...
               if (parent.length==0){ //Если это не СДЕК
                   return false;
               }
