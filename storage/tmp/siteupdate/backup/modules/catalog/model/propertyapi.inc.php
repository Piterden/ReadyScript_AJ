...
$filter_active, $post_var = 'prop', $name_field = 'title', $obj_link = '\Catalog\Model\Orm\Property\Link', 

	$valueField = array('int' => 'val_int','string' => 'val_str','list' => 'val_str','bool' => 'val_int' 
	), $prop_item_table, $prop_link_table;
	function __construct()
	{
		parent::__construct ( new \Catalog\Model\Orm\Property\Item (), array('multisite' => true,'defaultOrder' => 'parent_sortn, sortn','sortField' => 'sortn' 
		) );
	...
	{
		$groups = \RS\Orm\Request::make ()->from ( new \Catalog\Model\Orm\Property\Dir () )->orderby ( 'title' )->where ( array('site_id' => \RS\Site\Manager::getSiteId () 
		) )->objects ( null, 'id' );
		
		$props = \RS\Orm\Request::make ()->from ( new \Catalog\Model\Orm\Property\Item () )->orderby ( 'title' )->where ( array('site_id' => \RS\Site\Manager::getSiteId () 
		) )->objects ();
		$list = array();
		if (! empty ( $props )) {
	...
	function updateParentSort($from, $to)
	{
		...
		$q->update ()->from ( $this->obj_instance )->asAlias ( 'P' )->from ( new Orm\Property\Dir () )->asAlias ( 'D' )->set ( 'P.parent_sortn = D.sortn' )->where ( "P.parent_id = D.id AND D.id >= '#from' AND D.id <= '#to'", array('from' => $from,'to' => $to 
		) )->exec ();
	}
	...
	{
		$properties = \RS\Orm\Request::make ()->from ( new Orm\Property\Item () )->whereIn ( 'id', $properties_id )->orderby ( 'sortn' )->objects ( null, 'parent_id', true );
		...
		if (count ( $dir_ids )) {
			$dirs += \RS\Orm\Request::make ()->from ( new Orm\Property\Dir () )->whereIn ( 'id', $dir_ids )->orderby ( 'sortn' )->objects ( null, 'id' );
		}
		
		return array('properties' => $properties,'groups' => $dirs 
		);
	}
	...
	function saveProperties($obj_id, $link_type, $property_list)
	{
		...
		\RS\Orm\Request::make ()->delete ()->from ( new Orm\Property\Link () )->where ( array('site_id' => $site_id,"{$link_type}_id" => $obj_id 
		) )->exec ();
		
		if (! empty ( $property_list )) {
			$link_sql = array();
			
			$key_type = \RS\Orm\Request::make ()->select ( 'id, type' )->from ( new Orm\Property\Item () )->whereIn ( 'id', array_keys ( $property_list ) )->exec ()->fetchSelected ( 'id', 'type' );
			...
			foreach($property_list as $prop) {
				...
				if (isset ( $prop['value'] ) || ! empty ( $prop['usevalue'] )) {
					...
					foreach($value as $one_value) {
						$link = array('site_id' => $site_id,'prop_id' => $prop['id'],"{$link_type}_id" => $obj_id,'val_str' => 'NULL','val_int' => 'NULL','xml_id' => isset ( $prop['xml_id'] ) ? $prop['xml_id'] : 'NULL','public' => isset ( $prop['public'] ) ? $prop['public'] : 0 
						);
						
						if (! empty ( $prop['usevalue'] ) || isset ( $prop['is_my'] )) {
							$link[$this->valueField[$key_type[$prop['id']]]] = "'" . \RS\Db\Adapter::escape ( $one_value ) . "'";
						}
						$link_sql[] = "(" . implode ( ',', \RS\Helper\Tools::arrayQuote ( $link, array('val_str','val_int' 
						), "'", true ) ) . ")";
					}
				}
			}
	...
	function getAllPropertiesAndGroups($allow_empty_group = false)
	{
		...
		$dirs = \RS\Orm\Request::make ()->from ( new \Catalog\Model\Orm\Property\Dir () )->where ( array('site_id' => \RS\Site\Manager::getSiteId () 
		) )->orderby ( 'sortn' )->objects ( null, 'id' );
		
		// Получим все характеристики сгруппированными
		$properties = \RS\Orm\Request::make ()->from ( new \Catalog\Model\Orm\Property\Item () )->where ( array('site_id' => \RS\Site\Manager::getSiteId () 
		) )->orderby ( 'sortn' )->objects ( null, 'parent_id', true );
		...
		if (! empty ( $properties[0] ) || $allow_empty_group) {
			$data[0]['group'] = array('id' => 0,'title' => 'Все' 
			);
	...
	function getGroupProperty($group_id, $include_parent = true, $public = null)
	{
		...
		$res = \RS\Orm\Request::make ()->select ( 'T.*,  TL.val_str, TL.val_int, TL.group_id, TL.public' )->from ( $this->prop_item_table )->asAlias ( 'T' )->join ( $this->prop_link_table, 'T.id=TL.prop_id', 'TL' )->where ( array('TL.site_id' => \RS\Site\Manager::getSiteId () 
		) )->whereIn ( 'TL.group_id', $groups_id )->where ( $public_where )->orderby ( 'T.parent_id, T.sortn' )->exec ();
		...
		if (count ( $all_property_parents )) {
			$dirs += \RS\Orm\Request::make ()->from ( new Orm\Property\Dir () )->whereIn ( 'id', $all_property_parents )->orderby ( 'sortn' )->objects ( null, 'id' );
		}
	...
	function createOrUpdate(array $item)
	{
		...
		return array('group' => $dir,'property' => $this->getElement () 
		);
	}
	...
	{
		$obj_list = ($obj instanceof Orm\Product) ? array($obj 
		) : $obj;
		...
		foreach($obj_list as $product) {
			if (! is_null ( $product['id'] )) $product_list_sql[] = $product['id'];
		}
		
		if (! empty ( $product_list_sql )) {
			$res = \RS\Orm\Request::make ()->select ( 'T.*, L.val_str, L.val_int, L.product_id' )->from ( $this->prop_link_table )->asAlias ( 'L' )->join ( $this->prop_item_table, 'T.id = L.prop_id', 'T' )->whereIn ( 'L.product_id', $product_list_sql )->exec ();
...
{
					$res = \RS\Orm\Request::make ()->from ( $this->prop_link_table )->asAlias ( 'L' )->join ( $this->prop_item_table, 'T.id = L.prop_id', 'T' )->where ( array('L.site_id' => \RS\Site\Manager::getSiteId () 
					) )->whereIn ( 'L.group_id', $group_list )->exec ();
	...
	function getFilteredQuery(array $filters, $product_table_alias, \RS\Orm\Request $q)
	{
		...
		foreach($list as $n => $prop) {
			...
			if (is_array ( $value ) && isset ( $value['from'] ) && isset ( $value['to'] )) {
				if ($prop['interval_from'] == $value['from'] && $prop['interval_to'] == $value['to']) {
	...
	{
		$q = new \RS\Orm\Request ();
		$q->select ( 'id' )->from ( new Orm\Product () )->asAlias ( 'P' );
	...
	protected function listFilter($pn, $prop, $value, \RS\Orm\Request $q)
	{
		...
		$this->addPropertyJoin ( $pn, $prop, $q )->openWGroup ()->where ( "$pn.prop_id = '{$prop['id']}'" )->whereIn ( "$pn.val_str", ( array ) $value )->closeWGroup ();
	}
	...
	{
		$expr = "$pn.prop_id = '{$prop['id']}'"; // Выражение
		$this->addPropertyJoin ( $pn, $prop, $q )->where ( "(" . $expr . " AND $pn.val_str LIKE '#value%')", array('value' => $value 
		) );
	}
	...
	protected function boolFilter($pn, $prop, $value, \RS\Orm\Request $q)
	{
		if ($value == 0) {
			...
			$q->leftjoin ( $this->prop_link_table, "$pn.product_id = {$this->pta}.id", $pn )->where ( "($pn.prop_id = '{$prop['id']}' AND ($pn.val_int IS NULL OR $pn.val_int = 0)) " );
		} else {
			$this->addPropertyJoin ( $pn, $prop, $q )->where ( "($pn.prop_id = '{$prop['id']}' AND $pn.val_int > " . (1 - self::ECLIPSE_FLOAT_COMPARE) . ")" );
		}
	}
	...
	{
		$q->join ( $this->prop_link_table, "$pn.product_id = {$this->pta}.id", $pn );
		return $q;
	}
	...
	{
		$groups = \RS\Orm\Request::make ()->from ( new Orm\Property\Dir () )->objects ();
	...
	function cleanUnusedProperty()
	{
		...
		\RS\Orm\Request::make ()->delete ( 'L' )->from ( new Orm\Property\Link (), 'L' )->leftjoin ( new Orm\Product (), 'P.id = L.product_id', 'P' )->where ( "L.site_id = '#site_id' AND L.product_id > 0 AND P.id IS NULL", array('site_id' => $site_id 
		) )->exec ();
		
		\RS\Orm\Request::make ()->delete ( 'L' )->from ( new Orm\Property\Link (), 'L' )->leftjoin ( new Orm\Dir (), 'D.id = L.group_id', 'D' )->where ( "L.site_id = '#site_id' AND L.group_id > 0 AND D.id IS NULL", array('site_id' => $site_id 
		) )->exec ();
		
		$count = \RS\Orm\Request::make ()->delete ()->from ( new Orm\Property\Item () )->where ( array('site_id' => $site_id 
		) )->where ( "id NOT IN (SELECT prop_id FROM $link_table WHERE site_id='$site_id')" )->exec ()->affectedRows ();
		
		$count += \RS\Orm\Request::make ()->delete ()->from ( new Orm\Property\Dir () )->where ( array('site_id' => $site_id 
		) )->where ( "id NOT IN (SELECT parent_id FROM $item_table WHERE site_id='$site_id')" )->exec ()->affectedRows ();
		return $count;
	}
	...
	{
		$values = \RS\Orm\Request::make ()->select ( 'DISTINCT val_str' )->from ( new Orm\Property\Link () )->where ( array('site_id' => \RS\SIte\Manager::getSiteId (),'prop_id' => $property_id 
		) )->orderby ( 'val_str' )->exec ()->fetchSelected ( 'val_str', 'val_str' );
		unset ( $values[''] );
		return $values;
	}
}
