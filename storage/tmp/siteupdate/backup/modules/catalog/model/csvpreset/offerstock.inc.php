...
class OfferStock extends \RS\Csv\Preset\AbstractPreset
{
	...
	protected $link_preset_id, $link_id_field, $link_foreign_field, $link_offer_id_field, $array_field = 'stock_num', $ormObject, $offer_api, $warehouse_api;
	function __construct($options)
	{
		$defaults = array('ormObject' => new \Catalog\Model\Orm\Xstock () 
		);
	...
	function loadWarehouses()
	{
		if (! isset ( self::$warehouses )) {
			self::$warehouses = \RS\Orm\Request::make ()->from ( new \Catalog\Model\Orm\WareHouse () )->where ( array('site_id' => \RS\Site\Manager::getSiteId () 
			) )->objects ( null, 'id' );
		}
	}
	...
	function getColumns()
	{
		...
		if (! empty ( self::$warehouses )) {
			foreach(self::$warehouses as $warehouse_id => $warehouse) {
				$columns[$this->id . '-offerstock_' . $warehouse_id] = array('key' => 'offerprice_' . $warehouse_id,'warehouse_id' => $warehouse_id,'title' => 'Остаток по складу "' . $warehouse['title'] . '"' 
				);
			}
		}
	...
	function loadData()
	{
		$this->row = array();
		if ($this->schema->ids) {
			$this->rows = \RS\Orm\Request::make ()->from ( $this->ormObject )->whereIn ( $this->link_foreign_field, $this->schema->ids )->objects ( null, $this->link_foreign_field, true );
		}
	}
}