...
class Property extends \RS\Csv\Preset\AbstractPreset
{
	...
	protected $delimiter = ";", $value_delimiter = "|", $id_field = 'id', $link_id_field, $link_preset_id, $mask, $mask_fields = array(), $mask_pattern, $title, $array_field = 'prop', $manylink_foreign_id_field = 'prop_id', $manylink_id_field = 'product_id', $manylink_orm;
	function __construct($options)
	{
		$defaults = array('ormObject' => new \Catalog\Model\Orm\Property\Item (),'manylinkOrm' => new \Catalog\Model\Orm\Property\Link (),'mask' => '({group_name}){property}:{value}','multisite' => true 
		);
	...
	function loadData()
	{
		$this->row = array();
		if ($this->schema->ids) {
			$this->rows = \RS\Orm\Request::make ()->from ( $this->manylink_orm )->whereIn ( $this->manylink_id_field, $this->schema->ids )->objects ( null, $this->manylink_id_field, true );
		}
	}
	function getColumns()
	{
		return array($this->id . '-properties' => array('key' => 'properties','title' => $this->title 
		) 
		);
	}
			...
			foreach($items as $item) {
				...
				if (preg_match ( $this->mask_pattern, $item, $match )) {
					...
					if (isset ( self::$index[$index_name] )) {
						...
						$result_array[$prop_id] = array('id' => $prop_id,'is_my' => 1,'value' => $match['value'] 
						);
					}
				}
			}
