...
{
	protected static $type_cost = array(), $type_cost_by_title = array(), $default_currency = null, // Валюта по умолчанию
$currencies = array(), $currencies_by_title = array();
	protected $delimiter = ';', $id_field, $link_preset_id, $link_id_field, $link_id_field_product, $sortn_field, $array_product_field, // Поле для сохранения цены основной комлектации у товара
$array_offer_field, // Поле для сохранения цены всех комплектаций, кроме нулевой
$manylink_orm, $orm_object;
	...
	function loadData()
	{
		...
		$this->row = array();
		if ($product_ids) {
			$this->row = \RS\Orm\Request::make ()->from ( $this->manylink_orm, 'X' )->whereIn ( $this->manylink_id_field, $product_ids )->objects ( null, $this->manylink_id_field, true );
		}
	}
