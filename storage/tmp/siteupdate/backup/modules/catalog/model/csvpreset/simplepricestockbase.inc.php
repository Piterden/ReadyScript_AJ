...
{
	protected $fields = array(), $select_request, $id_field = 'id', $saved_request = null, // Объект запроса из сессии с параметрами текущего просмотра списка
$select_order, $orm_product_object, $show_fields = array(), $extra_fields = array(), $titles = array(), $search_fields = array(), $load_expression, $is_multisite = false, $use_cache = true, $cache = array(), 

	$null_fields = array(), $orm_unset_fields = array(), $replace_mode = false, $use_temporary_id, $uniq_fields;
	...
	function loadObject()
	{
		...
		if (! $this->use_cache || ! isset ( $this->cache[$cache_key] )) {
			...
			if (! $object) {
				$product_sql = \RS\Orm\Request::make ()->select ( 'P.id' )->from ( $this->getOrmProductObject (), 'P' )->where ( $this->getSearchExpr () )->toSql ();
				
				$object = \RS\Orm\Request::make ()->from ( $this->getOrmObject (), 'OFFER' )->where ( 'OFFER.product_id = (' . $product_sql . ')' )->where ( array('OFFER.sortn' => 0 
				) )->object ();
			}
			if ($object) {
