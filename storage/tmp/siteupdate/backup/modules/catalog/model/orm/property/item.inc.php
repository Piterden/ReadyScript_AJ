...
class Item extends \RS\Orm\OrmObject
{
	...
	function _init()
	{
		parent::_init ()->append ( array('site_id' => new Type\CurrentSite (),'xml_id' => new Type\String ( array('maxLength' => '255','description' => t ( 'Идентификатор товара в системе 1C' ),'visible' => false 
		) ),'title' => new Type\String ( array('maxLength' => '255','index' => true,'description' => t ( 'Название характеристики' ),'checker' => array('chkEmpty','Название характеристики не должно быть пустым' 
		),'meVisible' => false 
		) ),'type' => new Type\String ( array('maxLength' => '10','description' => t ( 'Тип' ),'Attr' => array(array('size' => 1 
		) 
		),'ListFromArray' => array(array('int' => 'Число','string' => 'Строка','list' => 'Список','bool' => 'Да/Нет' 
		) 
		),'meVisible' => false 
		) ),'_tpl_' => new Type\Mixed ( array('description' => '','template' => '%catalog%/form/property/values.tpl','visible' => true,'meVisible' => false 
		) ),'values' => new Type\Text ( array('maxLength' => '2000','description' => t ( 'Возможные значения' ),'visible' => false 
		) ),'sortn' => new Type\Integer ( array('maxLength' => '11','description' => t ( 'Сорт. индекс' ),'visible' => false 
		) ),'parent_sortn' => new Type\Integer ( array('description' => t ( 'Сорт. индекс группы' ),'visible' => false 
		) ),'unit' => new Type\String ( array('maxLength' => '30','description' => t ( 'Единица измерения' ) 
		) ),'group_id' => new Type\Integer ( array('maxLength' => '11','runtime' => true,'visible' => false 
		) ),'product_id' => new Type\Integer ( array('maxLength' => '11','runtime' => true,'visible' => false 
		) ),

		'interval_from' => new Type\Float ( array('description' => t ( 'Минимальное значение' ),'Attr' => array(array('size' => 8 
		) 
		),'visible' => false,'runtime' => true 
		) ),'interval_to' => new Type\Float ( array('description' => t ( 'Максимальное значение' ),'Attr' => array(array('size' => 8 
		) 
		),'visible' => false,'runtime' => true 
		) ),'step' => new Type\Float ( array('description' => t ( 'Шаг' ),'Attr' => array(array('size' => 3 
		) 
		),'visible' => false,'runtime' => true 
		) ),

		'parent_id' => new Type\Integer ( array('description' => t ( 'Группа' ),'allowEmpty' => false,'List' => array(array('\Catalog\Model\PropertyDirApi','selectList' 
		) 
		) 
		) ),'hidden' => new Type\Integer ( array('description' => t ( 'Не отображать в карточке товара' ),'checkboxView' => array(1,0 
		),'maxLength' => 1,'default' => 0 
		) ),'value' => new Type\Mixed ( array('runtime' => true,'visible' => false 
		) ),'is_my' => new Type\Mixed ( array('runtime' => true,'visible' => false 
		) ),'public' => new Type\Integer ( array('runtime' => true,'visible' => false 
		) ),'useval' => new Type\Integer ( array('runtime' => true,'visible' => false 
		) ),'allowed_values' => new Type\ArrayList ( array('visible' => false 
		) ) 
		) );
	...
	function beforeWrite($flag)
	{
		if ($flag == self::INSERT_FLAG) {
			$this['sortn'] = \RS\Orm\Request::make ()->select ( 'MAX(sortn) as max' )->from ( $this )->exec ()->getOneField ( 'max', 0 ) + 1;
		}
		
		if ($this['parent_id'] !== null) {
			if ($this['parent_id'] > 0) {
				$parent_sortn = \RS\Orm\Request::make ()->select ( 'sortn' )->from ( new Dir () )->where ( array('id' => $this['parent_id'] 
	...
	function delete()
	{
		...
		\RS\Orm\Request::make ()->delete ()->from ( new Link () )->where ( array('prop_id' => $this['id'] 
		) )->exec ();
	...
	function sliderData()
	{
		if (! isset ( $this->slider_data )) {
			...
			$this->slider_data = array('scale' => implode ( ',', $scale ),'heterogeneity' => implode ( ',', $heterogeneity ),'delta' => $delta,'points' => $points 
			);
		}
	...
	function getUrl($filter_var, $value)
	{
		...
		return $url->replaceKey ( array($filter_var => array($this['id'] => $my_filter 
		) + $filter 
		) );
	}
