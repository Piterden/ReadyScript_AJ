...
/**
 * Класс предоставляет возможность встраивать диалоги выбора товаров и групп товаров в админ.
 * панель.
 */
class ProductDialog
{
	protected $fieldname, $hide_group_checkbox, $hide_product_checkbox, $data, $template = '%catalog%/dialog/view_selected.tpl';
	...
	function getHtml()
	{
		...
		if (! empty ( $this->data['product'] )) {
			...
			$list = $productapi->setFilter ( 'id', $this->data['product'], 'in' )->loadAssocList ( 'id' );
			
			$extdata_products = array();
			foreach($list as $id => $product) {
				$extdata['product'][$id] = array('obj' => $product,'dirs' => isset ( $product_dirs[$id] ) ? ',' . implode ( ',', $product_dirs[$id] ) . ',' : '' 
				) // Раскладываем id в строку (тех.данные - необходимо для JavaScript)
;
			}
		}
		...
		if (! empty ( $this->data['group'] )) {
			...
			foreach($this->data['group'] as $dir_id) {
				...
				$extdata['group'][$dir_id] = array('parents' => ($dir_id == 0) ? '' : $parents,'obj' => ($dir_id == 0) ? array('name' => 'Все' 
				) : $dirapi->getOneItem ( $dir_id ) 
				);
			}
		}
		
		$view = new \RS\View\Engine ();
		$view->assign ( \RS\Module\Item::getResourceFolders ( $this ) )->assign ( array('hide_group_checkbox' => $this->hide_group_checkbox,'hide_product_checkbox' => $this->hide_product_checkbox,'fieldName' => $this->fieldname,'productArr' => $this->data,'extdata' => $extdata 
		) );
