...
class Dirapi extends \RS\Module\AbstractModel\TreeCookieList
{
	...
	{
		parent::__construct ( new \Catalog\Model\Orm\Dir (), array('multisite' => true,'parentField' => 'parent','sortField' => 'sortn','idField' => 'id','aliasField' => 'alias','nameField' => 'name','defaultOrder' => 'sortn' 
		) );
	}
	 ...
	 * @return array
	 */
	function getAllCategory($cache = true)
	{
		$query_hash = crc32 ( $this->queryObj ()->toSql () );
		
		if ($cache) {
			list ( $this->cat, $this->catByParent, $this->catByAlias, $this->catByAliasParent )

			 = \RS\Cache\Manager::obj ()->watchTables ( $this->obj_instance )->request ( array($this,'getAllCategory' 
			), false, $query_hash );
			
			$this->afterloadList ();
			return $this->cat;
		} else {
			parent::getAllCategory ();
			return array($this->cat,$this->catByParent,$this->catByAlias,$this->catByAliasParent 
			);
		}
	}
	public function listWithAll()
	{
		...
		foreach($treedata as &$item) {
			if ($item['fields']['is_spec_dir'] == 'Y') {
				$item['fields']['treeTools'] = new \RS\Html\Table\Type\Actions ( 'id', array(new \RS\Html\Table\Type\Action\Edit ( $router->getAdminPattern ( 'edit_dir', array('id' => $item['fields']['id'] 
				) ), null, array('attr' => array('@data-id' => '@id' 
				) 
				) ),new \RS\Html\Table\Type\Action\DropDown ( array(array('title' => t ( 'показать на сайте' ),'attr' => array('@href' => $router->getUrlPattern ( 'catalog-front-listproducts', array('category' => $item['fields']['alias'] 
				) ),'target' => 'blank' 
				) 
				),array('title' => t ( 'удалить' ),'attr' => array('@href' => $router->getAdminPattern ( 'del_dir', array(':chk[]' => $item['fields']['id'] 
		...
		array_unshift ( $treedata, array('fields' => array('id' => 0,'name' => 'Все',
				// Устанавливаем собственные инструменты
				'treeTools' => new \RS\Html\Table\Type\Actions ( 'id', array(new \RS\Html\Table\Type\Action\Edit ( \RS\Router\Manager::obj ()->getAdminPattern ( 'edit_dir', array('id' => 0 
				) ) ) 
				) ),'noDraggable' => true,'noCheckbox' => true,'noRedMarker' => true 
		),'child' => null 
		) );
		return $treedata;
	}
	...
	function loadParents($list)
	{
		...
		if (count ( $parent_ids )) {
			$parents = \RS\Orm\Request::make ()->from ( $this->obj_instance )->whereIn ( 'id', $parent_ids )->objects ( null, 'id' );
	...
	static function selectList($include_root = true)
	{
		...
		return ($include_root ? array(0 => 'Верхний уровень' 
		) : array()) + $_this->getSelectList ( 0 );
	}
	...
	public static function specSelectList($include_no_select = false)
	{
		...
		return ($include_no_select ? array(0 => 'Не выбрано' 
		) : array()) + $_this->getSelectList ( 0 );
	}
