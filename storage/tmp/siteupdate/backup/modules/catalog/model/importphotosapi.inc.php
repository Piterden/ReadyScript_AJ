...
{
	protected $data = array(), $photo_api, $timeout, $allow_ext = array('png','jpg','gif' 
	), $zip_name = 'photos.zip', $csv_name = 'files.csv', $log_rel = '/storage/tmp/importphotos/log.txt', $zip_folder, $zip_folder_rel = '/storage/tmp/importphotos', $extract_folder, $extract_folder_rel = '/storage/tmp/importphotos/unpack', $touch_products = array(), $csv_file, $csv_file_rel = '/storage/tmp/importphotos/unpack/names.csv', $names_replace = array(), $tmp_data_file = '/data.srz';
	...
	function uploadFile($file_arr)
	{
		...
		$uploader = new \RS\File\Uploader ( array('zip' 
		), $this->zip_folder_rel );
	...
	function extractFile($start_pos = 0)
	{
		...
		if ($zip->open ( $zip_filename ) === true) {
			...
			for($i = $start_pos; $i < $zip->numFiles; $i ++) {
				...
				if (substr ( $file, - 1 ) == '/' || in_array ( strtolower ( substr ( $file, - 3 ) ), $this->allow_ext )) {
	...
	{
		$this->setParam ( null, array('import_done_percent' => 0,'zip_done_percent' => 0 
		), true );
	}
	...
	function importPhoto($start_pos = 0, $field, $separator)
	{
		...
		if ($start_pos == 0) {
			$this->loadNamesCsv ();
			$this->setParam ( 'statistic', array('touch_images' => 0,'touch_products' => 0,'images_imported' => 0,'no_match_images' => 0 
			) );
		...
		$dir = $this->extract_folder;
		$current_pos = $this->importRecursive ( $dir, 0, $start_pos, array('field' => $field,'separator' => $separator 
		), microtime ( true ) );
		...
		$this->setParam ( 'statistic', array('touch_products' => count ( $this->getParam ( 'touch_products' ) ) 
		) + $this->getParam ( 'statistic' ) );
		return $result;
	}
	...
	private function importPhotoFile($dir, $file, $params)
	{
		...
		if (in_array ( strtolower ( $ext ), $this->allow_ext )) {
			...
			if (isset ( $this->names_replace[$search_term] )) $search_term = $this->names_replace[$search_term];
			
			$products_ids = $this->searchProducts ( $search_term, $file, $params['field'] );
			if ($products_ids) {
	...
	{
		$q = \RS\Orm\Request::make ()->select ( 'P.id' )->from ( new Orm\Product (), 'P' )->where ( array('P.site_id' => \RS\Site\Manager::getSiteId () 
		) );
		
		switch ($field) {
			case 'maindir' :
				$q->join ( new Orm\Dir (), 'D.id = P.maindir', 'D' )->where ( array('D.name' => $term 
				) );
				break;
			case 'brand_id' :
				$q->join ( new Orm\Brand (), 'B.id = P.brand_id', 'B' )->where ( array('B.title' => $term 
				) );
				break;
			default :
				$q->where ( array($field => $term 
				) );
		}
		...
		if ($found) {
			...
			$exclude = \RS\Orm\Request::make ()->from ( new \Photo\Model\Orm\Image () )->where ( array('filename' => $file,'type' => 'catalog' 
			) )->whereIn ( 'linkid', $found )->exec ()->fetchSelected ( 'linkid', 'linkid' );
