...
{
	protected $property_api, $properties = array(), $abstract_tpl = '%catalog%/filter/property_filter.tpl';
	...
	/**
	 * Не возвращает условие для выборки, т.к.
	 * это делает modificateQuery
	 * 
	...
	public function getParts($current_filter_values)
	{
		...
		foreach($filters as $prop_id => $value) {
			...
			$parts[] = array('title' => $property->title,'value' => $value,'href_clean' => \RS\Http\Request::commonInstance ()->replaceKey ( array($this->wrap_var => $without_this 
			) ) 
