...
class Install extends \RS\Module\AbstractInstall
{
	function install()
	{
		...
		if ($result) {
			...
			\Catalog\Config\Handlers::onSiteCreate ( array('orm' => \RS\Site\Manager::getSite (),'flag' => \RS\Orm\AbstractObject::INSERT_FLAG 
			) );
			
			// Добавляем виджеты на рабочий стол
			$widget = new \Main\Model\Orm\Widgets ();
			
			// Виджет Что смотрят сейчас на сайте
			$widget->getFromArray ( array('user_id' => 1,'col' => 'left','position' => 0,'class' => 'catalog-widget-watchnow' 
			) )->insert ();
		}
	...
	{
		return $this->importCsvFiles ( array(array('\Catalog\Model\CsvSchema\Brand','brand' 
	...
	function deferredAfterInstall($options)
	{
		if ($options['set_demo_data']) {
			...
			if ($site_config->getThemeName () == 'default') {
				...
				\Templates\Model\PageApi::setupModule ( 'main.index', 'catalog\controller\block\topproducts', array('dirs' => $top_dir_id 
				) );
				
				// Настраиваем блок товары в виде баннера
				\Templates\Model\PageApi::setupModule ( 'main.index', 'catalog\controller\block\bannerview', array('categories' => array($dir_api->getIdByAlias ( 'popular' ),$dir_api->getIdByAlias ( 'newest' ) 
				) 
				) );
				
				// Настраиваем спецкатегорию, которую выводим в брендах
				$actual_dir = \Catalog\Model\Orm\Dir::loadByWhere ( array('alias' => 'actual' 
				) );
				if ($actual_dir['id']) {
