...
class ListProducts extends \RS\Controller\Front
{
	...
	public $config, $query, $page, $pageSize, $search_type, $can_rank_sort, $items_on_page, $cur_sort, $cur_n_sort, $api, $dirapi;
	function init()
	{
		...
		$this->app->headers->addCookie ( 'viewAs', $this->view_as, $cookie_expire, $cookie_path )->addCookie ( 'pageSize', $this->pageSize, $cookie_expire, $cookie_path )->addCookie ( 'sort', $this->cur_sort, $cookie_expire, $cookie_path )->addCookie ( 'nsort', $this->cur_n_sort, $cookie_expire, $cookie_path );
		...
		if (($dir_id !== false) || ($dir === "0" && $this->config['show_all_products']) || $this->url->isKey ( 'query' )) {
			if ($this->query != '' || $dir_id || ($this->config['show_all_products'] && ! $this->url->isKey ( 'query' ))) { // Если есть категория или задан поисковый запрос или флаг отображать /catalog/
				if ($debug_group = $this->getDebugGroup ()) {
					$create_href = $this->router->getAdminUrl ( 'add', array('dir' => $dir_id 
					), 'catalog-ctrl' );
...
'dateof' => 'desc','title' => 'asc','cost' => 'asc','rating' => 'desc','rank' => 'desc','num' => 'desc','barcode' => 'asc' 
				);
				...
				if ($config['price_like_slider']) {
					...
					$money_array += array('step' => 1,'round' => 1,'unit' => \Catalog\Model\CurrencyApi::getDefaultCurrency ()->stitle,'heterogeneity' => $this->api->getHeterogeneity ( $money_array['interval_from'], $money_array['interval_to'] ) 
					);
				...
				if ($category['meta_title'] || $category['meta_keywords'] || $category['meta_description']) { // Если задано мета описание
					...
					$seoGen = new \Catalog\Model\SeoReplace\Dir ( array($category 
					) );
				...
				$this->view->assign ( array('query' => $this->query,'can_rank_sort' => $this->can_rank_sort,'dirapi' => $this->dirapi,'path' => $path,'dir' => $dir,'category' => $category,'sub_dirs' => $sub_dirs,'dir_id' => $dir_id,'cur_sort' => $this->cur_sort,'cur_n' => $this->cur_n_sort,'sort' => $sort,'total' => $total,'list' => $list,'view_as' => $this->view_as,'paginator' => $paginator,'prop_list' => $prop_list,'page_size' => $this->pageSize,'items_on_page' => $this->items_on_page,'filter' => $prop_api->cleanNoActiveFilters ( $filters ),'bfilter' => $this->api->getBaseFilters (),'is_filter_active' => ($prop_api->isFilterActive () || $basefilter),'clean_filter_url' => $prop_api->getCleanFilterUrl () 
				) );
			} else {
				$this->view->assign ( array('list' => array(),'no_query_error' => true 
				) );
			}
		} else {
