...
{
	protected $id, $api, $dirapi, $lastpage;
	function init()
	{
	...
	function actionIndex()
	{
		...
		if ($debug_group = $this->getDebugGroup ()) {
			$edit_href = $this->router->getAdminUrl ( 'edit', array('id' => $item['id'] 
			), 'catalog-ctrl' );
		...
		foreach($path as $one_dir) {
			$seoGen = new \Catalog\Model\SeoReplace\Dir ( array($one_dir 
			) );
		...
		$seoGen = new \Catalog\Model\SeoReplace\Product ( array($item,'cat_' => $dir_obj,'price' => $item->getCost () 
		) );
