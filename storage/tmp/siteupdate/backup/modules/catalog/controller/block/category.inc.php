...
{
	protected static $controller_title = 'Список категорий', $controller_description = 'Отображает список категорий в боковой колонке';
	public $api;
	protected $pathids = array(), $default_params = array('indexTemplate' => 'blocks/category/category.tpl','root' => 0 
	);
	function getParamObject()
	{
		return parent::getParamObject ()->appendProperty ( array('root' => new Type\String ( array('description' => t ( 'Корневая директория' ),'list' => array(array('\Catalog\Model\Dirapi','selectList' 
		) 
		) 
		) ) 
		) );
	...
	function actionIndex()
	{
		...
		if ($route && $route->getId () == 'catalog-front-listproducts') {
			...
			$category = $this->api->getBreadcrumbDir ();
			if (! $category) {
		...
		if (! $this->view->cacheOn ( $cache_id )->isCached ( $this->getParam ( 'indexTemplate' ) )) {
			...
			$this->view->assign ( array('dirlist' => $this->api->getTreeList ( $root_id ),'current_item' => $item,'pathids' => $this->pathids 
			) );
		}
