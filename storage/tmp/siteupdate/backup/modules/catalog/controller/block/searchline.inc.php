...
{
	protected static $controller_title = 'Поиск товаров по названию', $controller_description = 'Отображает форму для поиска товаров по ключевым словам';
	protected $action_var = 'sldo', $default_params = array('searchLimit' => 5,'hideAutoComplete' => 0,'indexTemplate' => 'blocks/searchline/searchform.tpl' 
	);
	function getParamObject()
	{
		return parent::getParamObject ()->appendProperty ( array('hideAutoComplete' => new Type\Integer ( array('description' => t ( 'Отключить подсказку результатов поиска в выпадающем списке' ),'checkboxView' => array(1,0 
		) 
		) ),'searchLimit' => new Type\Integer ( array('description' => t ( 'Количество результатов в выпадающем списке' ) 
		) ) 
		) );
	}
	...
	function actionAjaxSearchItems()
	{
		...
		if (! empty ( $query )) {
			...
			foreach($list as $product) {
				...
				$result_json[] = array('value' => $product->title,'label' => preg_replace ( "%($query)%iu", '<b>$1</b>', $product->title ),'barcode' => preg_replace ( "%($query)%iu", '<b>$1</b>', $product->barcode ),'image' => $product->getMainImage ()->getUrl ( 62, 62, "xy" ),'price' => $price,'url' => $product->getUrl () 
				);
			}
		}
