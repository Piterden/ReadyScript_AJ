...
{
	const LOG_TYPE_CLASS = 'Catalog\Model\Logtype\ShowProduct';
	protected $info_title = 'Что смотрят на сайте', $info_description = 'Список товаров, которые пользователи смотрят прямо сейчас', 
	...
	function actionIndex()
	{
		...
		foreach($list as $n => $event) {
			...
			$user_href = ($user['id'] > 0) ? $this->router->getAdminUrl ( 'edit', array('id' => $user['id'] 
			), 'users-ctrl' ) : false;
			...
			$path_href = $this->router->getAdminUrl ( false, array('dir' => $product['maindir'] 
			), 'catalog-ctrl' );
			
			$eventTime = strtotime ( $event->getEventDate () );
			$item = array('id' => $product['id'],'editUrl' => $this->router->getAdminUrl ( 'edit', array('id' => $product['id'] 
			), 'catalog-ctrl' ),'eventDate' => date ( \RS\Helper\Tools::dateExtend ( '%k, H:i', $eventTime ), $eventTime ),'user' => array('href' => $user_href,'name' => $user_name 
		...
		$this->view->assign ( array('list' => $data,'offset' => $offset,'total' => $total 
		) );
