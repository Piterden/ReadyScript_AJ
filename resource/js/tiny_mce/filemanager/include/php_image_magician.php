			...
			} else // More subtle and personally more desirable
{
				...
				$sharpenMatrix = array(array(- 1,- 2,- 1 
...
array(- 1,- 2,- 1 
	...
	{
		$sharpenMatrix = array(array($level,$level,$level 
...
array($level,$level,$level 
	...
	//
	{
		...
		for($i = 0; $i <= STEPS; $i ++) {
			...
			$points = array($blurWidth * $t,$blurHeight, // Point 1 (x, y)
...
;
	...
	// Purpose: Convert #hex color to RGB
	{
		...
		$rgb = array('r' => hexdec ( substr ( $color, 0, 2 ) ),'g' => hexdec ( substr ( $color, 2, 2 ) ),'b' => hexdec ( substr ( $color, 4, 2 ) ),'a' => 0 
		);
