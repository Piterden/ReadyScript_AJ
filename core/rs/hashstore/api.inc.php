...
class Api extends \RS\Orm\AbstractObject
{
	...
	function _init()
	{
		...
		$this->getPropertyIterator ()->append ( array('hash' => new Type\Bigint ( array('description' => t ( 'Хэш ключа' ),'maxLength' => 12,'unsigned' => true,'primaryKey' => true 
		) ),'value' => new Type\String ( array('description' => t ( 'Значение для ключа' ),'maxLength' => 4000 
		) ) 
		) );
	}
