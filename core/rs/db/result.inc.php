...
class Result
{
	public $auto_free = true; // Автоматически освобождать память после получения результатов запроса.
	protected /**
	 *
	 * @var mysqli_result
	 */
	$resource, $res_type = MYSQLI_ASSOC;
	...
	function fetchSelected($key, $value = null, $allow_sublist = false)
	{
		...
		while ( $row = $this->fetchRow () ) {
			if ($value === null)
				$val = $row;
			else $val = $is_array ? array_intersect_key ( $row, $value ) : $row[$value];
			if (is_null ( $key ))
				$tmp[] = $val;
			else if ($allow_sublist)
				$tmp[$row[$key]][] = $val;
			else $tmp[$row[$key]] = $val;
		}
		return $tmp;
	}
