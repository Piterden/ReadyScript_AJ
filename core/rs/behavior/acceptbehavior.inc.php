...
abstract class AcceptBehavior implements AcceptBehaviorInterface
{
	 ...
	 * 
	 * @return [ 'класс библиотеки подключения' => тип подключения (class | instance),
	 *         ...
	 *         ]
	 */
	...
	{
		$behaviors = $this->instance_behaviors + (isset ( self::$class_behaviors[get_called_class ()] ) ? self::$class_behaviors[get_called_class ()] : array());
		...
		foreach($behaviors as $class => $behavior_template) {
			if (is_callable ( array($behavior_template,$method_name 
				...
				return call_user_func_array ( array($this->cache_behavior_instances[$class],$method_name 
				), $arguments );
			}
		}
