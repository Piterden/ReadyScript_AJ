...
{
	public $config, $cms_config, $theme, $theme_settings, $site;
	function __construct()
	{
		...
		$this->assign ( array('Setup' => \Setup::varsAsArray (),'app' => \RS\Application\Application::getInstance (),'router' => \RS\Router\Manager::obj (),'url' => \RS\Http\Request::commonInstance (),'CONFIG' => $this->config,'CMS_CONFIG' => $this->cms_config,'SITE' => $this->site,'THEME_CSS' => \Setup::$SM_RELATIVE_TEMPLATE_PATH . '/' . $this->theme['theme'] . \Setup::$RES_CSS_FOLDER,'THEME_JS' => \Setup::$SM_RELATIVE_TEMPLATE_PATH . '/' . $this->theme['theme'] . \Setup::$RES_JS_FOLDER,'THEME_IMG' => \Setup::$SM_RELATIVE_TEMPLATE_PATH . '/' . $this->theme['theme'] . \Setup::$RES_IMG_FOLDER,'THEME_SHADE' => $this->theme['shade'],'THEME_SETTINGS' => $this->theme_settings 
		) );
	}
	...
	function getThemeSettings($cache = true)
	{
		if (! \RS\Router\Manager::obj ()->isAdminZone ()) {
			...
			if ($cache) {
				...
				), false, $this->site ? $this->site->id : '', $theme_str );
			} else {
