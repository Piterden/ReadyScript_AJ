...
namespace RS\Language
{
	...
	class Core
	{
		const COOKIE_ADMIN_LANG = 'admin_lang', COOKIE_CUSTOMER_LANG = 'customer_lang', DEFAULT_BASE_LANG = 'ru', DEFAULT_LANG = 'ru';
		public static $cache_current_lang, $cache_js_folder = CACHE_LANG_FOLDER, $lang_folder = '/lang', $mod_lang_folder = '/view/lang', $theme_lang_folder = '/resource/lang', $lang_php_file = 'messages.lng.php', $lang_js_file = 'messages.js.php', $messages = array(), // Сообщения текущей локали для PHP
$messages_js = array(), // Сообщения текущей локали для JS
$messages_js_lastmodify;
		private static $params, $translated;
		...
		{
			if ($cache_enabled) {
				list ( self::$messages, self::$messages_js, self::$messages_js_lastmodify ) = CacheManager::obj ()->expire ( 0 )->tags ( CACHE_TAG_MODULE )->request ( array(__CLASS__,'init' 
				), false, self::getCurrentLang () );
			} else {
				...
				return array(self::$messages,self::$messages_js,self::$messages_js_lastmodify 
				);
			}
		}
		...
		{
			if ($cache_enabled) {
				return CacheManager::obj ()->expire ( 0 )->request ( array(__CLASS__,'getSystemLanguages' 
				), false );
			} else {
				$folder_list = glob ( self::getSystemLangFolder () . '/*', GLOB_ONLYDIR );
				$locale_list = array(strtolower ( self::getBaseLang () ) => ucfirst ( self::getBaseLang () ) 
				);
		...
		public static function translate($phrase, $params = array(), $alias = null)
		{
			...
			$phrase = preg_replace_callback ( '/(\[(.*?):%(.*?):(.*?)\])/', array(__CLASS__,'execPlugin' 
			), $phrase );
...
namespace 
{
	...
	/**
	 * Возвращает фразу в текущей локали.
