...
abstract class AbstractSetup
{
	...
	// Общие параметры
	$VERSION = '2.0.1.196', // Текущая версия ядра
...
$ZONE, // Текущая зона домена, например: com
	...
	$DB_HOST = 'localhost', $DB_NAME = ' ', $DB_USER = ' ', $DB_PASS = ' ', $DB_CHARSET = 'utf8', $DB_AUTOINIT = true, $DB_INSTALL_MODE = true, $DB_TABLE_PREFIX = '', 
...
$SM_TEMPLATE_PATH, $SM_COMPILE_PATH = '/cache/smarty/compile', // Папка для компилированных шаблонов
...
$SM_COMPILE_CHECK = false, // Если true, то перекомпилировать шаблоны по мере надобности. (Для разработки необходимо указывать true, на продакшн сервере - false)
	...
	$CHECK_DOMAIN_TIMEOUT = 4, $CHECK_LICENSE_SERVER = 'http://update.readyscript.ru/checklicense/', // URL для запросов проверки лицензии
$BUY_LICENSE_URL = 'http://readyscript.ru/product/{script_type}/', // URL страницы для покупки лицензии
$UPDATE_URL = 'http://update.readyscript.ru/update/', // URL для запросов на обновление системы
$UPDATE_CHANNEL = 'release', // Канал обновления
	...
	$INTERNAL_ENCODING = 'UTF-8', $LOCALE = 'ru_RU.UTF-8', $NUMERIC_LOCALE = 'en_US.UTF-8', 
	
	// Пути к общим ресурсам
	$RESOURCE_PATH = '/resource', $JS_PATH = '/resource/js', $CSS_PATH = '/resource/css', $IMG_PATH = '/resource/img', 
...
$DEFAULT_THEME = 'default', // Тема, которая будет установлена сразу после инсталяции системы
	...
	$SCRIPT_TYPE, $MODULE_WATCH_TPL = '/moduleview', // путь к альтернативной папке шаблонов модулей
...
$LOG_EXECUTE_FILE = '/logs/exectime.log', $LOG_SQLQUERY_TIME = false, // Записывает все запросы к базе и время их выполнения. false - не записывать. Рекомендуется включать исключительно на время отладки
...
$TMP_DIR, $DOCTYPE = 'HTML PUBLIC "-//W3C//DTD HTML 4.01//EN" "http://www.w3.org/TR/html4/strict.dtd"', 
...
$NOPHOTO_THEME_FILE = 'nophoto.jpg', 
	...
	protected static 
	// Подключаемые модули
	$include_list = array('/core/system/autoload.inc.php','/core/system/constants.inc.php','/core/system/exceptions.inc.php','/core/smarty/Smarty.class.php','/core/csstidy/class.csstidy.php' 
	);
	...
	public static function init()
	{
		...
		if (! empty ( $_SERVER["REMOTE_ADDR"] )) {
			...
			if (class_exists ( '\Main\Model\BlockedIpApi' ) && \Main\Model\BlockedIpApi::isIpBanned ( $_SERVER["REMOTE_ADDR"] )) {
