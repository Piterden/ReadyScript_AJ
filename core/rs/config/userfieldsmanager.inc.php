...
{
	private $field, $list_delimiter = ',', $admin_tpl = '%SYSTEM%/admin/userfields.tpl', $tpl = '%SYSTEM%/coreobject/userform.tpl', $arr_wrap, $structure, $error_prefix = '', $errors = array(), $values = array();
	...
	/**
	 * Устанавливает поле в котором у объекта хранятся сведения о доп.
	 * полях
	 * 
	...
	/**
	 * Возвращает поле, в котором у объекта хранятся сведения о доп.
	 * полях
	 * 
	 * @return string
	 */
	...
	{
		if ($data !== null) $this->setValues ( $data );
		
		$this->errors = array();
		foreach($this->structure as $key => $item) {
			if ($item['necessary']) {
				if (empty ( $this->values[$key] )) {
					$this->errors[$key] = "Поле '{$item['title']}' является обязательным";
				}
			}
		}
	...
	function getForm($key)
	{
		...
		if ($this->arr_wrap !== null) $fld['fieldname'] = $this->arr_wrap . '[' . $fld['alias'] . ']';
		
		$tpl->assign ( array('fld' => $fld,'values' => $this->values,'has_error' => isset ( $this->errors[$key] ) 
		) );
	...
	function textView($key)
	{
		if ($this->structure[$key]['type'] == 'bool') {
			return empty ( $this->values[$key] ) ? 'Нет' : 'Да';
		} else {
	...
	function getAdminForm($before_phrase = '')
	{
		...
		$view->assign ( array('before_phrase' => $before_phrase,'manager' => $this 
		) );
