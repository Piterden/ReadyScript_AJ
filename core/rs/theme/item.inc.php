...
use Templates\Model as TemplatesModel, Templates\Model\Orm\SectionContext;
...
class Item
{
	protected $preview_file = 'preview{SHADE}.jpg', $template_path = SM_TEMPLATE_PATH, // Берется из \Setup;
$selfpath, $simplexml, $blocks_xml, $relative_self_path, $shade, $name, $context, $fullname;
	...
	function removeContext($context = null, $site_id = null)
	{
		$context = $context ?  : $this->getContext ();
		if ($context != 'theme') {
			return \RS\Orm\Request::make ()->delete ()->from ( new TemplatesModel\Orm\SectionPage () )->where ( array('site_id' => $site_id ?  : \RS\Site\Manager::getSiteId (),'context' => $context 
			) )->exec ();
		}
	}
	...
	function getContextOptions($site_id = null, \SimpleXMLElement $blocks_xml = null)
	{
		...
		$search_data = array('site_id' => $site_id ?  : \RS\Site\Manager::getSiteId (),'context' => $this->getContext () 
		);
		...
		if (! $context_options['context'] || $blocks_xml) {
			...
			$context_options->getFromArray ( $search_data + array('grid_system' => $grid_system 
			) );
		}
	...
	function resetContextOptions($site_id = null, $blocks_xml = null)
	{
		if ($blocks_xml === false) {
			...
			\RS\Orm\Request::make ()->delete ()->from ( new SectionContext () )->where ( array('site_id' => $site_id ?  : \RS\Site\Manager::getSiteId (),'context' => $this->getContext () 
			) )->exec ();
		} else {
