...
class Manager extends \RS\Module\AbstractModel\BaseModel
{
	...
	protected $tmp_folder = '/storage/tmp/theme', $allow_theme_mime = array('application/zip','application/x-zip-compressed' 
	 ...
	 * 
	 * @return [ ['title' => НАЗВАНИЕ КОНТЕКСТА, 'theme' => 'НАЗВАНИЕ ТЕМЫ, К КОТОРОЙ ОН ПРИНАДЛЕЖИТ']
	 *         ]
	 */
	public static function getContextList()
	{
		$event_result = \RS\Event\Manager::fire ( 'theme.getcontextlist', array() );
		$result = array('theme' => array('title' => t ( 'Основная тема' ),'theme' => \RS\Config\Loader::getSiteConfig ()->getThemeName () 
		) 
	...
	{
		$result = array('theme' => $theme_str,'shade' => '','blocks_context' => 'theme', // Контекст блоков по-умолчанию
'full_name' => $theme_str 
		);
