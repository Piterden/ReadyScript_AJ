...
class Request
{
	...
	private $get, $post, $request, $cookie, $server, $files, $parsed_url, $router_defaults, $session_save_url_var = 'SAVEDURL_';
	...
	public function getSource($storage)
	{
		switch ($storage) {
			...
			default :
				throw new \RS\Exception ( 'Неизвестный Storage' );
		}
	}
	...
	protected function _returner(array $src, $keys, $type, $default, $strip)
	{
		...
		foreach($_keys as $key) {
			...
			if ($isset_key || $default === null) {
				if ($isset_key && $strip !== false && $type != TYPE_ARRAY) {
	...
	/**
	 * Возвращает доменное имя, в кодировке utf8.
	 * В том числе и интернациональные домены
	 * 
	 * @return string
	 */
