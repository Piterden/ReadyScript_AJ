...
class Manager
{
	protected static $adminSite, $defaultSite, $site, $siteId;
	...
	public static function getSiteByUrl($host, $uri, $cache = true)
	{
		...
		foreach($sites as $site) {
			...
			if (in_array ( strtolower ( $host ), $domains )) { // Проверяем соответствие домену
	...
	{
		if ($cache) {
			return \RS\Cache\Manager::obj ()->tags ( CACHE_TAG_SITE )->request ( array(__CLASS__,'getSiteList' 
			), false );
		} else {
			$result = array();
			
			try {
				$result = \RS\Orm\Request::make ()->from ( new \Site\Model\Orm\Site () )->orderby ( 'folder DESC' )->objects ();
