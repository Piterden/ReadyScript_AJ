...
class PHPMailer
{
			...
			foreach($list as $address) {
				...
				if (strpos ( $address, '<' ) === false) {
					...
					if ($this->validateAddress ( $email )) {
						$addresses[] = array('name' => trim ( str_replace ( array('"',"'" 
						), '', $name ) ),'address' => $email 
						);
					}
				}
	...
	protected function sendmailSend($header, $body)
	{
		...
		if ($this->SingleTo) {
			foreach($this->SingleToArray as $toAddr) {
				...
				fputs ( $mail, $body );
				$result = pclose ( $mail );
				$this->doCallback ( ($result == 0), array($toAddr 
	...
	protected function smtpSend($header, $body)
	{
		...
		foreach(array($this->to,$this->cc,$this->bcc 
		) as $togroup) {
		...
		if ($this->isError ()) {
			...
			try {
				...
				if (empty ( $this->sign_extracerts_file )) {
					$sign = @openssl_pkcs7_sign ( $file, $signed, 'file://' . realpath ( $this->sign_cert_file ), array('file://' . realpath ( $this->sign_key_file ),$this->sign_key_pass 
					), null );
				} else {
					$sign = @openssl_pkcs7_sign ( $file, $signed, 'file://' . realpath ( $this->sign_cert_file ), array('file://' . realpath ( $this->sign_key_file ),$this->sign_key_pass 
...
6 => $disposition,7 => 0 
	...
	public function encodeQP($string, $line_max = 76)
	{
		...
		$string = str_replace ( array('%20','%0D%0A.','%0D%0A','%' 
		), array(' ',"\r\n=2E","\r\n",'=' 
		), rawurlencode ( $string ) );
		return preg_replace ( '/[^\r\n]{' . ($line_max - 3) . '}[^=\r\n]{2}/', "$0=\r\n", $string );
	}
