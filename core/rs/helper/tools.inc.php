...
class Tools
{
	 ...
	 * @return s string Строка содержащая обрезанный текст
	 */
	...
	public static function sendEmailSimple($subject, $email, $email_tpl, $data)
	{
		...
		self::sendEmail ( $subject, $system_config->getNoticeFrom ( false ), $system_config->getNoticeReply ( false ), $system_config->getNoticeFrom ( true ), $system_config->getNoticeReply ( true ), $email, $email_tpl, $data );
	}
	 ...
	 * %v на месяц на русском языке в род.
	 * падеже. например: мая, июня
	 * 
	 * @param mixed $format
	...
	public static function dateExtend($format, $timestamp)
	{
		...
		if (strpos ( $format, '%k' ) !== false) {
			...
			if ($tdate == date ( 'j-n-Y' ))
				$format = str_replace ( '%k', t ( 'сегодня' ), $format );
			elseif ($tdate == date ( 'j-n-Y', strtotime ( '-1 day' ) ))
				$format = str_replace ( '%k', t ( 'вчера' ), $format );
		...
		if (strpos ( $format, '%v' ) !== false) {
			$months = array(1 => t ( 'января' ),t ( 'февраля' ),t ( 'марта' ),t ( 'апреля' ),t ( 'мая' ),t ( 'июня' ),t ( 'июля' ),t ( 'августа' ),t ( 'сентября' ),t ( 'октября' ),t ( 'ноября' ),t ( 'декабря' ) 
			);
	...
	public static function priceToString($price)
	{
		...
		$cur = t ( '[plural:%0:рубль|рубля|рублей]', array($price 
		) ); // Плюрал по рублям
