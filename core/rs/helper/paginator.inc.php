...
class Paginator
{
	const PATTERN_TEMPLATE = - 2, // Заменять в %PAGE% на номе страницы в строке $pattern
PATTERN_KEYREPLACE = - 1; // Заменять параметр $pattern в строке GET
	protected $paginator_len = 9, $pattern, $pattern_type, $route_params, $pagelist = null;
	public $total, $page, $per_page, $total_pages;
	...
	{
		$this->pagelist[] = array('n' => $page,'act' => ($page == $this->page),'class' => $class,'href' => $this->getPageHref ( $page ) 
		);
	}
	...
	function getPageHref($page)
	{
		switch ($this->pattern_type) {
			...
			default :
				{
					$href = \RS\Router\Manager::obj ()->getUrl ( $this->pattern_type, array($this->pattern => $page 
					) + $this->route_params );
				}
		}
		return $href;
