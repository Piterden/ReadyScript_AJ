...
/**
 * Менеджер маршрутов.
 * Встречает запрос и передает управление подходящему маршруту модуля
 * или контроллеру.
 */
class Manager
{
	const ADMIN_ROUTE = 'main.admin';
	private $host, $uri, $admin_route, $is_admin_zone, $default_route, $enable_gzip = true;
	private static $instance, $routes = array();
	protected $http_request;
	/**
	 *
	 * @var RS_Router_RouteAbstract
	 */
	...
	protected function __construct()
	{
		...
		$this->default_route = new Route ( 'default', array('/{controller}/{Act}/','/{controller}/' 
		), null, t ( 'По умолчанию' ) );
		
		// Создаем маршут административной панели
		$admin = \Setup::$ADMIN_SECTION;
		$this->admin_route = new Route ( self::ADMIN_ROUTE, array("/{$admin}/{mod_controller}/","/{$admin}/","/{$admin}" 
		), array('controller' => 'main-admin-index','mod_controller' => 'main-widgets' 
		), t ( 'Панель Администратора' ), true );
	...
	public function initRoutes()
	{
		...
		if (\Setup::$INSTALLED) {
			...
			self::$routes = array('install' => new \RS\Router\Route ( 'install', '/install/', array('controller' => 'install-wizard' 
			), t ( 'Мастер установки' ) ) 
			);
		}
	}
	 ...
	 * 
	 * @param RS_Router_RouteAbstract $route маршрут
	 ...
	 * В случае если маршрут не найден возвращает пустую строку.
	 * Генерирует warning
	 * 
	 ...
	 * @param mixed $mod_controller - фронт контроллер, который будет вызван (обратный слеш заменяется на -(минус), можно вырезать часть "-controller-admin"), если не указан берется из текущего URL
	 */
	public function getAdminPattern($mod_action = null, $params = null, $mod_controller = null)
	{
	 ...
	 */
	public function setCurrentRoute(RouteAbstract $route)
	{
	 ...
	 * 
	 * @param RS_Router_RouteAbstract $route
	 */
