...
use RS\Cache\Manager as CacheManager, RS\Module\Manager as ModuleManager, RS\Module\Item as ModuleItem;

/**
 * Класс обработки событий.
 * Во время инициализации системы событий (выполняется в \Setup::init),
...
class Manager
{
	const SYSTEM_MODULE = 'main', USER_CALLBACK_CLASS = '\Config\MyHandlers', DEFAULT_CALLBACK_CLASS = '\Config\Handlers';
	protected static $initialized = false, $closure = 0, $base = array();
	...
	public static function init()
	{
		...
		if (\RS\Site\Manager::getSite () !== false) {
			self::$base = CacheManager::obj ()->expire ( 0 )->request ( array(__CLASS__,'loadBase' 
			), $is_admin_zone );
	...
	public static function bind($event, $callback, $priority = 10)
	{
		...
		self::$base[$event][$callback_class . '.' . $callback_method] = array('callback' => array($callback_class,$callback_method 
		),'priority' => $priority 
		);
	...
	/**
	 * Вызывает событие.
	 * Сообщает об этом слушателям, передает каждому слушателю результат выполнения предыдущего в виде параметров
	 * 
	 * @param string $event Имя события
