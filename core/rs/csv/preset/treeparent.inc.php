...
{
	protected $fields = array(), $fields_map = array(), 

	$is_multisite, $tree_delimiter = '/', $tree_field, $parent_field, $root_value, $id_field, $orm_object, $titles, $null_sign_fields, $link_foreign_field, $link_preset_id, $link_default_value;
	...
	function getColumns()
	{
		$fields = array_merge ( array($this->tree_field 
		), $this->fields );
		$result = array();
	...
	{
		$q = \RS\Orm\Request::make ()->from ( $this->getOrmObject () )->where ( array($this->getMappedField ( $this->tree_field ) => $title,$this->parent_field => $parent 
		) );
	...
	protected function getRecursiveField($current_id)
	{
		...
		while ( $id && $id != $this->root_value ) {
			$row = \RS\Orm\Request::make ()->select ( $this->getMappedField ( $this->tree_field ), $this->parent_field )->from ( $this->getOrmObject () )->where ( array($this->id_field => $id 
			) )->exec ()->fetchRow ();
			if ($row) {
