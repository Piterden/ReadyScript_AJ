...
{
	protected $search_fields = array(), $delimiter = ";", $orm_object, $id_field, $link_id_field, $link_preset_id, $mask, $mask_fields = array(), $mask_pattern, $title, $array_field, $array_key, $array_value, $manylink_foreign_id_field, $manylink_id_field, $manylink_orm;
	...
	function loadData()
	{
		...
		$this->row = array();
		if ($ids) {
			$this->row = \RS\Orm\Request::make ()->from ( $this->manylink_orm, 'X' )->join ( $this->orm_object, "X.{$this->manylink_foreign_id_field} = M.{$this->id_field}", 'M' )->whereIn ( $this->manylink_id_field, $ids )->objects ( null, $this->manylink_id_field, true );
		}
	}
	...
	function getColumns()
	{
		return array($this->id . '-manydata' => array('key' => 'manydata','title' => $this->title 
		) 
		);
	}
	function getColumnsData($n)
	{
		...
		return array($this->id . '-manydata' => $line 
		);
	}
		...
		if (isset ( $this->row['manydata'] )) {
			...
			foreach($items as $item) {
				...
				if (preg_match ( $this->mask_pattern, $item, $values_arr )) {
					...
					$loaded_object = \RS\Orm\Request::make ()->from ( $this->orm_object )->where ( $expr )->exec ()->fetchRow ();
					if ($loaded_object) {
