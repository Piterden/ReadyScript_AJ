...
{
	protected $link_id_field, $link_preset_id, $item, $delimiter = ',';
	...
	function loadData()
	{
		...
		if ($ids) {
			$this->row = \RS\Orm\Request::make ()->from ( new \Tags\Model\Orm\Link (), 'L' )->whereIn ( 'link_id', $ids )->where ( array('type' => $this->item 
			) )->join ( new \Tags\Model\Orm\Word (), 'L.word_id=W.id', 'W' )->objects ( null, 'link_id', true );
		}
	}
	...
	function getColumns()
	{
		return array($this->id . '-tags' => array('key' => 'tags','title' => t ( 'Теги' ) 
		) 
		);
	}
	...
	function getColumnsData($n)
	{
		...
		return array($this->id . '-tags' => implode ( $value, $this->delimiter ) 
		);
	}
