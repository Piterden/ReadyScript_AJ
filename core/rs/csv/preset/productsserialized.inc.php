...
{
	protected static $cache_groups = array(), $cache_products = array();
	protected $export_pattern, $import_pattern, $delimeter = ';', // Разделитель между значениями
$loaded_products = array(), $loaded_groups = array(), $is_multisite = true, 

	$export_product_field = 'title', $export_group_field = 'name', $link_foreign_field, $link_id_field, $link_preset_id, $title;
	...
	function setExportPattern($pattern)
	{
		...
		$this->import_pattern = str_replace ( array('{products}','{groups}' 
		), array('(?P<products>.*?)','(?P<groups>.*?)' 
		), quotemeta ( $pattern ) );
	}
	...
	function getColumns()
	{
		return array($this->id . '-products' => array('key' => 'products','title' => $this->title 
		) 
		);
	}
	...
	function loadData()
	{
		...
		if ($products_all) {
			$this->loaded_products = \RS\Orm\Request::make ()->from ( new \Catalog\Model\Orm\Product () )->whereIn ( 'id', $products_all )->objects ( null, 'id' );
		}
		
		if ($groups_all) {
			$this->loaded_groups = \RS\Orm\Request::make ()->from ( new \Catalog\Model\Orm\Dir () )->whereIn ( 'id', $groups_all )->objects ( null, 'id' );
		}
	}
	...
	function getColumnsData($n)
	{
		...
		$export_str = str_replace ( array('{groups}','{products}' 
		), array(implode ( $this->delimeter, $groups ),implode ( $this->delimeter, $products ) 
		), $this->export_pattern );
		return array($this->id . '-products' => $export_str 
		);
	}
			...
			if (preg_match ( "/^{$this->import_pattern}$/", $this->row['products'], $match )) {
				...
				foreach($groups as $group) {
					if (! isset ( self::$cache_groups[$group] )) {
						$item = \Catalog\Model\Orm\Dir::loadByWhere ( array($this->export_group_field => $group 
				...
				foreach($products as $product) {
					if (! isset ( self::$cache_products[$product] )) {
						$item = \Catalog\Model\Orm\Product::loadByWhere ( array($this->export_product_field => $product 
						) + $this->getMultisiteExpr () );
