...
{
	protected $use_cache = true, $cache = array(), $search_fields = array(), $orm_object, $is_multisite, $load_expression, $titles = array(), 

	$fields = array(), $exclude_fields = array(), $save = true, $id_field, $null_sign_fields, $link_foreign_field, $link_preset_id, $link_default_value;
	...
	function getColumns()
	{
		...
		foreach($this->orm_object->getProperties () as $key => $property) {
			...
			if (! in_array ( $key, $this->exclude_fields ) && (! $this->fields || isset ( $this->fields[$key] )) && (! $property->isRuntime () || isset ( $this->fields[$key] ))) {
				...
				$result[$this->id . '-' . $key] = array('key' => $key,'title' => $title 
				);
			}
		}
	...
	/**
	 * Проверяет, подходит ли текущие значения под определения NULL.
	 * (Если такие признаки имеются, то это значит, что этот объект создавать не нужно)
	 * 
	...
	function loadData()
	{
		...
		if (count ( $ids )) {
			...
			$this->rows = \RS\Orm\Request::make ()->select ( implode ( ',', $fields ) )->from ( $this->getOrmObject () )->whereIn ( $this->id_field, $ids )->exec ()->fetchSelected ( $this->id_field );
		}
	}
	...
	function loadObject()
	{
		...
		if (! $this->use_cache || ! isset ( $this->cache[$cache_key] )) {
			$q = \RS\Orm\Request::make ()->from ( $this->getOrmObject () )->where ( $this->getSearchExpr () )->where ( $this->getMultisiteExpr () );
