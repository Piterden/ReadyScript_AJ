...
{
	const	// Константы видимости полей при импорте и экспорте
	FIELDSCOPE_IMPORT = 'import', FIELDSCOPE_EXPORT = 'export';
	public $ids = array(), $rows = array();
	protected $csv_import_skip_first = true, $csv_delimiter = ';', $csv_enclosure = '"', $csv_charset = 'windows-1251', $fieldscope_fields = null, // Поля ограниченные областью видимости
$limit = 100, $before_line_import, $after_line_import, $after_import, $query, $action, // Текущее действие
$uploader, $work_fields, $base_id_field = 'id', $base_preset, $params = array(), $presets;
	...
	{
		return str_replace ( array('\\','-model-csvschema' 
		), array('-','' 
		...
		while ( $offset == 0 || count ( $this->rows ) ) {
			...
			foreach($this->rows as $n => $row) {
				...
				foreach($this->presets as $preset) {
					...
					\RS\Event\Manager::fire ( 'csv.beforelineexport.' . $class_short_name, array('schema' => $this 
					) );
	...
	function export()
	{
		...
		$app->cleanOutput ();
		$app->headers->addHeaders ( array('Content-Type' => $mime,'Content-Transfer-Encoding' => 'binary','Content-Disposition' => 'attachment; filename="' . $filename . '"','Connection' => 'close' 
		) );
	...
	function import($file, $check_timeout = false, $start_pos = 0, $site_id = null)
	{
		...
		$class_short_name = $this->getShortName ();
		if ($start_pos == 0) {
			\RS\Event\Manager::fire ( 'csv.beforeimport.' . $class_short_name, array('schema' => $this 
			) );
		}
		
		while ( $fp && (! $check_timeout || ((time () - $start_time) < ($max_exec_time - 2))) && ($row = fgetcsv ( $fp, null, $this->csv_delimiter, $this->csv_enclosure )) !== false ) {
			
			if ($i ++ == 0 && $this->csv_import_skip_first) continue;
			...
			\RS\Event\Manager::fire ( 'csv.beforelineimport.' . $class_short_name, array('schema' => $this 
			) );
			...
			\RS\Event\Manager::fire ( 'csv.afterlineimport.' . $class_short_name, array('schema' => $this 
			) );
		}
		...
		\RS\Event\Manager::fire ( 'csv.afterimport.' . $class_short_name, array('schema' => $this 
		) );
