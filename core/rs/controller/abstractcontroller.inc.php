...
class AbstractController implements IController
{
	...
	$app, /**
	 *
	 * @var \RS\Router\Manager
	 */
	$router;
	protected $act = null, $action_var = 'Act', /**
	 *
	 * @var Result\Standart
	 */
	$result;
	...
	/**
	 * Базовый конструктор контроллера.
	 * желательно вызывать в конструкторах порожденных классов.
	 */
	...
	function wrapHtml($body, $html_template = null)
	{
		...
		\RS\Event\Manager::fire ( 'controller.beforewrapoutput', $this );
	...
	/**
	 * Отдает в output страницу с ошибкой 404.
	...
	/**
	 * Перенаправляет браузер пользователя на другую страницу.
	 * Прекращает выполнение скрипта
	 * 
	...
	function redirect($url = null)
	{
		...
		$this->app->headers->addHeader ( 'location', $url )->sendHeaders ();
		exit ();
	}
