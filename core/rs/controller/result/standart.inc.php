...
{
	private $template, $checkAjaxOutput = true, $noAjaxRedirect, $controller, $result = array('html' => null 
	);
	...
	function addMessage($text, array $options = null)
	{
		...
		$this->result['messages'][] = array('text' => $text,'options' => $options 
		);
		return $this;
	}
	...
	function addEMessage($text, array $options = null)
	{
		...
		return $this->addMessage ( $text, array_merge ( array('theme' => 'error' 
		), $options ) );
	}
