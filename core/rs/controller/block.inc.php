...
$controller_description = ''; // Описание контроллера
	...
	{
		$info = array('title' => t ( static::$controller_title ),'description' => t ( static::$controller_description ) 
		);
	...
	public function exec($returnAsIs = false)
	{
		...
		if (\RS\Debug\Mode::isEnabled () && ! $this->router->isAdminZone ()) {
			...
			if ($this->getParam ( 'generate_by_template' )) { // Если блок вставлен в шаблон с помощью moduleinsert
			                                              // Генерируем кнопку
				$this->debug_group->addTool ( 'block_options', new Debug\Tool\BlockOptions ( $this->getBlockId (), array('attr' => array('href' => $this->router->getAdminUrl ( 'editTemplateModule', array('_block_id' => $this->getBlockId (),'block' => $this->getUrlName () 
				), 'templates-blockctrl' ) 
				) 
				) )
	...
	public function getSettingUrl($absolute = false)
	{
		if ($this->getParam ( 'generate_by_template' )) {
			$url = $this->router->getAdminUrl ( 'editTemplateModule', array('_block_id' => $this->getBlockId (),'block' => $this->getUrlName () 
			), 'templates-blockctrl' );
		} else {
			$url = $this->router->getAdminUrl ( 'editModule', array('id' => $this->getParam ( '_block_id' ) 
			), 'templates-blockctrl', $absolute );
		}
		return $url;
	}
}
