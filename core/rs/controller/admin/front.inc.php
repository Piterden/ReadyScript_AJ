...
class Front extends \RS\Controller\AbstractAdmin
{
	protected /**
	 *
	 * @var \RS\Controller\Result\Standart
	 */
	$result, $action_var = 'do', $wrap_output = true, $wrap_template = '%SYSTEM%/admin/body.tpl', $block_tpl = '%SYSTEM%/admin/block.tpl', // стандартный шаблон для оборачивания вывода в админке
$block_form_tpl = '%SYSTEM%/admin/crud_form.tpl'; // стандартный шаблон для оборачивания форм в админке
	private $caption, $help;
	function __construct()
	{
		parent::__construct ();
		$this->result = new \RS\Controller\Result\Standart ( $this ); // Helper, который помогает возвращать стандартизированый вывод
	}
