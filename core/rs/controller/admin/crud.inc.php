...
{
	protected $allow_crud_actions, $sqlMultiUpdate = true, // Если True - то используется метод api->multiUpdate иначе открывается каждый coreobject и персонально обновляется во время группового редактирования.
$selectAllVar = 'selectAll', $edit_url_var = 'edit_url', $sess_where = '_list', // имя переменной в сесии с условием последней выборки должно быть следующим ИМЯ_КОНТРОЛЕРА.
...
$api; // Наследник \RS\Module\AbstractModel\EntityList
	...
	{
		return new Helper\CrudCollection ( $this, $this->api, $this->url, array('paginator','topToolbar' => $this->buttons ( array('add' 
	...
	{
		return new Helper\CrudCollection ( $this, $this->api, $this->url, array('bottomToolbar' => $this->buttons ( array('save','cancel' 
		) ),'viewAs' => 'form','formTitle' => t ( 'Добавить' ) 
		) );
	}
	...
	{
		return $this->helperAdd ()->setTopTitle ( t ( 'Редактировать' ) );
	}
	...
	function helperTableOptions()
	{
		...
		$helper->setBottomToolbar ( new Toolbar\Element ( array('Items' => array('save' => new ToolbarButton\Button ( null, t ( 'сохранить' ), array('attr' => array('class' => 'saveform saveToCookie' 
		) 
		) ),'cancel' => $this->buttons ( 'cancel' ),'reset' => new ToolbarButton\Button ( null, t ( 'Сброс' ), array('attr' => array('class' => 'button reset' 
		) 
		) ) 
		) 
		...
		if (isset ( $index_helper['table'] )) {
			...
			$helper['tableControl'] = $index_helper['table'];
		}
	...
	function actionMultiedit()
	{
					...
					if ($this->sqlMultiUpdate) {
						...
						foreach($ids as $id) {
							...
							if ($prototype->hasError ()) {
								$prototype->addError ( "Во время обработки элемента $id произошла ошибка" );
		...
		$this->app->addJs ( 'coreobject.js', 'coreobject.js' );
		
		$helper = $this->getHelper ();
		$helper['hiddenFields'] = $hidden_fields;
		
		$this->view->assign ( array('elements' => $helper,'errors' => isset ( $error ) ? $error : array(),'param' => array('doedit' => $doedit,'ids' => $ids,'sel_count' => count ( $ids ) 
		) 
		) );
	...
	{
		return new Helper\CrudCollection ( $this, $this->api, $this->url, array('topTitle' => t ( 'Редактировать' ),'bottomToolbar' => $this->buttons ( array('save','cancel' 
		) ),'template' => '%system%/admin/crud_form.tpl','multieditMode' => true 
		) );
	}
	...
	{
		$default_buttons = array('add' => new ToolbarButton\Add ( $this->url->replaceKey ( array($this->action_var => 'add' 
		) ), null, array('noajax' => ! $ajax 
		) ),'delete' => new ToolbarButton\Delete ( null, null, array('attr' => array('data-url' => $this->router->getAdminUrl ( 'del' ) 
		),'noajax' => ! $ajax 
		) ),'multiedit' => new ToolbarButton\DropUp ( array(array('title' => t ( 'редактировать' ),'attr' => array('data-url' => $this->router->getAdminUrl ( 'multiedit' ),'class' => 'edit crud-multiedit' 
		) 
		) 
		), array('noajax' => ! $ajax,'attr' => array('class' => 'edit' 
		) 
		...
		) ),'moduleconfig' => new ToolbarButton\ModuleConfig ( $this->router->getAdminUrl ( 'edit', array('mod' => $this->mod_name 
		), 'modcontrol-control' ) ) 
		);
	...
	{
		$this->allow_crud_actions = func_get_args ();
	}
	...
	function exec($returnAsIs = false)
	{
		...
		if (is_callable ( array($this,$helper_method 
			...
			if ($helper_result !== null) {
				 ...
				 * Вызывается перед рендерингом страницы.
				 * Обработчики данного события могут изменить содержимое helper'а
