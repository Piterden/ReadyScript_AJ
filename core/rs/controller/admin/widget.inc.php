...
{
	protected $info_title = 'Не задано', // Определить у наследников название виджета.
...
$title; // Заголовок окна виджета
	...
	function getWidgetInfo()
	{
		...
		return array('title' => t ( $this->info_title ),'description' => t ( $this->info_description ),

		'full_title' => t ( $this->title ),'full_description' => t ( $this->info_full_description ),'module' => (isset ( $config ) ? $config['name'] : $this->mod_name),'short_class' => $this->getUrlName () 
		);
	}
	...
	function wrapWidget($return_data)
	{
		...
		$tpl->assign ( 'widget', array('id' => $this->getParam ( 'id' ),'class' => $this->getUrlName (),'title' => $this->title,'inside_html' => $return_data 
		) );
