...
use \RS\Html, \RS\Html\Toolbar\Button as ToolbarButton;
...
{
	const VIEW_CAT_VAR = 'viewcat', VIEW_CAT_LEFT = 'left', VIEW_CAT_TOP = 'top';
	private $api, $other_api = array(), $url, $appendModuleOptionsButton = true, $controller, $router, $collection = array('hiddenFields' => array(),'listFunction' => 'getList','treeListFunction' => 'getTreeList' 
	);
	...
	/**
	 * Устанавливает как будет выглядеть страница.
	 * В виде таблицы, формы, древовидного списка?
	 ...
	 * 
	 * @param Html\Table\Element $table
	 * @param mixed $id
	 * @return CrudCollection
	 */
	...
	{
		if ($id === null) {
			$id = md5 ( $this->controllerNameStr () . '-index' );
		}
		$this->collection['table'] = new Html\Table\Control ( array('Id' => $id,'Table' => $table,'AutoFill' => false 
		) );
		return $this;
	}
	...
	{
		$this->collection['paginator'] = new Html\Paginator\Control ( array('Paginator' => new Html\Paginator\Element (),'AutoFill' => false 
		) );
		return $this;
	}
	 ...
	 * 
	 * @param Html\Filter\Control $filter
	 * @return CrudCollection
	 */
	...
	/**
	 * Устанавливает имя шаблона, которых будет использован для генерации формы ORM объекта
	 * 
	 * @param string $tpl
	 * @return CrudCollection
	 */
	 ...
	 * 
	 * @param Html\Toolbar\Element $toolbar
	 * @return CrudCollection
	 */
	...
	{
		$this['topToolbar']->addItem ( new ToolbarButton\Dropdown ( array(array('title' => t ( 'Импорт/Экспорт' ),'attr' => array('class' => 'button','onclick' => "JavaScript:\$(this).parent().rsDropdownButton('toggle')" 
		) 
		),array('title' => t ( 'Экспорт CSV' ),'attr' => array('href' => \RS\Router\Manager::obj ()->getAdminUrl ( 'exportCsv', array('schema' => $schema,'referer' => $this->url->selfUri () 
		), 'main-csv' ),'class' => 'crud-add' 
		) 
		),array('title' => t ( 'Импорт CSV' ),'attr' => array('href' => \RS\Router\Manager::obj ()->getAdminUrl ( 'importCsv', array('schema' => $schema,'referer' => $this->url->selfUri () 
		), 'main-csv' ),'class' => 'crud-add' 
		) 
		) 
		) ), 'import' );
	...
	function setTree(Html\Tree\Element $tree, $api = null)
	{
		...
		$this->collection['tree'] = new Html\Tree\Control ( array('Tree' => $tree,'AutoFill' => false 
		) );
		return $this;
	}
	...
	/**
	 * Устанавливает, какую форму генерировать методу getForm.
	 * В режиме мультиредактирования форма отличается от обычной
	 * 
	 * @param mixed $bool
	 * @return CrudCollection
	 */
	...
	function active()
	{
		if ($this->appendModuleOptionsButton) {
			...
			$this['topToolbar']->addItem ( new ToolbarButton\ModuleConfig ( \RS\Router\Manager::obj ()->getAdminUrl ( 'edit', array('mod' => $module_name 
			), 'modcontrol-control' ) ) );
		}
	...
	{
		if ($this->getTreeViewType () == self::VIEW_CAT_LEFT) {
			$this->setTemplate ( '%system%/admin/crud_table_tree.tpl' );
		} else {
			$this->setTemplate ( '%system%/admin/crud_table_tree_viewtop.tpl' );
		}
		return $this;
	}
	
	/**
	 * Отображать страницу в виде таблицы с данными и дерева категорий (дерево категорий развернуто)
	 * 
	 * @return CrudCollection
	 */
	public function viewAsTableTreeView2()
	{
		$this->setTemplate ( '%system%/admin/crud_table_tree.tpl' );
		return $this;
	}
	...
	{
		$method = 'get' . $offset;
		return isset ( $this->collection[$offset] ) || method_exists ( $this, $method );
	}
