...
abstract class AbstractModule extends AbstractController
{
	 ...
	 * @var array
	 */
	$default_params = array(), $param = array(), 

	$mod_name = '', 
	// Путь к ресурсам модуля
	$controller_name = '', $mod_css = '/modules/%MODULE%/view/css/', $mod_js = '/modules/%MODULE%/view/js/', $mod_img = '/modules/%MODULE%/view/img/', $mod_tpl = '/modules/%MODULE%/view/';
	...
	function getParam($key = null, $default = null, $checkempty = false)
	{
		...
		if ($default === null && $checkempty === false && isset ( $this->default_params[$key] ) && empty ( $this->param[$key] )) {
			return $this->default_params[$key];
		} else {
			$isset = $checkempty ? ! empty ( $this->param[$key] ) : isset ( $this->param[$key] );
	...
	{
		$this->view->assign ( array('com' => $this,'error_text' => $error_text 
		) );
