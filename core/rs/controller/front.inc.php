...
abstract class Front extends AbstractClient
{
	...
	protected $wrap_template = DEFAULT_LAYOUT, $wrap_output = true;
	...
	function exec($returnAsIs = false)
	{
		...
		if (\Setup::$INSTALLED) {
			$pageseo = \RS\Cache\Manager::obj ()->watchTables ( new \PageSeo\Model\Orm\PageSeo () )->request ( array('\PageSeo\Model\PageSeoApi','getPageSeo' 
			), $this->router->getCurrentRoute ()->getId (), \RS\Site\Manager::getSiteId () );
			if ($pageseo) {
		...
		if ($this->wrap_output && substr ( $this->act, 0, 4 ) != 'ajax' && $this->url->request ( 'ajax', TYPE_INTEGER ) != 1) {
			$this->app->blocks->setRouteId ( $this->router->getCurrentRoute ()->getId () )->setMainContent ( $result_html )->setView ( $this->view );
			...
			if ($this->user->isAdmin () && $this->user->checkSiteRights ( \RS\Site\Manager::getSiteId () )) {
				...
				if (\RS\Config\Loader::getSystemConfig ()->show_debug_header) {
					...
					$debug_top_wrapper->assign ( array('result_html' => $result 
	...
	/**
	 * Добавляет ссылку на favicon, загруженный в админ.
	 * панели в секцию HEAD
	 * 
	 * @return void
	 */
	private function addFavicon()
	{
		...
		if ($site_config['favicon']) {
			...
			$params = array('type' => false,'rel' => 'shortcut icon' 
			);
			
			if (substr ( strtolower ( $favicon ), - 4 ) == '.png') {
				$params = array('type' => 'image/png','rel' => 'icon' 
				);
			}
	...
	function authPage($error = "", $referer = null)
	{
		...
		return $this->result->setNeedAuthorize ( true )->setNoAjaxRedirect ( $this->router->getUrl ( 'users-front-auth', array('referer' => $referer 
		) ) );
	}
}
