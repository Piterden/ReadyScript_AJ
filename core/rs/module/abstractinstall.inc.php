...
/**
 * Базовый класс установщика модуля.
 * Выполняет роль установщика "по-умолчанию" для модуля.
...
{
	protected $module, $mod_folder, $demo_data_folder = '/config/demo', /**
	 *
	 * @var \RS\Module\AbstractPatches
	 */
	$patches, $errors = array();
	...
	function update()
	{
		if ($this->patches) $this->patches->runBeforeUpdatePatches ();
		...
		if ($this->patches) $this->patches->runAfterUpdatePatches ();
	...
	{
		$params += array('schema' => null,'file' => null,'start_pos' => null 
		);
		...
		foreach($demo_schemas_array as $demo_schema) {
			...
			if (is_numeric ( $result ) && $result > 0) { // Если вернулась позиция в файле, то вернём массив с информацией
				$result = array('schema' => $schema_class_name,'file' => $file_name,'start_pos' => $result 
				);
				break;
			} elseif ($result) {
				$start_pos = 0;
	...
	function importCsv(\RS\Csv\AbstractSchema $schema, $csv_file, $site_id = null, $charset = 'utf-8', $start_pos = false)
	{
		...
		$search_files = array("{$csv_file}_{$lang}.csv","{$csv_file}.csv" 
		);
