...
/**
 * Класс - информация об одном модуле.
 * + действия с модулем
 */
class Item
{
	protected static $mod_css = '/modules/%MODULE%/view/css/', $mod_js = '/modules/%MODULE%/view/js/', $mod_img = '/modules/%MODULE%/view/img/', $mod_tpl = '/modules/%MODULE%/view/';
	protected $mod_name, $config, $changelog_filename = 'changelog', $module_folder = MODULE_FOLDER, // Берется из \Setup
$config_folder = CONFIG_FOLDER, $config_class = CONFIG_CLASS, $handlers_class = HANDLERS_CLASS, $my_handlers_class = MY_HANDLERS_CLASS, 
	...
	function getConfig()
	{
		return \RS\Config\Loader::byModule ( $this->mod_name );
	}
					...
					if (preg_match ( '/(.*?)\.(.*)/', $file, $matches )) {
						if ($matches[2] == \Setup::$CLASS_EXT) {
							...
							if (class_exists ( $class_name ) && is_subclass_of ( $class_name, '\RS\Controller\Block' )) {
								$ret[] = array('class' => $class_name,'short_class' => $matches[1],'info' => $class_name::getInfo () 
								);
							}
						}
					}
		...
		if ($installer) {
			...
			if ($installer->$method ()) {
				...
				foreach($options as $option => $value) { // Модуль установлен, запускаем опции
					if ($value) {
						if (is_callable ( array($installer,$option 
						) ) && ! $installer->$option ()) {
							$errors[] = t ( 'Во время установки опции "%0" произошла ошибка', array($option 
	...
	function uninstall()
	{
		...
		if (! $uninstall->uninstall ()) {
			...
			\RS\Orm\Request::make ()->delete ()->from ( new ModuleConfig () )->where ( array('module' => $this->mod_name 
			) )->exec ();
	...
	public static function getResourceFolders($module_name)
	{
		...
		$folders = array('mod_css' => strtolower ( str_replace ( '%MODULE%', $module_name, \Setup::$FOLDER . self::$mod_css ) ),'mod_js' => strtolower ( str_replace ( '%MODULE%', $module_name, \Setup::$FOLDER . self::$mod_js ) ),'mod_img' => strtolower ( str_replace ( '%MODULE%', $module_name, \Setup::$FOLDER . self::$mod_img ) ),'mod_tpl' => strtolower ( str_replace ( '%MODULE%', $module_name, \Setup::$FOLDER . self::$mod_tpl ) ) 
		);
		return $folders;
	}
	...
	public function getTools()
	{
		...
		$tools = array(array('url' => $router->getAdminUrl ( 'AjaxReInstall', array('module' => $this->mod_name 
		), 'modcontrol-control' ),'title' => t ( 'Переустановить модуль' ),'description' => t ( 'Обновляет структуру базы данных, заново создает пункты меню, устанавливает модуль' ),'confirm' => t ( 'Вы действительно хотите переустановить модуль?' ) 
		) 
		);
		
		if (($install = $this->getInstallInstance ()) !== false) {
			if ($install->canInsertDemoData ()) {
				$tools[] = array('url' => $router->getAdminUrl ( 'AjaxInstallDemoData', array('module' => $this->mod_name 
				), 'modcontrol-control' ),'title' => t ( 'Установить демо-данные' ),'description' => t ( 'Добавляет демонстационные данные к данным, которые сейчас присуствуют на сайте' ),'confirm' => t ( 'Вы действительно хотите добавить демонстрационные данные для данного модуля?' ) 
				);
			}
		}
		
		if ($this->issetChangelog ()) {
			$tools[] = array('url' => $router->getAdminUrl ( 'AjaxShowChangelog', array('module' => $this->mod_name 
			), 'modcontrol-control' ),'title' => t ( 'История изменений' ),'description' => t ( 'Показывает список изменений, произошедших в каждой версии модуля' ),'class' => 'crud-add' 
			);
		}
