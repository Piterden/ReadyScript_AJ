...
abstract class AbstractPatches
{
	...
	{
		// Не запускаем патчи во время установки системы
		...
		foreach($this->init () as $name) {
			...
			if (method_exists ( $this, $method )) {
				$patch_id = "MODULE_{$module}_PATCH_{$name}";
				if (\Setup::$INSTALLED && ! \RS\HashStore\Api::get ( $patch_id )) {
					try {
