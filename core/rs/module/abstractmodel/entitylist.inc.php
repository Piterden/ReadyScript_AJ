...
/**
 * Класс модели.
 * Содержит стандартное API для работы с плоским (не древовидным) списком.
 */
class EntityList extends BaseModel
{
	protected $default_order, $load_on_delete = false, $id_field, $alias_field, $sort_field, $name_field, $is_multisite = false, $site_context, $def_table_alias = 'A', // Псевдоним таблицы по умолчанию.

	$obj, // string - Объект с которым работает этот класс
$obj_instance, $filter_active = false, // Если true, значит фильтр применен
$multiedit_template = '%system%/coreobject/multiedit_form.tpl', $assocPlainList = array(), // Полностью загруженный список
$parsePrefixes = array('&' => 'AND','|' => 'OR' 
	...
	{
		$this->setElement ( $orm_element )->setSiteContext ();
	...
	/**
	 * Устанавливает, связаны ли данные ORM объекта с конкретным сайтом.
	 * Или объект имеет общие данные для всех сайтов.
	 * 
	...
	{
		$q = \RS\Orm\Request::make ()->select ( '*' )->from ( $this->obj_instance )->asAlias ( $this->defAlias () )->orderby ( $this->default_order );
		
		if ($this->is_multisite) {
			$q->where ( array($this->defAlias () . '.site_id' => $this->getSiteContext () ?  : 0 
			) );
		}
	...
	protected function filterLike($key, $value, $type, $prefix, $options)
	{
		...
		$this->queryObj ()->where ( "{$key} LIKE '#phrase'", array('phrase' => $value 
		), $prefix );
	}
	...
	{
		$this->queryObj ()->where ( "MATCH({$key}) AGAINST('#phrase')", array('phrase' => $value 
		), $prefix );
	}
	...
	protected function filterDefault($key, $value, $type, $prefix, $options)
	{
		if ($value === null) {
			...
			$this->queryObj ()->where ( "{$key} {$type} '#data'", array('data' => $value 
			), $prefix );
		}
	}
	...
	protected function setPage($page, $pageSize)
	{
		if ($page) {
			...
			$this->q->offset ( $offset )->limit ( $pageSize );
		}
		return $this;
	}
	...
	function getOneItem($id)
	{
		...
		return $q->where ( array($this->id_field => $id 
		) )->orderby ( null )->object ();
	}
	 ...
	 * 
	 * @param mixed $id - если число, то будет воспринят как id, иначе как псевдоним
	...
	function getById($id, $alias_field = null)
	{
		...
		if (is_numeric ( $id ) && $id > 0) {
			$q->where ( array($this->id_field => $id 
			) );
		} else {
			$alias_field = $alias_field ?  : $this->alias_field;
			$q->where ( array($alias_field => $id 
			) );
		}
	...
	/**
	 * Удаляет элементы, используя механизм orm-object'ов.
	 * удаляем каждый объект персонально.
	...
	function multiUpdate(array $data, $ids = array())
	{
		...
		if (! empty ( $data )) {
			...
			$u->update ()->set ( $data )->orderby ( false )->exec ();
		}
	...
	function moveElement($from, $to, $flag, \RS\Orm\Request $extra_expr = null)
	{
		...
		$res = $extra_expr->select ( "{$this->id_field}, {$this->sort_field}" )->from ( $this->obj_instance )->where ( $filter )->orderby ( $this->sort_field );
		$res = $res->exec ();
		...
		foreach($list as $newValues) {
			\RS\Orm\Request::make ()->update ( $this->obj_instance )->set ( array($this->sort_field => $newValues[$this->sort_field] 
			) )->where ( array($this->id_field => $newValues[$this->id_field] 
			) )->exec ();
		}
		return true;
	}
	...
	function getIdByAlias($alias_or_id, $alias_field = null)
	{
		if (! isset ( $this->_cache_alias[$alias_or_id] )) {
			...
			$q = $this->getCleanQueryObject ();
			$expr = "(`$alias_field` = '#alias' " . ((is_numeric ( $alias_or_id )) ? " OR `{$this->id_field}` = '#alias'" : '') . ")";
			$res = $q->where ( $expr, array('alias' => $alias_or_id 
			) )->orderby ( false )->limit ( 1 )->exec ();
	...
	{
		return $request->select ( $this->defAlias () . '.' . $this->id_field )->limit ( null )->orderby ( null )->exec ()->fetchSelected ( null, $this->id_field );
	}
}