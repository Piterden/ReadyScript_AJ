 ...
 * Содержит только базовые функции обработки ошибок
 */
abstract class BaseModel
{
	protected $default_error_fieldname = 'Информация', $form_fieldname = array(), $errors_by_form = array(), $errors_non_form = array(), $errors = array();
	...
	function getDisplayErrors()
	{
		...
		if (count ( $this->errors_non_form )) {
			$errors['@system'] = array('class' => 'system','fieldname' => t ( $this->default_error_fieldname ),'errors' => $this->errors_non_form 
			);
		}
		
		foreach($this->errors_by_form as $key => $error_list) {
			$errors[$key] = array('class' => 'field','fieldname' => $this->form_fieldname[$key],'errors' => $error_list 
			);
		}
		return $errors;
	}
