 ...
 */
/**
 * Функции по работе с древовидными списками.
 * (с полной загрузкой списка в массив)
 */
namespace RS\Module\AbstractModel;

abstract class TreeList extends EntityList
{
	protected $case_insensitive = true, $parent_field, 

	$cat, $catByParent, $catByAlias, $catByAliasParent, $cache;
	 ...
	 * @return \RS\Orm\AbstarctObject | falses
	 */
	public function getByAlias($alias, $parent = null)
	{
		...
		if ($parent === null) {
			if (isset ( $this->catByAlias[$alias] )) return $this->cat[$this->catByAlias[$alias]];
		} else {
			if (isset ( $this->catByAliasParent[$alias][$parent] )) return $this->cat[$this->catByAliasParent[$alias][$parent]];
		}
		return false;
	}
	...
	/**
	 * Возвращает массив родителей элемента.
	 * Аналог getPathToFirst только с использованием запросов к БД
	 * 
	 * @param mixed $id
	 */
	function queryParents($id)
	{
		...
		while ( $obj ) {
			$q->where = '';
			$obj = $q->where ( array($this->id_field => $id 
			) )->object ();
			if ($obj) {
	...
	function queryGetChilds($id, $order = null)
	{
		...
		return \RS\Orm\Request::make ()->select ( '*' )->from ( $this->obj_instance )->where ( array($this->parent_field => $id 
		) )->orderby ( $order )->objects ( $this->obj );
	}
	 ...
	 */
	public function moveElement($from, $to, $flag, \RS\Orm\Request $extra_expr = null)
	{
		if (! $extra_expr) {
