...
{
	const STATUS_OK = 1, // Модуль распакован, импортирован.
DELBEFORE_TYPE_FULL = 'full', DELBEFORE_TYPE_PART = 'part', DELBEFORE_TYPE_NONE = 'none';
	protected $module_folder = MODULE_FOLDER, // Берется из \Setup
$config_folder = CONFIG_FOLDER, // Относительно $module_folder
$config_class = CONFIG_CLASS, 

	$tmp_path = '/storage/tmp/new_module', $allow_mime = array('application/zip' 
	), $path, $prefix, $error = false, 

	$options = array('insertDemoData' => false 
	), 
	// Заполняется при валидации
	$valid, $module_info, $module_name, $module_already_exists;
	...
	function extractFromPost($postVar)
	{
		...
		if (($error = \RS\File\Tools::checkUploadError ( $postVar['error'] )) !== false) {
			return $this->addError ( $error, t ( 'Файл модуля' ), 'module' );
		}
	...
	function getTmpInfo()
	{
		...
		if ($this->valid) {
			...
			return array('info' => $this->module_info,'name' => $this->module_name,'changelog' => $this->getChangeLog (),'can_insert_demo_data' => $this->can_insert_demo_data,'already_exists' => $this->module_already_exists,'current_version' => $current_module_config['version'] 
			);
		}
		return false;
	}
	...
	function validateTmp()
	{
		...
		$files = array();
		if ($dh = opendir ( $this->path )) while ( ($file = readdir ( $dh )) !== false ) {
	...
	protected function checkCoreVersion($need)
	{
		...
		foreach($need_parts as $need_ver) {
			...
			if (preg_match ( '/^>=(.*)/', $need_ver, $match )) {
				if ($this->compareVersion ( $match[1], $core_version, '>=' )) return true;
			} elseif (preg_match ( '/^<=(.*)/', $need_ver, $match )) {
				if ($this->compareVersion ( $match[1], $core_version, '<=' )) return true;
			} elseif (preg_match ( '/^(.*?)-(.*)/', $need_ver, $match )) {
				if ($this->compareVersion ( $match[1], $core_version, '>=' ) && $this->compareVersion ( $match[2], $core_version, '<=' )) return true;
			} else {
				if ($this->compareVersion ( $need_ver, $core_version, '==' )) return true;
			}
		}
		return false;
	}
	 ...
	 */
	protected function compareVersion($need, $version, $compare = '>=')
	{
		$need_parts = explode ( '.', $need );
