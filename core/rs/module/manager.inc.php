...
/**
 * Класс содержит функции по работе с модулями.
 * (которые находятся в папке /modules/)
...
$config_class = CONFIG_CLASS;
	...
	public function getList($cache_enabled = true)
	{
		if (! isset ( self::$mod_list )) {
			if ($cache_enabled) {
				self::$mod_list = CacheManager::obj ()->expire ( 0 )->tags ( CACHE_TAG_MODULE )->request ( array($this,'getList' 
				), false );
			} else {
	 ...
	 */
	public function getActiveList($cache_enabled = true)
	{
		if (! \Setup::$INSTALLED) {
		...
		if ($cache_enabled) {
			...
			return CacheManager::obj ()->expire ( 0 )->tags ( CACHE_TAG_MODULE )->request ( array($this,'getActiveList' 
			), false, $is_admin_zone );
		} else {
	...
	/**
	 * Функция для статического вызова.
	 * Возвращает true, если модуль существует
	 */
	...
	/**
	 * Функция для статического вызова.
	 * Возвращает true, если модуль включён
	 */
	public static function staticModuleEnabled($mod_name)
	{
		 ...
		 */
		$class_config = \RS\Config\Loader::byModule ( $mod_name );
	...
	public function getBlockControllers()
	{
		...
		foreach($active_modules as $mod) {
			...
			if (! empty ( $controllers )) {
				$list[$mod->getName ()] = array('moduleItem' => $mod,'moduleTitle' => $mod->getConfig ()->name,'controllers' => $controllers 
				);
			}
		}
		return $list;
