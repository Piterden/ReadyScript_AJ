...
class Auth
{
	const COOKIE_AUTH_TICKET = 'user_ticket', COOKIE_AUTH_TICKET_LIFETIME = 63072000; // Время жизни cookie с тикетом авторизации в секундах.
	...
	public static function staticInit()
	{
		...
		$auth_ticket = \RS\Http\Request::commonInstance ()->cookie ( self::COOKIE_AUTH_TICKET, TYPE_STRING );
	...
	public static function login($login, $pass, $remember = false, $pass_encrypted = false)
	{
		...
		$user = \RS\Orm\Request::make ()->select ( '*' )->from ( new User () )->where ( array('login' => $login 
		) )->object ();
		
		if ($user && $user['pass'] === $pass) {
			if (self::isUserBanned ( $user )) {
				self::$error = t ( 'Заблокирован до %0. ', array(date ( 'd.m.Y', strtotime ( $user['ban_expire'] ) ) 
				) ) . $user['ban_reason'];
				return false;
			} else {
				...
				\RS\Event\Manager::fire ( 'user.auth', array('user' => $user 
				) );
				return true;
			}
		} else {
	...
	public static function loginByCookie($auth_ticket)
	{
		if (! empty ( $auth_ticket )) {
			...
			$user = \RS\Orm\Request::make ()->from ( new User () )->where ( array('id' => $id 
			) )->object ();
	...
	public static function logout()
	{
		...
		\RS\Event\Manager::fire ( 'user.logout', array('user' => self::getCurrentUser () 
		) );
	...
	public static function getCurrentUser()
	{
		...
		if ($guest_hash === false) {
			...
			setcookie ( 'guest', $guest_hash, time () + 60 * 60 * 24 * 730, '/' ); // 2 года помним неавторизованного пользователя
		}
