...
class Headers
{
	private $cookies = array(), $headers = array();
	...
	{
		$item = array('key' => $key_value,'value' => $value 
		);
	...
	public function sendHeaders()
	{
		foreach($this->headers as $item) {
			...
			), '', $header_str ); // Защита от CRLF атак
			header ( $header_str );
		}
		$this->cleanHeaders ();
	...
	public function addCookie($name, $value = null, $expire = null, $path = null, $domain = null, $secure = false, $httponly = false, $other_key = null)
	{
		...
		$this->cookies[$key] = array('name' => $name,'value' => $value,'expire' => $expire,'path' => $path,'domain' => $domain,'secure' => $secure,'httponly' => $httponly 
		);
		return $this;
	}
