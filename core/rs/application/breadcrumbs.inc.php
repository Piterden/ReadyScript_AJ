...
class BreadCrumbs
{
	protected $add_main = true, $breadcrumbs = array();
	...
	{
		$item = array('title' => $title,'href' => $href 
		);
		
		if ($position !== null) {
			$this->breadcrumbs = array_merge ( array_slice ( $this->breadcrumbs, 0, $position ), array($item 
			), array_slice ( $this->breadcrumbs, $position ) );
		} else {
	...
	{
		$main = array(array('title' => t ( 'Главная' ),'href' => \RS\Site\Manager::getSite ()->getRootUrl () 
		) 
		);
