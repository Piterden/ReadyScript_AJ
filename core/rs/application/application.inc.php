...
class Application
{
	const JS_POSITION_HEADER = 'header', JS_POSITION_FOOTER = 'footer', DEFAULT_ERROR_PAGE_TPL = '%THEME%/exception.tpl', // Путь к шаблону страницы ошибок
DEFAULT_ADMIN_ERROR_PAGE_TPL = '%SYSTEM%/admin/exception.tpl'; // Путь к шаблону страницы ошибок в административной части
	...
	protected $doctype = DOCTYPE, // Значение по умолчанию берем из \Setup
$is_admin_zone, $body_class, $js_no_compress = array(), $css_no_compress = array(), $js_default_position = self::JS_POSITION_HEADER, $js = array(self::JS_POSITION_HEADER => array(),self::JS_POSITION_FOOTER => array() 
	), $jscode = array(), $css = array(), $js_vars = array(), $base_js, $base_css, $any_head_data = '', $head_attributes = array();
	...
	protected function __construct()
	{
		...
		$this->addJsVar ( array('folder' => ( string ) \Setup::$FOLDER 
		) );
	}
	...
	public function addJs($jspath, $name = null, $basepath = null, $no_compress = false, $params = array())
	{
		...
		$file = array($name => array('file' => $jspath,'params' => $params 
		) 
		);
	...
	public function getJs($position = self::JS_POSITION_HEADER)
	{
		...
		if (\Setup::$COMPRESS_JS && ! $this->is_admin_zone) {
			...
			return $compressJs->getCompressed ( $this->js_no_compress );
		}
	...
	public function addCss($csspath, $name = null, $basepath = null, $no_compress = false, $params = array())
	{
		...
		$file = array($name => array('file' => $csspath,'params' => $params 
		) 
		);
	...
	public function getCss()
	{
		if (\Setup::$COMPRESS_CSS && ! $this->is_admin_zone) {
			...
			return $compressCss->getCompressed ( $this->css_no_compress );
		}
		return $this->css;
	}
	...
	{
		$this->cleanOutput ();
		$this->headers->cleanHeaders ()->setStatusCode ( $status_code, $http_code_phrase )->sendHeaders ();
		
		$template = $this->is_admin_zone ? self::DEFAULT_ADMIN_ERROR_PAGE_TPL : self::DEFAULT_ERROR_PAGE_TPL;
		
		$tpl = new \RS\View\Engine ();
		$tpl->assign ( array('error' => array('code' => $status_code,'comment' => $comment 
		),'site' => \RS\Site\Manager::getSite (),'site_config' => \RS\Config\Loader::getSiteConfig () 
		) );
