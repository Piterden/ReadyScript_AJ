...
{
	protected $uniqName = '', $return_list = array(), $file_list = array(), $group_list = array(), $checked_file_list = array(), $output_path, $extension, $file_time = 0; // Самая максимальная дата модификации файла CSS
	 ...
	 */
	protected function merge($group)
	{
		$uniq_t = array('file' => $group['uniq']['file'] . '?t' . $group['time'] 
		) + // Добавляем параметр (чтобы браузер не использовал кэшированный файл)
$group['files'][0];
		...
		if ($this->compress ( $merged_css, \Setup::$PATH . $group['uniq']['file'] )) {
	...
	/**
	 * Функция должна быть определена у наследников
	 * 
	 * @param mixed $css_source
	 * @param mixed $output_file
	 */
	abstract function compress($source, $output_file);
	
	/**
	 * Возвращает массив с одним файлом в списке.
	 * Файл - объединенный, оптимизированный
	 */
	public function getCompressed($no_compress = array())
	{
		...
		foreach($this->file_list as $name => $filearr) {
			...
			if (! isset ( $no_compress[$name] ) && file_exists ( \Setup::$PATH . $pfilename )) {
				if (! isset ( $groups[$i] )) {
					$groups[$i] = array('files' => array(),'uniq_src' => '','time' => 0 
					);
				}
		...
		foreach($groups as $group) {
			if (! isset ( $group['uniq'] )) {
				$group['uniq'] = array('file' => $this->output_path . '/' . sprintf ( "%u", crc32 ( $group['uniq_src'] ) ) . $this->extension 
				) + $group['files'][0];
			}
			
			if (isset ( $group['files'] ))
				$this->return_list = array_merge ( $this->return_list, $this->merge ( $group ) );
