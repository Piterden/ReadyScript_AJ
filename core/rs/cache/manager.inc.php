...
class Manager
{
	protected $cache_folder = CACHE_FOLDER, // Определено в \Setup;
$cache_table_folder = CACHE_TABLE_FOLDER, 

	$tags = array(), $before_expire, $watch_tables = null;
	public $enabled = CACHE_ENABLED, // Определено в \Setup
$expire = CACHE_TIME; // в секундах, срок в течении котороо кэш считается валидным. 0 - бессрочно (для ручной инвалидации)
	...
	/**
	 * Устанавливает срок в секундах в течении котороо кэш считается валидным.
	 * Сбрасывается после вызова метода request
	 * 
	...
	protected function checkTableActual(array $tables, $key)
	{
		...
		foreach($tables as $table) {
			list ( $db, $table ) = $this->normalizeTable ( $table );
			$file = $this->tableFilename ( $table, $db );
			$result = $result && (file_exists ( $file ) && (strpos ( file_get_contents ( $file ), $key . "\n" ) !== false));
		}
		return $result;
	}
