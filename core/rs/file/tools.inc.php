...
class Tools
{
	 ...
	 * @param mixed $parseDir - Устанавливайте true, если в $dstFilename присутствует еще имя файла, в этом случае оно будет игнорироваться.
	 */
	...
	public static function checkUploadError($err_status)
	{
		switch ($err_status) {
			...
			case UPLOAD_ERR_INI_SIZE :
				$res = t ( 'Загрузка файлов такого размера не поддерживается' );
				break;
			...
			case UPLOAD_ERR_CANT_WRITE :
				...
				break;
			default :
				$res = t ( 'Файл не загружен на сервер' );
		}
		return $res;
	}
	 ...
	 * @param string $filename - имя файла в диалоге созранения файла
	...
	public static function normalizeFilePost($post_files_arr)
	{
		...
		if (isset ( $post_files_arr['name'] )) {
			foreach($post_files_arr['name'] as $n => $value) {
				$result[$n] = array('name' => $value,'type' => $post_files_arr['type'][$n],'tmp_name' => $post_files_arr['tmp_name'][$n],'error' => $post_files_arr['error'][$n],'size' => $post_files_arr['size'][$n] 
				);
			}
		}
		return $result;
	...
	/**
	 * Закрывает директорию от публичного просмотра, добавляя в ней файл .
	 * htaccess со строкой deny from all
	 * 
