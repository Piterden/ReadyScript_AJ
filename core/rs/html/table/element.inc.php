...
{
	protected $data, $table_attr = array('class' => 'rs-table' 
	), $row_attr_param, $tr_param = array(), 

	$columns = array(), $rows = array(), $anyrows = array();
	...
	{
		if ($pos !== null) {
			$this->columns = array_merge ( array_slice ( $this->columns, 0, $pos ), array($column 
			), array_slice ( $this->columns, $pos ) );
		} else {
			$this->columns[] = $column;
		}
		return $this;
	...
	{
		$str = '';
		if (isset ( $this->row_attr[$n] )) foreach($this->row_attr[$n] as $key => $val) {
			$str .= " $key=\"$val\"";
		}
		return $str;
	}
	...
	{
		$str = '';
		if (isset ( $this->any_row_attr[$n] )) foreach($this->any_row_attr[$n] as $key => $val) {
			$str .= " $key=\"$val\"";
		}
		return $str;
	}
	...
	function setSortColumn($column, $direction)
	{
		foreach($this->columns as $n => $obj) {
			if ($n == $column)
				$obj->property['CurrentSort'] = $direction;
