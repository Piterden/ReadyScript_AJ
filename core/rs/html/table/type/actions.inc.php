...
{
	const AUTO_WIDTH = 22;
	public $property = array('ThAttr' => array('class' => 'settings' 
	),'customizable' => false 
	);
	protected $actions, $head_template = 'system/admin/html_elements/table/coltype/actions_head.tpl', $body_template = 'system/admin/html_elements/table/coltype/actions.tpl';
	...
	{
		if ($position !== null) {
			$this->actions = array_merge ( array_slice ( $this->actions, 0, $position ), array($action 
			), array_slice ( $this->actions, $position ) );
		} else {
			$this->actions[] = $action;
		}
	}
	...
	{
		$str = '';
		if (isset ( $action->options[$index] )) foreach($action->options[$index] as $key => $val) {
			if ($key{0} == '@') {
