...
abstract class AbstractType
{
	...
	protected $option_prefixes = array('set','add' 
	), $field, $row, $title, $value;
	protected $head_template = 'system/admin/html_elements/table/coltype/strhead.tpl', $body_template = '';
	...
	protected function abstractGetAttr($index, array $concat_arr = array(), $source = null)
	{
		...
		if (isset ( $source[$index] )) foreach($source[$index] as $key => $val) {
	...
	function getHref($href_pattern = null)
	{
		...
		if (strpos ( $href, '@' ) !== false) {
			$href = preg_replace_callback ( '/(@([^&\/]+))/', array($this,'replaceCallback' 
			), $href );
		}
		if (strpos ( $href, '{' ) !== false) {
			$href = preg_replace_callback ( '/({([^&\/]+)})/', array($this,'replaceCallback' 
