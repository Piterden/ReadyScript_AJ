...
{
	protected $id, $auto_fill = true, $table_var = 'table', $sort_column_var = 'sort', $sort_direction_var = 'direction', $table;
	...
	function fill()
	{
		...
		foreach($this->table->getColumns () as $n => $obj) {
			...
			if (! empty ( $obj->property['Sortable'] )) {
				...
				$obj->sorturl = $this->url->replaceKey ( array($this->table_var => array($this->sort_column_var => $n,$this->sort_direction_var => $direction 
				) 
				) );
			}
