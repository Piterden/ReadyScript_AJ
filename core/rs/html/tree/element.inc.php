...
{
	const VIEWTYPE_BIGTREE = 'bigtree', VIEWTYPE_INLINE = 'inline';
	protected $checked = array(), $checkbox_name = 'chk[]', $tools, $main_column, $other_columns, $head_buttons, $path_to_first = array(), $data;
	
	/**
	 * Задает массив выделенных элементов.
	 * id Выделенных элементов должны быть в ключах массива
	 * 
	...
	function getView($local_options = null)
	{
		...
		$view->assign ( array('tree' => $this,'local_options' => $local_options 
		) );
		
		if (isset ( $local_options['viewtype'] ) && $local_options['viewtype'] == self::VIEWTYPE_INLINE) {
			$template = 'system/admin/html_elements/tree/tree_inline.tpl';
		} else {
	...
	/**
	 * Устанавливает поле, значение в котором система будет сравнивать с activeValue.
	 * Если значение совпадет, то строка с данными будет выделена, как текущая
	 * 
	 * @param string $field - имя поля
	 * @return Element
	 */
