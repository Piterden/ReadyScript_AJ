...
/**
 * Класс управления фильтром.
 * Класс умеет заполнять формы контейнера данными,
...
{
	public $uniq, $auto_fill = true, $filter_var = 'f'; // GET-параметр, содержащий сведения об установленных фильтрах
	protected $tpl = 'system/admin/html_elements/filter/control.tpl', $container, $exclude_get_params = array('p' 
	), $before_sql_where_callback, $add_param = array();
	...
	function getCleanFilterUrl()
	{
		...
		return \RS\Http\Request::commonInstance ()->replaceKey ( array($this->getFilterVar () => null 
		) + $exclude );
	}
	
	/**
	 * Возвращает - какие элементы были закрыты.
	 * берет информацию из cookie
	 * 
	 * @return array
	 */
	function getElementsStatus()
	{
		...
		foreach($cookie as $str) {
			$keyval = explode ( '=', $str );
			if (count ( $keyval ) == 2) $estatus[$keyval[0]] = ( int ) $keyval[1];
		}
		return $estatus;
	}
	...
	/**
	 * Сохраняет текущие GET параметры.
	 * Они будут добавлены к URL при применении фильтра
	 * 
	 * @return Control
	 */
	...
	{
		return array_merge ( array($this->container 
		), $this->container->getSecContainers () );
	}
