...
{
	public $tpl = 'system/admin/html_elements/filter/type/daterange.tpl';
	protected $search_type = 'daterange';
	...
	public function getParts($current_filter_values, $exclude_keys = array())
	{
		...
		if ($this->getNonEmptyValue () !== null) {
			...
			if (! empty ( $from )) { // Если есть дата с
				...
				$parts[] = array('title' => $this->getTitle () . t ( ' с' ),'value' => $from_formated,'href_clean' => \RS\Http\Request::commonInstance ()->replaceKey ( array($this->wrap_var => $without_this 
				) + $exclude ) 
...
;
			...
			if (! empty ( $to )) { // Если есть дата по
				...
				$parts[] = array('title' => $this->getTitle () . t ( ' до' ),'value' => $to_formated,'href_clean' => \RS\Http\Request::commonInstance ()->replaceKey ( array($this->wrap_var => $without_this 
				) + $exclude ) 
				) // Url, для очистки данной части фильтра
;
			}
