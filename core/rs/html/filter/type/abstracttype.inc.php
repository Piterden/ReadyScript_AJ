...
{
	public $wrap_var, $emptynull = true; // true - считать пустое значение, не установленнным фильтром
	protected $type_array_sql = array(1 => '<','2' => '>',3 => '=' 
	), $is_prefilter = false, $field_prefix = '', // если задан префикс, то в sqlwhere будет дополнять строку префиксом: prefix.key = 'value';
$search_type = 'eq', $title_attr = array('class' => 'standartkey' 
	), $attr = array(), $trim = true, $abstract_tpl = 'system/admin/html_elements/filter/type/abstract.tpl', $key, $title, $value = '', $prefilter_list = array();
	 ...
	 */
	function setEmptyNull($emptynull)
	{
	 ...
	 * (ключ сответствует колонке в БД)
	 * 
	 * @return string
	 */
	function getKey()
	...
	function getSqlKey()
	{
		if (empty ( $this->field_prefix ))
			return $this->key;
		else return $this->field_prefix . '.' . $this->key;
	}
	function getName()
	{
	...
	public function getParts($current_filter_values, $exclude_keys = array())
	{
		...
		if ($this->getNonEmptyValue () !== null) {
			...
			$parts[] = array('title' => $this->getTitle (),'value' => $prefilters . $this->getTextValue (),'href_clean' => \RS\Http\Request::commonInstance ()->replaceKey ( array($this->wrap_var => $without_this 
			) + $exclude ) 
			) // Url, для очистки данной части фильтра
;
		}
		return $parts;
	}
