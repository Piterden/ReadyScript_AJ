 ...
 * Сообщаем ему, папку исходников.
 * папку, в которую будут рендериться уменьшенные фото.
 */
class Core
{
	const WM_POSX_LEFT = 'left', WM_POSX_CENTER = 'center', WM_POSX_RIGHT = 'right', 

	WM_POSY_TOP = 'top', WM_POSY_MIDDLE = 'middle', WM_POSY_BOTTOM = 'bottom';
	protected static $_cache = array();
	protected $base, $watermark = false, $srcFolder, $dstFolder, 
	...
	function __construct($base, $srcFolder, $dstFolder)
	{
		...
		if (! empty ( $sysConfig['watermark'] )) {
			$this->setWatermark ( $sysConfig->getProp ( 'watermark' )->getFullPath (), $sysConfig['wmark_min_width'], $sysConfig['wmark_min_height'], $sysConfig['wmark_pos_x'], $sysConfig['wmark_pos_y'] );
		}
	 ...
	 */
	function setWatermark($file, $minWidth = 0, $minHeight = 0, $posX = self::WM_POSX_CENTER, $posY = self::WM_POSY_MIDDLE)
	{
		$this->watermark = array('file' => $file,'minWidth' => $minWidth,'minHeight' => $minHeight,'posX' => $posX,'posY' => $posY 
		);
	}
	...
	{
		if ($source->width < $this->watermark['minWidth'] || $source->height < $this->watermark['minHeight']) return false;
		...
		switch ($this->watermark['posX']) {
			case self::WM_POSX_LEFT :
				$xDest = $offset;
				break;
			case self::WM_POSX_CENTER :
				$xDest = round ( $source->width / 2 - ($x / 2) );
				break;
			case self::WM_POSX_RIGHT :
				$xDest = $source->width - ($x + $offset);
		}
		
		switch ($this->watermark['posY']) {
			case self::WM_POSY_TOP :
				$yDest = $offset;
				break;
			case self::WM_POSY_MIDDLE :
				$yDest = round ( $source->height / 2 - ($y / 2) );
				break;
			case self::WM_POSY_BOTTOM :
				$yDest = $source->height - ($y + $offset);
				break;
		}
		...
		// imagejpeg($source->image_handler, $source->filename, 100);
	}
	...
	public function getImageUrl($origFileName, $width, $height, $imgType = 'xy', $absolute = false)
	{
		...
		$dstFileName = strtolower ( $this->dstFolder . '/' . $imgType . '_' . $width . 'x' . $height . '/' . $fname . '_' . $openKey . '.' . $ext );
		return $absolute ? \RS\Site\Manager::getSite ()->getAbsoluteUrl ( $dstFileName ) : $dstFileName;
	}
				...
				if ($dh = opendir ( $dir )) {
					while ( ($file = readdir ( $dh )) !== false ) {
						...
						if (is_dir ( $dir . $file ) && preg_match ( '/^(.+?)_(\d+)x(\d+)$/', $file, $match )) {
							$types[] = array('scale' => $match[1],'width' => $match[2],'height' => $match[3] 
							);
						}
					}
					closedir ( $dh );
