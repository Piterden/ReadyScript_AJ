...
class Request
{
	public $select = '', $ignore = false, $delete, $having, $groupby, $from = '', $joins, $where, $limit, $offset, $orderby, $set, $map_obj; // какое поле какому объекту соответствует
	protected $return_class, $last_action, $action = 'select';
	
	/**
	 * Возвращает новый экземпляр текущего класса.
	 * Предназначен для краткой записи ::make()->from()->exec();
	 * 
	 * @return Request
	 */
	...
	/**
	 * Возвращает количество результатов звпроса.
	 * Подставляет COUNT(*) в секцию SELECT
	 */
		...
		if ($expression !== null) {
			...
			foreach($args as $fields) {
				...
				if (preg_match ( '/\{(.*)\}:(.*)/u', $fields, $match )) {
					...
					$this->map_obj[] = array('start' => ($start > 0) ? $start + 1 : 0,'length' => substr_count ( $match[1], ',' ) + 1,'class' => $match[2],'fields' => $this->parseFields ( $match[1] ) 
					);
					$fields = $match[1];
				}
	...
	function update($table = null, $ignore = null)
	{
		...
		if ($table !== null) {
			call_user_func ( array($this,'from' 
			), $table );
		}
		return $this;
	}
