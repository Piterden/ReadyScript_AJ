...
{
	const INSERT_FLAG = 'insert', UPDATE_FLAG = 'update', REPLACE_FLAG = 'replace', INDEX_PRIMARY = 'primary key', INDEX_UNIQUE = 'unique', INDEX_KEY = 'index', INDEX_FULLTEXT = 'fulltext';
...
$table = null, // Имя таблицы
	...
	$iterator = 0, $init_default_method = '_initDefaults', // Имя метода, который вызывается во время создания объекта
$local = array(), $default_local_parameters = array('modified' => array(), // Измененные свойства
...
'escape_all' => false 
	) // Перевести в entity все значения при записи
, $class = array(), $default_class_parameters = array('property_template' => '%system%/coreobject/prop_form.tpl','properties' => null, // Свойства объекта
'indexes' => array() 
	);
	...
	/**
	 * Производит внутреннюю инициализацию объекта.
	 * Вызывается один раз для одного имени класса
	 */
	...
	/**
	 *
	 * @deprecated Возвращает параметры, заданные для всех объектов данного класса
	...
	/**
	 *
	 * @deprecated Устанавливает параметр, заданные для всех объектов данного класса
	...
	/**
	 *
	 * @deprecated Возвращает true, если задан $key и параметр $key существует.
	...
	public static function loadByWhere($expr, array $values = null, $prefix = 'AND')
	{
		...
		$result = \RS\Orm\Request::make ()->from ( new $called_class () )->where ( $expr, $values, $prefix )->limit ( 1 )->object ();
	...
	public function insert($ignore = false, $on_duplicate_update_keys = array(), $on_duplicate_uniq_fields = array())
	{
		 ...
		 */
		$can_do = $this->beforeWrite ( self::INSERT_FLAG, $on_duplicate_update_keys ) !== false && ! \RS\Event\Manager::fire ( 'orm.beforewrite.' . $this->getShortAlias (), array('orm' => $this,'flag' => self::INSERT_FLAG,'on_duplicate_update_keys' => $on_duplicate_update_keys 
		) )->getEvent ()->isStopped ();
		...
		if ($can_do && $ret = $storage->insert ( $type, $on_duplicate_update_keys, $on_duplicate_uniq_fields )) {
			 ...
			 * index 'flag' => string "insert" | "replace" | "update"
			 */
			\RS\Event\Manager::fire ( 'orm.afterwrite.' . $this->getShortAlias (), array('orm' => $this,'flag' => self::INSERT_FLAG,'on_duplicate_update_keys' => $on_duplicate_update_keys 
			) );
		}
		return $ret;
	}
	...
	public function update($primaryKey = null)
	{
		 ...
		 * index 'flag' => string "insert" | "replace" | "update"
		 */
		$can_do = $this->beforeWrite ( self::UPDATE_FLAG ) !== false && ! \RS\Event\Manager::fire ( 'orm.beforewrite.' . $this->getShortAlias (), array('orm' => $this,'flag' => self::UPDATE_FLAG 
		) )->getEvent ()->isStopped ();
		...
		if ($can_do && $ret = $storage->update ( $primaryKey )) {
			 ...
			 * index 'flag' => string "insert" | "replace" | "update"
			 */
			\RS\Event\Manager::fire ( 'orm.afterwrite.' . $this->getShortAlias (), array('orm' => $this,'flag' => self::UPDATE_FLAG 
			) );
		}
		return $ret;
	}
	...
	public function replace()
	{
		 ...
		 * index 'flag' => string "insert" | "replace" | "update"
		 */
		$can_do = $this->beforeWrite ( self::REPLACE_FLAG ) !== false && ! \RS\Event\Manager::fire ( 'orm.beforewrite.' . $this->getShortAlias (), array('orm' => $this,'flag' => self::REPLACE_FLAG 
		) )->getEvent ()->isStopped ();
		...
		if ($can_do && $ret = $storage->replace ()) {
			 ...
			 * index 'flag' => string "insert" | "replace" | "update"
			 */
			\RS\Event\Manager::fire ( 'orm.afterwrite.' . $this->getShortAlias (), array('orm' => $this,'flag' => self::REPLACE_FLAG 
			) );
		}
		return $ret;
	}
	...
	public function delete()
	{
		...
		$eresult = \RS\Event\Manager::fire ( 'orm.delete.' . $this->getShortAlias (), array('orm' => $this 
		) );
	...
	public function getFromArray(array $data, $remove_prefix = null, $mark_modify = true)
	{
		...
		\RS\Event\Manager::fire ( 'orm.afterload.' . $this->getShortAlias (), array('orm' => $this 
		) );
	...
	public function getPrimaryKeyProperty()
	{
		foreach($this->getPropertyIterator () as $key => $prop)
			if ($prop->isPrimaryKey ()) return $key;
	...
	/**
	 * Проверка на возможность сохранения данных.
	 * при этом объект заполняется из POST. насколько это возможно.
	 * 
	...
	{
		if (! isset ( $post_var )) $post_var = $_POST;
		if (! isset ( $files_var )) $files_var = $_FILES;
		if (! isset ( $usekeys )) $usekeys = $this->getLocalParameter ( 'use_keys' );
		if (! isset ( $exclude )) $exclude = $this->getLocalParameter ( 'exclude_keys' );
		...
		foreach($this->getProperties () as $key => $property) {
			...
			if (isset ( $usekeys ) && ! in_array ( $key, $usekeys )) continue;
			if (isset ( $exclude ) && in_array ( $key, $exclude )) continue;
			if (! $property->isListenPost () && ! isset ( $user_post[$key] )) continue;
	...
	public function checkField($property)
	{
		...
		foreach($property->getCheckers () as $checker) {
			...
			$param = array_merge ( array($this,$property->get () 
			), $checker['param'] );
	...
	public static function chkDepend($coreobj, $value, $real_errtext, $real_chk, array $conditions, $real_params = array())
	{
		...
		foreach($conditions as $key => $val) {
			if (is_array ( $val )) {
				if (! in_array ( $coreobj[$key], $val )) $flag = false;
			} else {
				if ($coreobj[$key] != $val) $flag = false;
			}
		}
		if (! $flag) return true; // Сообщаем, что поле прошло прверку, если условия использования чекера не подходят
		
		$param = array_merge ( array($coreobj,$value,$real_errtext 
		), $real_params );
	...
	public function addErrors($array, $form = null)
	{
		...
		if ($form !== null) {
			...
			else $local['formerror'][$form] = array_merge ( $local['formerror'][$form], $array );
		} else {
	...
	/**
	 *
	 * @deprecated Возвращает массив с текстами всех ошибок.
	 */
	...
	{
		$local = &self::$local[$this->_local_id];
		if ($key === null) return $local['formerror'];
		if ($sep === null) {
	...
	public function getDisplayErrors()
	{
		...
		if (count ( $non_form_errors )) {
			$errors['@system'] = array('class' => 'system','fieldname' => t ( 'Системное сообщение' ),'errors' => $non_form_errors 
			);
		}
		...
		foreach($errors_by_form as $key => $error_list) {
			...
			$errors[$key] = array('class' => 'field','fieldname' => $fieldname,'errors' => $error_list 
			);
		}
		return $errors;
	}
	...
	function addIndex($fields, $type = self::INDEX_KEY, $name = null, $using = null)
	{
		...
		self::$class[$this->_self_class]['indexes'][$name] = array('fields' => $fields,'type' => $type,'name' => $name,'using' => $using 
		);
	...
	function getForm(array $tpl_vars = null, $switch = null, $is_multiedit = false, $template = null, $tpl_maker = null, $tpl_folder = null)
	{
		...
		if (! file_exists ( $template )) {
			...
			$make_form->assign ( array('prop' => $properties,'elem' => $this,'switch' => $switch 
			) );
	...
	public function getPropertyView($key, $attributes = array(), $view_params = array())
	{
		...
		$sm->assign ( array('prop' => $property,'view_params' => $view_params,'object' => $this 
		) );
	...
	/**
	 *
	 * @deprecated Необходимо реализовывать метод getDebugActions()
	 */
