...
class DbMap
{
	private $dbfields = array(), $dbindexes = array(), $properties = null, $indexes = array(), $skip_update_indexes = array(), $no_table = false, $db = null, $table = null, $engine = null, $charset = null;
	...
	private function getDbIndexes()
	{
		...
		while ( $row = $dbresult->fetchRow () ) {
			...
			if (! isset ( $indexes[$row['Key_name']] )) {
				$indexes[$row['Key_name']] = array('unique' => $row['Non_unique'],'type' => $row['Index_type'] 
				);
			}
		...
		foreach($this->properties as $name => $prop) // обработка индексов
{
	...
	private function FieldFormat($name, \RS\Orm\Type\AbstractType $prop, $is_create = false)
	{
		...
		$skip_add_primary_index = $prop->isPrimaryKey () && ! $is_create && isset ( $this->dbindexes['PRIMARY'] ) && $this->dbindexes['PRIMARY']['fields'][0] == $name;
