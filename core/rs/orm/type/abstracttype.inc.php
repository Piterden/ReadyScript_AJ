...
abstract class AbstractType {
	...
	$value, $hidden = false, $checkers = array(), $use_to_save = true, // Сохранять свойство в хранилище
	$read_only = false, $array_wrap_name = false, $form_template = '%system%/coreobject/type/form/string.tpl', $template, $me_template, $change_size_for_list = true, $always_modify = false, $errors = array(), // Ошибки данного свойства
	$listen_post = true, // Принимать переменные из пост при вызове метода save()
	$hint = '', // Подсказка поля
	$checkbox_param = array(), $checkbox_list = false, // Отображать список в виде чекбокосв

	$radio_list = false, $radio_list_inline = false, $vis_form = true, // Отображать в формах
	$me_visible, $view_attr = array(), $php_type = '', // Mixed
	...
	$sql_notation = '', $autoincrement = false, // Поле автоинкрементное
	$allowempty = true, $primary_key = false, // Это первичный ключ
	$default = null, $is_default_func, $unique = false, $index = false, $has_len = true, // Имеется ли длина в обозначении SQL типа
	...
	/**
	 * Вызывает методы set....
	 * или add... для ключей массива options
	 *
	 * @param array $options
	 * @return void
	 */
	...
	public function setChecker($callmethod, $errortxt = '') {
		$this->checkers[] = array('callmethod' => $callmethod, 'errortext' => $errortxt, 'param' => array_slice(func_get_args(), 1),
		);
	}
	 ...
	 * @param boolean $use_default_visible - Если ture, то по умолчанию возвращается значение общей видимости (Visible)
	 * @return bool
	 */
	 ...
	 * @return bool
	 */
	public function isMeVisible($switch = null) {
		if ($switch !== null) {
	 ...
	 * @return void
	 */
	public function setList(array $listfunc) {
	...
	function textView() {
		...
		if (isset($this->list) && in_array(gettype($this->value), array('integer', 'string', 'double', 'float',
	...
	/**
	 * Устанавливает, сохранять ли данное свойство.
	 * Если нет, то будет всегда испоьзоваться значение по-умолчанию
	 *
	 * @param bool $bool
	 * @return void
	 */
	 ...
	 *
	 * @param bool $bool - если true, то связано
	 * @return void
	 */
	...
	function getFromRequest(array $src) {
		...
		if (count($this->getCheckboxParam()) && !isset($value)) {
			$value = $this->getCheckboxParam('off');
		}
