...
{
	/**
	 * Проверяет на валидность значения поля "псевдоним".
	 * Ожидается, что данное значение должно строить ЧПУ
	 * 
	...
	/**
	 * Возвращает true, если проверяемое значение не пустое.
	 * 0 - также считается пустым значением
	 * 
	 * @param \RS\Orm\AbstractObject $orm_object - объект, который валидируется
	 * @param mixed $value - значение для проверки
	 * @param string $errtext - текст с ошибкой
	...
	/**
	 * Возвращает true, если значение не равно -1.
	 * Удобно использовать для проверки списков на поле "не выбрано"
	 * 
	 * @param \RS\Orm\AbstractObject $orm_object - объект, который валидируется
	 * @param mixed $value - значение для проверки
	 * @param string $errtext - текст с ошибкой
	...
	/**
	 * Возвращает true, если min <= значение <= max.
	 * Для числовых полей
	 * 
	 * @param \RS\Orm\AbstractObject $orm_object - объект, который валидируется
	 * @param mixed $value - значение для проверки
