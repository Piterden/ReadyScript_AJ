...
{
	private $multiedit_keys = array(), $data = array(), $valid = false, $group_set, $groups = array(), $currentgroup = null;
	...
	{
		if (! ($value instanceof Type\AbstractType)) throw new Exception ( "Неправильно установлено свойство $offset" );
	 ...
	 * @return array
	 */
	public function getGroups($hide_empty_multiedit = null, $switch = null)
	{
		...
		if (! isset ( $this->cache_groups[( int ) $hide_empty_multiedit] )) {
			$result = array();
			foreach($groups as $name => $items) {
				foreach($items as $k => $item) {
					if ($check_func == '-' || $item->$check_func ( $switch )) {
						$result[$index[$name]]['group'] = $name;
						$result[$index[$name]]['items'][$k] = $item;
					}
				}
			}
			$this->cache_groups[( int ) $hide_empty_multiedit] = $result;
		}
		
		return $this->cache_groups[( int ) $hide_empty_multiedit];
	}
	...
	{
		$this->groups = array_merge_recursive ( $this->groups, $property_iterator->exportGroups () );
