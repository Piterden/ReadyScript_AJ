...
class Serialized extends AbstractTableStorage
{
	...
	{
		$row = Request::make ()->from ( $this->table )->where ( $this->getPrimaryKeyExpr ( $primaryKeyValue ) )->exec ()->fetchRow ();
	...
	{
		$row = Request::make ()->from ( $this->table )->where ( $this->getPrimaryKeyExpr ( $primaryKeyValue ) )->count ();
		return ($row > 0);
	}
	...
	{
		return Request::make ()->delete ()->from ( $this->table )->where ( $this->getPrimaryKeyExpr () )->exec ()->affectedRows ();
	}
