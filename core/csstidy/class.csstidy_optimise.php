...
class csstidy_optimise
{
	 ...
	 * Example: margin:1px 1px 1px 1px -> margin:1px
	 * 
	 * @param string $value
	 * @access public
	 * @return string
	...
	/**
	 * Color compression function.
	 * Converts all rgb() values to #-values and uses the short-form if possible. Also replaces 4 color names by #-values.
	 * 
	 * @param string $color
	 * @return string
	 * @version 1.1
	...
	public function cut_color($color)
	{
		...
		if (strncmp ( $color, "'", 1 ) == 0 || strncmp ( $color, '"', 1 ) == 0) return $color;
		...
		if (strpos ( $color, '(' ) !== false && strncmp ( $color, 'rgb(', 4 ) != 0) {
			...
			if (stripos ( $color, 'progid:' ) !== false) return $color;
	...
	/**
	 * Compresses numbers (ie.
	 * 1.0 becomes 1 or 1.100 becomes 1.1 )
	 * 
	...
	/**
	 * Checks if a given string is a CSS valid number.
	 * If it is,
	...
	/**
	 * Merges selectors with same properties.
	...
	public function dissolve_short_bg($str_value)
	{
		...
		if (stripos ( $str_value, 'gradient(' ) !== false) return array('background' => $str_value 
		);
	...
	public function merge_bg($input_css)
	{
		...
		if (isset ( $input_css['background'] ) && $input_css['background']) return $input_css;
		...
		for($i = 0; $i < $number_of_values; $i ++) {
			foreach($background_prop_default as $bg_property => $default_value) {
				...
				if (stripos ( $cur_value, 'gradient(' ) !== false) return $input_css;
	...
	public function merge_font($input_css)
	{
		...
		if (isset ( $input_css['font-family'] ) && isset ( $input_css['font-size'] )) {
			...
			foreach($font_prop_default as $font_property => $default_value) {
				if ($font_property !== 'font' || ! $new_font_value) unset ( $input_css[$font_property] );
			}
