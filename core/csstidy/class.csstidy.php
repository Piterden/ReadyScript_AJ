...
class csstidy
{
	 ...
	 * The code is saved as array in $this->css
	 * 
					...
					if ($this->is_token ( $string, $i ) || $pn) {
						...
						if (($string{$i} === '}' || $string{$i} === ';' || $pn) && ! empty ( $this->selector )) {
							...
							while ( count ( $this->sub_value_arr ) ) {
								...
								if ($sub != '') $this->value .= ((! strlen ( $this->value ) || substr ( $this->value, - 1, 1 ) === ',') ? '' : ' ') . $sub;
							}
					...
					if ($string{$i} === $_str_char && ! $this->escaped ( $string, $i )) {
						...
						if ($this->status === 'iv') {
							if (! $_quoted_string) {
								...
								if (strpos ( $_cur_string, '  ' ) !== false) $_cur_string = preg_replace ( ",\s+,", ' ', $_cur_string );
							}
	...
	public function css_new_selector($media, $selector)
	{
		...
		if (strncmp ( $selector, '@font-face', 10 ) != 0) {
			if ($this->settings['merge_selectors'] != false) return $selector;
			
			if (! $this->css || ! isset ( $this->css[$media] ) || ! $this->css[$media]) return $selector;
	...
	public function css_new_property($media, $selector, $property)
	{
		...
		if (! $this->css || ! isset ( $this->css[$media][$selector] ) || ! $this->css[$media][$selector]) return $property;
	...
	{
		return (strpos ( $value, '!' ) !== false and // quick test
! strcasecmp ( substr ( str_replace ( $this->data['csstidy']['whitespace'], '', $value ), - 10, 10 ), '!important' ));
	 ...
	 * and returns a list of the strings.
