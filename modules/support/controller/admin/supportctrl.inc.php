...
class SupportCtrl extends \RS\Controller\Admin\Crud
{
	...
	function helperIndex()
	{
		...
		$helper->setTable ( new Table\Element ( array('Columns' => array(new \RS\Html\Table\Type\String ( 'dateof', 'Дата', array('Sortable' => SORTABLE_BOTH,'ThAttr' => array('width' => 150 
		) 
		) ),new \RS\Html\Table\Type\Usertpl ( 'is_admin', '', '%support%/user_type_cell.tpl', array('ThAttr' => array('width' => 15 
		) 
		) ),

		new \RS\Html\Table\Type\String ( 'message', 'Сообщение' ),new TableType\Actions ( 'id', array(new TableType\Action\Edit ( $this->router->getAdminPattern ( 'edit', array(':id' => '~field~' 
		) ), null, array('attr' => array('@data-id' => '@id' 
		) 
		) ),new TableType\Action\DropDown ( array(array('title' => t ( 'удалить' ),'attr' => array('@href' => $this->router->getAdminPattern ( 'del', array(':id' => '~field~' 
