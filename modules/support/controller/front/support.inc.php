...
{
	protected $api, $topic_api;
	function init()
	{
		$this->api = new \Support\Model\Api ();
		$this->topic_api = new \Support\Model\TopicApi ();
		
		$this->app->breadcrumbs->addBreadCrumb ( t ( 'Личный кабинет' ) );
	}
	function actionIndex()
	{
		...
		if ($this->url->isPost ()) {
			$addpost = array('user_id' => $this->user['id'],'is_admin' => 0 
			);
		...
		$this->view->assign ( array('supp' => $support_item,'list' => $list 
		) );
	...
	function actionViewTopic()
	{
		...
		if (! $topic || $topic['user_id'] != $this->user['id']) {
			$this->e404 ( 'Такой темы не существует' );
		}
		
		$this->app->breadcrumbs->addBreadCrumb ( t ( 'Поддержка' ), $this->router->getUrl ( 'support-front-support' ) )->addBreadCrumb ( $topic['title'] );
		
		$support_item = $this->api->getNewElement ();
		$support_item->escapeAll ( true );
		
		if ($this->url->isPost ()) {
			$addpost = array('user_id' => $this->user['id'],'is_admin' => 0,'topic_id' => $topic_id 
			);
		...
		$this->view->assign ( array('supp' => $support_item,'topic' => $topic,'list' => $list 
		) );
	...
	function actionDelTopic()
	{
		...
		return $this->result->setSuccess ( true )->setNoAjaxRedirect ( $this->router->getUrl ( 'support-front-support' ) );
	}
}