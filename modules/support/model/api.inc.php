...
class Api extends \RS\Module\AbstractModel\EntityList
{
	...
	{
		parent::__construct ( new \Support\Model\Orm\Support (), array('defaultOrder' => 'dateof DESC' 
		) );
	}
	...
	function getNewMessageCount($user_id)
	{
		...
		$sum = $q->select ( 'SUM(newcount) as sum' )->from ( new \Support\Model\Orm\Topic () )->where ( array('user_id' => $user_id 
