...
class Support extends \RS\Orm\OrmObject
{
	...
	function _init()
	{
		parent::_init ()->append ( array('site_id' => new Type\CurrentSite (),'topic' => new Type\String ( array('description' => 'Тема','checker' => array(array(__CLASS__,'checkTopic' 
		),t ( 'Укажите, пожалуйста, тему' ) 
		),'visible' => false 
		) ),'user_id' => new Type\Integer ( array('description' => 'Пользователь','visible' => false 
		) ),'dateof' => new Type\Datetime ( array('description' => 'Дата отправки' 
		) ),'message' => new Type\Text ( array('description' => 'Сообщение','checker' => array('chkEmpty',t ( 'Не задан вопрос' ) 
		) 
		) ),'processed' => new Type\Integer ( array('maxLength' => 1,'description' => 'Флаг прочтения','visible' => false 
		) ),'is_admin' => new Type\Integer ( array('maxLength' => 1,'description' => 'Это администратор','visible' => false 
		) ),'topic_id' => new Type\Integer ( array('description' => 'ID темы','visible' => false 
		) ) 
		) );
	}
	...
	function updateTopicCounts()
	{
		...
		// Общее количество
		$total_msg = $q->from ( $this )->where ( array('topic_id' => $this['topic_id'] 
		) )->count ();
		
		// Новые для пользователей (которые написал администратор)
		$q = new \RS\Orm\Request ();
		$new_msg = $q->from ( $this )->where ( array('topic_id' => $this['topic_id'],'is_admin' => '1','processed' => '0' 
		) )->count ();
		
		// Новые для администратора
		$q = new \RS\Orm\Request ();
		$new_admin_msg = $q->from ( $this )->where ( array('topic_id' => $this['topic_id'],'is_admin' => '0','processed' => '0' 
		) )->count ();
		
		$q = new \RS\Orm\Request ();
		$updated = $q->select ( "dateof" )->from ( $this )->where ( array('topic_id' => $this['topic_id'] 
		) )->orderby ( "dateof DESC" )->limit ( 1 )->exec ()->getOneField ( "dateof" );
