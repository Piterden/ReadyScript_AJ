...
class Install extends \RS\Module\AbstractInstall
{
	...
	function deferredAfterInstall($options)
	{
		if ($options['set_demo_data']) {
			...
			if ($site_config->getThemeName () == 'default') {
				...
				\Templates\Model\PageApi::setupModule ( 'default', 'menu\controller\block\menu', array('indexTemplate' => 'blocks/menu/foot_menu.tpl','root' => $bottom_root_id 
				), 'footmenu' );
			}
		}
		return true;
	...
	{
		return $this->importCsvFiles ( array(array('\Menu\Model\CsvSchema\Menu','menu' 
		) 
		), 'utf-8', $params );
	}
