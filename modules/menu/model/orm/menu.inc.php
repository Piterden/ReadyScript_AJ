...
{
    const
        TYPELINK_LINK = 'link', //Ссылка (пункт меню - это любая ссылка)
    ...
    protected
        $article_category_title = 'Привязанные к меню';
    ...
    protected function _init()
    {
        ...
        $properties = $this->getPropertyIterator()->append(array(
            t('Основные'),
                    ...
                    'menutype' => new Type\String(array(
                        'maxLength' => '70',
                    ...
                    'typelink' => new Type\String(array(
                        'maxLength' => '20',
                        'description' => 'Тип элемента',
                        'ListFromArray' => array(array(
                            self::TYPELINK_ARTICLE  => 'Статья',                         
                            self::TYPELINK_LINK     => 'Ссылка', 
                            self::TYPELINK_EMPTY    => 'Страница'
                        )),
                        'Attr' => array(array('size' => 0)),
                        'hint' => t('<strong>Статья</strong> - позволяет задать произвольный текст(или HTML) на странице данного пункта меню<br>'.
                                    '<strong>Ссылка</strong> - предназначена для переадресации на любую страницу, при нажатии на пункт меню<br>'.
                                    '<strong>Страница</strong> - означает, что Вы можете сконструировать страницу для данного пункта меню в разделе Конструктор сайта'),
                        'meVisible' => false,
                        'specVisible' => false,
                    )),
                    '__setlink__' => new Type\Mixed(array(
                        'description' => '',
                        'template' => '%menu%/form/menu/link.tpl',
                        'visible' => true,
                        'meVisible' => false,
                        'specVisible' => false,
                    )),
                    'link' => new Type\String(array(
                        'maxLength' => '255',
                        'description' => 'Ссылка',
                    ...
                    'acontent' => new Type\Richtext(array(
                        'description' => 'Статья',
                        'runtime' => true,
                        'visible' => false
                    )),                    
    ...
    function afterWrite($save_flag)
    {
        if ($this['typelink'] == self::TYPELINK_ARTICLE) { 
            if (!$article = $this->getLinkedArticle()) {
                ...
                $category['parent'] = 0;
                if (!$category->insert()) {
                    $category['id'] = \RS\Orm\Request::make()
                        ->select('id')
                        ->from($category)
                        ->where(array(
                            'site_id' => \RS\Site\Manager::getSiteId(),
                            'alias' => self::ARTICLE_CATEGORY_ALIAS
                        ))
                        ->exec()
                        ->getOneField('id');
                }
    ...
    */
    function getHref()
    {
        if ($this['typelink'] != self::TYPELINK_LINK) {
            ...
            return 
                str_replace('%ADMINPATH%', \Setup::$FOLDER.'/'.\Setup::$ADMIN_SECTION, $this['link']);
        }
    }
