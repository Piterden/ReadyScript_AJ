...
class Api extends \RS\Module\AbstractModel\TreeCookieList
{
	...
	protected $sort_field = 'sortn', $checkAccess = true, // Если true, то возвращаем только те пункты меню, на которые есть права у текущего пользователя. false - не прверять права
$accessFilters = null;
	protected static $instance;
	function __construct()
	{
		parent::__construct ( new \Menu\Model\Orm\Menu (), array('multisite' => true,'parentField' => 'parent','nameField' => 'title','aliasField' => 'alias','sortField' => 'sortn','defaultOrder' => 'parent, sortn' 
		) );
		...
		if ($add_tools_column) {
			foreach($list as $object) {
				if ($object['typelink'] == Orm\Menu::TYPELINK_SEPARATOR) {
					$object['treeTools'] = new TableType\Actions ( 'id', array(new TableType\Action\Edit ( \RS\Router\Manager::obj ()->getAdminUrl ( 'edit', array('id' => $object['id'],'mod_controller' => 'menu-ctrl' 
					) ) ) 
					) );
				}
			}
	...
	static function selectList()
	{
		...
		return array(0 => 'Верхний уровень' 
		) + $_this->getSelectList ( 0 );
	}
	
	/**
	 * Возвращает текущий пункт меню.
	 * В случае успешного обнаружения объект будет загружен (id>0)
	 * 
		...
		foreach($items as $item) {
			...
			if (isset ( $item['parent'] ) && ! is_numeric ( $item['parent'] )) {
				if (! isset ( $cache[$item['parent']] )) {
					$cache[$item['parent']] = \RS\Orm\Request::make ()->from ( $this->obj_instance )->where ( array('alias' => $item['parent'],'menutype' => $item['menutype'] 
			...
			$menu_item = Orm\Menu::loadByWhere ( array('alias' => $item['alias'],'parent' => ( int ) $item['parent'] 
			) );
			$menu_item->getFromArray ( $item );
			$menu_item->replace ();
			
			if (! empty ( $item['childs'] )) {
				$this->appendItems ( $item['childs'], array('parent' => $menu_item['id'] 
				) + $common );
			}
		}
	}
	...
	function moveElement($from, $to, $flag, \RS\Orm\Request $extra_expr = null)
	{
		...
		if (! $extra_expr) {
			$extra_expr = \RS\Orm\Request::make ()->where ( array('parent' => $from_obj['parent'],'menutype' => $from_obj['menutype'] 
			) );
		}
	...
	function getAdminMenu($hide_separators = false, $cache = true)
	{
		...
		if ($cache) {
			...
			return \RS\Cache\Manager::obj ()->request ( array($this,'getAdminMenu' 
			), $hide_separators, false, $site_id, $user->id );
		} else {
	...
	public static function getHref($item)
	{
		if ($item['typelink'] != self::TYPELINK_LINK) {
			...
			return str_replace ( '%ADMINPATH%', \Setup::$FOLDER . '/' . \Setup::$ADMIN_SECTION, $item['link'] );
		}
	}
	...
	{
		$parent = Orm\Menu::loadByWhere ( array('alias' => $parent_alias,'menutype' => $menutype 
		) );
		
		if ($parent['id']) {
			return \RS\Orm\Request::make ()->delete ()->from ( new Orm\Menu () )->where ( array('alias' => $alias,'parent' => $parent['id'],'menutype' => $menutype 
			) )->exec ()->affectedRows () > 0;
		}
		return false;
	}
	...
	function getMenuItems($root, $cache = true)
	{
		if ($cache) {
			$site_id = \RS\Site\Manager::getSiteId ();
			return \RS\Cache\Manager::obj ()->request ( array($this,__FUNCTION__ 
			), $root, false, $site_id );
		} else {
			...
			return array('root' => $root_orm,'items' => $items 
			);
		}
	}
}
