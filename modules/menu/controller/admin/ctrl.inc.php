...
use \RS\Html\Table\Type as TableType, \RS\Html\Toolbar\Button as ToolbarButton, \RS\Html\Toolbar, \RS\Html\Tree;
...
class Ctrl extends \RS\Controller\Admin\Crud
{
	...
	function helperIndex()
	{
		...
		$helper->setTopToolbar ( new Toolbar\Element ( array('Items' => array(new ToolbarButton\Dropdown ( array(array('title' => t ( 'добавить пункт меню' ),'attr' => array('class' => 'crud-add button add','href' => $this->router->getAdminUrl ( 'add' ) 
		) 
		),array('title' => t ( 'добавить разделитель' ),'attr' => array('class' => 'crud-add','href' => $this->router->getAdminUrl ( 'add', array('sep' => 1 
		) ) 
		) 
		) 
		...
		$helper->setTree ( new Tree\Element ( array('disabledField' => 'public','disabledValue' => '0','activeField' => 'id','sortIdField' => 'id','hideFullValue' => true,'sortable' => true,'mainColumn' => new TableType\Usertpl ( 'title', 'Название', '%menu%/tree_column.tpl' ),'tools' => new TableType\Actions ( 'id', array(new TableType\Action\Edit ( $this->router->getAdminPattern ( 'edit', array(':id' => '~field~' 
		) ), null, array('attr' => array('@data-id' => '@id' 
		) 
		) ),new TableType\Action\DropDown ( array(array('title' => 'Редактировать мета-теги','attr' => array('@href' => $this->router->getAdminPattern ( 'edit', array(':id' => 'menu.item_{id}','create' => 1 
		), 'pageseo-ctrl' ),'class' => 'crud-add' 
		) 
		),array('title' => t ( 'Клонировать' ),'attr' => array('class' => 'crud-add','@href' => $this->router->getAdminPattern ( 'clone', array(':id' => '~field~' 
		) ) 
		) 
		),array('title' => 'Добавить дочерний элемент','attr' => array('@href' => $this->router->getAdminPattern ( 'add', array(':pid' => '@id' 
		) ),'class' => 'crud-add' 
		) 
		),array('title' => 'Добавить разделитель','attr' => array('@href' => $this->router->getAdminPattern ( 'add', array(':pid' => '@id','sep' => 1 
		) ),'class' => 'crud-add' 
		) 
		),array('title' => 'Показать на сайте','attr' => array('@href' => $this->router->getAdminPattern ( 'itemRedirect', array(':id' => '@id' 
		) ),'target' => '_blank' 
		) 
		) 
		) ) 
	...
	function actionItemRedirect()
	{
		...
		if ($id > 0) {
			$this->redirect ( $this->router->getUrl ( 'menu.item_' . $id ) );
		}
	}
