...
{
	const COMMENT_TYPE_FOLDER = '/model/commenttype'; // Путь к папке с классами типов комментариев. Относительно корня модуля
	protected $config, // Конфиг комментариев
$replace_by_ip = false; // только 1 комментарий от одного ip на один aid
	function __construct()
	{
		$this->config = \RS\Config\Loader::byModule ( 'comments' );
		parent::__construct ( new \Comments\Model\Orm\Comment (), array('loadOnDelete' => true,'multisite' => true,'defaultOrder' => 'dateof DESC' 
		) );
	}
	...
	{
		$res = \RS\Orm\Request::make ()->select ( 'type' )->from ( $this->obj_instance )->groupby ( 'type' )->exec ();
	...
	function addComment($aid, $type)
	{
		...
		$data = array('aid' => $aid,'type' => $type,'moderated' => 0,'dateof' => date ( 'Y-m-d H:i:s' ),'ip' => $_SERVER['REMOTE_ADDR'],'useful' => 0,'help_yes' => 0,'help_no' => 0 
		);
		...
		if ($this->getElement ()->checkData ( $data )) {
			if ($this->replace_by_ip) {
				...
				\RS\Orm\Request::make ()->delete ( 'A, B' )->from ( $this->obj_instance )->asAlias ( 'A' )->leftjoin ( new Orm\Vote (), 'A.id = B.comment_id', 'B' )->where ( "A.aid = '#aid' AND A.ip = '#ip'", array('aid' => $data['aid'],'ip' => $data['ip'] 
				) )->exec ();
			}
	...
	function markHelpful($comment_id, $help)
	{
		...
		\RS\Orm\Request::make ()->update ( $this->obj_instance )->set ( "help_yes = (SELECT COUNT(*) FROM " . $vote->_getTable () . " WHERE comment_id = id AND help=1)" )->set ( "help_no = (SELECT COUNT(*) FROM " . $vote->_getTable () . " WHERE comment_id = id AND help='-1')" )->set ( "useful = (SELECT SUM(help) FROM " . $vote->_getTable () . " WHERE comment_id = id)" )->where ( array('id' => $comment_id 
		) )->exec ();
	}
	...
	{
		$result = \RS\Orm\Request::make ()->select ( 'SUM(rate) sum, COUNT(*) cnt' )->from ( $comment )->where ( array('aid' => $comment['aid'],'type' => $comment['type'] 
		) )->exec ()->fetchRow ();
		$cnt = isset ( $result['cnt'] ) ? $result['cnt'] : 0;
		$rating = round ( ($cnt > 0) ? (isset ( $result['sum'] ) ? $result['sum'] : 0) / $cnt : 0, 1 );
		
		\RS\Orm\Request::make ()->update ( $object )->set ( array('rating' => $rating,'comments' => $cnt 
	...
	function alreadyWrite($aid, $ip = null)
	{
		...
		$count = \RS\Orm\Request::make ()->from ( $this->obj_instance )->where ( array('aid' => $aid,'ip' => $ip 
		) )->count ();
		return ($count > 0);
	}
}
