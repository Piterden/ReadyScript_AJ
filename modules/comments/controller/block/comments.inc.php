...
{
	protected static $controller_title = 'Комментарии', $controller_description = 'Отображает список комментариев к объекту. Позволяет добавить комментарий пользователю';
	protected $default_params = array('indexTemplate' => 'blocks/comments/comment_block.tpl','listTemplate' => 'blocks/comments/list.tpl','pageSize' => 10 
	), $action_var = 'cmdo', $type, $aid, $config, 

	$sortby, $page, $pageSize, $api;
	...
	{
		return parent::getParamObject ()->appendProperty ( array('listTemplate' => new Type\Template ( array('description' => t ( 'Шаблон списка' ) 
		) ),'type' => new Type\String ( array('description' => t ( 'Тип комментариев' ),'list' => array(array('\Comments\Model\Api','getAllowTypes' 
		) 
		) 
		) ),'pageSize' => new Type\Integer ( array('description' => t ( 'Количество комментариев на странице' ) 
		) ) 
		) );
	}
	function checkType()
	{
		...
		if (! class_exists ( $this->type ) || ! is_subclass_of ( $this->type, '\Comments\Model\Abstracttype' )) {
	...
	function actionIndex()
	{
		...
		if ($this->isMyPost () && ($this->config['need_authorize'] == 'N' || \RS\Application\Auth::isAuthorize ())) {
			if ($this->api->addComment ( $this->aid, $this->type )) {
				$this->refreshPage ();
			} else {
				$this->view->assign ( 'error', $this->api->getElement ()->getLastError () );
			}
		} else {
		...
		$this->view->assign ( array('already_write' => $already_write,'aid' => $this->aid,'comment' => $this->api->getElement (),'mod_config' => $this->config,'list_html' => $this->actionGetCommentList ()->getHtml () 
		) );
	...
	function actionGetCommentList()
	{
		...
		$this->api->setFilter ( 'type', $this->type )->setFilter ( 'aid', $this->aid );
		...
		$this->view->assign ( array('commentlist' => $list,'total' => $total,'paginator' => $paginator 
		) );
