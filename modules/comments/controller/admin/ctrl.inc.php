...
class Ctrl extends \RS\Controller\Admin\Crud
{
	...
	function helperIndex()
	{
		...
		) ),new TableType\String ( 'message', 'Комментарий' ),new TableType\String ( 'aid', 'Связь №', array('Sortable' => SORTABLE_BOTH 
		) ),new TableType\String ( 'ip', 'IP' ),new TableType\StrYesno ( 'moderated', 'Промодерированно' ),new TableType\String ( 'type_obj_title', 'Тип', array('TdAttr' => array('class' => 'cell-sgray' 
		) 
		...
		) ),new Filter\Type\Select ( 'type', 'Категория', $typelist ),new Filter\Type\Select ( 'moderated', 'Промодерированно?', array(0 => 'Нет',1 => 'Да' 
		) ),new Filter\Type\String ( 'aid', 'Связь №' ),new Filter\Type\Date ( 'dateof', 'Дата', array('showtype' => true 
		) ) 
		) 
		) ) 
		),'open' => true 
	...
	function helperEdit($primaryKey = null)
	{
		...
		$helper['bottomToolbar']->addItem ( new ToolbarButton\delete ( $this->router->getAdminUrl ( 'delComment', array('id' => $id,'dialogMode' => $this->url->request ( 'dialogMode', TYPE_INTEGER ) 
		) ), null, array('noajax' => true,'attr' => array('class' => 'delete crud-get crud-close-dialog','data-confirm-text' => t ( 'Вы действтельно хотите удалить данный комментарий?' ) 
		) 
		) ), 'delete' );
