...
class FormApi extends \RS\Module\AbstractModel\EntityList
{
	...
	{
		parent::__construct ( new \Feedback\Model\Orm\FormItem (), array('aliasField' => 'alias','nameField' => 'title','multisite' => true,'defaultOrder' => 'title' 
		) );
	}
	...
	function send(Orm\FormItem $form, \RS\Http\Request $request)
	{
		...
		foreach($fields as $k => $form_field) {
			...
			switch ($form_field['show_type']) {
				case Orm\FormFieldItem::SHOW_TYPE_EMAIL : // Если E-mail
					...
					if (! filter_var ( $value, FILTER_VALIDATE_EMAIL )) {
						$this->addError ( t ( 'В поле %0 E-mail заполнен не правильно', array($form_field['title'] 
						) ), $form_field['alias'] );
					}
			...
			$mail[] = array('field' => $form_field,'value' => $value 
			);
		}
