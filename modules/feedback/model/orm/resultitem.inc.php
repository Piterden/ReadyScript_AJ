 ...
 * Объект результата отправки формы
...
'form_id' => new Type\Integer ( array('description' => t ( 'Форма' ),'List' => array(array('\Feedback\Model\FormApi','staticSelectList' 
		...
		) ),'title' => new Type\String ( array('maxLength' => '150','description' => t ( 'Название' ),'meVisible' => false 
		) ),'dateof' => new Type\Datetime ( array('maxLength' => '150','description' => t ( 'Дата отправки' ) 
		) ),'status' => new Type\Enum ( array('new','viewed' 
		), array('maxLength' => '1','allowEmpty' => false,'default' => 'new','listFromArray' => array(array('new' => 'Новое','viewed' => 'Просморен' 
		) 
		),'description' => t ( 'Статус' ) 
		) ),'ip' => new Type\String ( array('maxLength' => '150','description' => t ( 'IP Пользователя' ),'meVisible' => false 
		) ),'stext' => new Type\Text ( array('description' => t ( 'Содержимое результата формы' ),'Template' => 'form/field/stext.tpl','meVisible' => false 
		) ),'answer' => new Type\Text ( array('description' => t ( 'Ответ' ),'meVisible' => false 
		) ),'send_answer' => new Type\Integer ( array('maxLength' => '1','meVisible' => false,'checkBoxView' => array(1,0 
		),'runtime' => true,'description' => t ( 'Отправить ответ на E-mail пользователя' ) 
		) ) 
		) );
	}
	...
	function afterWrite($flag)
	{
		if ($flag == self::INSERT_FLAG) { // Флаг вставки
			$this['title'] = "Сообщение №" . $this['id']; // Обновим название
			$this->update ();
		}
	}
