	...
	function deferredAfterInstall($options)
	{
		if ($options['set_demo_data']) {
			...
			if ($site_config->getThemeName () == 'default') {
				$form = \Feedback\Model\Orm\FormItem::loadByWhere ( array('title' => 'Обратная связь' 
				) );
				
				// Настраиваем блок новости в дефолтном шаблоне
				\Templates\Model\PageApi::setupModule ( null, 'feedback\controller\block\button', array('form_id' => $form['id'] 
				) );
			}
		}
		return true;
