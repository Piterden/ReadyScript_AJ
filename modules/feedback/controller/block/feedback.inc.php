...
{
	protected static $controller_title = 'Форма отправки на E-mail', $controller_description = 'Отображает форму связи для пользователя с уведомлением, на E-mail';
	protected $default_params = array('indexTemplate' => 'blocks/feedback/feedback.tpl' 
	) // Шаблон отображения произвольной формы
;
	...
	{
		return parent::getParamObject ()->appendProperty ( array('form_id' => new Type\String ( array('description' => t ( 'Выберите форму из списка' ),'list' => array(array('\Feedback\Model\FormApi','staticSelectList' 
		) 
		) 
		) ),'hvalues' => new Type\ArrayList ( array('description' => t ( 'Массив для передачи скрытых полей key=>value' ),'visible' => false 
		) ),'values' => new Type\ArrayList ( array('description' => t ( 'Массив для передачи в уже существующие поля key=>value' ),'visible' => false 
		) ) 
		) );
	}
	...
	function actionIndex()
	{
		...
		$this->view->assign ( array('form' => $form,'error_fields' => $errors,'request' => $request 
		) );
