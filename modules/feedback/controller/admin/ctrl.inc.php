...
class Ctrl extends \RS\Controller\Admin\Crud
{
	...
	function helperIndex()
	{
		...
		$collection->setTopToolbar ( new Toolbar\Element ( array('Items' => array(new ToolbarButton\Dropdown ( array($dir_count > 0 ? // Если форм ещё нету, то скроем кнопку добавления поля
array('title' => t ( 'добавить поле' ),'attr' => array('href' => $this->router->getAdminUrl ( 'add', array('dir' => $this->dir 
		) ),'class' => 'button add crud-add' 
		) 
		) : null,array('title' => t ( 'добавить форму' ),'attr' => array('href' => $this->router->getAdminUrl ( 'add_dir' ),'class' => 'crud-add ' . (($dir_count == 0) ? 'button add' : '') 
		) 
		) 
		...
		) ),new TableType\String ( 'show_type', 'Тип' ),new TableType\StrYesno ( 'required', 'Обязательное' ),new TableType\String ( 'hint', 'Подпись', array('hidden' => true 
		...
		),'SecContainers' => array(new Filter\Seccontainer ( array('Lines' => array(new Filter\Line ( array('items' => array(new Filter\Type\String ( 'alias', t ( 'Псевдоним' ), array('SearchType' => 'like%' 
		) ) 
		) 
		) ) 
		) 
		...
		if ($dir['id'] && $collection->getTreeViewType () == \RS\Controller\Admin\Helper\CrudCollection::VIEW_CAT_TOP) {
			...
			$path_to_first = array(array('title' => t ( 'Не создано ни одной формы' ),'hideInlineButtons' => array('edit','delete' 
			) 
			) 
			);
		}
		...
		$collection->setTree ( new Tree\Element ( array('sortIdField' => 'sortn','activeField' => 'id','activeValue' => $this->dir,'noExpandCollapseButton' => true,'sortable' => false,'pathToFirst' => $path_to_first ?  : array(),'mainColumn' => new TableType\String ( 'title', t ( 'Название' ), array('href' => $this->router->getAdminPattern ( false, array(':dir' => '@id' 
		) ) 
		) ),'tools' => new TableType\Actions ( 'id', array(new TableType\Action\Edit ( $this->router->getAdminPattern ( 'edit_dir', array(':id' => '~field~' 
		) ), null, array('attr' => array('@data-id' => '@id' 
		) 
		) ),new TableType\Action\DropDown ( array(array('title' => t ( 'показать форму на сайте' ),'attr' => array('@href' => $this->router->getUrlPattern ( 'feedback-front-form', array(':form_id' => '~field~' 
		), true ),'target' => '_blank' 
		) 
		...
		) ),'inlineButtons' => array('add' => array('attr' => array('title' => t ( 'создать форму' ),'href' => $this->router->getAdminUrl ( 'add_dir' ),'class' => 'add crud-edit' 
		) 
		...
		),'headButtons' => array(array('text' => t ( 'Название формы' ),'tag' => 'span','attr' => array('class' => 'lefttext' 
		) 
		),array('attr' => array('title' => 'Создать форму','href' => $this->router->getAdminUrl ( 'add_dir' ),'class' => 'add crud-add' 
		) 
		) 
		) 
		) ), $this->dirapi );
