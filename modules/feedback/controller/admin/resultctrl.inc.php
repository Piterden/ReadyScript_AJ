...
class ResultCtrl extends \RS\Controller\Admin\Crud
{
	...
	function helperIndex()
	{
		...
		) ),new TableType\String ( 'dateof', 'Дата создания' ),new TableType\String ( 'status', 'Статус', array('Sortable' => SORTABLE_BOTH,'CurrentSort' => SORTABLE_ASC 
		) ),new TableType\String ( 'id', '№' ),new TableType\Actions ( 'id', array(new TableType\Action\Edit ( $this->router->getAdminPattern ( 'edit', array(':id' => '~field~' 
		...
		) ),new Filter\Type\Datetime ( 'dateof', t ( 'Дата создания' ) ),new Filter\Type\Select ( 'status', t ( 'Статус' ), array('new' => 'Новый','viewed' => 'Просморен' 
		) ) 
		) 
		) ) 
		) 
		...
		$collection->setTree ( new Tree\Element ( array('sortIdField' => 'sortn','activeField' => 'id','activeValue' => $this->dir,'noCheckbox' => true,'noExpandCollapseButton' => true,'sortable' => false,'pathToFirst' => $path_to_first ?  : array(),'mainColumn' => new TableType\String ( 'title', t ( 'Название' ), array('href' => $this->router->getAdminPattern ( false, array(':dir' => '@id' 
		) ) 
