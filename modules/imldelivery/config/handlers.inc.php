...
class Handlers extends \RS\Event\HandlerAbstract
{
    ...
    */
    function init()
    {
        $this
            ...
            ->bind('deliverytype.applyextrachangediscount')
    ...
    function initialize()
    {
        ...
        \Catalog\Model\Orm\Product::attachClassBehavior(new \Imldelivery\Model\Behavior\Product);
        \Users\Model\Orm\User::attachClassBehavior(new \Imldelivery\Model\Behavior\User);
    }
    ...
    * @param array $list - массив из передаваемых классов доставок
    */
    public static function ormInitShopPayment($params)
    {
        $params->getPropertyIterator()->append(array(
	        'Такса',
            'tax_sign' => new Type\Enum(array('+', '*'), array(
                'allowEmpty' => false,
                'description' => t('Знак таксы'),
                'listFromArray' => array(array(
                    '+' => 'Прибавить',
                    '*' => 'Умножить',
                ))
            )),
	        'tax_value' => new Type\Integer(array(
                'maxLength' => '11',
                'allowEmpty' => true,
                'description' => t('Величина таксы'),
            )),
	    ));
	    //return $params;
	    // echo "<pre>";
	    // print_r($params->getPropertyIterator());
    }

    public static function deliveryTypeApplyExtraChangeDiscount($data)
    {
    	$cost = $data['cost'];
