...
<script>
            ...
            IML.imlData.sd_list.features.push({
                ...
                "properties": {
                    balloonContent: '<span style="font-weight: bold">' + el.Name + '</span>' + '<br />' +
                        ...
                        '<br /><button class="balloonSelectSd" onclick="submitModal(event, ' + el.RequestCode +
    ...
    function bindHandlers() {
        ...
        $('#mapModal').unbind('hide.bs.modal').on('hide.bs.modal', function() {
            $this = $(this);
            $this.find('.modal-body').empty();
        });
        $('button.submitModal').unbind('click').on('click', function() {
            saveDataToOrder();
            return false;
        });
    }
