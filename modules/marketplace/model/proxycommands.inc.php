...
class ProxyCommands extends BaseModel
{
	...
	public function executeCommand($params)
	{
		...
		exit ( json_encode ( array('success' => false,'message' => 'Wrong Command: ' . $params['command'] 
		) ) );
	}
	private function commandDownloadAddon($params)
	{
		if ($error = Rights::CheckRightError ( $this, ACCESS_BIT_WRITE )) {
			return array('success' => false,'status' => 'error','message' => t ( 'Недостаточно прав на установку дополнений' ) 
			);
		}
		...
		if (! $res) {
			return array('success' => false,'status' => 'error','message' => t ( 'Не удалость соединиться с сервером Marketplace' ) 
			);
		}
		...
		if ($proxy_api->content_type == 'application/octet-stream') {
			...
			return array('success' => true,'status' => 'process' 
			);
		}
		...
		if ($proxy_api->content_type == 'application/json') {
			...
			if (! $decoded) {
				return array('success' => false,'status' => 'error','message' => t ( 'Сервер Marketplace вернул неверный JSON' ) 
				);
			}
			...
			if ($decoded->success && $decoded->status == 'done') {
				...
				return array('success' => true,'status' => 'done' 
				);
			}
			
			// Все прочие типы ответов являются сообщениями об ошибках
			return array('success' => false,'status' => 'error','message' => $decoded->message 
			);
		}
		
		// Не удалось распознать тип содержимого
		return array('success' => false,'status' => 'error','message' => t ( 'Недопустимый Content Type' ),'server_response' => $res 
		);
	}
	private function commandInstallAddon($params)
	{
		...
		if ($install_api->hasError ()) {
			return array('success' => false,'status' => 'error','message' => join ( ', ', $install_api->getErrors () ) 
			);
		}
		return array('success' => true,'status' => 'done' 
		);
