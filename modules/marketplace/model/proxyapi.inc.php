...
class ProxyApi extends BaseModel
{
	...
	public function request($url, $post_data = false)
	{
		...
		$headers_to_send = array('X-Readyscript-Iframe: True','X-ReadyScript-Lang: ' . \RS\Language\Core::getCurrentLang (),'X-ReadyScript-ClientVersion: ' . $config['version'],'X-ReadyScript-ProxyUrl: ' . \RS\Router\Manager::obj ()->getAdminUrl ( false, array(), 'marketplace-proxy', true ),'X-ReadyScript-Addons: ' . $installed_addons 
		);
	...
	public function getLicensesInfo()
	{
		...
		foreach($licenses as $one) {
			$license_arr = array('license' => $one['license'],'data' => $one['data'] 
			);
			$row = __GET_LICENSE_INFO ( $license_arr );
			
			$lic_keys[] = $one['license'];
			$signs[] = md5 ( $row['sites'] . $row['type'] . $row['update_months'] . $row['expire'] . $row['expire_month'] . $row['update_expire'] . $row['date_of_activation'] . $row['product'] . $row['domain'] . $row['upgrade_to_product'] );
		}
		
		return array('keys' => $lic_keys,'signs' => $signs 
		);
	}
