...
class InstallApi extends BaseModel
{
	...
	public function installAddon($archive_file_path)
	{
		...
		if (! $addon_type) {
			foreach($this->addonArchiveApi->getErrors () as $error)
				$this->addError ( $error );
			return false;
		}
		...
		else if ($addon_type === 'solution') return $this->installSolution ( $archive_file_path );
	...
	public function installTemplate($archive_file_path)
	{
		...
		$item = new \RS\Theme\Item ( $info['name'] );
	...
	{
		$url = Manager::obj ()->getUrl ( 'marketplace-front-checkforfatal', array(), true );
		
		$context = array('ssl' => array('allow_self_signed' => true,'verify_peer' => false,'verify_peer_name' => false 
		) 
		);
