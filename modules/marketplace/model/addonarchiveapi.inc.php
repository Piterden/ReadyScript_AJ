...
class AddonArchiveApi extends \RS\Module\AbstractModel\BaseModel
{
	...
	public function fetchModuleInfo($filename)
	{
		...
		if (! $config_content) return $this->addError ( t ( 'Отсутствует файл конфигурации <module-name>/config/module.xml' ) );
		...
		if (! $module_info['version']) return $this->addError ( t ( 'Не указана версия модуля в файле config/file.inc.php' ) );
	...
	public function fetchTemplateInfo($filename)
	{
		...
		if (! $config_content) return $this->addError ( t ( 'Отсутствует файл информации о теме оформления <theme-folder>/theme.xml' ) );
		
		$template_info = self::parseThemeConfig ( $config_content );
		$template_info['name'] = $theme_name;
		
		if (! $template_info['version']) return $this->addError ( t ( 'Не указана версия темы в файле <theme-folder>/theme.xml' ) );
	...
	{
		if (! file_exists ( $filename )) return $this->addError ( t ( 'Архив дополения не найден' ) );
		...
		if ($root_items_count != 1) return $this->addError ( t ( 'Архив дополения должен содержать только одну папку в корне. Найдено %0', array($root_items_count 
		) ) );
		...
		if ($zip->getFromName ( "{$first_folder_name}/config/file.inc.php" )) {
			...
			else return self::TYPE_MODULE;
		} else if ($zip->getFromName ( "{$first_folder_name}/theme.xml" ))
			return self::TYPE_TEMPLATE;
		else return $this->addError ( t ( 'Не найден файл конфигурации' ) );
	}
