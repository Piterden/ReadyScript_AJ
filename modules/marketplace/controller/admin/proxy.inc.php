...
use RS\Exception;
use \RS\Router\Manager as RouterManager, \RS\Html\Table\Type as TableType, \RS\Html\Table;

class Proxy extends \RS\Controller\Admin\Front
{
	private static $allowed_domains = array(), $protocol, $start_url;
	
	/** @var ProxyApi */
	private $proxyApi;
	static public function staticInit()
	{
		self::$allowed_domains = array(\Setup::$MARKETPLACE_DOMAIN,'readyscript.ru','readyscript.local' 
		);
		self::$protocol = (! empty ( $_SERVER['HTTPS'] ) && $_SERVER['HTTPS'] != 'off') || (isset ( $_SERVER['HTTP_X_FORWARDED_PROTO'] ) && $_SERVER['HTTP_X_FORWARDED_PROTO'] == 'https') ? 'https' : 'http';
	...
	private function makeUrlsAbsolute($html, $host)
	{
		...
		$html = preg_replace_callback ( '~((href|src|action)\s*=\s*[\"\'])([^\"\']+)~i', function ($x) use($host, $protocol) {
	...
	private function wrapUrlsWithProxy($html)
	{
		...
		$html = preg_replace_callback ( '/(<a.*?\shref=["\'])([^"\']+?)(["\'].*?>)/', function ($match) use($router) {
			...
			return $match[1] . $router->getAdminUrl ( false, array('url' => $match[2] 
			), 'marketplace-proxy' ) . $match[3];
		}, $html );
