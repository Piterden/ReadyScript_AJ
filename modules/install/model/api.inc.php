...
{
	const NEED_PHP_VERSION = '5.3.0', NEED_MYSQL_VERSION = '5.0';
	private $local_storage, $local_storage_file, $check_write_rights = array('/storage' => array('description' => 'Папка для хранения пользовательских файлов' 
	),'/cache' => array('description' => 'Папка для хранения кэш-данных' 
	),'/templates' => array('description' => 'Папка для шаблонов. Доступ необходим для корректной работы функции установки новых тем оформления' 
	),'/modules' => array('description' => 'Папка для модулей. Доступ необходим для корректной работы функции установки и обновления модулей' 
	),'/' => array('description' => 'В процессе установки в корне будет создан файл конфигурации' 
	) 
	), $exts;
	...
	public function resetInstall($config)
	{
		...
		$this->setKey ( 'progress', array('config' => $config,'stack' => $stack,'steps' => $steps,'percent' => 0,'pos' => 0 
		) );
	...
	public function doAfterInstall()
	{
		...
		foreach($status['stack'] as $module) {
			...
			if ($install = $mod_item->getInstallInstance ()) {
				...
				if ($result !== true) {
					return $this->makeError ( t ( "Модуль '%0'", array($module 
					) ), $result );
				}
			}
		}
	...
	public function progress()
	{
		$status = $this->getKey ( 'progress' );
		if (! $status) {
			return $this->makeError ( null, 'Внутренняя ошибка, попробуйте запустить установку заново' );
		}
		$install_result = array();
		if ($status['pos'] < count ( $status['steps'] )) {
			$do = $status['steps'][$status['pos']];
			
			$install_result = call_user_func_array ( array($this,$do['action'] 
			), array_diff_key ( $do, array('action' => true,'caption' => true 
			) ) );
	...
	public function doInstall($module)
	{
		...
		if (($install_result = $current_module->install ()) !== true) {
			...
			return $this->makeError ( t ( "Модуль '%0'", array($mod 
			) ), $install_result );
		}
		return true;
	}
	...
	protected function finishInstall()
	{
		...
		$htaccess_php_deny = '# Запрещаем открывать php файлы напрямую' . "\n" . '<Files ~ "(\.php.?)$">' . "\n" . 'Order allow,deny' . "\n" . 'Deny from all' . "\n" . '</Files>';
		...
		$result = array('next' => t ( 'Установка завершена' ),'complete' => true,'percent' => 100 
		);
	...
	public function installComplete()
	{
		...
		$notice->init ( $progress['config']['supervisor_email'], $progress['config']['supervisor_pass'], $progress['config']['admin_section'], \RS\Helper\IdnaConvert::getInstance ()->decode ( \Setup::$DOMAIN ) );
	...
	protected function makeError($module_title, $message)
	{
		...
		foreach($message as $text) {
			$return['errors'][] = array('moduleTitle' => $module_title,'message' => $message 
			);
		}
	...
	{
		$check_result = array('php_version' => $this->checkPhpVersion (),'mysql_support' => $this->checkMysqlSupport (),'safe_mode' => $this->checkSafeMode (),'upload_files' => $this->checkUploadFiles (),'gd' => $this->checkGDSupport (),'zip' => $this->checkZipSupport (),'mbstring' => $this->checkMbSupport (),'curl' => $this->checkCurl (),'mcrypt' => $this->checkMCrypt (),'write_rights' => $this->checkWriteRights (),'can_continue' => true 
		);
	...
	public function checkGDSupport()
	{
		...
		return array('decision' => in_array ( 'gd', $exts ) 
		);
	}
	...
	{
		$exts = $this->getLoadedExtensions ();
		return array('decision' => in_array ( 'zip', $exts ) 
		);
	}
	...
	public function checkmbSupport()
	{
		...
		return array('decision' => in_array ( 'mbstring', $exts ) && ($func_overload == 0 || $func_overload == 1) 
		);
	}
	
	/**
	 * Проверяет наличие модуля mcrypt с шифрованием twofish
	 */
	public function checkMCrypt()
	{
		$exts = $this->getLoadedExtensions ();
		return array('decision' => in_array ( 'mcrypt', $exts ) && @mcrypt_module_open ( 'twofish', '', 'ecb', '' ) !== false 
		);
	}
	...
	{
		$ver = phpversion ();
		return array('server' => $ver,'decision' => $this->compareVersion ( self::NEED_PHP_VERSION, $ver ),'need' => self::NEED_PHP_VERSION 
		);
	}
	...
	{
		$exts = $this->getLoadedExtensions ();
		return array('decision' => in_array ( 'mysqli', $exts ),'need' => self::NEED_MYSQL_VERSION 
		);
	}
	...
	public function checkSafeMode()
	{
		...
		return array('decision' => $safe_mode == 0 
		);
	}
	...
	public function checkUploadFiles()
	{
		...
		return array('decision' => $file_uploads == 1 
		);
	}
	...
	{
		$exts = $this->getLoadedExtensions ();
		return array('decision' => in_array ( 'curl', $exts ) 
		);
	}
