...
class Wizard extends \RS\Controller\AbstractModule
{
	...
	function step1()
	{
		...
		$this->view->assign ( array('license_text' => $this->api->getLicenseText ( $current_lang ),'locale_list' => $locale_list,'current_lang' => $current_lang,'step' => 1 
		) );
		return $this->wrapHtml ( $this->view->fetch ( '%install%/step1.tpl' ) );
	}
	function step2()
	{
		$this->view->assign ( array('check' => $this->api->checkServerParams (),'step' => 2 
		) );
	...
	function step3()
	{
		if ($this->url->isPost ()) {
			...
			$config = $this->url->post ( array('start','db_host','db_name','db_prefix',

			'supervisor_email','supervisor_pass','supervisor_pass_confirm','admin_section','set_demo_data' 
			), TYPE_STRING );
			...
			if (! $CHK->isErr ()) {
				...
				foreach($CHK->GetErrArr () as $err_text) {
					$errors[] = array('moduleTitle' => count ( $errors ) + 1,'message' => $err_text 
					);
				}
		...
		$this->view->assign ( array('step' => 3,'generated_prefix' => \RS\Helper\Tools::generatePassword ( 4, range ( 'a', 'z' ) ) . '_' 
		) );
	...
	function step4()
	{
		...
		$this->view->assign ( array('license_key' => $license_key,'license' => $license,'result' => $result,'step' => 4 
		) );
	...
	function step4a()
	{
		...
		$this->view->assign ( array('license' => $license,'step' => '4a' 
		) );
	...
	function step5()
	{
		...
		$this->view->assign ( array('step' => 5,'email' => $progress['config']['supervisor_email'],'password' => $progress['config']['supervisor_pass'],'admin_section' => $progress['config']['admin_section'] 
		) );
