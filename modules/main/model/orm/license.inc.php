...
{
	const HASHSTORE_ACTIVATION_DATA = 'ACTIVATION_DATA', 

	ERROR_NO_LICENSE_KEY = 1, ERROR_LICENSE_NOT_FOUND = 2, ERROR_WRONG_COPY_ID = 3, ERROR_WRONG_INSTALL_ID = 4, ERROR_KEY_NEED_ACTIVATE = 5, ERROR_ACTIVATE = 6, ERROR_CHECK_DOMAIN = 7, ERROR_EXTRA_ACTIVATE = 8, ERROR_SCRIPT_TYPE = 9;
	protected static $table = 'license';
	protected $license_type, $license_expire_month, $need_type, $need_activation;
	function _init()
	{
		$this->getPropertyIterator ()->append ( array('license' => new Type\String ( array('maxLength' => 24,'attr' => array(array('size' => 50 
		) 
		),'description' => t ( 'Лицензионный номер' ),'primaryKey' => true 
		) ),'is_activation' => new Type\Integer ( array('description' => 'Активировать ключ','runtime' => true,'visible' => false,'actmode' => true 
		) ),
				// Сведения для активации ключа
				'person' => new Type\String ( array('description' => 'Контактное лицо','activationVisible' => true,'visible' => false,'runtime' => true,'actmode' => true 
				) ),'company_name' => new Type\String ( array('description' => 'Наименование компании','activationVisible' => true,'visible' => false,'runtime' => true,'actmode' => true 
				) ),'inn' => new Type\String ( array('maxLength' => 12,'description' => 'ИНН','activationVisible' => true,'visible' => false,'runtime' => true,'actmode' => true 
				) ),'phone' => new Type\String ( array('maxLength' => 100,'description' => 'Телефон','activationVisible' => true,'visible' => false,'runtime' => true,'actmode' => true 
				) ),'email' => new Type\String ( array('maxLength' => 120,'description' => 'E-mail','activationVisible' => true,'visible' => false,'runtime' => true,'actmode' => true 
				) ),'domain' => new Type\String ( array('description' => 'Домен','activationVisible' => true,'hint' => t ( 'Сайт обязательно должен быть доступен на данном доменном имени' ),'visible' => false,'runtime' => true,'actmode' => true 
				) ),'check_domain' => new Type\Integer ( array('description' => 'Проверить отклик магазина по доменному имени','hint' => t ( 'Это поможет исключить активацию ключа на неверное доменное имя. Снимайте эту отметку, только если уверены в корректности написания доменного имени' ),'checkboxView' => array(1,0 
				),'activationVisible' => true,'default' => 1,'visible' => false,'runtime' => true,'actmode' => true 
				) ),'data' => new Type\Blob ( array('visible' => false 
				) ),'type' => new Type\String ( array('maxLength' => 50,'description' => 'Тип лицензии','visible' => false 
				) ) 
		) );
	}
	...
	function fillDefaultActivationValue()
	{
		...
		if (! ($data = \RS\HashStore\Api::get ( self::HASHSTORE_ACTIVATION_DATA ))) {
			...
			$data = array('domain' => \RS\Http\Request::commonInstance ()->server ( 'HTTP_HOST', TYPE_STRING ) 
			);
		}
