...
class Widgets extends \RS\Orm\OrmObject
{
	...
	function _init()
	{
		parent::_init ()->append ( array('site_id' => new Type\CurrentSite (),'user_id' => new Type\Integer ( array('maxLength' => '11' 
		) ),'col' => new Type\String ( array('maxLength' => '20' 
		) ),'position' => new Type\Integer ( array('maxLength' => '5' 
		) ),'class' => new Type\String ( array('maxLength' => '255' 
		) ),'vars' => new Type\Text ( array() ) 
		) );
		
		$this->addIndex ( array('site_id','user_id','class' 
		), self::INDEX_UNIQUE );
	}
	function beforeWrite($flag)
	{
		if ($this['position'] === null) {
			...
			$this['position'] = \RS\Orm\Request::make ()->from ( $this )->where ( array('site_id' => $this['__site_id']->get (),'user_id' => $this['user_id'],'col' => $this['col'] 
			) )->count ();
		}
	}
