...
class Widgets
{
	protected $cur_user_id, $column_schema = array('left','right','center' 
	), // Список колонок на рабочем столе
$default_add_column = 'center', // Добавляем в колонку 1 по умолчанию, нумерация начинается с 0.
$obj = '\Main\Model\Orm\Widgets', $obj_instance, $site_id, $widget_folder = 'controller/admin/widget';
	...
	{
		$list = \RS\Orm\Request::make ()->from ( $this->obj_instance )->orderby ( "col, position" )->where ( array('site_id' => $this->site_id,'user_id' => $this->cur_user_id 
		) )->objects ( null, 'class' );
	...
	function removeWidget($widget_controller)
	{
		...
		\RS\Orm\Request::make ()->delete ()->from ( $this->obj_instance )->where ( array('class' => $widget_controller,'site_id' => $this->site_id,'user_id' => $this->cur_user_id 
		) )->exec ();
	}
	...
	function moveWidget($id, $to_column, $to_pos)
	{
		...
		if ($cur_widget['col'] != $to_column) { // У виджета сменилась колонка
			...
			$sqls[] = \RS\Orm\Request::make ()->update ( $this->obj_instance )->set ( 'position = position-1' )->where ( array('site_id' => $this->site_id,'user_id' => $this->cur_user_id,'col' => $cur_widget['col'] 
			) )->where ( "position>'#cur_pos'", array('cur_pos' => $cur_widget['position'] 
			) );
			
			// Последняя позиция в новой колонке
			$next_position = \RS\Orm\Request::make ()->select ( "MAX(position)+1 as next_pos" )->from ( $this->obj_instance )->where ( array('site_id' => $this->site_id,'user_id' => $this->cur_user_id,'col' => $to_column 
			) )->exec ()->getOneField ( 'next_pos', 0 );
			
			// Вставляем виджет в самый низ новой колонки
			$sqls[] = \RS\Orm\Request::make ()->update ( $this->obj_instance )->set ( array('position' => $next_position,'col' => $to_column 
			) )->where ( array('site_id' => $this->site_id,'user_id' => $this->cur_user_id,'id' => $id 
			) );
		...
		if (! isset ( $next_position ) || $next_position != 0) // Если виджет не один в колонке
{
			...
			if ($cur_widget['position'] < $to_pos) {
				// Вниз
				$sqls[] = \RS\Orm\Request::make ()->update ( $this->obj_instance )->set ( 'position = position-1' )->where ( array('site_id' => $this->site_id,'user_id' => $this->cur_user_id,'col' => $to_column 
			...
			$sqls[] = \RS\Orm\Request::make ()->update ( $this->obj_instance )->set ( array('position' => $to_pos 
			) )->where ( array('site_id' => $this->site_id,'user_id' => $this->cur_user_id,'id' => $id 
			) );
		}
	...
	{
		return \RS\Orm\Request::make ()->select ( 'MAX(position)+1 as max' )->from ( $this->obj_instance )->where ( array('col' => $column,'site_id' => $this->site_id,'user_id' => $this->cur_user_id 
	...
	{
		$widget = \RS\Orm\Request::make ()->select ( '*' )->from ( $this->obj_instance )->where ( array('id' => $id,'user_id' => $this->cur_user_id,'site_id' => $this->site_id 
		) )->object ();
		return $widget;
	}
	...
	function getFullList($appendInfo = false)
	{
		...
		if ($appendInfo) $widgets = $this->appendInfo ( $widgets );
...
from ( $this->obj_instance )->where ( array('user_id' => $this->cur_user_id,'site_id' => $this->site_id 
			) )->whereIn ( 'class', array_keys ( $widget_list ) )->exec ();
		...
		if (is_dir ( $widget_folder )) {
			...
			foreach($files as $file)
				if ($file != '.' && $file != '..' && $file != '.svn') {
					...
					$widget_name = str_replace ( array('.my.' . \Setup::$CLASS_EXT,'.' . \Setup::$CLASS_EXT 
					), '', $filename );
	...
	{
		return \RS\Orm\Request::make ()->from ( $this->obj_instance )->where ( array('class' => $wclass,'site_id' => $this->site_id 
		) )->object ();
	}
}
