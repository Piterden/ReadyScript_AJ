...
class LangApi extends \RS\Module\AbstractModel\BaseModel
{
	...
	static public function createLangFilesForDirectory($directory_path, $php_strings_output_file, $js_strings_output_file)
	{
		...
		$patterns = array('/\Wt\s*?\(\s*?\'(.*?)\'\s*?[,\)]/s','/\Wt\s*?\(\s*?\"(.*?)\"\s*?[,\)]/s' 
		);
		$php_strings = \Main\Model\LangApi::getStringsFromDir ( $directory_path, array('php' 
		), $patterns );
		
		// Регулярное выражение для поиска tpl тэга {t} {/t}
		$patterns = array('/\{t(?:\s+?.*?)?}(.+?)\{\/t\}/s' 
		);
		
		$tpl_strings = \Main\Model\LangApi::getStringsFromDir ( $directory_path, array('tpl' 
		), $patterns );
		
		$php_strings = array_merge ( $php_strings, $tpl_strings );
		
		// Регулярное выражение для поиска JS функции lang.t()
		$patterns = array('/\Wlang\.t\s*?\(\s*?\'(.*?)\'\s*?[,\)]/s','/\Wlang\.t\s*?\(\s*?\"(.*?)\"\s*?[,\)]/s' 
		);
		$js_strings = \Main\Model\LangApi::getStringsFromDir ( $directory_path, array('js','tpl' 
		), $patterns );
