...
class Install extends \RS\Module\AbstractInstall
{
	function install()
	{
		...
		if ($result) {
			...
			// Виджет Комментарии
			$widget->getFromArray ( array('user_id' => $user_id,'col' => 'center','position' => 0,'class' => 'comments-widget-newlist' 
			) )->insert ();
			
			// Виджет Новости компании
			$widget->getFromArray ( array('id' => null,'user_id' => $user_id,'col' => 'center','position' => 1,'class' => 'main-widget-readyscriptnews' 
			) )->insert ();
			
			// Виджет Обновления
			$widget->getFromArray ( array('id' => null,'user_id' => $user_id,'col' => 'right','position' => 0,'class' => 'siteupdate-widget-checkupdates' 
			) )->insert ();
			
			// Виджет Последние авторизации
			$widget->getFromArray ( array('id' => null,'user_id' => $user_id,'col' => 'right','position' => 1,'class' => 'users-widget-authlog' 
			) )->insert ();
		}
		return $result;
	}
