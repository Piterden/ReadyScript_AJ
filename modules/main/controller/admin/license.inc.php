...
use \RS\Html\Toolbar\Button as ToolbarButton, \RS\Html\Table\Type as TableType, \RS\Html\Table;
...
class License extends \RS\Controller\Admin\Crud
{
	...
	{
		$helper = parent::helperIndex ()->setAppendModuleOptionsButton ( false )->removeSection ( 'paginator' )->setListFunction ( 'getTableList' )->setTopTitle ( t ( 'Управление лицензиями' ) )->setTable ( new Table\Element ( array('Columns' => array(new TableType\Checkbox ( 'license_key' ),new TableType\Usertpl ( 'license_key', 'Лицензионный номер', '%main%/license_col_number.tpl' ),new TableType\String ( 'license_type_str', 'Тип' ),new TableType\Usertpl ( 'object', 'Объект лицензирования', '%main%/license_col_object.tpl', array('TdAttr' => array('class' => 'cell-small' 
		) 
		) ),new TableType\Datetime ( 'date_of_activation', 'Дата активации', array('format' => 'd.m.Y' 
		) ),new TableType\String ( 'domain', 'Домен' ) 
		) 
		) ) );
		
		$helper['topToolbar']->addItem ( new ToolbarButton\Add ( $this->url->replaceKey ( array($this->action_var => 'addLic' 
		) ), t ( 'Добавить лицензию' ) ), 'add' )->addItem ( new ToolbarButton\Button ( $this->api->getBuyLicenseUrl (), t ( 'Купить лицензию' ), array('attr' => array('target' => '_blank' 
		) 
		) ) );
	...
	{
		$helper = parent::helperAdd ()->setTopTitle ( t ( 'Введите номер лицензии' ) );
