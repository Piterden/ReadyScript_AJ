...
/**
 * Контроллер списка маршрутов.
 * В данном разделе можно увидеть все маршруты, которые присутствуют в системе
 * @ingroup Main
 */
class Routes extends \RS\Controller\Admin\Front
{
	function actionIndex()
	{
		...
		$helper->setTable ( new Table\Element ( array('Columns' => array(new TableType\String ( 'id', 'ID' ),new TableType\String ( 'description', 'Описание' ),new TableType\Usertpl ( 'patterns', 'Маска пути', '%main%/pageseo_column_route.tpl', array('TdAttr' => array('class' => 'cell-small' 
		) 
		...
		foreach($routes as $route) {
			$data[$i] = array('id' => $route->getId (),'patterns' => $route->getPatternsView (),'description' => $route->getDescription (),'hidden' => $route->isHidden () 
			);
		...
		$this->view->assign ( array('elements' => $helper,'uri' => $uri,'host' => $host,'selected' => $selected 
		) );
