...
use \RS\Html\Toolbar, \Main\Model\Orm\CsvMap, \RS\Html\Toolbar\Button as ToolbarButton;
...
class Csv extends \RS\Controller\Admin\Front
{
	...
	function actionExportCsv()
	{
		...
		$helper = new \RS\Controller\Admin\Helper\CrudCollection ( $this, null, null, array('topTitle' => 'Выберите колонки для экспорта','bottomToolbar' => new Toolbar\Element ( array('Items' => array(new ToolbarButton\SaveForm ( null, t ( 'Экспорт' ) ),new ToolbarButton\Cancel ( $referer ) 
		) 
		) ),'viewAs' => 'form' 
		) );
		...
		if ($csv_schema = \RS\Csv\AbstractSchema::getByShortName ( $schema )) {
			...
			if ($this->url->isPost ()) {
				...
				if (empty ( $columns )) {
					return $this->result->setSuccess ( false )->setErrors ( array('destination' => array('class' => 'field','fieldname' => t ( 'Колонки на экспорт' ),'errors' => array(t ( 'Не задана ни одна колонка для экспорта' ) 
					) 
					) 
					) );
				} else {
					...
					$_SESSION['export_data'] = array('schema' => $schema,'work_fields' => $columns,'params' => $params 
					);
			...
			$this->view->assign ( array('schema' => $schema,'columns' => $csv_schema->getColumns (),'maps' => CsvMap::loadList ( $schema, CsvMap::TYPE_EXPORT ) 
			) );
	...
	function actionImportCsv()
	{
		...
		$helper = new \RS\Controller\Admin\Helper\CrudCollection ( $this, null, null, array('topTitle' => 'Импорт CSV файла','bottomToolbar' => new Toolbar\Element ( array('Items' => array('save' => new ToolbarButton\SaveForm ( null, t ( 'Загрузить файл' ) ),new ToolbarButton\Cancel ( $referer ) 
		) 
		) ),'viewAs' => 'form' 
		) );
		...
		if ($csv_schema = \RS\Csv\AbstractSchema::getByShortName ( $schema )) {
			...
			$this->view->assign ( array('schema' => $schema,'maps' => CsvMap::loadList ( $schema, CsvMap::TYPE_IMPORT ),'referer' => $referer 
			) );
			if ($this->url->isPost ()) {
				if (isset ( $_SESSION['export_data']['params'] )) {
					$csv_schema->setParams ( $_SESSION['export_data']['params'] );
	...
	function actionProcessImport($schema_def = null, $filename_def = null, $referer_def = null)
	{
		...
		if ($csv_schema = \RS\Csv\AbstractSchema::getByShortName ( $schema )) {
			...
			$helper = new \RS\Controller\Admin\Helper\CrudCollection ( $this, null, null, array('topTitle' => t ( 'Параметры импорта CSV файла' ),'bottomToolbar' => new Toolbar\Element ( array('Items' => array('save' => new ToolbarButton\SaveForm ( null, t ( 'Начать импорт' ) ),new ToolbarButton\Cancel ( $referer ) 
			) 
			) ),'viewAs' => 'form' 
			) );
			...
			if ($this->url->isPost () && $this->url->post ( 'import_start', TYPE_INTEGER )) {
				...
				if ($csv_schema->validateImportWorkField ( $columns )) {
					...
					if ($result === true) {
						// Импорт завершён
						return $this->result->setSuccess ( true )->addMessage ( t ( 'Импорт успешно завершен' ) );
					} elseif (is_integer ( $result )) {
						
						// Импорт нужно продолжить
						return $this->result->setSuccess ( true )->addSection ( 'repeat', true )->addSection ( 'start_pos', $result )->addSection ( 'queryParams', array('url' => $this->url->getSelfUrl (),'data' => array('ajax' => 1,'referer' => $referer,'schema' => $schema,'start_pos' => $result,'import_start' => 1,'filename' => $filename,'columns' => $this->url->request ( 'columns', TYPE_ARRAY, null ) 
						) 
						) )->addSection ( 'redirect', \RS\Router\Manager::obj ()->getAdminUrl ( 'processImport' ) );
					}
				} else {
					$this->result->setSuccess ( false )->setErrors ( $csv_schema->getDisplayErrors () );
				}
				// Импортируем данные
			}
			
			$this->view->assign ( array('schema' => $schema,'maps' => CsvMap::loadList ( $schema, CsvMap::TYPE_IMPORT ),'csv_columns' => $csv_columns,'selected_columns' => $columns,'columns' => $csv_schema->getColumns (),'filename' => $filename 
			) );
	...
	function actionSaveMap()
	{
		...
		$this->view->assign ( array('map' => $map 
		) );
