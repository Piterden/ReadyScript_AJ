...
use \RS\Application\Auth as AppAuth, \RS\Cache\Cleaner as CacheCleaner;
...
class Index extends \RS\Controller\AbstractController
{
	function __construct()
	{
		...
		$this->app->addJsVar ( array('authUrl' => $this->router->getAdminUrl ( false, array('Act' => 'auth' 
		), false ) 
		) );
	}
	...
	function actionIndex()
	{
		...
		if (! $this->url->isAjax ()) {
			...
			$this->app->addJsVar ( array('adminSection' => '/' . \Setup::$ADMIN_SECTION,'scriptType' => \Setup::$SCRIPT_TYPE,'resImgUrl' => \Setup::$IMG_PATH 
			) );
			
			$this->app->addJs ( 'jquery.min.js', 'jquery' )->addJs ( 'ui.jquery.all.min.js', null, BP_COMMON )->addJs ( 'jquery.datetimeaddon.min.js', null, BP_COMMON )->addJs ( 'lab.min.js', null, BP_COMMON )->addJs ( 'jquery.form.js', null, BP_COMMON )->addJs ( 'jquery.cookie.js', null, BP_COMMON )->addJs ( 'admindebug.js' )->addJs ( 'admin.js' )->addJs ( 'updatable.js', null, BP_COMMON )->addJs ( 'coreobject.js', null, BP_COMMON )->addJs ( 'tabs.js', 'tabs' )->addJs ( 'jquery.tooltip.js', 'tooltip' )->meta->add ( array('http-equiv' => 'X-UA-Compatible','content' => 'IE=Edge','unshift' => true 
			) );
		}
	...
	function actionAuth()
	{
		...
		$data = array('login' => $this->url->request ( 'login', TYPE_STRING, '' ),'pass' => $this->url->request ( 'pass', TYPE_STRING, '' ),'remember' => $this->url->request ( 'remember', TYPE_INTEGER ),'do' => $this->url->request ( 'do', TYPE_STRING ) 
		);
	...
	function actionChangePassword()
	{
		...
		$this->view->assign ( array('current_lang' => \RS\Language\Core::getCurrentLang (),'locale_list' => \RS\Language\Core::getSystemLanguages (),'uniq' => $hash,'user' => $user,'err' => $error 
		) );
	...
	{
		$result_helper = new \RS\Controller\Result\Standart ( $this );
		...
		if (! $this->url->isAjax ()) {
			...
			$this->view->assign ( array('data' => $data,'current_lang' => \RS\Language\Core::getCurrentLang (),'locale_list' => \RS\Language\Core::getSystemLanguages (),'js' => $js,'err' => $error,'referer' => $referer,'auth_url' => $this->router->getAdminUrl ( false, array('Act' => 'auth' 
			), false ) 
			) );
