...
class SelectTheme extends \RS\Controller\Admin\Front
{
	...
	function actionAjaxThemeList($as_string = false)
	{
		...
		$this->view->assign ( array('theme_list' => $theme_list,'current' => $current_theme 
		) );
	...
	function actionUploadTheme()
	{
		...
		return $this->result->setSuccess ( $this->api->uploadThemeZip ( $file, $overwrite ) )->setErrors ( $this->api->getDisplayErrors () );
	}
