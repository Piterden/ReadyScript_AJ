...
use \RS\Module\AbstractModel, \RS\Html\Toolbar\Button as ToolbarButton, \RS\Html\Toolbar;
...
{
	protected $pageApi, $containerApi, $sectionApi, $sectionModuleApi;
	function __construct()
	{
		...
		$this->sectionApi = new AbstractModel\EntityList ( new \Templates\Model\Orm\Section (), array('loadOnDelete' => true 
		) );
		
		$this->sectionModuleApi = new AbstractModel\EntityList ( new \Templates\Model\Orm\SectionModule (), array('loadOnDelete' => true 
		) );
	}
	function actionIndex()
	{
		...
		$this->view->assign ( array('defaultPage' => $default_page,'currentPage' => $page,'currentTheme' => $site_config['theme'],'pages' => $this->pageApi->getList (),'context_list' => \RS\Theme\Manager::getContextList (),'context' => $context,'grid_system' => $theme->getGridSystem () 
		) );
	...
	function helperIndex()
	{
		...
		) ), t ( 'добавить страницу' ) ),new ToolbarButton\Space (),new ToolbarButton\Button ( null, t ( 'сохранить эталон темы' ), array('attr' => array('title' => t ( 'Сохраняет block.xml в папке с темой' ),'class' => 'crud-get','data-confirm-text' => t ( 'Вы действительно хотите перезаписать структуру блоков blocks.xml в каталоге темы ?' ),'data-url' => $this->router->getAdminUrl ( 'saveTheme' ) 
		) 
		) ),new ToolbarButton\Button ( $this->url->replaceKey ( array($this->action_var => 'export' 
		) ), t ( 'экспорт' ), array('attr' => array('title' => t ( 'Экспорт в XML файл' ) 
		) 
		) ),new ToolbarButton\Button ( $this->url->replaceKey ( array($this->action_var => 'import' 
		) ), t ( 'импорт' ), array('attr' => array('class' => 'crud-add','title' => t ( 'Импорт из XML файла' ) 
		) 
		) ) 
		) 
		) ) );
	...
	function actionAdd($primaryKeyValue = null, $returnOnSuccess = false, $helper = null)
	{
		...
		if ($primaryKeyValue) {
			$helper->setTopTitle ( 'Редактировать страницу' );
		} else {
			$helper->setTopTitle ( 'Добавить страницу' );
		}
	...
	{
		$this->view->assign ( 'dialogTitle', 'Редактирование страницы' );
	...
	function actionCopyContainer()
	{
		...
		$containers = ! $pages ? array() : $this->containerApi->setFilter ( 'page_id', array_keys ( $pages ), 'in' )->queryObj ()->objects ( null, 'page_id', true );
		
		$this->view->assign ( array('pages' => $pages,'containers' => $containers,'elements' => $helper 
		) );
	...
	function actionAjaxMoveSection()
	{
		...
		$result = array('success' => $this->sectionApi->getElement ()->moveToPosition ( $position ) 
		);
	...
	function actionAjaxMoveContainer()
	{
		...
		return $this->result->setSuccess ( $container->load ( $source_id ) && $container->changePosition ( $destination_id ) );
	}
	function actionAjaxMoveBlock()
	{
		...
		$result = array('success' => $this->sectionModuleApi->getElement ()->moveToPosition ( $position ) 
		);
	...
	function actionAddModule()
	{
		...
		$this->view->assign ( array('controllers_tree' => $controllers_tree,'section_id' => $section_id 
		) );
	...
	function actionAddModuleStep2($primaryKeyValue = null)
	{
		...
		if ($object) {
			...
			if (! $primaryKeyValue) {
				if ($access_error = \RS\AccessControl\Rights::CheckRightError ( $this, ACCESS_BIT_WRITE )) {
					return $this->result->addSection ( 'close_dialog', true )->addEMessage ( $access_error );
				}
				
				$sectionModule->insert ();
				// Сбрасываем кэш
				\RS\Cache\Manager::obj ()->invalidateByTags ( CACHE_TAG_BLOCK_PARAM );
				return $this->result->addSection ( 'close_dialog', true )->setNoAjaxRedirect ( $this->url->getSavedUrl ( $this->controller_name . 'index' ) )->getOutput ();
			}
		}
		...
		$this->view->assign ( array('elements' => $helper,'errors' => $errors 
		) );
		return $this->result->setTemplate ( $helper['template'] );
	}
	...
	function actionEditTemplateModule()
	{
		...
		if ($block = $api->getBlockFromCache ( $block_id, $block_url )) { // Подгрузим блок
			...
			if ($object) { // Если получить объекть удалось
				...
				\RS\Cache\Manager::obj ()->invalidateByTags ( CACHE_TAG_BLOCK_PARAM );
				return $this->result->setSuccess ( true )->setSuccessText ( 'Попытка получить блок не удалась.<br/> Нужна перезагрука страницы' );
			}
	...
	function actionImport()
	{
		...
		$helper->setTopTitle ( 'Импорт структуры блоков' );
	...
	function actionExport()
	{
		...
		$filename = 'blocks.xml';
		$this->app->headers->addHeaders ( array('Content-Type' => 'text/xml','Content-Transfer-Encoding' => 'binary','Content-Disposition' => 'attachment; filename="' . $filename . '"','Connection' => 'close' 
		) );
		return $this->api->getBlocksXML ( $context );
	}
	function actionSaveTheme()
	{
		$this->result->setSuccess ( $this->api->saveThemeBlocks () )->addSection ( 'noUpdate', true );
	...
	function actionContextOptions()
	{
		...
		if ($this->url->isPost ()) {
			...
			if ($this->result->isSuccess ()) {
				$this->result->setSuccess ( true )->setSuccessText ( t ( 'Изменения успешно сохранены' ) );
			} else {
