...
class PageApi extends \RS\Module\AbstractModel\EntityList
{
	...
	{
		parent::__construct ( new \Templates\Model\Orm\SectionPage (), array('multisite' => true,'loadOnDelete' => true 
		) );
	}
	...
	{
		static		// Технические параметры, которые нужно исключать из blocks.xml
	...
	public static function setupModule($route_id, $block_class, array $params, $section_alias = null)
	{
		...
		$q = \RS\Orm\Request::make ()->from ( new Orm\SectionModule () )->where ( array('module_controller' => strtolower ( ltrim ( $block_class, '\\' ) ) 
		) );
		
		if ($route_id !== null) {
			$q->where ( array('page_id' => $page['id'] 
			) );
		}
		
		if ($section_alias !== null) {
			$sections_id = \RS\Orm\Request::make ()->from ( new Orm\Section () )->where ( array('alias' => $section_alias 
			) + ($page['id'] ? array('page_id' => $page['id'] 
			) : array()) )->exec ()->fetchSelected ( null, 'id' );
			if (! $sections_id) {
