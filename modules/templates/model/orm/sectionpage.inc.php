...
{
	public $max_container_type;
	protected static $table = 'section_page';
	...
	function _init()
	{
		...
		$this->getPropertyIterator ()->append ( array('site_id' => new Type\CurrentSite ( array('no_export' => true 
		) ),'route_id' => new Type\String ( array('maxLength' => '255','allowempty' => false,'description' => t ( 'Маршрут' ),'hint' => t ( 'Маршрут означает определенный тип страниц, Например: "просмотр товара", или "список продукции"' ),'List' => array(array(__CLASS__,'getRouteSelectList' 
		) 
		),'Attr' => array(array('size' => 1 
		) 
		) 
		) ),'context' => new Type\String ( array('maxLength' => 32,'description' => 'Дополнительный идентификатор темы','visible' => false 
		) ),'template' => new Type\String ( array('maxLength' => '255','description' => t ( 'Шаблон' ),'template' => '%templates%/form/sectionpage/template.tpl' 
		) ),'inherit' => new Type\Integer ( array('maxLength' => 1,'default' => 1,'description' => t ( 'Наследовать шаблон по-умолчанию?' ),'checkboxView' => array(1,0 
		) 
		) ) 
		) );
		
		$this->addIndex ( array('site_id','route_id','context' 
		), self::INDEX_UNIQUE );
	}
	...
	public static function loadByRoute($route_id, $context = 'theme', $site_id = null)
	{
		...
		$result = self::loadByWhere ( array('site_id' => $site_id,'route_id' => $route_id,'context' => $context 
		) );
	...
	protected function loadContainers($page_id)
	{
		...
		return \RS\Orm\Request::make ()->from ( new SectionContainer () )->where ( array('page_id' => $page_id 
		) )->orderby ( 'type' )->objects ( null, 'type' );
	}
	...
	public function getContainers($route_id = null)
	{
		...
		for($type = 1; $type <= $max; $type ++) {
			$containers[$type] = array('type' => $type,'defaultObject' => isset ( $default_containers[$type] ) ? $default_containers[$type] : false,'object' => isset ( $loaded_containers[$type] ) ? $loaded_containers[$type] : false 
			);
		}
	...
	function delete()
	{
		...
		$sub_containers = \RS\Orm\Request::make ()->from ( new SectionContainer () )->where ( array('page_id' => $this['id'] 
		) )->objects ();
