...
{
	const GS_NONE = 'none', GS_GS960 = 'gs960', GS_BOOTSTRAP = 'bootstrap';
	protected static $table = 'section_context';
	protected $before_grid_system;
	function _init()
	{
		$this->getPropertyIterator ()->append ( array('site_id' => new Type\CurrentSite (),'context' => new Type\String ( array('maxLength' => 50,'description' => t ( 'Контекст темы оформления' ),'visible' => false 
		) ),'grid_system' => new Type\Enum ( array('none','gs960','bootstrap' 
		), array('description' => t ( 'Тип сеточного фреймворка' ),'listFromArray' => array(array('none' => t ( 'Без сетки' ),'gs960' => t ( 'GridSystem 960' ),'bootstrap' => t ( 'Bootstrap 3' ) 
		) 
		),'allowEmpty' => false,'template' => '%templates%/form/sectioncontext/grid_system.tpl' 
		) ),'options' => new Type\Text ( array('description' => t ( 'Настройки темы в сериализованном виде' ),'visible' => false 
		) ),'options_arr' => new Type\ArrayList ( array('description' => t ( 'Настройки темы' ),'visible' => false 
		) ) 
		) );
		
		$this->addIndex ( array('site_id','context' 
		), self::INDEX_PRIMARY );
	}
	...
	function beforeWrite($flag)
	{
		...
		$before = self::loadByWhere ( array('site_id' => $this['site_id'],'context' => $this['context'] 
		) );
		if ($before['context']) {
	...
	function afterWrite($flag)
	{
		if ($this->before_grid_system !== null && $this->before_grid_system != $this['grid_system']) {
			...
			$pages_id = \RS\Orm\Request::make ()->select ( 'id' )->from ( new SectionPage () )->where ( array('site_id' => $this['site_id'],'context' => $this['context'] 
	...
	private function generateField($option)
	{
		switch (( string ) $option['type']) {
			case 'checkbox' :
				{
					$field = new Type\Integer ( array('checkboxView' => array(1,0 
					) 
					) );
					break;
				}
			...
			case 'select' :
				{
					...
					$field = new Type\String ( array('listFromArray' => array($items 
					) 
					) );
					break;
				}
