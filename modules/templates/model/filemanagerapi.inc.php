...
{
	const PATH_SESSION_VAR = 'TMANAGER_PATH', ERROR_FILE_ALREADY_EXISTS = 1;
	protected $field_list, $theme_list, 
	...
	{
		$this->field_list = array('filename' => t ( 'Имя файла' ) 
		);
	}
	...
	public function getRootSections($only_themes = false)
	{
		...
		$result = array('themes' => array(),'modules' => array() 
		);
		foreach($this->theme_list as $theme) {
			$info = $theme->getInfo ();
			$result['themes'][$theme->getName ()] = array('title' => $info['name'] . " ({$theme->getName()})" 
			);
		}
		...
		if (! $only_themes) {
			...
			foreach($module_list as $short_name => $module) {
				$result['modules'][$short_name] = array('title' => $module->getConfig ()->name 
				);
			}
		}
		return $result;
	...
	function getFileList($path, array $allow_extension = null, $only_themes = false)
	{
		...
		if (is_array ( $folder_list )) foreach($folder_list as $folder) {
			$name = basename ( $folder );
			$result['items'][] = array('name' => $name,'type' => 'dir','link' => $epath['public_dir'] . $name . '/','descr' => '' 
			);
		}
		...
		foreach($file_list as $file) {
			...
			if ($info !== false) {
				...
				$result['items'][] = array('path' => $epath['public_dir'],'filename' => $name,'name' => $fname,'ext' => ltrim ( $fext, '.' ),'link' => $epath['public_dir'] . $name,'type' => 'file','descr' => $info 
				);
			}
		}
	...
	{
		if (preg_match ( '/^[a-zA-Z0-9\-\_.]+$/', $filename ) && (! $filterExtension || preg_match ( '/^[a-zA-Z0-9\-\_.]+?\.(' . implode ( $this->allow_ext, '|' ) . ')$/', $filename ))) {
			return true;
		}
