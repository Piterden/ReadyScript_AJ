...
class Ctrl extends \RS\Controller\Admin\Crud
{
	...
	function helperIndex()
	{
		...
		$helper->setTable ( new Table\Element ( array('Columns' => array(new TableType\Checkbox ( 'id' ),new TableType\String ( 'title', t ( 'Название материала' ), array('Sortable' => SORTABLE_BOTH,'href' => $this->router->getAdminPattern ( 'edit', array(':id' => '@id' 
		...
		) ),new TableType\Action\DropDown ( array(array('title' => t ( 'клонировать материал' ),'attr' => array('class' => 'crud-add','@href' => $this->router->getAdminPattern ( 'clone', array(':id' => '~field~' 
		) ) 
		) 
		) 
		) ) 
