...
class Install extends \RS\Module\AbstractInstall
{
	function install()
	{
		...
		if ($result) {
			...
			\Shop\Config\Handlers::onSiteCreate ( array('orm' => \RS\Site\Manager::getSite (),'flag' => \RS\Orm\AbstractObject::INSERT_FLAG 
			) );
			...
			$widget->getFromArray ( array('user_id' => $user_id,'col' => 'left','class' => 'shop-widget-lastorders' 
			) )->insert ();
			
			// Виджет Статусы заказов
			$widget->getFromArray ( array('id' => null,'position' => null,'user_id' => $user_id,'col' => 'left','class' => 'shop-widget-orderstatuses' 
			) )->insert ();
		}
		return $result;
	}
	...
	{
		return $this->importCsvFiles ( array(array('\Shop\Model\CsvSchema\Delivery','delivery' 
