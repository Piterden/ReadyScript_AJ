...
{
	protected $use_ssl = true, // Использовать защищенное соединение
$force_ssl_v3 = true, // SSL протокол
...
$response = ''; // Сам ответ
	...
	function getFormObject()
	{
		...
		) ),'bussiness_email' => new Type\String ( array('description' => t ( 'E-mail продавца на PayPal' ) 
		) ),'culture' => new Type\String ( array('description' => t ( 'Язык интерфейса PayPal' ),'listFromArray' => array(array('RU' => 'Русский','EN' => 'Анлийский' 
		) 
		) 
		) ),

		'__help__' => new Type\Mixed ( array('description' => t ( '' ),'visible' => true,'template' => '%shop%/form/payment/paypal/help.tpl' 
		) ) 
		) );
	...
	function getPayUrl(Transaction $transaction)
	{
		...
		$params['item_name'] = "Заказ №" . $order_id . ". " . $current_site; // Id заказа
		...
		$fail = $router->getUrl ( 'shop-front-onlinepay', array('Act' => 'fail','custom' => $inv_id,'PaymentType' => $this->getShortName () 
		), true );
		$success = $router->getUrl ( 'shop-front-onlinepay', array('Act' => 'success','custom' => $inv_id,'PaymentType' => $this->getShortName () 
		), true );
	 ...
	 */
	protected function curlPost($url_to_server, $encoded_data)
	{
		if ($this->use_ssl) { // Адрес соединения
			$uri = 'https://' . $url_to_server . '/cgi-bin/webscr';
		} else {
			$uri = 'http://' . $url_to_server . '/cgi-bin/webscr';
		}
	...
	private function checkSign(\Shop\Model\Orm\Transaction $transaction, \RS\Http\Request $request)
	{
		...
		$this->curlPost ( $url, $encoded_data ); // curl
