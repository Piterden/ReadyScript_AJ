...
class Bill extends AbstractType
{
	...
	{
		return array('bill' => array('title' => t ( 'Счет' ) 
		) 
		);
	}
	...
	function getFormObject()
	{
		...
		$properties = new \RS\Orm\PropertyIterator ( array('number_prefix' => new Type\String ( array('description' => t ( 'Префикс для номера счета' ) 
		) ),'seal_url' => new Type\String ( array('description' => 'Ссылка на изображение печати' 
		) ),'use_site_company' => new Type\Integer ( array('description' => t ( 'Использователь основные реквизиты компании' ),'checkboxView' => array(1,0 
