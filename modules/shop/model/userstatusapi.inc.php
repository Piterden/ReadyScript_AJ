...
class UserStatusApi extends \RS\Module\AbstractModel\EntityList
{
	...
	{
		parent::__construct ( new \Shop\Model\Orm\UserStatus (), array('nameField' => 'title','aliasField' => 'type','defaultOrder' => 'id','multisite' => true 
		) );
	}
	function tableData()
	{
	...
	public static function getStatusIdByType($type = null)
	{
		if (! isset ( self::$id_by_type )) {
			self::$id_by_type = \RS\Orm\Request::make ()->select ( 'id, type' )->from ( new Orm\UserStatus () )->where ( array('site_id' => \RS\Site\Manager::getSiteId () 
	...
	function getAdminTreeList()
	{
		...
		array_unshift ( $list, array('fields' => array('id' => 0,'title' => 'Все','noOtherColumns' => true,'noCheckbox' => true,'noDraggable' => true,'noRedMarker' => true,'orders_count' => $total 
		),'child' => null 
		) );
		return $list;
	}
