...
class Zone extends \RS\Orm\OrmObject
{
	...
	function _init()
	{
		parent::_init ()->append ( array('site_id' => new Type\CurrentSite (),'title' => new Type\String ( array('checker' => array('chkEmpty',t ( 'Введите имя зоны' ) 
		),'description' => t ( 'Название' ) 
		) ),'xregion' => new Type\ArrayList ( array('checker' => array('chkEmpty',t ( 'Укажите регионы' ) 
		),'description' => t ( 'Регионы' ),'list' => array(array('\Shop\Model\RegionApi','selectListAll' 
		) 
		),'attr' => array(array('size' => 20,'multiple' => true 
		) 
		) 
		) ) 
		) );
	...
	{
		\RS\Orm\Request::make ()->delete ()->from ( new Xregion () )->where ( array('zone_id' => $this->id 
		) )->exec ();
	}
	...
	{
		$regions = \RS\Orm\Request::make ()->select ( 'region_id' )->from ( new Xregion () )->where ( array('zone_id' => $this->id 
