...
{
	protected static $table = 'order_tax', $cache_rates = array();
	function _init()
	{
		parent::_init ()->append ( array(t ( 'Основные' ),'site_id' => new Type\CurrentSite (),'title' => new Type\String ( array('description' => t ( 'Название' ) 
		) ),'alias' => new Type\String ( array('description' => t ( 'Идентификатор (Английские буквы или цифры)' ) 
		) ),'description' => new Type\String ( array('description' => t ( 'Описание' ) 
		) ),'enabled' => new Type\Integer ( array('description' => t ( 'Включен' ),'checkboxView' => array(1,0 
		) 
		) ),'user_type' => new Type\Enum ( array('all','user','company' 
		), array('description' => t ( 'Тип плательщиков' ),'listFromArray' => array(array('all' => t ( 'Все' ),'user' => t ( 'Физические лица' ),'company' => t ( 'Юридические лица' ) 
		) 
		) 
		) ),'included' => new Type\Integer ( array('description' => t ( 'Входит в цену' ),'checkboxView' => array(1,0 
		) 
		) ),'sortn' => new Type\Integer ( array('description' => t ( 'Порядок применения' ) 
		) ),

		t ( 'Ставки налога' ),'__rates__' => new Type\UserTemplate ( '%shop%/form/tax/rates.tpl' ),'rates' => new Type\ArrayList () 
		) );
	}
	...
	function afterWrite($flag)
	{
		...
		if ($this->isModified ( 'rates' )) {
			\RS\Orm\Request::make ()->delete ()->from ( new TaxRate () )->where ( array('tax_id' => $this['id'] 
			) )->exec ();
	...
	{
		if ($this['rates'] === null) {
			$this['rates'] = ( array ) \RS\Orm\Request::make ()->from ( new TaxRate () )->where ( array('tax_id' => $this['id'] 
			) )->exec ()->fetchSelected ( 'region_id', 'rate' );
		}
	}
	...
	function getRate(\Shop\Model\Orm\Address $address)
	{
		...
		if (! isset ( self::$cache_rates[$this['id']][$address_id] )) {
			$q = \RS\Orm\Request::make ()->select ( 'rate' )->from ( new TaxRate () )->where ( array('tax_id' => $this['id'] 
			) );
			
			if ($address['region_id'] > 0) {
				$q->where ( "(region_id = '#country_id' OR region_id = '#region_id')", array('country_id' => $address['country_id'],'region_id' => $address['region_id'] 
				) );
			} else {
				$q->where ( "(region_id = '#country_id')", array('country_id' => $address['country_id'] 
				) );
			}
			$tax_rate = $q->object ();
