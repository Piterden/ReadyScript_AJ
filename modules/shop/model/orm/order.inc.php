...
use \RS\Orm\Type, \Catalog\Model\Orm\Product;
...
{
	const ACL_WRITE_ORDER_BIT = 2, ORDER_SESS_VAR = 'ORDER-ORMOBJECT';
...
$this_before_write;
	public $address, $order_cart, $session_cart;
	protected static $currency, $table = 'order';
	function _init()
	{
		$properties = parent::_init ()->groupSet ( 'condition', array('step' => 'init' 
		) )->append ( array('site_id' => new Type\CurrentSite (),'order_num' => new Type\String ( array('maxLength' => '20','description' => t ( 'Уникальный идентификатор номера заказа' ) 
		) ),'step' => new Type\Mixed ( array() ),'user_id' => new Type\User ( array('maxLength' => '11','attr' => array(array('data-autocomplete-body' => '1' 
		) 
		),'description' => t ( 'ID покупателя' ) 
		) ),'basket' => new Type\Mixed ( array('description' => t ( 'Объект - корзина' ) 
		) ),'currency' => new Type\String ( array('maxLength' => '5','description' => t ( 'Трехсимвольный идентификатор на момент оформления заказа' ) 
		) ),'currency_ratio' => new Type\Float ( array('description' => t ( 'Курс относительно базововй валюты' ) 
		) ),'currency_stitle' => new Type\String ( array('description' => t ( 'Символ валюты' ),'maxLength' => 10 
		) ),'ip' => new Type\String ( array('maxLength' => '15','description' => t ( 'IP' ) 
		) ),'notify_user' => new Type\Integer ( array('runtime' => true,'description' => t ( 'Уведомлять пользователя об изменении в заказе' ) 
		) ),
				
				// Подтверждение
				'dateof' => new Type\Datetime ( array('description' => t ( 'Дата заказа' ) 
				) ),'totalcost' => new Type\Decimal ( array('maxLength' => '15','decimal' => 2,'description' => t ( 'Общая стоимость' ) 
				) ),'user_delivery_cost' => new Type\Decimal ( array('maxLength' => '15','decimal' => 2,'description' => t ( 'Стоимость доставки, определенная администратором' ) 
				) ),'is_payed' => new Type\Integer ( array('maxLength' => '1','description' => t ( 'Заказ полностью оплачен?' ),'CheckBoxView' => array(1,0 
				) 
				) ),'status' => new Type\Integer ( array('maxLength' => '11','description' => t ( 'Статус' ),'index' => true 
				) ),'admin_comments' => new Type\Text ( array('description' => t ( 'Комментарии администратора (не отображаются пользователю)' ),'Attr' => array(array('class' => 'fullwide' 
				) 
				) 
				) ),'user_text' => new Type\Richtext ( array('description' => t ( 'Текст для покупателя' ),'Attr' => array(array('class' => 'fullwide' 
				) 
				) 
				) ),'_serialized' => new Type\Text (),'userfields' => new Type\Text ( array('description' => t ( 'Дополнительные сведения' ),'condition' => array('step' => 'address' 
				),'visible' => false 
				) ),'extra' => new Type\ArrayList ( array('visible' => false 
				) ),'hash' => new Type\String ( array('maxLength' => '32' 
				) ),'expired' => new Type\Mixed (),'is_exported' => new Type\Integer ( array('maxLength' => '1','default' => 0,'description' => t ( 'Выгружен ли заказ' ),'visible' => false 
				) ) 
		) )->cancelGroupSet ();
		
		$this->addIndex ( array('site_id','order_num' 
		), self::INDEX_UNIQUE );
		...
		$chk_condition = array('user_type' => array('person','company' 
		),'step' => 'address' 
		);
		
		$properties->append ( array('user_type' => new Type\String ( array('maxLength' => '30','condition' => array('step' => 'address' 
		),'runtime' => true 
		) ) 
		) )->groupSet ( 'condition', $chk_condition )->groupSet ( 'runtime', true )->append ( array('reg_name' => new Type\String ( array('description' => t ( 'Имя' ),'maxLength' => '200' 
		) ),'reg_surname' => new Type\String ( array('description' => t ( 'Фамилия' ),'maxLength' => '200' 
		) ),'reg_midname' => new Type\String ( array('description' => t ( 'Отчество' ),'maxLength' => '200' 
		) ),'reg_phone' => new Type\String ( array('description' => t ( 'Телефон' ),'maxLength' => '100' 
		) ),'reg_e_mail' => new Type\String ( array('description' => t ( 'E-mail' ),'maxLength' => '100' 
		) ),'reg_autologin' => new Type\Integer ( array('maxLength' => '1','CheckBoxView' => array(1,0 
		) 
		) ),'reg_login' => new Type\String ( array('description' => t ( 'Логин' ),'maxLength' => '70' 
		) ),'reg_openpass' => new Type\String ( array('description' => t ( 'Пароль' ),'maxLength' => '70','attr' => array(array('type' => 'password' 
		) 
		) 
		) ),'reg_pass2' => new Type\String ( array('maxLength' => '70','attr' => array(array('type' => 'password' 
		) 
		) 
		) ),'regfields' => new Type\ArrayList ( array('description' => t ( 'Дополнительные сведения' ) 
		) ) 
		) );
		
		// поля для регистрации предприятия
		$chk_condition = array('user_type' => 'company','step' => 'address' 
		);
		$properties->groupSet ( 'condition', $chk_condition )->append ( array('reg_company' => new Type\String ( array('maxLength' => '255' 
		) ),'reg_company_inn' => new Type\String ( array('maxLength' => '50' 
		) ) 
		) );
		
		// поля для авторизации пользователя
		$chk_condition = array('user_type' => 'user','step' => 'address' 
		);
		$properties->groupSet ( 'condition', $chk_condition )->append ( array('login' => new Type\String ( array('maxLength' => '100' 
		) ),'password' => new Type\String ( array('maxLength' => '100','Attr' => array(array('type' => 'password' 
		) 
		) 
		) ) 
		) )->cancelGroupSet ();
		...
		$properties->append ( array('contact_person' => new Type\String ( array('maxLength' => '255','description' => t ( 'Контактное лицо' ),'condition' => array('step' => 'address' 
		) 
		) ),'use_addr' => new Type\Integer ( array('maxLength' => '11','description' => t ( 'ID адреса доставки' ),'condition' => array('step' => 'address' 
		) 
		) ) 
		) );
		
		$chk_condition = array('use_addr' => '0','step' => 'address' 
		);
		
		$properties->groupSet ( 'condition', $chk_condition )->groupSet ( 'runtime', true )->append ( array('addr_country_id' => new Type\String ( array('maxLength' => '100','description' => t ( 'Страна' ),'Checker' => array('chkEmpty',t ( 'Страна - обязательное поле' ) 
		),'List' => array(array('\Shop\Model\RegionApi','countryList' 
		) 
		),'Attr' => array(array('size' => 1 
		) 
		) 
		) ),'addr_region' => new Type\String ( array('maxLength' => '100' 
		) ),'addr_region_id' => new Type\String ( array('maxLength' => '100','description' => t ( 'Область/Край' ),'List' => array(array(__CLASS__,'regionList' 
		) 
		),'Attr' => array(array('size' => 1 
		) 
		) 
		) ),'addr_city' => new Type\String ( array('maxLength' => '100','description' => t ( 'Город' ),'Checker' => array('chkEmpty',t ( 'Город - обязательное поле' ) 
		) 
		) ),'addr_zipcode' => new Type\String ( array('maxLength' => '20','description' => t ( 'Индекс' ) 
		) ),'addr_address' => new Type\String ( array('maxLength' => '255','description' => t ( 'Адрес' ),'Checker' => array(function ($object, $value, $error) {
		...
		$properties->append ( array('userfields_arr' => new Type\ArrayList ( array('description' => t ( 'Дополнительные сведения' ),'condition' => array('step' => 'address' 
		) 
		) ),'code' => new Type\Captcha ( array('condition' => array('step' => 'address' 
		) 
		) ),
				
				// Шаг 2
				'delivery' => new Type\Integer ( array('maxLength' => '11','description' => t ( 'Доставка' ),'condition' => array('step' => 'delivery' 
				),'Checker' => array('chkEmpty','Укажите тип доставки' 
				),'List' => array(array('\Shop\Model\DeliveryApi','staticSelectList' 
				) 
				) 
				)
				 ),'deliverycost' => new Type\Decimal ( array('maxLength' => '15','decimal' => 2,'description' => t ( 'Стоимость доставки' ),'condition' => array('step' => 'delivery' 
				) 
				) ),
				
				// Шаг 2.2 - только при самовывозе
				'warehouse' => new Type\Integer ( array('maxLength' => '11','description' => t ( 'Склад' ),'condition' => array('step' => 'warehouses' 
				...
				// Шаг 3
				'payment' => new Type\Integer ( array('description' => t ( 'Тип оплаты' ),'condition' => array('step' => 'pay' 
				...
				// Шаг 4
				'comments' => new Type\Text ( array('description' => t ( 'Комментарий' ),'condition' => array('step' => 'confirm' 
				) 
				) ) 
		)
		 );
		...
		$properties->append ( array('user_fio' => new Type\String ( array('description' => t ( 'Ф.И.О.' ),'Attr' => array(array('size' => 40 
		) 
		),'maxLength' => 255 
		) ),'user_email' => new Type\String ( array('description' => t ( 'E-mail' ),'Attr' => array(array('size' => 40 
		) 
		),'maxLength' => 255 
		) ),'user_phone' => new Type\String ( array('description' => t ( 'Телефон' ),'Attr' => array(array('size' => 40 
		) 
		),'maxLength' => 255 
		) ) 
		) );
	...
	function afterWrite($flag)
	{
		...
		if ($flag == self::INSERT_FLAG) { // При вставке
			...
			if (! $config['use_generated_order_num']) {
				...
				\RS\Orm\Request::make ()->update ( $this )->set ( 'order_num = id' )->where ( array('id' => $this['id'] 
				) )->exec ();
...
($old_order['is_payed'] != $this['is_payed']); // Проверяем флаг оплаты
	...
	function getAllowCurrencies()
	{
		...
		$result = array('0' => $base_currency->title 
		);
		
		if ($base_currency->title == $this['currency']) {
			$my = 0;
		} else {
			$result = array('1' => $this['currency'] 
			) + $result;
			$my = 1;
		}
	...
	/**
	 * Возвращает дополнительные пары ключ => значение для отображения в админ.
	 * панели в разделе "Информация о заказе"
	 * 
	 * @return array
	 */
	...
	{
		$extra = $this['extra'];
		$item = array('title' => $title,'value' => $value,'data' => $data 
		);
	...
	function getFiles($access = array('visible', 'afterpay'), $include_product_files = true)
	{
		...
		if ($this['id'] && \RS\Module\Manager::staticModuleExists ( 'files' )) {
			...
			if ($access) {
				...
				$file_api->setFilter ( array('link_type_class' => 'files-shoporder','link_id' => $this['id'] 
				) );
			...
			if ($include_product_files && in_array ( 'afterpay', $access )) {
				...
				if ($ids) {
					...
					$file_api->setFilter ( array('link_type_class' => 'files-catalogproduct','access' => 'afterpay' 
					) );
