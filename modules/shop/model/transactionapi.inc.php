...
use \Shop\Model\Orm\Transaction, \Shop\Model\Orm\Order, \Shop\Model\PaymentType\PersonalAccount;
...
class TransactionApi extends \RS\Module\AbstractModel\EntityList
{
	...
	{
		parent::__construct ( new \Shop\Model\Orm\Transaction (), array('multisite' => true 
		) );
	}
	
	/**
	 * Создать транзакцию для данного заказа.
	 * Создает новую транзакцию со статусом new
	...
	function createTransactionFromOrder($order_id)
	{
		...
		$order = \Shop\Model\Orm\Order::loadByWhere ( array('order_num' => $order_id 
		) );
		
		if ($order_id && ! $order->id) {
			throw new \Exception ( t ( 'Не найден заказ %0', array($order_id 
			) ) );
		}
	 ...
	 * @return bool
	 */
	function addFunds($user_id, $amount, $writeoff, $reason, $ckeck_rights = true, $entity = null, $entity_id = null)
	{
		if ($ckeck_rights) {
		...
		try {
			$transaction = $this->createTransactionForAddFunds ( $user_id, 0, $writeoff ? - $amount : $amount );
	...
	/**
	 * Разпознать в запросе идентификатор транзакции.
	 * Каждый конкретный тип оплаты это делает по своему.
	 * 
	...
	function recognizeTransactionFromRequest($payment_type, \RS\Http\Request $request)
	{
		...
		if (! isset ( $payment_types[$payment_type] )) {
			throw new \Exception ( t ( 'Неверный тип оплаты: %0', array($payment_type 
			) ) );
		}
		if (! ($payment_types[$payment_type] instanceof \Shop\Model\PaymentType\AbstractType)) {
			throw new \Exception ( t ( 'Тип оплаты должен быть наследником PaymentType\AbstractType: %0', array($payment_type 
			) ) );
		}
		if (! $payment_types[$payment_type]->canOnlinePay ()) {
			throw new \Exception ( t ( 'Тип оплаты не поддерживает online платежи: %0', array($payment_type 
			) ) );
		}
		...
		if (! $transaction_id) {
			throw new \Exception ( t ( "Не передан идентификатор транзакции" ) );
		}
		$transaction = new \Shop\Model\Orm\Transaction ( ( int ) $transaction_id );
		if (! $transaction->id) {
			throw new \Exception ( t ( "Транзакция с идентификатором %0 не найдена", array($transaction_id 
			) ) );
		}
		return $transaction;
	}
	...
	{
		if (! $transaction->id) throw new \Exception ( t ( 'Невозможно подписать транзакцию с нулевым идентификатором' ) );
		$parts = array();
