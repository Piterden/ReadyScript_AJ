...
{
	const API_URL = "http://gw.edostavka.ru:11443/", // Основной URL
...
API_CALCULATE_VERSION = "1.0"; // Версия API для подсчёта стоимости доставки
	private $tariffId = array(), // Идентификатор тарифа по которому будет произведена доставка
...
$cache_api_requests = array(); // Кэш запросов к серверу рассчета
	...
	{
		$properties = new \RS\Orm\PropertyIterator ( array('channel' => new Type\String ( array('maxLength' => 255,'description' => t ( 'Канал для обмена данными с СДЕК' ),'default' => 0,'listFromArray' => array(array(t ( 'По умолчанию' ),t ( 'Запасной канал' ) 
		) 
		),'hint' => t ( 'По умолчанию - ' . self::API_URL . '<br/>Запасной - ' . self::API_URL_ADDITIONAL ) 
		) ),'cash_on_delivery' => new Type\Integer ( array('maxLength' => 1,'default' => 0,'CheckboxView' => array(1,0 
		),'description' => t ( 'Наложенный платёж?' ),'hint' => t ( 'Платёж при получении товаров' ) 
		) ),'secret_login' => new Type\String ( array('maxLength' => 150,'hint' => t ( 'Используется дополнительно для расчёта стоимости<br/>Выдаётся СДЕК' ),'description' => t ( 'Логин для доступа к серверу расчётов' ) 
		) ),'secret_pass' => new Type\String ( array('maxLength' => 150,'hint' => t ( 'Используется дополнительно для расчёта стоимости<br/>Выдаётся СДЕК' ),'description' => t ( 'Пароль для доступа к серверу расчётов' ) 
		) ),'day_apply_delivery' => new Type\Integer ( array('maxLength' => 11,'default' => 1,'description' => t ( 'Количество дней, через сколько будет произведена планируемая отправка заказа' ) 
		) ),'city_from_name' => new Type\String ( array('maxLength' => 150,'description' => t ( 'Название города отправления<br/>Например: Краснодар' ) 
		) ),'city_from_zipcode' => new Type\String ( array('maxLength' => 11,'description' => t ( 'Почтовый индекс города отправителя<br/>Например: 350000' ) 
		) ),'tariffTypeCode' => new Type\Integer ( array('description' => t ( 'Тариф' ),'maxLength' => 11,'visible' => false,'List' => array(array('\Shop\Model\DeliveryType\Cdek\CdekInfo','getAllTariffs' 
		),false 
		),'ChangeSizeForList' => false,'attr' => array(array('size' => 16 
		) 
		) 
		) ),'tariffTypeList' => new Type\ArrayList ( array('description' => t ( 'Список тарифов по приоритету' ),'maxLength' => 1000,'runtime' => false,'attr' => array(array('multiple' => true 
		) 
		),'template' => '%shop%/form/delivery/cdek/tariff_list.tpl','hint' => t ( 'При расчёте стоимости, если указаный тариф будет не доступен для отправления, то расчёт будет вестись по нижеследующему тарифу указанному в списке' ),'listFromArray' => array(array() 
		) 
		) ),'additional_services' => new Type\ArrayList ( array('maxLength' => 1000,'default' => 0,'attr' => array(array('size' => 7,'multiple' => true 
		) 
		),'list' => array(array('\Shop\Model\DeliveryType\Cdek','getAdditionalServices' 
		) 
		),'description' => t ( 'Добавлять допполнительные услуги к заказу:<br/>(Необязательно)' ),'hint' => t ( 'Дополнительные услуги зависят от Ваших условий договора. Обратитесь к менеджеру, за дальнейшими разъяснениями по использованию доп. услуг.' ) 
		) ),'width' => new Type\Integer ( array('description' => t ( 'Свойство со значением ширины товара(см)' ),'default' => 0,'list' => array(array('\Catalog\Model\PropertyApi','staticSelectList' 
		),true 
		) 
		) ),'height' => new Type\Integer ( array('description' => t ( 'Свойство со значением высоты товара(см)' ),'default' => 0,'list' => array(array('\Catalog\Model\PropertyApi','staticSelectList' 
		),true 
		) 
		) ),'length' => new Type\Integer ( array('description' => t ( 'Свойство со значением длинны товара(см)' ),'default' => 0,'list' => array(array('\Catalog\Model\PropertyApi','staticSelectList' 
		),true 
		) 
		) ) 
		) );
	...
	/**
	 * Запрос к серверу рассчета стоимости доставки.
	 * Ответ сервера кешируется
	 * 
	...
	private function apiRequest($script, $params, $method = "POST")
	{
		...
		if (! isset ( $this->cache_api_requests[$cache_key] )) {
			...
			$requst_array = array('http' => array('ignore_errors' => true, // Игнорируем ошибки(статусы ошибок) в заголовках, т.к. может быть 500 ошибка, но контент есть
'method' => $method,'timeout' => self::REQUEST_TIMEOUT 
			) 
			);
	...
	private function requestPochtomat(\Shop\Model\Orm\Order $order, \Shop\Model\Orm\Address $address = null)
	{
		...
		$params = array('citypostcode' => $zipcode 
		);
	...
	private function requestGetInfo(\Shop\Model\Orm\Order $order)
	{
		...
		try {
			return @simplexml_load_string ( $this->apiRequest ( "info_report.php", array('xml_request' => $xml 
			) ) );
		} catch ( \Exception $ex ) {
	...
	private function requestGetCallCourier($call, \Shop\Model\Orm\Order $order)
	{
		...
		try {
			return @simplexml_load_string ( $this->apiRequest ( "call_courier.php", array('xml_request' => $xml 
			) ) );
		} catch ( \Exception $ex ) {
			throw new \RS\Exception ( $ex->getMessage () . "<br/> Файл:" . $ex->getFile () . "<br/> Строка:" . $ex->getLine (), $ex->getCode () );
		}
	...
	private function requestOrderStatus(\Shop\Model\Orm\Order $order)
	{
		...
		try {
			return @simplexml_load_string ( $this->apiRequest ( "status_report_h.php", array('xml_request' => $xml 
			) ) );
		} catch ( \Exception $ex ) {
			throw new \RS\Exception ( $ex->getMessage () . "<br/> Файл:" . $ex->getFile () . "<br/> Строка:" . $ex->getLine (), $ex->getCode () );
		}
	...
	private function requestCreateOrder(\Shop\Model\Orm\Order $order, \Shop\Model\Orm\Address $address = null)
	{
		...
		foreach($products as $n => $item) {
			...
			$sxml->Order->Package->Item[$i]['Comment'] = $offer_title ? $offer_title : $product['title'];
		...
		try {
			return @simplexml_load_string ( $this->apiRequest ( "new_orders.php", array('xml_request' => $xml 
			) ) );
		} catch ( \Exception $ex ) {
			throw new \RS\Exception ( $ex->getMessage () . "<br/> Файл:" . $ex->getFile () . "<br/> Строка:" . $ex->getLine (), $ex->getCode () );
		}
	...
	private function requestDeliveryInfo(\Shop\Model\Orm\Order $order, \Shop\Model\Orm\Address $address = null)
	{
		...
		$request_array = array('http' => array('method' => "POST",'header' => "Content-Type: application/json\r\n",'timeout' => self::REQUEST_TIMEOUT 
		) 
		);
		
		// Параметры
		$query_info = array('version' => self::API_CALCULATE_VERSION,'senderCityPostCode' => $this->getOption ( 'city_from_zipcode' ), // Zip код отправителя
'receiverCityPostCode' => $address['zipcode'] 
		) // Zip код получателя
;
		...
		if (! empty ( $tariffsList )) {
			...
			foreach($tariffsList as $priority => $tariff) {
				$arr[] = array('priority' => $priority,'id' => $tariff 
				);
			}
	...
	private function getCurrencyByName($name)
	{
		...
		) )->orderby ( '`default` DESC' )->objects ( null, 'title' );
	...
	{
		\RS\Orm\Request::make ()->from ( new \Shop\Model\Orm\Order () )->set ( array('admin_comments' => $text 
	...
	private function cdekGetCallCourierHTML(\Shop\Model\Orm\Order $order)
	{
		...
		if ($request->isPost ()) {
			...
			if (isset ( $responce->CallCourier['ErrorCode'] )) {
				...
				$view->assign ( array('success' => 'Сделан вызов курьера на ' . $call['Date'] . '.<br/>
                Ожидание курьера с ' . $call['TimeBeg'] . ':00 по ' . $call['TimeEnd'] . ':00' 
				) );
			}
		}
		
		$view->assign ( array('title' => 'Вызов курьера для забора товара СДЕКом','current_date' => date ( 'Y-m-d', time () + 24 * 60 * 60 ),'time_range' => range ( 1, 24 ),'time_default_start' => 10,'time_default_end' => 13,'order' => $order,'delivery_type' => $this,'current_city' => $this->getOption ( 'city_from_name', '' ),'regions' => $cdekInfo->getAllCities (),'errors' => $this->getErrors () 
		) );
	...
	private function cdekGetInfo(\Shop\Model\Orm\Order $order)
	{
		...
		if (isset ( $response->Order->AddedService ) && ! empty ( $response->Order->AddedService['ServiceCode'] )) {
			...
			if (isset ( $addServices[$service_code] )) {
				$view->assign ( array('addTariffCode' => $addServices[$service_code] 
				) );
			}
		}
		
		$view->assign ( array('order_info' => $response->Order,'title' => 'Информация о заказе СДЕК','tariffCode' => $tariffs[( integer ) $response->Order['TariffTypeCode']],'tariffs' => $tariffs,'order' => $order,'delivery_type' => $this 
		) );
	...
	private function cdekGetHtmlStatus(\Shop\Model\Orm\Order $order)
	{
		...
		$view = new \RS\View\Engine ();
		$view->assign ( array('order_info' => $response->Order,'title' => 'Статус заказа','order' => $order,'delivery_type' => $this 
		) );
	...
	function getAddittionalHtml(\Shop\Model\Orm\Delivery $delivery, \Shop\Model\Orm\Order $order = null)
	{
		...
		$pochtomates = array();
		if ($tariff && in_array ( $tariff['regim_id'], array(2,4 
		) )) { // Если нужны почтоматы
			$pochtomates = $this->requestPochtomat ( $order );
		}
		
		$this->getDeliveryCostText ( $order );
		
		$view->assign ( array('errors' => $this->getErrors (),'order' => $order,'extra_info' => $order->getExtraKeyPair (),'delivery' => $delivery,'cdek' => $this,'pochtomates' => $pochtomates 
	...
	function getAdminAddittionalHtml(\Shop\Model\Orm\Order $order = null)
	{
		...
		$view->assign ( array('errors' => $this->getErrors (),'order' => $order,'cost' => $cost,'extra_info' => $order->getExtraKeyPair (),'delivery' => $delivery,'cdek' => $this,'pochtomates' => $pochtomates 
		) + \RS\Module\Item::getResourceFolders ( $this ) );
	...
	function onOrderCreate(\Shop\Model\Orm\Order $order, \Shop\Model\Orm\Address $address = null)
	{
		...
		if (! isset ( $extra['cdek_order_id'] )) { // Если заказ в СДЕК ещё не создан
			...
			if ($created_order) { // Если дождались ответ от СДЕК
				if (isset ( $created_order->Order[0]['ErrorCode'] )) { // Если есть ошибки, добавим в комметарий
					...
					$order->addExtraInfoLine ( 'id заказа СДЕК', '<a href="http://lk.cdek.ru/" target="_blank">Перейти к заказу №' . $cdek_order_id . '</a>', array('orderId' => $cdek_order_id 
					), 'cdek_order_id' );
				}
			} else {
		...
		\RS\Orm\Request::make ()->update ()->from ( new \Shop\Model\Orm\Order () )->set ( array('_serialized' => serialize ( $order['extra'] ) 
