...
abstract class AbstractType
{
    ...
    * @return double
    */
    abstract function getDeliveryCost(\Shop\Model\Orm\Order $order, \Shop\Model\Orm\Address $address = null, \Shop\Model\Orm\Delivery $delivery, $use_currency = true);
    ...
    */
    function getDeliveryCostText(\Shop\Model\Orm\Order $order, \Shop\Model\Orm\Address $address = null, \Shop\Model\Orm\Delivery $delivery)
    {
        ...
        return ($cost) ? \RS\Helper\CustomView::cost($cost).' '.$order->getMyCurrency()->stitle : t('бесплатно');
    }
    ...
    */
    function applyExtraChangeDiscount($delivery, $cost)
    {
        if ($cost && !empty($delivery['extrachange_discount'])){
            $plus_cost = 0;
