...
{
	const API_URL = "http://spsr.ru/cgi-bin/post09.pl?TARIFFCOMPUTE_2", DEFAULT_COUNTRY = '209|0', // Россия
...
REQUEST_TIMEOUT = 10; // 10 сек. таймаут запроса к api
	...
	{
		$this->setOption ( array('city_from' => self::DEFAULT_CITY_FROM 
		) );
	}
	...
	{
		$properties = new \RS\Orm\PropertyIterator ( array('city_from' => new Type\String ( array('description' => t ( 'Город отправления' ),'ListFromArray' => array($this->getCities () 
		) 
		) ),'tariff' => new Type\String ( array('description' => t ( 'Тариф' ),'ListFromArray' => array($this->getAllTariffs () 
		) 
		) ) 
		) );
	...
	private function apiRequest($params)
	{
		...
		if (! isset ( $this->cache_api_requests[$cache_key] )) {
			$ctx = stream_context_create ( array('http' => array('timeout' => self::REQUEST_TIMEOUT 
			) 
			) );
	...
	private function requestDeliveryInfo(\Shop\Model\Orm\Order $order, \Shop\Model\Orm\Address $address = null)
	{
		...
		$data = $this->apiRequest ( array('FromCountry' => self::DEFAULT_COUNTRY,'FromCity' => $this->getOption ( 'city_from', self::DEFAULT_CITY_FROM ),'Country' => self::DEFAULT_COUNTRY,'ToRegion' => '','to_Cities_name' => '','ToCity' => $this->getCityId ( $address->city ),'Weight' => $order->getWeight () / 1000 
		) );
	...
	{
		$data = $this->apiRequest ( array('FromCountry' => self::DEFAULT_COUNTRY,'FromCity' => self::DEFAULT_CITY_FROM,'Country' => self::DEFAULT_COUNTRY,'ToRegion' => '','to_Cities_name' => '','ToCity' => self::DEFAULT_CITY_TO,'Weight' => 10 
		) );
