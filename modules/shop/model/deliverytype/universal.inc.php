...
class Universal extends AbstractType
{
	...
	function getFormHtml()
	{
		...
		$view->assign ( 'zones', array('all' => t ( ' - все - ' ) 
		) + \Shop\Model\ZoneApi::staticSelectList () );
		$view->assign ( 'ruletypes', array(self::RULE_TYPE_WEIGHT => t ( 'Вес, грамм' ),self::RULE_TYPE_SUM => t ( 'Сумма заказа' ) 
		) );
		$view->assign ( 'actiontypes', array(self::ACTION_TYPE_FIXED => t ( 'Фиксированная' ),self::ACTION_TYPE_ORDER_PERCENT => t ( 'Процент от стоимости заказа' ),self::ACTION_TYPE_DELIVERY_PERCENT => t ( 'Процент от стоимости доставки' ) 
		) );
	...
	function validate(\Shop\Model\Orm\Delivery $delivery)
	{
		...
		foreach($rules as $one) {
			try {
				$formula_vars = array('S' => 1000,'W' => 1000 
				);
	...
	function getDeliveryCost(\Shop\Model\Orm\Order $order, \Shop\Model\Orm\Address $address = null, $use_currency = true)
	{
		...
		if (! empty ( $rules )) {
			foreach($rules as $one_rule) {
				$last_delivery_cost += $this->applyRule ( $order, $address, $one_rule, $last_delivery_cost );
			}
		}
