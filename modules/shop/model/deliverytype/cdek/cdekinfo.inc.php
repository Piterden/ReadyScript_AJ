...
/**
 * Класс с доп.
 * информацией доставки СДЕКа
 */
class CDEKInfo
{
	const FILENAME_RUS_TARIFF = 'rus_tariffs.csv', // Тарифы внутри страны
...
FILENAME_ALL_SERVICES = 'additional_services_all.csv'; // Все дополнительные услуги
	private $tariff_type_compare = array( // Массив типа доставки и соответствия числу
'дверь-дверь (Д-Д)' => 1,'дверь-склад (Д-С)' => 2,'склад-дверь (С-Д)' => 3,'склад-склад (С-С)' => 4 
	);
	...
	function getFilePath($filename)
	{
		...
		if (! file_exists ( $path )) {
			throw new \RS\Exception ( "Нет файла CSV для обработки " . $path . ". ", 1 );
		}
		return $path;
	}
	...
	function getCSVInfo($filename, $format = array())
	{
		...
		while ( ($row = fgetcsv ( $handler, null, $csv_delimiter, $csv_enclosure )) !== false ) {
			...
			foreach($row as $key => $field) {
				$row_value = iconv ( "WINDOWS-1251", "UTF-8", $field );
				$row_arr[$format[$key]] = $row_value;
				if ($format[$key] == "regim") {
					$row_arr['regim_id'] = $this->tariff_type_compare[$row_value];
				}
			}
			$arr[$row[0]] = $row_arr;
		}
	...
	{
		return $this->getCSVInfo ( self::FILENAME_INT_TARIFF, array('code','title','regim','weight_limit','group','description' 
		) );
	}
	...
	{
		return $this->getCSVInfo ( self::FILENAME_RUS_TARIFF, array('code','title','regim','weight_limit','group','description' 
		) );
	}
	...
	{
		return $this->getCSVInfo ( self::FILENAME_RUS_CITIES, array('code','fullname','title','region' 
		) );
	}
	...
	{
		return $this->getCSVInfo ( self::FILENAME_KAZ_CITIES, array('code','fullname','title','region' 
		) );
	}
	...
	{
		return $this->getCSVInfo ( self::FILENAME_UKR_CITIES, array('code','fullname','title','region' 
		) );
	}
	...
	{
		return $this->getCSVInfo ( self::FILENAME_BLR_CITIES, array('code','fullname','title','region' 
		) );
	}
	...
	{
		$_this = new self ();
		return $_this->getCSVInfo ( self::FILENAME_ADD_SERVICES, array('code','agree','title','description' 
		) );
	}
	
	/**
	 * Получает полный массив массив доп.
	 * услуг
	 * 
	 * @return array
	 */
	...
	{
		$_this = new self ();
		return $_this->getCSVInfo ( self::FILENAME_ALL_SERVICES, array('code','agree','title','description' 
		) );
	}
	
	/**
	 * Получает все тарифы СДЕК на отправку
	 * 
	 * @return array
	 */
