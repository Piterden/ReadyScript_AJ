...
{
	protected static $controller_title = 'Текущий шаг оформления заказа', $controller_description = 'Отображает цепочку шагов оформления заказа, с подсветкой текущего и возможностью вернуться к предыдущему';
	protected $default_params = array('indexTemplate' => 'blocks/checkoutstep/step.tpl' 
	);
		...
		if ($route->getId () == 'shop-front-checkout') {
			...
			switch ($act) {
				...
				case 'warehouses' :
					...
					break;
				case 'payment' :
					$config['hide_delivery'] ? $step = 2 : $step = 3;
					break;
				case 'confirm' :
