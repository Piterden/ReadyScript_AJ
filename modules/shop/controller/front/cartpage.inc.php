...
class CartPage extends \RS\Controller\Front
{
	...
	function actionIndex()
	{
		...
		if (! empty ( $add_product_id )) {
			...
			$this->cart->addProduct ( $add_product_id, $add_product_amount, $add_product_offer, $add_product_multioffers, $add_product_concomitants, $add_product_concomitants_amount );
		...
		$this->view->assign ( array('cart' => $this->cart,'cart_data' => $cart_data 
		) );
		...
		return $this->result->addSection ( 'cart', array('can_checkout' => ! $cart_data['has_error'],'total_unformated' => $cart_data['total_unformatted'],'total_price' => $cart_data['total'],'items_count' => $cart_data['items_count'] 
		) )->setTemplate ( 'cartpage.tpl' );
	}
	
	/**
	 * Обновляет информацию о товарах, их количестве в корзине.
	 * Добавляет купон на скидку, если он задан
	 */
	function actionUpdate()
	{
		if ($this->url->isPost ()) {
			...
			if ($apply_coupon !== true) {
				...
				$this->view->assign ( array('coupon_code' => $coupon 
				) );
			}
		}
