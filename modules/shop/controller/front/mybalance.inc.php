...
class MyBalance extends \RS\Controller\AuthorizedFront
{
	function actionIndex()
	{
		...
		$this->view->assign ( array('paginator' => $paginator,'list' => $list 
		) );
	...
	function actionAddFunds()
	{
		...
		$this->app->title->addSection ( $title );
		$this->app->breadcrumbs->addBreadCrumb ( t ( 'Лицевой счет' ), $this->router->getUrl ( 'shop-front-mybalance' ) )->addBreadCrumb ( $title );
		...
		), 'in' ); // Разный список оплат, в зависимости от того, компания ли это или обынчй пользователь
		$pay_api->setFilter ( 'class', PersonalAccount::SHORT_NAME, '!=' ); // Исключаем из способов пополнения счета оплату с самого себя
		$pay_api->setFilter ( 'target', array('all','refill' 
		), 'in' );
		...
		if ($this->url->isPost ()) {
			...
			if (! $transApi->hasError ()) {
				...
				if ($transaction->getPayment ()->getTypeObject ()->isPostQuery ()) { // Если нужен пост запрос
					...
					$this->view->assign ( array('url' => $url,'transaction' => $transaction 
					) );
