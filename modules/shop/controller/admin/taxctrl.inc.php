...
class TaxCtrl extends \RS\Controller\Admin\Crud
{
	...
	function helperIndex()
	{
		...
		) ),new TableType\String ( 'description', t ( 'Описание' ) ),new TableType\Yesno ( 'enabled', t ( 'Включен' ), array('toggleUrl' => $this->router->getAdminPattern ( 'ajaxTogglePublic', array(':id' => '@id' 
		) ) 
		) ),new TableType\String ( 'sortn', t ( 'Приоритет' ) ),
		...
		) ),new TableType\Action\DropDown ( array(array('title' => t ( 'Клонировать налог' ),'attr' => array('class' => 'crud-add','@href' => $this->router->getAdminPattern ( 'clone', array(':id' => '~field~' 
		) ) 
		) 
		) 
		) ) 
