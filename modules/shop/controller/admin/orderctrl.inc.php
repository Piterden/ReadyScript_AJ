...
use \RS\Html\Table\Type as TableType, \RS\Controller\Admin\Helper, \RS\Html\Toolbar\Button as ToolbarButton, \RS\Html\Toolbar, \RS\Html\Filter, \RS\Html\Table, \RS\Html\Tree, \Shop\Model;
...
class OrderCtrl extends \RS\Controller\Admin\Crud
{
	...
	function helperIndex()
	{
		...
		$helper->setTopToolbar ( new Toolbar\Element ( array('Items' => array(new ToolbarButton\Dropdown ( array(array('title' => t ( 'создать заказ' ),'attr' => array('href' => $this->router->getAdminUrl ( 'add' ),'class' => 'button add' 
		) 
		),

		array('title' => t ( 'добавить статус' ),'attr' => array('href' => $this->router->getAdminUrl ( 'add', array(), 'shop-userstatusctrl' ),'class' => 'crud-add' 
		) 
		) 
		...
		) ) )->viewAsTableTree ()->setTopTitle ( t ( 'Заказы' ) )->setTable ( new Table\Element ( array('Columns' => array(new TableType\Checkbox ( 'id', array('showSelectAll' => true 
		) ),new TableType\String ( 'order_num', t ( 'Номер' ), array('Sortable' => SORTABLE_BOTH,'href' => $edit_href 
		...
		) ), null, array('noajax' => true,'attr' => array('@data-id' => '@id' 
		) 
		) ) 
		...
		$helper['topToolbar']->addItem ( new ToolbarButton\Dropdown ( array(array('title' => t ( 'Экспорт/Отчёт' ),'attr' => array('class' => 'button','onclick' => "JavaScript:\$(this).parent().rsDropdownButton('toggle')" 
		) 
		),array('title' => t ( 'Экспорт заказов в CSV' ),'attr' => array('data-url' => \RS\Router\Manager::obj ()->getAdminUrl ( 'exportCsv', array('schema' => 'shop-order','referer' => $this->url->selfUri () 
		), 'main-csv' ),'class' => 'crud-add' 
		) 
		),array('title' => t ( 'Показать отчёт' ),'attr' => array('data-url' => \RS\Router\Manager::obj ()->getAdminUrl ( 'ordersReport' ),'class' => 'crud-add' 
		) 
		) 
		) ) );
		...
		if ($this->status > 0 && $helper->getTreeViewType () == \RS\Controller\Admin\Helper\CrudCollection::VIEW_CAT_TOP) {
			...
			$path_to_first = array(array('title' => t ( 'Все' ),'hideInlineButtons' => array('edit','delete' 
			) 
			) 
			);
		}
		
		$tree = new Tree\Element ( array('classField' => '_class','sortIdField' => 'id','activeField' => 'id','activeValue' => $this->status,'pathToFirst' => $path_to_first,'sortable' => false,'noExpandCollapseButton' => true,'mainColumn' => new TableType\Usertpl ( 'title', 'Название', '%shop%/order_tree_cell.tpl', array('href' => $this->router->getAdminPattern ( false, array(':status' => '@id' 
		) ) 
		) ),'tools' => new TableType\Actions ( 'id', array(new TableType\Action\Edit ( $this->router->getAdminPattern ( 'edit', array(':id' => '~field~' 
		), 'shop-userstatusctrl' ), null, array('attr' => array('@data-id' => '@id' 
		) 
		) ) 
		) ),'inlineButtons' => array('add' => array('attr' => array('title' => t ( 'Добавить статус' ),'href' => $this->router->getAdminUrl ( 'add', array(), 'shop-userstatusctrl' ),'class' => 'add crud-add' 
		) 
		),'edit' => array('attr' => array('title' => t ( 'редактировать' ),'href' => $this->router->getAdminUrl ( 'edit', array('id' => $this->status 
		), 'shop-userstatusctrl' ),'class' => 'edit crud-edit' 
		) 
		) 
		) + ((! $current_status || ! $current_status->isSystem ()) ? array('delete' => array('attr' => array('title' => t ( 'удалить' ),'href' => $this->router->getAdminUrl ( 'del', array('chk[]' => $this->status 
		), 'shop-userstatusctrl' ),'class' => 'delete crud-remove-one' 
		) 
		) 
		) : array()),'headButtons' => array(array('text' => t ( 'Статус' ),'tag' => 'span','attr' => array('class' => 'lefttext' 
		) 
		),array('attr' => array('title' => t ( 'Добавить статус' ),'href' => $this->router->getAdminUrl ( 'add', array(), 'shop-userstatusctrl' ),'class' => 'add crud-add' 
		) 
		) 
		) 
		) );
		
		$helper->setTreeListFunction ( 'getAdminTreeList' )->setTree ( $tree, $this->status_api )->setTreeBottomToolbar ( new Toolbar\Element ( array('Items' => array(new ToolbarButton\Delete ( null, null, array('attr' => array('data-url' => $this->router->getAdminUrl ( 'del', array(), 'shop-userstatusctrl' ) 
		) 
		) ) 
		) 
		) ) );
		
		$helper->setFilter ( new Filter\Control ( array('Container' => new Filter\Container ( array('Lines' => array(new Filter\Line ( array('Items' => array(new Filter\Type\String ( 'order_num', '№', array('attr' => array('class' => 'w50' 
		) 
		) ),new Filter\Type\DateRange ( 'dateof', t ( 'Дата оформления' ) ),new Filter\Type\String ( 'totalcost', t ( 'Сумма' ), array('showtype' => true 
		) ) 
		) 
		) ) 
		)
		,'SecContainer' => new Filter\Seccontainer ( array('Lines' => array(new Filter\Line ( array('Items' => array(new Filter\Type\User ( 'user_id', 'Пользователь' ),new Filter\Type\String ( 'user_fio', 'ФИО без регистрации', array('searchType' => '%like%' 
		) ) 
		) 
		) ) 
		) 
	...
	function helperEdit()
	{
		...
		$helper = new \RS\Controller\Admin\Helper\CrudCollection ( $this, $this->api, $this->url, array('bottomToolbar' => $this->buttons ( array('save','cancel' 
		) ),'viewAs' => 'form','formTitle' => t ( 'Создание заказа' ) 
		) );
		if ($id > 0) { // Если заказ уже существует
			$helper['bottomToolbar']->addItem ( new ToolbarButton\delete ( $this->router->getAdminUrl ( 'delOrder', array('id' => $id,'dialogMode' => $this->url->request ( 'dialogMode', TYPE_INTEGER ) 
			) ), null, array('noajax' => true,'attr' => array('class' => 'delete crud-get crud-close-dialog','data-confirm-text' => t ( 'Вы действительно хотите удалить заказ?' ) 
			) 
			) ), 'delete' );
		}
	...
	function actionEdit()
	{
		...
		if ($this->url->isPost ()) {
			...
			$fields = array('delivery','user_id','use_addr','is_payed','payment','contact_person','status','user_delivery_cost','admin_comments','user_text','notify_user','user_fio','user_email','user_phone' 
			);
			...
			if (! $refresh_mode) {
				...
				if (isset ( $order['id'] ) && $order['id'] < 0) {
					...
					if ($order->insert ()) { // Перенаправим на ректирование, если создался заказ
						...
						return $this->result->setSuccess ( true )->setSuccessText ( t ( 'Заказ успешно создан' ) )->setHtml ( false )->addSection ( 'windowRedirect', $this->router->getAdminUrl ( 'edit', array('id' => $order['id'] 
						), 'shop-orderctrl' ) );
					} else {
						return $this->result->setSuccess ( false )->addEMessage ( $order->getErrorsStr () );
					}
		...
		$this->view->assign ( array('order_footer_fields' => $order->getForm ( null, 'footer', false, null, '%shop%/order_footer_maker.tpl' ),'catalog_folders' => \RS\Module\Item::getResourceFolders ( 'catalog' ),'elem' => $order,'user_id' => $order['user_id'],'warehouse_list' => $warehouses,'status_list' => $user_status_api->getGroupedList (),'default_statuses' => $user_status_api->getStatusIdByType (),'refresh_mode' => $refresh_mode,'show_delivery_buttons' => $show_delivery_buttons,'user_num_of_order' => $user_num_of_order 
		) );
		...
		if ($refresh_mode) { // Если режим обновления
			...
			$this->view->assign ( array('elements' => $helper->active () 
			) );
	 ...
	 * Создание заказа
	 * 
	 * @param integer|null $primaryKey - первичный ключ
	 * @param \RS\Controller\Admin\Helper\CrudCollection|null $helper - объект Helperа
	 * @return Standart
	 */
	function actionAdd($primaryKey = null, $helper = null)
	{
		...
		$user_status_api = new Model\UserStatusApi ();
		$this->view->assign ( array('elem' => $order,'order_footer_fields' => $order->getForm ( null, 'footer', false, null, '%shop%/order_footer_maker.tpl' ),'catalog_folders' => \RS\Module\Item::getResourceFolders ( 'catalog' ),'warehouse_list' => $warehouses,'status_list' => $user_status_api->getGroupedList (),'user_num_of_order' => 0,'refresh_mode' => false 
		) );
		$helper['form'] = $this->view->fetch ( '%shop%/orderview.tpl' );
		return $this->result->setTemplate ( $helper['template'] );
	}
	...
	function actionUserDialog()
	{
		...
		$helper->setTopTitle ( t ( 'Добавление пользователя' ) )->viewAsForm ();
		...
		if ($this->url->isPost ()) {
			...
			if ($is_reg_user) { // Если нужно регистрировать
				...
				if ($user->save ()) {
					...
					$this->view->assign ( array('user' => $user,'user_num_of_order' => $user_num_of_order 
					) );
					return $this->result->setSuccess ( true )->addSection ( 'noUpdateTarget', true )->addSection ( 'user_id', $user['id'] )->addSection ( 'insertBlockHTML', $this->view->fetch ( '%shop%/form/order/user.tpl' ) );
				} else {
				...
				if ($user_id) {
					...
					return $this->result->setSuccess ( true )->addSection ( 'noUpdateTarget', true )->addSection ( 'user_id', $user_id )->addSection ( 'insertBlockHTML', $this->view->fetch ( '%shop%/form/order/user.tpl' ) );
				} else {
		...
		$this->view->assign ( array('user' => $user,'elem' => $order 
		) );
		
		$helper->setBottomToolbar ( new Toolbar\Element ( array('Items' => array('save' => new ToolbarButton\SaveForm ( null, t ( 'применить' ) ),'cancel' => new ToolbarButton\Cancel ( null, t ( 'отмена' ) ) 
		) 
		) ) );
	...
	function actionDeliveryDialog()
	{
		...
		if (isset ( $order['use_addr'] ) && $order['use_addr']) {
			$this->view->assign ( array('current_address' => $address_api->getOneItem ( $order['use_addr'] ),'address_part' => $this->actionGetAddressRecord ( $order['use_addr'] )->getHtml () 
			) );
			...
			$this->view->assign ( array('address_part' => $this->actionGetAddressRecord ( $use_addr )->getHtml () 
			) );
		}
		...
		if ($this->url->isPost ()) { // Если пришёл запрос
			...
			$this->view->assign ( array('elem' => $order,'delivery' => $delivery,'user_id' => $user_id,'address' => $address,'warehouse_list' => $warehouses,'user_delivery_cost' => $user_delivery_cost 
			) );
			
			return $this->result->setSuccess ( true )->setHtml ( false )->addSection ( 'noUpdateTarget', true )->addSection ( 'delivery', $delivery_id )->addSection ( 'address', $post_address )->addSection ( 'use_addr', $use_addr )->addSection ( 'user_delivery_cost', $user_delivery_cost )->addSection ( 'insertBlockHTML', $this->view->fetch ( '%shop%/form/order/delivery.tpl' ) );
		}
		
		$this->view->assign ( array('dlist' => $dlist,'order' => $order,'delivery_id' => $delivery_id,'address_list' => $address_list 
		) );
	...
	function actionPaymentDialog()
	{
		...
		if ($this->url->isPost ()) { // Если пришёл запрос
			...
			$this->view->assign ( array('elem' => $order,'payment_id' => $pay_id,'pay' => $order->getPayment () 
			) );
			
			return $this->result->setSuccess ( true )->setHtml ( false )->addSection ( 'noUpdateTarget', true )->addSection ( 'payment', $pay_id )->addSection ( 'insertBlockHTML', $this->view->fetch ( '%shop%/form/order/payment.tpl' ) );
		}
		
		$pay_api = new Model\PaymentApi ();
		$plist = $pay_api->getList ();
		
		$this->view->assign ( array('order' => $order,'plist' => $plist 
		) );
	...
	function actionGetAddressRecord($_address_id = null)
	{
		...
		$this->view->assign ( array('address' => $address,'country_list' => $country_list,'region_list' => $region_list 
		) );
	...
	function actionGetOfferPrice()
	{
		...
		foreach($cost_ids as $cost_id) {
			...
			$offer_costs[$cost_id] = array('title' => $cost_obj->title,'cost' => $product->getCost ( $cost_id, $offer_index, false ) 
			);
		}
	...
	function actionOrderQuery()
	{
		...
		if (! $type) {
			return $this->result->setSuccess ( false )->addSection ( 'close_dialog', true )->addEMessage ( 'Не указан параметр запроса - type (delivery или payment)' );
		}
		$order_id = $this->request ( 'order_id', TYPE_STRING, 0 );
		
		if (! $order_id) {
			return $this->result->setSuccess ( false )->addSection ( 'close_dialog', true )->addEMessage ( 'id заказа указан неправильно' );
		}
		 ...
		 * @var \Shop\Model\Orm\Order
		 */
		$order = \RS\Orm\Request::make ()->from ( new \Shop\Model\Orm\Order () )->where ( array('order_num' => $order_id 
		) )->object ();
		
		if (! $order) {
			return $this->result->setSuccess ( false )->addSection ( 'close_dialog', true )->addEMessage ( 'Такой заказ не найден' );
		}
		...
		switch ($type) {
			case "delivery" :
				...
				break;
			case "payment" :
				return $this->result->setSuccess ( true )->setHtml ( $order->getPayment ()->getTypeObject ()->actionOrderQuery ( $order ) );
				break;
		}
	}
	...
	function actionOrdersReport()
	{
		...
		if ($where_conditions) {
			...
			$this->view->assign ( array('order_report_arr' => $order_report_arr,'currency' => \Catalog\Model\CurrencyApi::getBaseCurrency (), // Базовая валюта
'payments' => \Shop\Model\PaymentApi::staticSelectList (),'deliveries' => \Shop\Model\DeliveryApi::staticSelectList () 
			) );
		}
