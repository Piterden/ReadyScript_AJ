...
class DeliveryCtrl extends \RS\Controller\Admin\Crud
{
	...
	function helperIndex()
	{
		...
		), array('add' => t ( 'Добавить способ доставки' ) 
		) ) );
		...
		) ),new TableType\String ( 'class', t ( 'Тип рассчета' ) ),new TableType\String ( 'user_type', t ( 'Доступен для' ) ),new TableType\Yesno ( 'public', 'Видим.', array('Sortable' => SORTABLE_BOTH,'toggleUrl' => $this->router->getAdminPattern ( 'ajaxTogglePublic', array(':id' => '@id' 
		) ) 
		...
		) ),new TableType\Action\DropDown ( array(array('title' => t ( 'Клонировать способ доставки' ),'attr' => array('class' => 'crud-add','@href' => $this->router->getAdminPattern ( 'clone', array(':id' => '~field~' 
		) ) 
		) 
		) 
		) ) 
