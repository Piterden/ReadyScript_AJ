...
{
	protected $info_title = 'Динамика продаж', $info_description = 'График заказов по месяцам с возможностью сравнивать продажи в разных годах';
	public $api;
	function init()
	{
	...
	function actionIndex()
	{
		...
		$this->app->headers->addCookie ( 'sellchart_range' . $site_id, $range, $cookie_expire, $cookie_path )->addCookie ( 'sellchart_orders' . $site_id, $orders, $cookie_expire, $cookie_path )->addCookie ( 'sellchart_show_type' . $site_id, $show_type, $cookie_expire, $cookie_path );
		...
		$this->view->assign ( array('range' => $range,'orders' => $orders,'show_type' => $show_type,'dynamics_arr' => $order_dynamics_arr,'dynamics' => json_encode ( $order_dynamics_arr ),'currency' => json_encode ( \Catalog\Model\CurrencyApi::getBaseCurrency ()->stitle ) 
		) );
