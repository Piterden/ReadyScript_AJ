...
class DiscountCtrl extends \RS\Controller\Admin\Crud
{
	...
	function helperIndex()
	{
		...
		$helper->setTable ( new Table\Element ( array('Columns' => array(new TableType\Checkbox ( 'id' ),new TableType\String ( 'code', t ( 'Код' ), array('Sortable' => SORTABLE_BOTH,'href' => $this->router->getAdminPattern ( 'edit', array(':id' => '@id' 
		...
		new TableType\String ( 'descr', t ( 'Описание' ), array('hidden' => true 
		) ),new TableType\Yesno ( 'active', t ( 'Включен' ), array('toggleUrl' => $this->router->getAdminPattern ( 'toggleCoupon', array(':id' => '@id' 
		) ),'Sortable' => SORTABLE_BOTH 
		) ),new TableType\Usertpl ( 'discount', t ( 'Скидка' ), '%shop%/discount/discount.tpl', array('Sortable' => SORTABLE_BOTH 
		) ),new TableType\String ( 'wasused', t ( 'Использована' ), array('Sortable' => SORTABLE_BOTH 
		) ),
		...
		) ),new TableType\Action\DropDown ( array(array('title' => t ( 'Клонировать купон на скидку' ),'attr' => array('class' => 'crud-add','@href' => $this->router->getAdminPattern ( 'clone', array(':id' => '~field~' 
		) ) 
		) 
		) 
		) ) 
		...
		$helper->setFilter ( new Filter\Control ( array('Container' => new Filter\Container ( array('Lines' => array(new Filter\Line ( array('Items' => array(new Filter\Type\String ( 'code', 'Код', array('attr' => array('class' => 'w100' 
		) 
		) ),new Filter\Type\String ( 'discount', 'Скидка', array('showType' => true 
