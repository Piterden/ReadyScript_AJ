...
{
	protected $filter_parts = array();
	function __construct()
	{
		parent::__construct ( new \Catalog\Model\Orm\Offer (), array('defaultOrder' => 'sortn','sortField' => 'sortn','titleField' => 'title' 
		) );
	}
	...
	function applyFormFilter($get_filters, $url_params = array())
	{
		...
		$allow_keys = array('title' => t ( 'Название' ),'barcode' => t ( 'Артикул' ),'num' => t ( 'Остаток' ) 
		);
		...
		foreach($get_filters as $key => $value) {
			...
			$this->filter_parts[] = array('text' => $allow_keys[$key] . ': ' . $value,'clean_url' => $router->getAdminUrl ( false, array('offer_filter' => $without_filter 
			) + $url_params, 'catalog-block-offerblock' ) 
			);
		}
	}
	...
	function saveOffers($product_id, $offers, $use_unconverted_propsdata = false)
	{
		...
		foreach($offers as $key => $item) {
			...
			$item += array('photos_arr' => array(),'pricedata_arr' => array() 
			);
			...
			if ($is_only_main_offer) {
				$offer = Orm\Offer::loadByWhere ( array('product_id' => $product_id,'sortn' => 0 
				) );
			}
	...
	{
		$sub_query = \RS\Orm\Request::make ()->select ( 'SUM(num)' )->from ( new Orm\Offer (), 'O' )->where ( 'O.product_id = P.id' )->toSql ();
		
		\RS\Orm\Request::make ()->update ( new \Catalog\Model\Orm\Product () )->asAlias ( 'P' )->set ( "P.num = ($sub_query)" )->where ( array('id' => $product_id 
		) )->update ()->exec ();
	}
	...
	function deleteExcludedOffers($product_id, $offers_ids)
	{
		...
		if ($offers_ids) {
			...
			\RS\Orm\Request::make ()->from ( new \Catalog\Model\Orm\Offer () )->where ( 'id NOT IN (' . implode ( ",", $offers_ids ) . ')' )->where ( array('product_id' => $product_id 
			) )->delete ()->exec ();
			// Удаляем количество товаров комплектаций
			\RS\Orm\Request::make ()->from ( new \Catalog\Model\Orm\Xstock () )->where ( 'offer_id NOT IN (' . implode ( ",", $offers_ids ) . ')' )->where ( array('product_id' => $product_id 
			) )->delete ()->exec ();
		}
	...
	function deleteOffersByProductId($product_id)
	{
		...
		if (is_array ( $product_id )) {
			// Удаляем комлектации
			\RS\Orm\Request::make ()->delete ()->from ( new Orm\Offer () )->whereIn ( 'product_id', $product_id )->where ( 'sortn > 0' )->exec ();
		} else {
			// Удаляем комлектации
		...
		$sub_query = \RS\Orm\Request::make ()->select ( 'id' )->from ( new \Catalog\Model\Orm\Offer () )->toSql ();
		$q = \RS\Orm\Request::make ()->from ( new \Catalog\Model\Orm\Xstock () )->where ( 'offer_id NOT IN (' . $sub_query . ')' )->delete ()->exec ();
	 ...
	 * т.к.
	 * в результате удаления комплектаций, могут появиться пропуски в sortn
	 */
	function rebuildSortn($product_id)
	{
		if ($this->noWriteRights ()) return false;
		
		$offers_sortn = \RS\Orm\Request::make ()->select ( 'id, sortn' )->from ( $this->obj_instance )->where ( 'sortn > 0' )->where ( array('product_id' => $product_id 
		) )->orderby ( 'sortn' )->exec ()->fetchSelected ( 'id', 'sortn' );
		
		$i = 1;
		foreach($offers_sortn as $id => $sortn) {
			if ($sortn != $i) {
				\rs\Orm\Request::make ()->update ( $this->obj_instance )->set ( array('sortn' => $i 
				) )->where ( array('id' => $id 
				) )->exec ();
			}
			$i ++;
	...
	function multiUpdate(array $data, $ids = array())
	{
		...
		if (isset ( $data['photos_arr'] )) {
			...
			\RS\Orm\Request::make ()->update ( new Orm\Offer () )->set ( array('photos' => $photos 
			) )->whereIn ( 'id', $ids )->exec ();
		...
		if (isset ( $data['pricedata_arr'] )) {
			...
			if (empty ( $data['pricedata_arr']['oneprice']['use'] )) {
				...
				$q = \RS\Orm\Request::make ()->select ( 'id, pricedata' )->from ( $this->obj_instance )->whereIn ( 'id', $ids )->limit ( 50 );
				...
				while ( $offer_rows = $q->offset ( $offset )->exec ()->fetchAll () ) {
					foreach($offer_rows as $row) {
						...
						\RS\Orm\Request::make ()->update ( $this->obj_instance )->set ( array('pricedata' => $pricedata 
		...
		if (isset ( $data['propsdata_arr'] )) {
			...
			\RS\Orm\Request::make ()->update ( new Orm\Offer () )->set ( array('propsdata' => $propsdata 
			) )->whereIn ( 'id', $ids )->exec ();
		...
		if (isset ( $data['stock_num'] )) {
			...
			\RS\Orm\Request::make ()->delete ()->from ( new \Catalog\Model\Orm\Xstock () )->whereIn ( 'offer_id', $ids )->exec ();
			
			$offerId_productId = \RS\Orm\Request::make ()->select ( 'id, product_id' )->from ( $this->obj_instance )->whereIn ( 'id', $ids )->exec ()->fetchSelected ( 'id', 'product_id' );
	...
	{
		return \RS\Orm\Request::make ()->delete ()->from ( new \Catalog\Model\Orm\Offer () )->where ( 'product_id < 0' )->exec ()->affectedRows ();
	}
	...
	{
		$result = array('params' => array(),'offers' => array(),'selected' => array() 
		);
		...
		), \RS\Orm\Request::make ()->from ( $this->obj_instance )->where ( 'sortn>0' )->where ( array('product_id' => $product_id 
		) )->orderby ( 'sortn' )->exec ()->fetchAll () );
		...
		foreach($offers as $offer_arr) {
			...
			$result['offers'][$offer['id']] = array('title' => ($title == '') ? $offer['title'] : $title,'params' => $offer['propsdata_arr'] 
			);
	...
	function linkPhotosToOffers($product_id, $photos_id, $offers_id, $mainoffer)
	{
		...
		), \RS\Orm\Request::make ()->select ( 'id, photos' )->from ( $this->obj_instance )->where ( array('product_id' => $product_id 
		) )->where ( 'sortn>0' )->exec ()->fetchAll () );
		...
		foreach($all_offers as $n => $offer) {
			...
			\RS\Orm\Request::make ()->update ( $this->obj_instance )->set ( array('photos' => serialize ( $photos_arr ) 
