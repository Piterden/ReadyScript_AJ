...
class SimplePriceStockUpdate extends \RS\Csv\AbstractSchema
{
	function __construct()
	{
		...
		parent::__construct ( new \Catalog\Model\CsvPreset\SimplePriceStockBase ( array('idField' => 'title','ormObject' => new Orm\Offer (),'ormProductObject' => new Orm\Product (),'ormUnsetFields' => array('pricedata','title' 
		),'fields' => array('title','barcode' 
		),'multisite' => true,'selectOrder' => 'product_id ASC, sortn ASC','selectRequest' => $request,'searchFields' => array('barcode' 
		...
		) ), array(new \Catalog\Model\CsvPreset\OfferStock ( array('linkPresetId' => 0,'linkOfferIdField' => 'id','linkForeignField' => 'offer_id' 
		) ),new \Catalog\Model\CsvPreset\StockCost ( array('linkPresetId' => 0,'linkIdField' => 'id','sortnField' => 'sortn','linkIdFieldProduct' => 'product_id','arrayProductField' => 'xcost', // Поле для обновления цены в основной комплектации в товаре
'arrayOfferField' => 'pricedata_arr' 
		) // Поле для обновления цены всех комплектаций, кроме нулевой
 ) 
		), array('afterImport' => array($this,'afterImport' 
		),'fieldScope' => array('title' => self::FIELDSCOPE_EXPORT 
		) 
		) );
	}
	...
	function getBaseQuery()
	{
		...
		if ($savedRequest = \Catalog\Model\Api::getSavedRequest ( 'Catalog\Controller\Admin\Ctrl_list' )) {
			...
			$q->limit ( null )->orderby ( 'OFFERS.product_id ASC, OFFERS.sortn ASC' )->leftjoin ( new \Catalog\Model\Orm\Offer (), 'OFFERS.product_id = A.id', 'OFFERS' )->where ( 'OFFERS.product_id>0' )->setReturnClass ( new \Catalog\Model\Orm\Offer () );
	...
	function afterBaseRowImport($preset)
	{
		...
		if ($preset->row['barcode'] && $offer && ! $offer['sortn'] && isset ( $preset->row['excost'] )) {
			...
			$product['excost'] = $preset->row['excost'];
			$product->update ();
		}
	}
}