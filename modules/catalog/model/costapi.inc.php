...
{
	protected static $session_old_cost_id, $session_default_cost_id, $instance, $full_costlist;
	function __construct()
	{
		parent::__construct ( new \Catalog\Model\Orm\Typecost (), array('multisite' => true,'nameField' => 'title','defaultOrder' => 'id' 
		) );
	}
	...
	{
		$no_select = $prepend_no_select ? array(0 => 'Не выбрано' 
		) : array();
	 ...
	 * Из списка исключаются автовычесляемые цены и редактируемая сейчас.
	 */
	...
	{
		$sites = \RS\Orm\Request::make ()->from ( new \Site\Model\Orm\Site () )->orderby ( 'title ASC' )->objects ();
		
		foreach($sites as $k => $site) {
			$prices = \RS\Orm\Request::make ()->from ( new \Catalog\Model\Orm\Typecost () )->where ( array('site_id' => $site['id'] 
			) )->orderby ( 'title ASC' )->objects ();
			
			$prices = array(- 1 => array('id' => 0,'title' => 'По умолчанию' 
			) 
			) + $prices;
	...
	public static function getUserCost(\Users\Model\Orm\User $user = null)
	{
		...
		// Для совместимости с предыдущей версией
		$user_cost = is_numeric ( $user['cost_id'] ) ? array($site_id => $user['cost_id'] 
		) : @unserialize ( $user['cost_id'] );
	...
	public static function getOldCostId()
	{
		if (! self::$session_old_cost_id) {
			...
			if (! $old_cost_id) {
				...
				$old_cost_type = orm\Typecost::loadByWhere ( array('site_id' => \RS\Site\Manager::getSiteId (),'title' => t ( 'Зачеркнутая цена' ) 
				) );
	...
	public static function recalculateCosts($site_id = null)
	{
		...
		$xcost = new Orm\Xcost ();
		$q = \RS\Orm\Request::make ()->select ( 'X.*' )->from ( new Orm\Product (), 'P' )->join ( $xcost, 'P.id = X.product_id', 'X' )->where ( array('P.site_id' => $site_id 
		...
		while ( $res->rowCount () ) {
			...
			$sql = "INSERT INTO " . $xcost->_getTable () . "(product_id, cost_id, cost_val) VALUES" . implode ( ',', $values ) . " ON DUPLICATE KEY UPDATE cost_val = VALUES(cost_val)";
		...
		$offset = 0;
		$pageSize = 200;
		$q = \RS\Orm\Request::make ()->from ( new Orm\Offer () )->where ( array('site_id' => $site_id 
