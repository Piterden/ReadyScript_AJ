...
{
	const COOKIE_CURRENCY_KEY = 'currency', CBR_LINK = "http://www.cbr.ru/scripts/XML_daily.asp"; // ссылка получения курса ЦБ РФ
	protected static $current_currency, $default_currency, $base_currency;
	function __construct()
	{
		parent::__construct ( new \Catalog\Model\Orm\Currency (), array('multisite' => true,'nameField' => 'title' 
		) );
	}
	...
	public static function getCurrentCurrency()
	{
		if (self::$current_currency === null) {
			...
			if (! empty ( $cookie )) {
				self::$current_currency = \RS\Orm\Request::make ()->from ( new Orm\Currency () )->where ( array('site_id' => \RS\Site\Manager::getSiteId (),'title' => $cookie 
				) )->object ();
			}
	...
	/**
	 * Возвращает объект с базовой валютой.
	 * (базовой считается валюта, в которой цены в системе)
	 * 
	 * @return Orm\Currency
	 */
	 ...
	 */
	public static function setCurrentCurrency($alias)
	{
		$currency = \Catalog\Model\Orm\Currency::loadByWhere ( array('site_id' => \RS\Site\Manager::getSiteId (),'title' => $alias 
		) );
	...
	{
		static $update_price_round, $update_price_round_value;
	...
	{
		return \RS\Orm\Request::make ()->from ( new Orm\Currency () )->where ( array('title' => $uid 
		) )->object ();
	}
	...
	function getCBRFCourseWithUpdate($cur_site = true)
	{
		...
		$q = \RS\Orm\Request::make ()->from ( new \Catalog\Model\Orm\Currency () )->where ( array('is_base' => 0 
		) );
		if ($cur_site === true) { // Если для текущего сайта
			$q->where ( array('site_id' => \RS\Site\Manager::getSiteId () 
			) );
		}
		if ($cur_site > 0 && ! is_bool ( $cur_site )) { // Если для конкретного сайта
			$q->where ( array('site_id' => $cur_site 
			) );
		}
