...
use \Catalog\Model\Orm\Xdir, \Catalog\Model\Orm\Xcost;
...
{
	protected $work_list, $work_list_ids;
	function __construct()
	{
		parent::__construct ( new \Catalog\Model\Orm\Product (), array('multisite' => true,'aliasField' => 'alias','defaultOrder' => 'dateof DESC' 
		) );
	}
	...
	function addCostQuery(array $cost_types)
	{
		foreach($cost_types as $cost_type) {
			...
			$this->queryObj ()->select ( "$alias.cost_val as cost_" . $cost_type['id'] )->leftjoin ( new \Catalog\Model\Orm\Xcost (), "$alias.product_id=A.id AND $alias.cost_id='{$cost_type['id']}'", $alias );
		}
	}
	...
	public function addProductsCost($products)
	{
		...
		if (empty ( $products_id )) return $products;
		
		$res = \RS\Orm\Request::make ()->from ( new Xcost () )->whereIn ( 'product_id', $products_id )->exec ();
	...
	public function addProductsFiles($products)
	{
		...
		$product_files = \RS\Orm\Request::make ()->from ( new \Files\Model\Orm\Linkfiles () )->where ( array('type' => 'catalog' 
		) )->whereIn ( 'linkid', $products_id )->exec ()->fetchSelected ( 'linkid', 'uniq', true );
	...
	public function addProductsPhotos($products)
	{
		...
		$product_photos = \RS\Orm\Request::make ()->from ( new \Photo\Model\Orm\Image () )->where ( array('type' => 'catalog' 
		) )->whereIn ( 'linkid', $products_id )->orderby ( 'sortn' )->objects ( null, 'linkid', true );
	...
	function genereteBarcode()
	{
		...
		$q = \RS\Orm\Request::make ()->select ( 'max(id) as id' )->from ( new \Catalog\Model\Orm\Product () )->exec ();
	...
	public function getMultiDirCount()
	{
		...
		return $q->groupby ( false )->orderby ( false )->exec ()->getOneField ( 'cnt', 0 );
	}
	...
	/**
	 * Применяет "базовые" фильтры, т.е.
	 * фильтры, которые не зависят от таблиц характеристик товара.
	 */
	public function applyBaseFilters($bfilters = null)
	{
		...
		foreach($keys as $n => $key) {
			...
			$func = $key . 'Filter';
			if (method_exists ( $this, $func )) $this->$func ( $bfilters[$key] );
		}
		return true;
	}
	...
	protected function costFilter($filter)
	{
		...
		$q->leftjoin ( new Xcost (), "A.id = XC.product_id AND XC.cost_id='{$current_cost_type}'", 'XC' )->orderby ( 'cost_val' );
	...
	public function addProductsDirs($products)
	{
		...
		$res = \RS\Orm\Request::make ()->select ( '*' )->from ( new Xdir () )->whereIn ( 'product_id', $products_id )->exec ()->fetchAll ();
	...
	public function addProductsOffers($products)
	{
		...
		$offers = \RS\Orm\Request::make ()->from ( new Orm\Offer () )->whereIn ( 'product_id', $products_id )->orderby ( 'product_id, sortn' )->objects ( null, 'product_id', true );
		
		// Раскладываем по товарам
		foreach($products as $item) {
			$one_offers = array('use' => 1,'items' => array() 
			);
	...
	public function addProductsMultiOffers($products)
	{
		...
		$levels_res = \RS\Orm\Request::make ()->select ( 'I.title as prop_title, I.values as prop_values, Level.*' )->from ( new Orm\MultiOfferLevel (), 'Level' )->join ( new Orm\Property\Item (), 'Level.prop_id = I.id', 'I' )->where ( array('I.type' => 'list','I.site_id' => \RS\Site\Manager::getSiteId () 
		) )->whereIn ( 'Level.product_id', $products_id )->orderby ( 'product_id, Level.sortn' )->exec ();
		...
		if ($props_id) {
			$values_res = \RS\Orm\Request::make ()->from ( new Orm\Property\Link () )->whereIn ( 'product_id', $products_id )->whereIn ( 'prop_id', $props_id )->orderby ( 'val_str' )->exec ();
	 ...
	 * комплектации без загрузки подробных сведений.
	 * Данный метод удобно использовать на странице со списком товаров
	 * 
	 * @param array $products список товаров
	 * @return array
	 */
	public function addProductsMultiOffersInfo($products)
	{
		...
		$use_multioffers = \RS\Orm\Request::make ()->select ( 'DISTINCT product_id' )->from ( new Orm\MultiOfferLevel () )->whereIn ( 'product_id', $products_id )->exec ()->fetchSelected ( 'product_id', 'product_id' );
		
		$use_offers = \RS\Orm\Request::make ()->select ( 'product_id, COUNT(*) as cnt' )->from ( new Orm\Offer () )->whereIn ( 'product_id', $products_id )->groupby ( 'product_id' )->exec ()->fetchSelected ( 'product_id', 'cnt' );
	...
	public function getProductsDirs($products_id, $addroot = false, $is_products = true)
	{
		...
		$list = \RS\Orm\Request::make ()->from ( new Xdir () )->whereIn ( 'product_id', $products_id )->exec ();
	...
	function multiDelete($ids, $dir = 0)
	{
		...
		// Удаляем связи
		$q = \RS\Orm\Request::make ()->delete ()->from ( new Xdir () )->whereIn ( 'product_id', $ids );
		...
		if ($dir > 0) {
			...
			$res = \RS\Orm\Request::make ()->from ( new Xdir () )->whereIn ( "product_id", $ids )->exec ();
		...
		if (! empty ( $del_ids )) {
			...
			$photo_ids = \RS\Orm\Request::make ()->select ( 'id' )->from ( $photo_api->getElement () )->where ( array('type' => 'catalog' 
			) )->where ( "linkid IN ($del_ids_in)" )->exec ()->fetchSelected ( null, 'id' );
			
			$photo_api->multiDelete ( $photo_ids ); // Удаляем связанные фото
			                                     // Удаляем сами товары
			$product_table = $this->obj_instance->_getTable ();
			\RS\Orm\Request::make ()->delete ()->from ( $this->obj_instance )->where ( "id IN($del_ids_in)" )->exec ();
			
			// Удаляем цены
			\RS\Orm\Request::make ()->delete ()->from ( new Xcost () )->where ( "product_id IN($del_ids_in)" )->exec ();
			
			// Удаляем товарные предложения (комплектации)
			\RS\Orm\Request::make ()->delete ()->from ( new \Catalog\Model\Orm\Offer () )->where ( "product_id IN($del_ids_in)" )->exec ();
			
			// Удаляем уровни многомерных комплектаций
			\RS\Orm\Request::make ()->delete ()->from ( new \Catalog\Model\Orm\MultiOfferLevel () )->where ( "product_id IN($del_ids_in)" )->exec ();
			
			// Удаляем характеристики
			\RS\Orm\Request::make ()->delete ()->from ( new \Catalog\Model\Orm\Property\Link () )->where ( "product_id IN($del_ids_in)" )->exec ();
			
			// Удаляем из поискового индекса
			\RS\Orm\Request::make ()->delete ()->from ( new \Search\Model\Orm\Index () )->where ( "entity_id IN($del_ids_in)" )->where ( array('result_class' => get_class ( $this->obj_instance ) 
			) )->exec ();
			
			// Удаляем остатки на складах
			\RS\Orm\Request::make ()->delete ()->from ( new \Catalog\Model\Orm\Xstock () )->where ( "product_id IN($del_ids_in)" )->exec ();
		}
		
		\Catalog\Model\Dirapi::updateCounts (); // Обновляем счетчики у категорий
		                                       // Добавим событие на удалении
		\RS\Event\Manager::fire ( 'orm.multidelete.catalog-product', array('ids' => $del_ids 
		) );
		return true;
	}
		...
		foreach($ids as $id) {
			foreach($excost as $cost_id => $cdata) {
				...
				if (isset ( $cdata['edit_multi'] )) {
					...
					$product_before_cost = isset ( $all_prices[$cost_id][$id] ) ? $all_prices[$cost_id][$id] : array('cost_original_currency' => 0,'cost_original_val' => 0 
					);
	...
	private function multiUpdateProductOffers($ids, $excost, $all_prices, $update_price_round = false, $update_price_round_value = 0)
	{
		...
		$pricedata_offers = ( array ) \RS\Orm\Request::make ()->from ( new \Catalog\Model\Orm\Offer () )->whereIn ( 'product_id', $ids )->where ( 'sortn > 0' )->exec ()->fetchSelected ( 'id' );
		if (empty ( $pricedata_offers )) {
			return;
		}
		
		// Получим валюты в системе
		$currencies = \RS\Orm\Request::make ()->from ( new \Catalog\Model\Orm\Currency () )->where ( array('public' => 1 
		) )->orderby ( '`default` DESC' )->objects ( null, 'id' );
		
		$currency_keys = array_keys ( $currencies );
		$currency_default_id = $currency_keys[0]; // id валюты по умолчанию
		
		static $prices; // Все сущесвующие цены в системе
		...
		foreach($pricedata_offers as $offer_id => $offer_price_info) {
			...
			if (isset ( $pricedata['oneprice'] ) && ($pricedata['oneprice']['znak'] == "=") && $pricedata['oneprice']['original_value'] > 0) {
				if ($prices === null) {
					$prices = \RS\Orm\Request::make ()->from ( new \Catalog\Model\Orm\Typecost () )->where ( array('site_id' => \RS\Site\Manager::getSiteId (),'type' => 'manual' 
					) )->objects ();
				}
				...
				foreach($prices as $price) {
					$pricedata['price'][$price['id']] = array('znak' => '=','unit' => $pricedata['oneprice']['unit'],'original_value' => $pricedata['oneprice']['original_value'],'value' => $pricedata['oneprice']['value'] 
					);
				}
			...
			foreach($excost as $cost_id => $cdata) {
				...
				if (isset ( $cdata['edit_multi'] ) && $cdata['edit_multi']) {
					...
					if (! isset ( $pricedata['price'][$from_price]['znak'] )) {
						$pricedata['price'][$from_price] = array('znak' => "=",'unit' => $all_prices[$cost_id][$product_id]['cost_original_currency'],'cost_original_currency' => "",'cost_original_val' => "" 
						);
					}
					
					// Если цены для которой считаем не существует
					if (! isset ( $pricedata['price'][$cost_id]['znak'] )) {
						$pricedata['price'][$cost_id] = array('znak' => "=",'unit' => $all_prices[$cost_id][$product_id]['cost_original_currency'],'cost_original_currency' => "",'cost_original_val' => "" 
						);
					}
					...
					if (($pricedata['price'][$cost_id]['znak'] == "=") && ($pricedata['price'][$from_price]['unit'] != "%") && ($pricedata['price'][$from_price]['original_value'] > 0)) {
						...
						$pricedata['price'][$cost_id] = array('znak' => '=','unit' => $currency_id,'original_value' => $original_cost,'value' => $cost 
						);
					}
			...
			\RS\Orm\Request::make ()->update ()->from ( new \Catalog\Model\Orm\Offer () )->set ( array('pricedata' => serialize ( ( array ) $pricedata ) 
			) )->where ( array('id' => $offer_id,'site_id' => \RS\Site\Manager::getSiteId () 
			) )->exec ();
		}
	}
	...
	function multiUpdateProductsAndOffersPrices($ids = array(), array $excost)
	{
		...
		foreach($excost as $cost_id => $cdata) {
			...
			if (isset ( $cdata['edit_multi'] ) && trim ( $cdata['plus_value'] ) !== "") {
				...
				$all_prices[$cost_id] = \RS\Orm\Request::make ()->from ( new Xcost () )->where ( array('cost_id' => $cdata['from_price'] 
		...
		if (! empty ( $replace_price )) { // Какие точно удалять, чтобы заменить
		                             // Удалим старые цены
			$q = \RS\Orm\Request::make ()->delete ()->from ( new Xcost () )->whereIn ( 'cost_id', $replace_price )->whereIn ( 'product_id', $ids )->exec ();
		}
	...
	function multiUpdate(array $data, $ids = array())
	{
		...
		$merged_xdir = array();
		$need_reindex = array_intersect ( array('title','_property_','barcode','short_description','meta_keywords' 
		...
		if (isset ( $data['_property_'] )) {
			...
			if (isset ( $data['_property_'][0] )) {
				$q = \RS\Orm\Request::make ()->delete ()->from ( new \Catalog\Model\Orm\Property\Link () )->where ( array('site_id' => \RS\Site\Manager::getSiteId () 
				) )->whereIn ( 'product_id', $ids )->exec ();
				unset ( $data['_property_'][0] );
			} else {
				// Удалим предыдущие значения этих характеристик
				foreach($data['_property_'] as $k => $property) {
					$q = \RS\Orm\Request::make ()->delete ()->from ( new \Catalog\Model\Orm\Property\Link () )->where ( array('prop_id' => $property['id'],'site_id' => $property['site_id'] 
					) )->whereIn ( 'product_id', $ids )->exec ();
		...
		if (isset ( $data['recommended_arr'] ) && isset ( $data['recommended_arr']['product'] )) {
			...
			\RS\Orm\Request::make ()->from ( new \Catalog\Model\Orm\Product () )->set ( array('recommended' => $recomended 
			) )->whereIn ( 'id', $ids )->where ( array('site_id' => \RS\Site\Manager::getSiteId () 
			) )->update ()->exec ();
		...
		if (isset ( $data['concomitant_arr'] ) && isset ( $data['concomitant_arr']['product'] )) {
			...
			\RS\Orm\Request::make ()->from ( new \Catalog\Model\Orm\Product () )->set ( array('concomitant' => $concomitant 
			) )->whereIn ( 'id', $ids )->where ( array('site_id' => \RS\Site\Manager::getSiteId () 
			) )->update ()->exec ();
		...
		if (isset ( $data['num'] )) {
			...
			$offers = \RS\Orm\Request::make ()->from ( new \Catalog\Model\Orm\Offer () )->where ( array('site_id' => \RS\Site\Manager::getSiteId (),'sortn' => 0 
			) )->whereIn ( 'product_id', $ids )->exec ()->fetchSelected ( 'product_id', 'id' );
			...
			foreach($data['num'] as $warehouse_id => $num) {
				if ($num !== '') {
					...
					if ($offers) {
						\RS\Orm\Request::make ()->from ( $stockClass )->whereIn ( 'offer_id', $offers )->where ( array('warehouse_id' => $warehouse_id 
						) )->delete ()->exec ();
					}
			...
			$offer = new \Catalog\Model\Orm\Offer ();
			\RS\Orm\Request::make ()->update ()->from ( $offer, 'O' )->set ( 'O.num = (SELECT SUM(stock) FROM ' . $stockClass->_getTable () . ' S WHERE S.offer_id = O.id)' )->whereIn ( 'O.product_id', $ids )->exec ();
		...
		if ($need_calculate_product_num) {
			...
			$sub_query = \RS\Orm\Request::make ()->select ( 'SUM(num)' )->from ( new Orm\Offer (), 'O' )->where ( 'O.product_id = P.id' );
			
			\RS\Orm\Request::make ()->update ()->from ( new \Catalog\Model\Orm\Product (), 'P' )->set ( 'P.num = (' . $sub_query . ')' )->whereIn ( 'P.id', $ids )->exec ();
		}
		...
		\RS\Event\Manager::fire ( 'orm.multiupdate.catalog-product', array('ids' => $ids 
		) );
		return $ret;
	}
	...
	function updateProductSearchIndex($ids)
	{
		...
		$q = \RS\Orm\Request::make ()->from ( $this->obj_instance )->whereIn ( 'id', $ids )->limit ( $page_size );
	...
	function filterRequest()
	{
		...
		\RS\Orm\Request::make ()->join ( $this->obj_instance, 'P.id = A.entity_id', 'P' )->where ( 'public = 1' );
	...
	function getDirList()
	{
		...
		$q->join ( new Orm\Dir (), 'A.maindir = DL.id', 'DL' )->where ( 'DL.public = 1' )->orderby ( null )->groupby ( 'DL.id' );
	...
	function getSameByCostProducts($product, $delta, $page_size)
	{
		...
		$this->costFilter ( array('from' => $cost - $cost * ($delta / 100),'to' => $cost + $cost * ($delta / 100) 
		) );
	...
	{
		if ($cache) {
			return \RS\Cache\Manager::obj ()->tags ( CACHE_TAG_UPDATE_CATEGORY )->request ( array($this,'getAllowableBrandsValues' 
			), $cache_key, false );
		} else {
			$q = clone $this->queryObj ();
			$q->select = 'Brand.*';
			$q->join ( new \Catalog\Model\Orm\Brand (), 'A.brand_id=Brand.id', 'Brand' )->groupby ( 'A.brand_id' )->where ( array('A.public' => 1 
	...
	{
		if ($cache) {
			return \RS\Cache\Manager::obj ()->tags ( CACHE_TAG_UPDATE_CATEGORY )->request ( array($this,'getAllowablePropertyValues' 
			), $dir_id, $cache_key, false );
		} else {
			...
			if ($public_prop_ids) {
				...
				$q->join ( new \Catalog\Model\Orm\Property\Link (), "ALP.product_id = A.id", "ALP" )->groupby ( null )->where ( 'A.public = 1' )->whereIn ( 'ALP.prop_id', $public_prop_ids );
							...
							case 'int' :
								{
									if (! isset ( $min_max[$prop_id] )) {
										$min_max[$prop_id] = array('interval_from' => $value,'interval_to' => $value,'step' => 1 
										);
									} else {
	...
	/**
	 * Перемещает элемент from на место элемента to.
	 * Если flag = 'up', то до элемента to, иначе после
	 * 
	...
	function moveProduct($dir_id, $from, $to, $flag)
	{
		...
		$from_obj = \RS\Orm\Request::make ()->from ( $this->obj_instance, 'P' )->join ( $xdir, 'X.product_id=P.id AND X.dir_id=' . $dir_id, 'X' )->where ( array('id' => $from 
		) )->object ();
		
		$to_obj = \RS\Orm\Request::make ()->from ( $this->obj_instance, 'P' )->join ( $xdir, 'X.product_id=P.id AND X.dir_id=' . $dir_id, 'X' )->where ( array('id' => $to 
		) )->object ();
		...
		$res = \RS\Orm\Request::make ()->from ( $xdir )->where ( array('dir_id' => $dir_id 
		) )->where ( $filter )->orderby ( $this->sort_field );
		$res = $res->exec ();
		...
		if ($is_up)
			$list = $this->moveArrayUp ( $list );
		else $list = $this->moveArrayDown ( $list );
		
		foreach($list as $newValues) {
			\RS\Orm\Request::make ()->update ( $xdir )->set ( array($this->sort_field => $newValues[$this->sort_field] 
			) )->where ( array('dir_id' => $newValues['dir_id'],'product_id' => $newValues['product_id'] 
			) )->exec ();
		}
		return true;
	}
	...
	function moveProductInDir($dir_id, $product_id, $flag)
	{
		...
		$q = \RS\Orm\Request::make ()->from ( new \Catalog\Model\Orm\Xdir () )->where ( array('dir_id' => $dir_id 
		) )->limit ( 1 );
