...
/**
 * Комплектация товара.
 * (или товарное предложение)
 */
class Offer extends \RS\Orm\OrmObject
{
	protected static $table = "product_offer";
	public $first_sortn = 0; // Сортировочный индекс, который следует присваивать первой добавляемой комплектации
	function _init()
	{
		parent::_init ()->append ( array('site_id' => new Type\CurrentSite (),'product_id' => new Type\Integer ( array('description' => t ( 'ID товара' ),'index' => true,'visible' => false 
		) ),'title' => new Type\String ( array('description' => t ( 'Название' ),'maxLength' => 300,'mainVisible' => false 
		) ),'barcode' => new Type\String ( array('description' => t ( 'Артикул' ),'maxLength' => 50,'allowEmpty' => false,'mainVisible' => false 
		) ),'pricedata' => new Type\TinyText ( array('description' => t ( 'Цена (сериализован)' ),'visible' => false 
		) ),'pricedata_arr' => new Type\ArrayList ( array('description' => t ( 'Цена' ),'template' => '%catalog%/form/offer/price_data.tpl','mainVisible' => false 
		) ),'propsdata' => new Type\TinyText ( array('description' => t ( 'Характеристики комплектации (сериализован)' ),'visible' => false 
		) ),'propsdata_arr' => new Type\ArrayList ( array('description' => t ( 'Характеристики комплектации' ),'visible' => false 
		) ),'_propsdata' => new Type\ArrayList ( array('description' => t ( 'Характеристики комплектации' ),'template' => '%catalog%/form/offer/props_data.tpl','mainVisible' => false 
		) ),'num' => new Type\Integer ( array('maxLength' => 11,'allowEmpty' => false,'default' => 0,'description' => t ( 'Остаток на складе' ),'visible' => false 
		) ),'stock_num' => new Type\Mixed ( array('description' => t ( 'Остатки на складах' ),'template' => '%catalog%/form/offer/stock_num.tpl','visible' => true,'mainVisible' => false 
		) ),'photos' => new Type\String ( array('maxLength' => 1000,'description' => t ( 'Фотографии комплектаций' ),'visible' => false 
		) ),'photos_arr' => new Type\ArrayList ( array('description' => t ( 'Связанные фото' ),'template' => '%catalog%/form/offer/photos.tpl','mainVisible' => false 
		) ),'sortn' => new Type\Integer ( array('description' => t ( 'Порядковый номер' ),'visible' => false 
		) ),'unit' => new Type\Integer ( array('description' => t ( 'Единица измерения' ),'default' => 0,'List' => array(array('\Catalog\Model\UnitApi','selectList' 
		) 
		) 
		) ),'position' => new Type\String ( array('description' => t ( '' ),'runtime' => true,'template' => '%catalog%/form/offer/position.tpl','visible' => false 
		) ),'processed' => new Type\Integer ( array('description' => t ( 'Флаг обработанной во время импорта комплектации' ),'maxLength' => '2','visible' => false 
		) ),'xml_id' => new Type\String ( array('maxLength' => '255','description' => t ( 'Идентификатор товара в системе 1C' ),'visible' => false 
		) ) 
		) );
	...
	function beforeWrite($flag)
	{
		...
		if (isset ( $this['photos_named_arr'] ) && count ( $this['photos_named_arr'] )) {
			...
			foreach($this['photos_named_arr'] as $filename) { // Переберём и найдём истенные id-шники
				...
				$photo = \RS\Orm\Request::make ()->from ( new \Photo\Model\Orm\Image () )->where ( array('site_id' => \RS\Site\Manager::getSiteId (),'type' => 'catalog','filename' => trim ( $filename ),'linkid' => $this['product_id'] 
				) )->object ();
				if ($photo) {
					$arr[] = $photo['id'];
				}
		...
		if ($flag != self::UPDATE_FLAG && ! $this->isModified ( 'sortn' )) {
			$q = \RS\Orm\Request::make ()->select ( 'MAX(sortn)+1 as next_sort' )->from ( $this )->where ( array('product_id' => ( int ) $this['product_id'] 
			) );
	...
	function afterWrite($flag)
	{
		...
		if ($this->isModified ( 'stock_num' )) {
			...
			\RS\Orm\Request::make ()->delete ()->from ( new \Catalog\Model\Orm\Xstock () )->where ( array('offer_id' => $this['id'],'product_id' => $this['product_id'] 
			) )->exec ();
			...
			foreach($this['stock_num'] as $warehouse_id => $stock_num) {
				...
				$offer_stock->insert ( false, array('stock' 
				), array('product_id','offer_id','warehouse_id' 
				) );
			}
		}
	}
	...
	{
		$this['stock_num'] = \RS\Orm\Request::make ()->from ( new \Catalog\Model\Orm\Xstock () )->where ( array('product_id' => $this['product_id'],'offer_id' => $this['id'] 
	 ...
	 * 
	 * @param string $sign - символ склейки
	 */
	...
	{
		$next = \RS\Orm\Request::make ()->from ( $this )->where ( array('product_id' => $this['product_id'] 
		) )->count () + 1;
