...
/**
 * Класс ORM-объектов "Добавить в 1 клик".
 * Объект добавить в 1 клик
...
'title' => new Type\String ( array('maxLength' => '150','description' => t ( 'Номер сообщения' ) 
		) ),'dateof' => new Type\Datetime ( array('maxLength' => '150','description' => t ( 'Дата отправки' ) 
		) ),'status' => new Type\Enum ( array('new','viewed' 
		), array('maxLength' => '1','allowEmpty' => false,'default' => 'new','listFromArray' => array(array('new' => 'Новое','viewed' => 'Закрыт' 
		) 
		),'description' => t ( 'Статус' ) 
		) ),'ip' => new Type\String ( array('maxLength' => '150','description' => t ( 'IP Пользователя' ) 
		) ),'stext' => new Type\Text ( array('description' => t ( 'Содержимое результата формы' ),'Template' => 'form/field/stext.tpl' 
		) ) 
		) );
	}
