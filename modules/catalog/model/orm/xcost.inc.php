...
{
	protected static $table = "product_x_cost", $cache_currency = array();
	function _init()
	{
		$this->getPropertyIterator ()->append ( array('product_id' => new Type\Integer ( array('description' => t ( 'ID товара' ) 
		) ),'cost_id' => new Type\Integer ( array('description' => t ( 'ID цены' ) 
		) ),'cost_val' => new Type\Decimal ( array('description' => t ( 'Рассчитанная цена в базовой валюте' ),'allowempty' => false,'maxlength' => 20,'decimal' => 2 
		) ),'cost_original_val' => new Type\Decimal ( array('description' => t ( 'Оригинальная цена товара' ),'allowempty' => false,'maxlength' => 20,'decimal' => 2 
		) ),'cost_original_currency' => new Type\Integer ( array('description' => t ( 'ID валюты оригинальной цены товара' ),'allowempty' => false,'default' => 0 
		) ) 
		) );
		
		$this->addIndex ( array('product_id','cost_id' 
		), self::INDEX_UNIQUE );
	}
	...
	function fillData($cost_id, $product_id, $data)
	{
		...
		if (is_array ( $data )) {
			$data += array('cost_original_val' => 0,'cost_original_currency' => 0 
			);
