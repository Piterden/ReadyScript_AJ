...
{
	protected $line_delimiter = ";", $link_preset_id, $link_foreign_field, $link_foreign_named_field = "photos_named_arr", $link_orm_type, $title;
	function loadData()
	{
	...
	function getColumns()
	{
		return array($this->id . '-photos_arr' => array('key' => 'photos_arr','title' => $this->title 
		) 
		);
	}
	...
	function getColumnsData($n)
	{
		...
		foreach($this->getColumns () as $id => $column) {
			...
			if (count ( ( array ) $photos_ids )) { // Если есть назначенные фото
				$photos = \RS\Orm\Request::make ()->from ( new \Photo\Model\Orm\Image () )->where ( array('site_id' => \RS\Site\Manager::getSiteId () 
				) )->whereIn ( 'id', $photos_ids )->objects ( null, 'id' );
