...
{
	protected static $props, $groups, $index;
	protected $delimiter = ";\n", $value_delimiter = "|", $id_field = 'id', $link_id_field, $link_preset_id, $mask, $mask_fields = array(), $title, $array_field = 'multioffers', $manylink_foreign_id_field = 'prop_id', $manylink_id_field = 'product_id', $manylink_orm;
	function __construct($options)
	{
		$defaults = array('ormObject' => new \Catalog\Model\Orm\MultiOfferLevel (),'propertyOrm' => new \Catalog\Model\Orm\Property\Item (),'manylinkOrm' => new \Catalog\Model\Orm\Property\Link (),'mask' => '{title}:({group_name}){property}','multisite' => true 
		);
	...
	{
		return $this->is_multisite ? array('site_id' => \RS\Site\Manager::getSiteId () 
		) : array();
	}
	function loadProperty()
	{
		if (! isset ( self::$props )) {
			self::$props = \RS\Orm\Request::make ()->from ( new \Catalog\Model\Orm\Property\Item () )->where ( $this->getMultisiteExpr () ?  : null )->objects ( null, 'id' );
			
			self::$groups = \RS\Orm\Request::make ()->from ( new \Catalog\Model\Orm\Property\Dir () )->where ( $this->getMultisiteExpr () ?  : null )->objects ( null, 'id' );
	...
	function loadData()
	{
		...
		if ($this->schema->ids) {
			...
			$this->rows = \RS\Orm\Request::make ()->from ( new \Catalog\Model\Orm\MultiOfferLevel () )->whereIn ( $this->manylink_id_field, $this->schema->ids )->objects ( null, $this->manylink_id_field, true );
		}
	}
	function getColumns()
	{
		return array($this->id . '-multiooffers' => array('key' => 'multioffer','title' => $this->title 
		) 
		);
	}
	...
	function importColumnsData()
	{
		if (isset ( $this->row['multioffer'] )) {
			...
			$result_array = array();
			if (! empty ( $items )) {
				$result_array = array('use' => 1,'levels' => array() 
				);
				...
				foreach($items as $item) {
					...
					if (preg_match ( $this->mask_pattern, $item, $match )) {
						...
						$result_array['levels'][] = array('title' => $match['title'],'prop' => self::$index[$index_name] 
						);
					}
				}
			}
