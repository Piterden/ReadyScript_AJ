...
class ImportYmlApi extends \RS\Module\AbstractModel\BaseModel
{
	...
	public $storage;
	protected $site_id, $yml_api, $timeout, $allow_ext = array('yml','xml' 
	), $tmp_data_file = 'data.tmp', $log_name = 'yml_import.log', $log_file_rel, $log_file, $yml_name = 'tmp.yml', $yml_folder, $yml_folder_rel = '/storage/tmp/importyml', $start_time, $params_fields = array('delivery' => 'Доставка','local_deliver_cost' => 'Стоимость доставки','typePrefix' => 'Тип','vendorCode' => 'Код производителя','model' => 'Модель','manufacturer_warranty' => 'Гарантия производителя','country_of_origin' => 'Страна-производитель','author' => 'Автор','publisher' => 'Издатель','series' => 'Серия','year' => 'Год','ISBN' => 'ISBN','volume' => 'Объем','part' => 'Часть','language' => 'Язык','binding' => 'Переплет','page_extent' => 'Количество страниц','downloadable' => 'Возможность скачать','performed_by' => 'Исполнитель','performance_type' => 'Тип представления','storage' => 'Носитель','format' => 'Формат','recording_length' => 'Длительность записи','artist' => 'Артист','media' => 'Носитель медиа','starring' => 'В главных ролях','country' => 'Страна','director' => 'Директор','originalName' => 'Оригинальное название','worldRegion' => 'Регион мира','region' => 'Регион','days' => 'Дни','title' => 'Название','dataTour' => 'Дата тура','hotel_stars' => 'Количество звезд','room' => 'Комнаты','meal' => 'Питание','included' => 'Включено','transport' => 'Транспорт','place' => 'Место проведения','hall' => 'Зал','hall_part' => 'Расположение','date' => 'Дата','is_premiere' => 'Премьера','is_kids' => 'Для детей','adult' => 'Для взрослых','pickup' => 'Самовывоз','store' => 'Точка продаж','sales_notes' => 'Ценовые предложения' 
	);
	...
	function process($step_data)
	{
		...
		if (is_array ( $result )) {
			...
			return $result + array('upload_images' => $step_data['upload_images'],'step' => $step_data['step'] 
			);
		} elseif ($result) {
			...
			if ($step_data['step'] == count ( $steps ) - 1) {
				...
				return array('upload_images' => $step_data['upload_images'],'step' => $next_step,'offset' => 0 
				) + (isset ( $steps[$next_step]['make_callback'] ) ? call_user_func ( $steps[$next_step]['make_callback'] ) : array());
			}
		}
	...
	{
		$_this = $this;
		$steps = array(array('title' => 'Подготовка к импорту','successTitle' => 'Подготовка к импорту завершена','method' => 'stepStart' 
		),array('title' => 'Импорт валют','successTitle' => 'Валюты импортированы','method' => 'stepCurrency' 
		),array('title' => 'Импорт категорий','successTitle' => 'Категории импортированы','method' => 'stepCategory','make_callback' => function () use($_this) {
			return array('total' => $_this->storage['yml_info']['category_count'] 
			);
		} 
		),array('title' => 'Импорт товаров','successTitle' => 'Товары импортированы','method' => 'stepProduct','make_callback' => function () use($_this) {
			return array('total' => count ( $_this->storage['yml_info']['products_xml_ids'] ) 
			);
		} 
		),array('title' => 'Импорт рекомендуемых товаров','successTitle' => 'Рекомендуемые товары импортированы','method' => 'stepRecommended','make_callback' => function () use($_this) {
			return array('total' => count ( $_this->storage['yml_info']['products_xml_ids'] ) 
			);
		} 
		) 
		...
		if (! empty ( $step_data['upload_images'] )) {
			$steps = array_merge ( $steps, array(array('title' => 'Импорт изображений','successTitle' => 'Изображения импортированы','method' => 'stepImages','make_callback' => function () use($_this) {
				return array('total' => $_this->storage['yml_info']['photo_count'] 
				);
			} 
			) 
			) );
	...
	private function stepStart($step_data)
	{
		...
		$this->saveLocalKey ( array('statistic' => array('inserted_categories' => 0,'updated_categories' => 0,'inserted_offers' => 0,'updated_offers' => 0,'inserted_photos' => 0,'already_exists_photos' => 0,'not_downloaded_photos' => 0 
		) 
		) );
	...
	private function stepCurrency($step_data)
	{
		...
		foreach($currencies as $curr_xml) {
			$currency_data = array('site_id' => $this->site_id,'title' => ( string ) $curr_xml->attributes ()->id 
			);
	...
	{
		// id категорий
		$exists_dir = \RS\Orm\Request::make ()->from ( new \Catalog\Model\Orm\Dir () )->where ( array('site_id' => $this->site_id 
		) )->where ( 'xml_id LIKE \'' . self::YML_ID_PREFIX . '%\'' )->exec ()->fetchSelected ( 'xml_id', 'id' );
		...
		do {
			...
			$dir->insert ( true, array('alias','parent' 
			), array('name','xml_id','site_id' 
			) );
		...
		if ($timeout) {
			return array('offset' => $offset,'percent' => round ( $offset / $this->storage['yml_info']['category_count'] * 100 ) 
			);
		}
		return true;
	}
	...
	private function stepProduct($step_data)
	{
		...
		if ($timeout) {
			return array('offset' => $offset,'percent' => round ( $offset / count ( $this->storage['yml_info']['products_xml_ids'] ) * 100 ) 
			);
		}
		return true;
	}
		...
		do {
			...
			if (( string ) $offer_xml->rec) {
				...
				if (! empty ( $recommended_arr )) {
					\RS\Orm\Request::make ()->update ( $product )->set ( array('recommended' => serialize ( $recommended_arr ) 
					) )->where ( array('id' => $this->storage['list']['products'][$xml_id],'site_id' => $this->site_id 
					) )->exec ();
				}
			}
		} while ( ! ($timeout = $this->checkTimeout ()) && $reader->next ( 'offer' ) );
	...
	private function stepImages($step_data)
	{
		...
		do {
			...
			if (( bool ) $offer_xml->picture) {
				...
				foreach($offer_xml->picture as $picture) {
					...
					$image = \RS\Orm\Request::make ()->from ( new \Photo\Model\Orm\Image () )->where ( array('site_id' => $this->site_id,'extra' => $xml_id,'filename' => basename ( $picture ),'linkid' => $product_id 
					) )->object ();
		...
		if ($timeout) {
			return array('offset' => $offset,'percent' => round ( $offset / $this->storage['yml_info']['photo_count'] * 100 ) 
			);
		}
	...
	private function loadLists()
	{
		...
		// Загружаем категории
		$list['categories'] = \RS\Orm\Request::make ()->from ( new Orm\Dir () )->where ( $common_where )->exec ()->fetchSelected ( 'xml_id', 'id' );
		
		// Загружаем цены
		$list['costs'] = \RS\Orm\Request::make ()->from ( new Orm\Typecost () )->where ( $common_where )->exec ()->fetchSelected ( 'title', 'id' );
		
		// Загружаем бренды
		$list['brands'] = \RS\Orm\Request::make ()->from ( new Orm\Brand () )->where ( $common_where )->exec ()->fetchSelected ( 'title', 'id' );
		
		// Загружаем валюты
		$list['currencies'] = \RS\Orm\Request::make ()->from ( new Orm\Currency () )->where ( $common_where )->exec ()->fetchSelected ( 'title', 'id' );
	...
	private function loadProductsList()
	{
		...
		$products = \RS\Orm\Request::make ()->select ( 'xml_id, id' )->from ( new Orm\Product () )->whereIn ( 'xml_id', $xml_ids )->where ( array('site_id' => $this->site_id 
	...
	private function prepareImport()
	{
		...
		return array('products_xml_ids' => $product_xml_ids,'category_count' => $category_count,'photo_count' => $photo_count 
		);
	}
	...
	private function importProduct($reader)
	{
		...
		if (isset ( $offer_xml->price )) {
			...
			$excost_array[\Catalog\Model\CostApi::getDefaultCostId ()] = array('cost_original_val' => ( string ) $offer_xml->price,'cost_original_currency' => $this->storage['list']['currencies'][( string ) $offer_xml->currencyId] 
			);
			
			if ($offer_xml->oldprice && \Catalog\Model\CostApi::getOldCostId ()) {
				$excost_array[\Catalog\Model\CostApi::getOldCostId ()] = array('cost_original_val' => ( string ) $offer_xml->oldprice,'cost_original_currency' => $this->storage['list']['currencies'][( string ) $offer_xml->currencyId] 
				);
			}
		...
		$product['description'] = $product['short_description'] = ( string ) $offer_xml->description;
	...
	private function updateProductParams(\SimpleXMLElement $offer_xml, $product_id)
	{
		...
		\RS\Orm\Request::make ()->delete ()->from ( new \Catalog\Model\Orm\Property\Link () )->where ( array('site_id' => $this->site_id,'product_id' => $product_id 
		) )->where ( 'xml_id LIKE \'' . self::YML_ID_PREFIX . '%\'' )->exec ();
		...
		foreach($offer_xml->param as $param) {
			...
			$product_property->insert ( false, array('title','type' 
			), array('xml_id','site_id' 
			) );
			...
			$product_property_link->insert ( false, array('val_str' 
			), array('xml_id','site_id','product_id' 
			) );
		}
