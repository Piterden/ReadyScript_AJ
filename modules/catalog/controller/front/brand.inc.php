...
class Brand extends \RS\Controller\Front
{
	...
	function actionIndex()
	{
		...
		// Хлебные крошки
		$this->app->breadcrumbs->addBreadCrumb ( t ( "Производители" ), $this->router->getUrl ( 'catalog-front-allbrands' ) )->addBreadCrumb ( $brand['title'] );
		...
		$this->view->assign ( array('products' => $products, // Товары бренда в спец. категориях
'brand' => $brand, // Бренд
'dirs' => $dirs 
		) // Категории бренда
 );
