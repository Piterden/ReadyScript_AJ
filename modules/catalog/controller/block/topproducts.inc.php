...
{
	protected static $controller_title = 'Продукты из категории', $controller_description = 'Отображает нужное количество товаров из заданной категории';
	protected $default_params = array('indexTemplate' => 'blocks/topproducts/top_products.tpl', // Должен быть задан у наследника
'pageSize' => 15 
	), $page;
	public $dirapi, $api;
	function getParamObject()
	{
		return parent::getParamObject ()->appendProperty ( array('pageSize' => new Orm\Type\Integer ( array('description' => t ( 'Количество элементов на страницу' ) 
		) ),'dirs' => new Orm\Type\String ( array('description' => t ( 'Категория, из которой выводить товары' ),'list' => array(array('\Catalog\Model\DirApi','selectList' 
		) 
		) 
		...
		) ),'only_in_stock' => new Orm\Type\Integer ( array('default' => 0,'description' => t ( 'Показывать только те, что в наличии?' ),'CheckboxView' => array(1,0 
		) 
		) ) 
		) );
	}
	...
	function actionIndex()
	{
		...
		if (! $this->view->cacheOn ( $cache_id )->isCached ( $this->getParam ( 'indexTemplate' ) )) {
			...
			if ($debug_group = $this->getDebugGroup ()) {
				$create_href = $this->router->getAdminUrl ( 'add', array('dir' => $dir_ids[0] 
				), 'catalog-ctrl' );
			...
			if (! empty ( $dir_ids )) {
				...
				$this->view->assign ( array('dir' => new \Catalog\Model\Orm\Dir ( $dir_ids[0] ),'paginator' => $paginator,'products' => $products,'page' => $page 
				) );
			}
		}
