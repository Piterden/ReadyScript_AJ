...
{
	protected $id, $dirapi, $productapi, $action_var = 'do';
	function init()
	{
	...
	function actionIndex()
	{
		...
		$this->view->assign ( array('treeList' => $tree_list,'products' => $this->actionGetProducts ( 0 ),'hideGroupCheckbox' => $this->url->request ( 'hideGroupCheckbox', TYPE_INTEGER, 0 ) 
		) );
		return $this->result->setHtml ( $this->view->fetch ( 'dialog/wrapper.tpl' ) )->getOutput ();
	}
	function actionGetChildCategory()
	{
		$dirlist = $this->dirapi->getTreeList ( $this->id );
		$this->view->assign ( array('hideGroupCheckbox' => $this->url->request ( 'hideGroupCheckbox', TYPE_INTEGER, 0 ),'dirlist' => $dirlist 
		) );
		return $this->result->setHtml ( $this->view->fetch ( 'dialog/tree_branch.tpl' ) )->getOutput ();
	}
	function actionGetProducts($category = null)
	{
		...
		foreach($filter as $key => $val)
			if ($val != '') {
				switch ($key) {
					case 'id' :
						$this->productapi->setFilter ( $key, ( int ) trim ( $val ) );
						break;
					case 'title' :
						$this->productapi->setFilter ( $key, ( string ) trim ( $val ), '%like%' );
						break;
					case 'barcode' :
						$this->productapi->setFilter ( $key, trim ( ( string ) $val ), '%like%' );
				}
			}
		...
		$this->view->assign ( array('list' => $list,'hideProductCheckbox' => $this->url->request ( 'hideProductCheckbox', TYPE_INTEGER, 0 ),'products_dirs' => $products_dirs,'costtypes' => $costApi->getList (),'paginator' => array('total' => $total,'totalPages' => ceil ( $total / $pageSize ),'page' => $page,'pageSize' => $pageSize 
		) 
		) );
		...
		if ($category !== null) {
			...
			return $this->result->setHtml ( $this->view->fetch ( 'dialog/products.tpl' ) )->getOutput ();
		}
	}
}
