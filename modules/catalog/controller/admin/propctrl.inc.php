...
{
	protected $form_tpl = 'form_prop.tpl', $me_form_tpl = 'me_prop.tpl', $action_var = 'do', $dir_api, $dir, $api;
	...
	function helperIndex()
	{
		...
		$helper->setTopToolbar ( $this->buttons ( array('add' 
		), array('add' => t ( 'добавить характеристику' ) 
		) ) );
		...
		$helper->setTable ( new Table\Element ( array('Columns' => array(new TableType\Checkbox ( 'id', array('showSelectAll' => true 
		...
		) ),new TableType\String ( 'title', t ( 'Название' ), array('LinkAttr' => array('class' => 'crud-edit' 
		),'Sortable' => SORTABLE_BOTH,'href' => $this->router->getAdminPattern ( 'edit', array(':id' => '@id','dir' => $this->dir 
		) ) 
		) ),new TableType\String ( 'unit', t ( 'Ед. изм' ) ),new TableType\String ( 'type', t ( 'Тип' ) ),new TableType\String ( 'id', '№', array('TdAttr' => array('class' => 'cell-sgray' 
		) 
		) ),new TableType\Actions ( 'id', array(new TableType\Action\Edit ( $this->router->getAdminPattern ( 'edit', array(':id' => '~field~','dir' => $this->dir 
		) ), null, array('attr' => array('@data-id' => '@id' 
		) 
		) ),new TableType\Action\DropDown ( array(array('title' => t ( 'Клонировать характеристику' ),'attr' => array('class' => 'crud-add','@href' => $this->router->getAdminPattern ( 'clone', array(':id' => '~field~' 
		) ) 
		) 
		),array('title' => t ( 'удалить' ),'class' => 'crud-get','attr' => array('@href' => $this->router->getAdminPattern ( 'del', array(':chk[]' => '@id' 
		) ) 
		) 
		) 
		) ) 
		...
		),'TableAttr' => array('data-sort-request' => $this->router->getAdminUrl ( 'move' ) 
		) 
		) ) );
		...
		if ($this->dir > 0 && $helper->getTreeViewType () == \RS\Controller\Admin\Helper\CrudCollection::VIEW_CAT_TOP) {
			...
			$path_to_first = array(array('title' => t ( 'Без группы' ),'hideInlineButtons' => array('edit','delete' 
			) 
			) 
			);
		}
		...
		$helper->setTree ( new Tree\Element ( array('sortIdField' => 'id','activeField' => 'id','disabledField' => 'hidden','disabledValue' => '1','activeValue' => $this->dir,'noExpandCollapseButton' => true,'sortable' => true,'pathToFirst' => $path_to_first,'mainColumn' => new TableType\String ( 'title', 'Название', array('href' => $this->router->getAdminPattern ( false, array(':dir' => '@id' 
		) ) 
		) ),'tools' => new TableType\Actions ( 'id', array(new TableType\Action\Edit ( $this->router->getAdminPattern ( 'edit_dir', array(':id' => '~field~' 
		) ), null, array('attr' => array('@data-id' => '@id' 
		) 
		) ),new TableType\Action\DropDown ( array(array('title' => t ( 'Клонировать категорию' ),'attr' => array('class' => 'crud-add','@href' => $this->router->getAdminPattern ( 'clonedir', array(':id' => '~field~' 
		) ) 
		) 
		) 
		) ) 
		) ),'inlineButtons' => array('add' => array('attr' => array('title' => t ( 'Создать категорию' ),'href' => $this->router->getAdminUrl ( 'add_dir' ),'class' => 'add crud-add' 
		) 
		...
		),'headButtons' => array(array('text' => t ( 'Название группы' ),'tag' => 'span','attr' => array('class' => 'lefttext' 
		) 
		),array('attr' => array('title' => 'Создать категорию','href' => $this->router->getAdminUrl ( 'add_dir' ),'class' => 'add crud-add' 
		) 
		) 
		) 
		) ), $this->dir_api );
		
		$helper->setTreeBottomToolbar ( new Toolbar\Element ( array('Items' => array(new ToolbarButton\Delete ( null, null, array('attr' => array('data-url' => $this->router->getAdminUrl ( 'del_dir' ) 
		) 
		) ) 
		) 
		) ) );
	...
	function actionAjaxGetPropertyList()
	{
		...
		$groups = array(0 => array('title' => 'Без группы' 
		) 
		) + $dir_api->getListAsResource ()->fetchSelected ( 'id' );
		
		$propapi = new \Catalog\Model\PropertyApi ();
		$propapi->setOrder ( 'parent_id, sortn' );
		$proplist = $propapi->getListAsResource ()->fetchAll ();
		
		$result = array('properties_sorted' => $proplist,'groups' => $groups 
		);
	...
	function actionAjaxCreateOrUpdateProperty()
	{
		...
		if ($result) {
			$this->view->assign ( 'group', array('group' => $result['group'] 
			) );
			$this->view->assign ( array('properties' => array($result['property'] 
	...
	function actionAjaxGetSomeProperties()
	{
		...
		foreach($list['groups'] as $group_id => $group) {
			if (isset ( $list['properties'][$group_id] )) {
				...
				$this->view->assign ( array('group' => array('group' => $group 
				),'owner_type' => 'group' 
				) );
				...
				foreach($list['properties'][$group_id] as $property) {
					$property['is_my'] = true;
					$this->view->assign ( array('properties' => array($property 
					) 
					) );
					
					$property_html = $this->view->fetch ( 'property_product.tpl' );
					
					$result[] = array('group' => $group->getValues (),'prop' => $property->getValues (),'group_html' => $group_html,'property_html' => $property_html 
					);
				}
			}
		}
