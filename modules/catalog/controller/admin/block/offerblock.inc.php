...
class OfferBlock extends \RS\Controller\Admin\Block
{
	...
	function actionIndex()
	{
		...
		$main_offer = \Catalog\Model\Orm\Offer::loadByWhere ( array('product_id' => $this->product['id'],'sortn' => 0 
		) );
		...
		$this->offer_api->setFilter ( array('sortn:>' => 0,'product_id' => $this->product['id'] 
		) );
		
		$this->offer_api->applyFormFilter ( $filter, array('product_id' => $this->product['id'],'offer_page_size' => $page_size 
		) );
		
		// Пагинация
		$total = $this->offer_api->getListCount ();
		$url_pattern = $this->router->getAdminPattern ( false, array(':offer_page' => '%PAGE%','product_id' => $this->product['id'],'offer_page_size' => $page_size,'offer_filter' => $filter 
		...
		$paginator->tpl = '%catalog%/adminblocks/offerblock/offer_paginator.tpl';
		...
		$this->view->assign ( array('elem' => $this->product,'filter_parts' => $this->offer_api->getFormFilterParts (),'product_id' => $this->product['id'],'filter' => $filter,'main_offer' => $main_offer,'other_fields_form' => $main_form_other_fields,'offer_page_size' => $page_size,'paginator' => $paginator,'offers_total' => $total,'render_type' => $render_type,'offers' => $this->offer_api->getList ( $paginator->page, $paginator->page_size ) 
		) );
		
		switch ($render_type) {
			case 'all' :
				$template = 'adminblocks/offerblock/offerblock.tpl';
				break;
			case 'all-offers' :
				$template = 'adminblocks/offerblock/offer_all.tpl';
				break;
			default :
				$template = 'adminblocks/offerblock/offer_ext.tpl';
		}
	...
	function actionOfferEdit()
	{
		...
		if ($this->url->isPost ()) {
			...
			if (! $offer_id) { // Создание комплектации
				...
				$offer['pricedata_arr'] = array('oneprice' => array('use' => true,'znak' => '+','original_value' => 0,'unit' => \Catalog\Model\CurrencyApi::getBaseCurrency ()->id 
				) 
				);
	...
	function actionOfferMultiEdit()
	{
		...
		$this->view->assign ( array('param' => array('hidden_fields' => $hidden_fields,'doedit' => $doedit,'sel_count' => count ( $ids ) 
		) 
		) );
	...
	function actionOfferMove()
	{
		...
		$q = \RS\Orm\Request::make ()->where ( array('product_id' => $this->product['id'] 
		) );
	...
	function actionOfferMakeFromMultioffers()
	{
		...
		if (! $multioffer_api->createOffersFromParams ( $this->product['id'], $product_barcode, $props )) {
			return $this->result->setSuccess ( false )->addEMessage ( $multioffer_api->getErrorsStr () );
		}
	...
	function actionOfferLinkPhoto()
	{
		...
		$this->view->assign ( array('photos_id' => $photos_id,'dialog_data' => $dialog_data 
		) );
	...
	function actionOfferLinkPhotoSave()
	{
		if ($this->url->isPost ()) {
			...
			return $this->result->setSuccess ( true )->addSection ( 'main_offer_photos', $main_offer_photos_arr );
		}
	}
	...
	private function getSelectedOffersId()
	{
		...
		if ($all_offers) {
			...
			if (! $q) return $this->result->setSuccess ( false );
