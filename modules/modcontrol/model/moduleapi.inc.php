...
class ModuleApi
{
	...
	function tableData()
	{
		...
		foreach($list as $alias => $module) {
			...
			$disable = $module['is_system'] ? array('disabled' => 'disabled' 
			) : null;
			$highlight = (time () - $module['lastupdate']) < 60 * 60 * 24 ? array('class' => 'highlight_new' 
			) : null;
			$module['class'] = $alias;
			...
			$table_rows[] = array('num' => $i,'name' => $module['name'],'description' => $module['description'],'checkbox_attribute' => $disable,'row_attributes' => $highlight 
			) + $module->getValues ();
		}
		return $table_rows;
	}
