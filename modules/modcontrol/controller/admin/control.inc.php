...
use \RS\Html\Table\Type as TableType, \RS\Html\Toolbar\Button as ToolbarButton, \RS\Html\Filter, \RS\Html\Toolbar, \RS\Html\Table;

class Control extends \RS\Controller\Admin\Front
{
	protected $action_var = 'do', $form_tpl = 'forms/%MODULE%_form.tpl', $modules;
	...
	function helperIndex()
	{
		...
		$helper->viewAsTable ();
		$helper->setTable ( new Table\Element ( array('Columns' => array(new TableType\Checkbox ( 'class', array('cellAttrParam' => 'checkbox_attribute' 
		) ),new TableType\String ( 'name', 'Название', array('href' => $this->router->getAdminPattern ( 'edit', array(':mod' => '@class' 
		) ) 
		) ),new TableType\String ( 'description', 'Описание' ),new TableType\String ( 'version', 'Версия', array('TdAttr' => array('class' => 'cell-small' 
		) 
		) ),new TableType\Usertpl ( 'enabled', 'Включен', '%modcontrol%/col_enabled.tpl' ),new TableType\String ( 'class', 'Идентификатор', array('ThAttr' => array('width' => '50' 
		) 
		) ),new TableType\Actions ( 'class', array(new TableType\Action\Edit ( $this->router->getAdminPattern ( 'edit', array(':mod' => '~field~' 
		) ), null, array('noajax' => true 
		) ) 
		) ) 
		),'rowAttrParam' => 'row_attributes' 
		) ) );
		
		$helper->setFilter ( new Filter\Control ( array('container' => new Filter\Container ( array('lines' => array(new Filter\Line ( array('items' => array(new Filter\Type\String ( 'name', 'Название' ),new Filter\Type\String ( 'class', 'Идентификатор' ) 
		) 
		) ) 
		) 
		) ) 
		...
		if (! defined ( 'CANT_UPLOAD_MODULE' )) {
			$helper->setTopToolbar ( new Toolbar\Element ( array('Items' => array(new ToolbarButton\Add ( $this->url->replaceKey ( array($this->action_var => 'add' 
			) ), t ( 'добавить модуль' ), array('noajax' => false 
			) ) 
			) 
			) ) );
		}
		
		$helper->setBottomToolbar ( new Toolbar\Element ( array('Items' => array(new ToolbarButton\Delete ( null, null, array('attr' => array('data-url' => $this->router->getAdminUrl ( 'del' ) 
		) 
		) ) 
		) 
		) ) );
	...
	function actionEdit()
	{
		...
		if ($config_obj === null) $this->e404 ( t ( 'Такого модуля не существует' ) );
		...
		$helper->setBottomToolbar ( new Toolbar\Element ( array('Items' => array(new ToolbarButton\SaveForm (),new ToolbarButton\Cancel ( $this->url->getSavedUrl ( $this->controller_name . 'index' ) ) 
		) 
		) ) );
		
		$helper['form'] = $config_obj->getForm ( null, null, false, null, '%system%/coreobject/config_form.tpl', $this->mod_tpl );
		
		$this->view->assign ( array('controller_list' => $mod->getBlockControllers (),'module_item' => $mod,'elements' => $helper,'errors' => isset ( $error ) ? $error : array() 
		) );
	...
	function actionAdd()
	{
		...
		// Если пост идет для текущего модуля
		if ($this->url->isPost ()) {
			$this->result->setSuccess ( $mod_install->extractFromPost ( $this->url->files ( 'module' ) ) );
		...
		$helper->setBottomToolbar ( new Toolbar\Element ( array('Items' => array(new ToolbarButton\SaveForm (),new ToolbarButton\Cancel ( $this->router->getadminUrl ( false ) ) 
		) 
		) ) );
		...
		$this->view->assign ( array('is_empty_tmp' => $mod_install->isEmptyTmp (),'elements' => $helper 
		) );
	...
	{
		$helper = new \RS\Controller\Admin\Helper\CrudCollection ( $this );
		$helper->setTopTitle ( t ( 'Параметры установки модуля' ) )->viewAsForm ();
		...
		$this->view->assign ( array('mod_validate' => $valid,'mod_errors' => $mod_install->getErrors (),'mod_info' => $mod_install->getTmpInfo (),'elements' => $helper 
		) );
		
		$helper->setBottomToolbar ( new Toolbar\Element ( array('Items' => array('next' => new ToolbarButton\SaveForm ( $this->router->getAdminUrl ( 'addStep3' ), t ( 'установить' ), array('attr' => array('class' => 'saveform' 
		) 
		) ),'back' => new ToolbarButton\Cancel ( $this->router->getAdminUrl ( false ), t ( 'назад' ) ),'clean' => new ToolbarButton\Button ( $this->router->getAdminUrl ( 'cleanTmp' ), t ( 'удалить модуль из временной папки' ) ) 
		) 
		) ) );
	...
	function actionAddSuccess()
	{
		...
		$helper = new \RS\Controller\Admin\Helper\CrudCollection ( $this );
		$helper->viewAsForm ()->setTopTitle ( t ( 'Установка модуля завершена' ) )->setBottomToolbar ( new Toolbar\Element ( array('Items' => array(new ToolbarButton\Cancel ( $this->router->getAdminUrl ( false ), t ( 'к списку модулей' ) ) 
		) 
		) ) );
		
		$this->view->assign ( array('elements' => $helper,'module_name' => $_SESSION['INSTALLED_MODULE'] 
		) );
			...
			if (! $access_error) {
				...
				if ($this->result->isSuccess ()) {
					if ($result === true) {
						...
						$this->result->addSection ( 'repeat', true )->addSection ( 'queryParams', array('data' => array('params' => $result 
						) 
						) );
					}
				} else {
	...
	function actionAjaxShowChangelog()
	{
		...
		if ($module->exists ()) {
			...
			$helper->setBottomToolbar ( new Toolbar\Element ( array('items' => array(new ToolbarButton\Cancel ( '' ) 
			) 
			) ) );
			$helper->viewAsForm ();
			
			$this->view->assign ( array('module_item' => $module 
			) );
