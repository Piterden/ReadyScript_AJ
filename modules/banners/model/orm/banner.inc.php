...
class Banner extends \RS\Orm\OrmObject
{
	...
	public static $src_folder = '/storage/banners/original', $dst_folder = '/storage/banners/resized';
	function _init()
	{
		parent::_init ()->append ( array('site_id' => new Type\CurrentSite (),'title' => new Type\String ( array('description' => t ( 'Название баннера' ) 
		) ),'file' => new Type\File ( array('description' => t ( 'Баннер' ),'storage' => array(\Setup::$ROOT,\Setup::$FOLDER . static::$src_folder 
		),'template' => '%banners%/form/banner/file.tpl' 
		) ),'use_original_file' => new Type\Integer ( array('description' => t ( 'Использовать оригинал файла для вставки' ),'checkboxView' => array(1,0 
		) 
		) ),'link' => new Type\String ( array('description' => t ( 'Ссылка' ) 
		) ),'targetblank' => new Type\Integer ( array('description' => t ( 'Открывать ссылку в новом окне' ),'checkboxView' => array(1,0 
		) 
		) ),'info' => new Type\Text ( array('description' => t ( 'Дополнительная информация' ) 
		) ),'public' => new Type\Integer ( array('maxLength' => 1,'description' => t ( 'Публичный' ),'checkboxView' => array(1,0 
		) 
		) ),'xzone' => new Type\ArrayList ( array('description' => t ( 'Связанные зоны (удерживая CTRL можно выбирать несколько зон)' ),'list' => array(array('\Banners\Model\ZoneApi','staticAdminSelectList' 
		) 
		),'attr' => array(array('size' => 10,'multiple' => 'multiple' 
		) 
		) 
		) ),'weight' => new Type\Integer ( array('description' => t ( 'Вес от 1 до 100' ),'default' => 100,'hint' => t ( 'Чем больше вес, тем больше вероятность того, что баннер будет показан в случае конкуренции' ) 
		) ) 
		) );
	}
	...
	function getDebugActions()
	{
		...
		), 'banners-ctrl' ) ),new \RS\Debug\Action\Delete ( \RS\Router\Manager::obj ()->getAdminPattern ( 'del', array(':chk[]' => '{id}' 
		), 'banners-ctrl' ) ) 
		);
	}
	...
	function afterWrite($flag)
	{
		if ($this->isModified ( 'xzone' )) {
			\RS\Orm\Request::make ()->delete ()->from ( new Xzone () )->where ( array('banner_id' => $this['id'] 
			) )->exec ();
	...
	function fillZones()
	{
		$this['xzone'] = \RS\Orm\Request::make ()->from ( new Xzone () )->where ( array('banner_id' => $this['id'] 
		) )->exec ()->fetchSelected ( null, 'zone_id' );
	}
	function delete()
	{
		if ($result = parent::delete ()) {
			\RS\Orm\Request::make ()->from ( new Xzone () )->where ( array('banner_id' => $this['id'] 
			) )->exec ();
		}
		return $result;
	}
	 ...
	 * @param integer $hwight - высота изображения
	 * @param string xy|cxy|axy $scale - тип масштабирования
