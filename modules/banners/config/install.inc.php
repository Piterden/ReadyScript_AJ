...
class Install extends \RS\Module\AbstractInstall
{
	...
	{
		return $this->importCsvFiles ( array(array('\Banners\Model\CsvSchema\Zone','zones' 
	...
	function deferredAfterInstall($options)
	{
		if ($options['set_demo_data']) {
			...
			if ($site_config->getThemeName () == 'default') {
				$zone = \Banners\Model\Orm\Zone::loadByWhere ( array('alias' => 'leftcolumn' 
				) );
				
				// Настраиваем блок новости в дефолтном шаблоне
				\Templates\Model\PageApi::setupModule ( null, 'banners\controller\block\bannerzone', array('zone' => $zone['id'],'rotate' => 1 
				) );
			}
		}
		return true;
