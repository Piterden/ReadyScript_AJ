...
class Img
{
	const SESSION_VAR = 'kaptcha_keystring', SESSION_CHECKCOUNT_VAR = 'kaptcha_keystring_checkcount';
	public static $checks = 0, $check_limit = 5; // Кол-во проверок, которое можно провести с одним кодом капчи
		...
		while ( true ) {
			...
			if ($no_fonts) {
				...
				for($i = 0; $i < $length; $i ++) {
					...
					else $y = ($height - $fontfile_height) / 2 + 2;
	...
	public static function checkKeyString($keystring)
	{
		...
		if (isset ( $_SESSION[self::SESSION_VAR] )) {
			$result = (! empty ( $_SESSION[self::SESSION_VAR] ) && (strcmp ( $keystring, $_SESSION[self::SESSION_VAR] ) == 0));
