...
{
	protected $action_var = 'files_do';
	public $file_api, $link_type, $link_id;
	function init()
	{
		...
		$this->view->assign ( array('link_type' => $type_class_name,'link_id' => $this->link_id 
		) );
	}
	...
	{
		$this->file_api->setFilter ( array('link_type_class' => $this->link_type->getShortName (),'link_id' => $this->link_id 
		) );
		
		$this->view->assign ( array('max_upload_size' => \RS\File\Tools::getMaxPostFileSize (),'files' => $this->file_api->getList () 
		) );
	...
	function actionMove()
	{
		...
		$this->result->setSuccess ( $this->file_api->moveFileElement ( $from, $to, $flag, $this->link_type, $this->link_id ) );
