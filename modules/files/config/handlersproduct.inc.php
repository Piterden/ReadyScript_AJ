...
class HandlersProduct extends \RS\Event\HandlerAbstract
{
	function init()
	{
		...
		$this->bind ( 'orm.init.catalog-product' )->bind ( 'orm.afterwrite.catalog-product' )->bind ( 'orm.delete.catalog-product' )->bind ( 'orm.multidelete.catalog-product' );
	}
	...
	{
		$product->getPropertyIterator ()->append ( array(t ( 'Файлы' ),'__files__' => new Type\UserTemplate ( '%files%/catalog/product_files.tpl' ) 
		) );
	}
	...
	public static function ormAfterwriteCatalogProduct($params)
	{
		...
		if ($product['_tmpid'] < 0) {
			\RS\Orm\Request::make ()->update ( new \Files\Model\Orm\File () )->set ( array('link_id' => $product['id'] 
			) )->where ( array('link_type_class' => 'files-catalogproduct','link_id' => $product['_tmpid'] 
			) )->exec ();
		}
	}
