...
class HandlersOrder extends \RS\Event\HandlerAbstract
{
	function init()
	{
		...
		$this->bind ( 'orm.init.shop-order' )->bind ( 'orm.afterwrite.shop-order' )->bind ( 'orm.delete.shop-order' )->bind ( 'order.change' );
	}
	...
	{
		$order->getPropertyIterator ()->append ( array(t ( 'Файлы' ),'__files__' => new Type\UserTemplate ( '%files%/shop/order_files.tpl', false, array('footerVisible' => true 
		) ) 
		) );
	}
	...
	public static function ormAfterwriteShopOrder($params)
	{
		...
		if ($order['_tmpid'] < 0) {
			\RS\Orm\Request::make ()->update ( new \Files\Model\Orm\File () )->set ( array('link_id' => $order['id'] 
			) )->where ( array('link_type_class' => 'files-shoporder','link_id' => $order['_tmpid'] 
			) )->exec ();
		}
	}
