...
{
	const UPLOAD_TYPE_COPY = 0, UPLOAD_TYPE_MOVE = 1, UPLOAD_TYPE_MOVEUPLOADED = 2, 
	...
	{
		parent::__construct ( new Orm\File (), array('sortField' => 'sortn','defaultOrder' => 'sortn','multisite' => true 
		) );
	}
	...
	function uploadFromPost($files, FilesType\AbstractType $type_class, $link_id)
	{
		...
		foreach($normalized_files as $file) {
			...
			if ($error = \RS\File\Tools::checkUploadError ( $file['error'] )) {
				...
				$uploaded_file = $this->uploadFromUrl ( $file['tmp_name'], $type_class, $link_id, $file['name'], self::UPLOAD_TYPE_MOVEUPLOADED, array('mime' => $file['type'] 
				) );
			}
			
			if ($uploaded_file) {
				$item = array('success' => true,'file' => $uploaded_file 
				);
			} else {
				$item = array('success' => false,'error' => $this->getErrorsStr () 
				);
			}
			$items[] = $item;
		}
	...
	function uploadFromUrl($file_path, FilesType\AbstractType $type_class, $link_id, $display_name = null, $upload_type = self::UPLOAD_TYPE_COPY, $info = array())
	{
		...
		switch ($upload_type) {
			case self::UPLOAD_TYPE_COPY :
				{
					$move_result = copy ( $file_path, $destination_path );
					break;
				}
			case self::UPLOAD_TYPE_MOVE :
				{
					$move_result = rename ( $file_path, $destination_path );
					break;
				}
			case self::UPLOAD_TYPE_MOVEUPLOADED :
				{
					$move_result = move_uploaded_file ( $file_path, $destination_path );
					break;
				}
		}
	...
	function getFile($id, $site_id = null)
	{
		...
		return \RS\Orm\Request::make ()->from ( $this->obj_instance )->where ( array('id' => $id,'site_id' => $site_id 
		) )->object ();
	}
	...
	{
		$extra_expr = \RS\Orm\Request::make ()->where ( array('link_type_class' => $type_class->getShortName (),'link_id' => $link_id 
		) );
