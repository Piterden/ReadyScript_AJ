...
class ShopOrder extends AbstractType
{
	...
	function getAccessTypes()
	{
		...
		if (\Setup::$SCRIPT_TYPE != 'Shop.Base') {
			$access_list += array('afterpay' => array('title' => t ( 'доступен после оплаты' ),'hint' => t ( 'Ссылка на скачивание данного файла будет доступна в разделе личного кабинета - Мои заказы' ) 
			) 
			);
		}
		return $access_list;
	...
	function checkDownloadRightErrors(\Files\Model\Orm\File $file)
	{
		...
		if ($file['access'] == self::ACCESS_TYPE_AFTERPAY) {
			...
			$is_purchased = \RS\Orm\Request::make ()->from ( new \Shop\Model\Orm\Order () )->where ( array('id' => $file['link_id'],'user_id' => $user['id'],'is_payed' => 1 
			) )->count ();
