...
{
	const ACCESS_TYPE_AFTERPAY = 'afterpay';
	...
	function getAccessTypes()
	{
		...
		if (\Setup::$SCRIPT_TYPE != 'Shop.Base') {
			$access_list += array('afterpay' => array('title' => t ( 'доступен после оплаты' ),'hint' => t ( 'Ссылка на скачивание данного файла будет отправлена покупателю по почте, а также будет доступна в разделе личного кабинета - Мои заказы' ) 
			) 
			);
		}
		return $access_list;
	...
	function checkDownloadRightErrors(\Files\Model\Orm\File $file)
	{
		...
		if ($file['access'] == self::ACCESS_TYPE_AFTERPAY) {
			...
			$is_purchased = \RS\Orm\Request::make ()->from ( new \Shop\Model\Orm\Order (), 'O' )->join ( new \Shop\Model\Orm\OrderItem (), 'I.order_id = O.id', 'I' )->where ( array('O.user_id' => $user['id'],'O.is_payed' => 1,'I.type' => \Shop\Model\Cart::TYPE_PRODUCT,'I.entity_id' => $file['link_id'] 
			) )->count ();
			
			if (! $is_purchased) return t ( 'Доступ к файлу запрещен' );
		}
		return false;
	}
