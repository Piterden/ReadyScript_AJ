...
{
	protected static $cache_path = array();
	protected $tree_delimiter = '/', $delimiter = "/", $id_field, $link_id_field, $link_preset_id, $orm_object, $is_multisite, $range, // Массив диапозона для обработки полей (Сколько одинаковых полей "Категория" включает)

	$title, $array_field, 

	$tree_field, $tree_parent_field, $root_value, 

	$manylink_foreign_id_field, $manylink_id_field, $manylink_orm;
	...
	function loadData()
	{
		...
		if ($ids) {
			$this->rows = \RS\Orm\Request::make ()->select ( "O.*, X.{$this->manylink_foreign_id_field} as foreignId" )->from ( $this->orm_object, 'O' )->join ( $this->manylink_orm, "X.{$this->manylink_id_field} = O.{$this->id_field}", 'X' )->where ( "X.{$this->manylink_foreign_id_field} in (" . implode ( ',', $ids ) . ")" )->objects ( null, 'foreignId', true );
		}
	}
	...
	function getColumns()
	{
		...
		for($i = 0; $i < count ( $colums_num ); $i ++) {
			$columns[$this->id . '-manytree' . ($i + 1)] = array('key' => 'manytree' . ($i + 1),'title' => t ( 'Категория' . ($i + 1) ) 
			);
		}
	...
	function followPath($path_array)
	{
		...
		foreach($path_array as $path) {
			$dir = \RS\Orm\Request::make ()->from ( $this->getOrmObject () )->where ( array('name' => trim ( $path ),'parent' => $parent_id 
			) )->object ();
	...
	{
		$q = \RS\Orm\Request::make ()->from ( $this->getOrmObject () )->where ( array($this->getMappedField ( $this->tree_field ) => $title,$this->tree_parent_field => $parent 
	...
	protected function getRecursiveField($current_id)
	{
		if (! isset ( self::$cache_path[$current_id] )) {
			...
			while ( $id && $id != $this->root_value ) {
				$row = \RS\Orm\Request::make ()->select ( $this->getMappedField ( $this->tree_field ), $this->tree_parent_field )->from ( $this->getOrmObject () )->where ( array($this->id_field => $id 
				) )->exec ()->fetchRow ();
				if ($row) {
