...
$select_order, $exclude_fields = array(), $titles = array(), $search_fields = array(), $load_expression, $is_multisite, $use_cache = true, $cache = array(), 

	$null_fields = array(), $replace_mode = false, $use_temporary_id, $uniq_fields;
			...
			foreach($product['properties'] as $key => $properties) {
				...
				if (! empty ( $properties ) && isset ( $properties['properties'] )) {
					...
					foreach($props as $key => $property) {
						$arr[$key] = array('id' => $key,'is_my' => true,'value' => $property['value'] 
						);
					}
				}
			}
