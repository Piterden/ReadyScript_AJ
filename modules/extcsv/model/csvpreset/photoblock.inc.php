...
{
	protected $root = PATH, $link_id_field, $link_preset_id, $range, // Массив диапозона для обработки полей (Сколько одинаковых полей "Фото" включает)
$type_item;
	...
	function loadData()
	{
		...
		if ($ids) {
			$this->row = \RS\Orm\Request::make ()->from ( new \Photo\Model\Orm\Image () )->whereIn ( 'linkid', $ids )->where ( array('type' => $this->type_item 
	...
	function getColumns()
	{
		...
		for($i = 0; $i < count ( $colums_num ); $i ++) {
			$columns[$this->id . '-photo' . ($i + 1)] = array('key' => 'photo' . ($i + 1),'title' => t ( 'Фото' . ($i + 1) ) 
			);
		}
