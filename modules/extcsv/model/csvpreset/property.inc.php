...
class Property extends \RS\Csv\Preset\AbstractPreset
{
	...
	protected $delimiter = ",", $value_delimiter = ",", $id_field = 'id', $link_id_field, $link_preset_id, $properties, // Массив со свойствами для колонок
$titles_prop_compire, // Массив с соотвествием имён и id
$mask, $mask_fields = array(), $mask_pattern, $title, $array_field = 'prop', $manylink_foreign_id_field = 'prop_id', $manylink_id_field = 'product_id', $manylink_orm;
	function __construct($options)
	{
		$defaults = array('ormObject' => new \Catalog\Model\Orm\Property\Item (),'manylinkOrm' => new \Catalog\Model\Orm\Property\Link (),'multisite' => true 
		);
...
'title' => "Характеристика_(" . $group['title'] . ")" . $property['title'], // Имя колонки
	...
	function importColumnsData()
	{
		if (isset ( $this->row )) { // Если есть что импортировать получим данные о группе и характеристиках
			...
			foreach($this->row as $column_title => $val) {
				...
				if (! empty ( $value )) {
					$result_array[$prop_id] = array('id' => $prop_id,'is_my' => 1,'value' => $value 
					);
				}
			}
