...
array('title' => t ( 'Расширенный импорт товаров из CSV' ),'attr' => array('href' => \RS\Router\Manager::obj ()->getAdminUrl ( 'importCsv', array('schema' => 'extcsv-product','referer' => \RS\Router\Manager::obj ()->getAdminUrl (),'params[dir]' => $dir 
		), 'main-csv' ),'class' => 'crud-add' 
		) 
		) );
		
		$import_buttons->addItem ( array('title' => t ( 'Расширенный экспорт товаров в CSV' ),'attr' => array('href' => \RS\Router\Manager::obj ()->getAdminUrl ( 'exportCsv', array('schema' => 'extcsv-product','referer' => \RS\Router\Manager::obj ()->getAdminUrl (),'params[dir]' => $dir 
		), 'main-csv' ),'class' => 'crud-add' 
		) 
		) ); // Добавим кнопку
	}
