...
MOVE_TYPE_RENAME = 'rename', MOVE_TYPE_COPY = 'copy', IMGUNIQ_TOTAL_LENGTH = 15; // общая длина имени файла изображения
	protected /**
	 *
	 * @var \Photo\Model\Orm\Image
	 */
	$obj_instance, $enable_upload_resize = true, $uploadError = array(), $allow_mime = array('image/pjpeg','image/jpeg','image/png','image/gif' 
	), $srcFolder = '/storage/photo/original', $dstFolder = '/storage/photo/resized';
	function __construct()
	{
		...
		parent::__construct ( new \Photo\Model\Orm\Image (), array('defaultOrder' => $order 
		) );
	...
	function fixSortNumbers($linkid, $type)
	{
		...
		\RS\Db\Adapter::SQLExec ( "
            ...
            order by `sortn` asc
        ", array('linkid' => $linkid,'type' => $type 
		) );
	}
	
	/**
	 * Генерирует уникальное имя картинки, в папке.
	 * Например: "a/h32k45h6hn"
	 */
	 ...
	 * 
	 * @param массив из $_FILES['имя Inputа']
	 */
	 ...
	 * 
	 * @param mixed $source путь к исходному файлу изображения
	 * @param mixed $destination путь к файлу изображения для сохранения
	 */
	...
	function multiDelete($ids)
	{
		if (! empty ( $ids )) {
			...
			$server_names = \RS\Orm\Request::make ()->select ( 'id, servername' )->from ( $this->obj_instance )->whereIn ( "id", $ids )->exec ()->fetchSelected ( 'id', 'servername' );
			...
			$except = \RS\Orm\Request::make ()->select ( 'id,servername' )->from ( $this->obj_instance )->whereIn ( "servername", $server_names )->exec ()->fetchSelected ( 'servername', 'id' );
	...
	{
		$exists = \RS\Orm\Request::make ()->select ( 'id, servername' )->from ( new $this->obj_instance () )->exec ()->fetchSelected ( 'servername', 'id' );
	...
	{
		return \RS\File\Tools::deleteFolder ( $this->dstFolder, true );
	}
