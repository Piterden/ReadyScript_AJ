...
class Tools extends \RS\Controller\Admin\Front
{
	function actionAjaxDelUnlinkPhotos()
	{
		...
		return $this->result->setSuccess ( true )->addMessage ( t ( 'Удалено %0 не связанных фото', array($count 
		) ) );
	}
	function actionAjaxDelPreviewPhotos()
	{
		...
		return $this->result->setSuccess ( true )->addMessage ( t ( 'Миниатюры фотографий удалены' ) );
	}
}