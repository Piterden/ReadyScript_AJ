...
{
	protected $api, $default_params = array('indexTemplate' => 'form.tpl', // Шаблон по умолчанию
'photoTemplate' => 'form_onepic.tpl' 
	) // Шаблон одной картинки
, $action_var = 'pdo';
	...
	{
		$this->view->assign ( array('addParam' => array('linkid' => $this->param['linkid'],'type' => $this->param['type'] 
	...
	function actionAddphoto()
	{
		...
		if (! empty ( $files )) {
			...
			for($i = 0; $i < count ( $files['name'] ); $i ++) {
				$normalize_files[] = array('name' => $files['name'][$i],'type' => $files['type'][$i],'tmp_name' => $files['tmp_name'][$i],'error' => $files['error'][$i],'size' => $files['size'][$i] 
				);
			}
			...
			foreach($normalize_files as $key => $file) {
				if ($file['error'] != UPLOAD_ERR_NO_FILE) {
					...
					$items[$key] = array('success' => ($photo !== false),'errors' => $photo_api->getUploadError () 
					);
	...
	function noWriteRights()
	{
		if ($acl_err = \RS\AccessControl\Rights::CheckRightError ( $this, ACCESS_BIT_WRITE )) {
			$this->result->setSuccess ( false )->setErrors ( array($acl_err 
			) );
			return true;
		}
		return false;
