...
/**
 * Api - для работы с индексной таблицей.
 * Позволяет добавлять и исключать объекты из поиска
 * @ingroup Search
 */
class IndexApi extends \RS\Module\AbstractModel\EntityList
{
	...
	{
		return \RS\Orm\Request::make ()->delete ()->from ( new Orm\Index () )->where ( array('result_class' => is_object ( $result_class ) ? get_class ( $result_class ) : $result_class,'entity_id' => $entity_id 
		) )->exec ();
	}
}
