...
class Mysql
{
	const ORDER_RELEVANT = 'relevant', ORDER_FIELD = 'field';
	protected $config, $order, $orderType = self::ORDER_RELEVANT, $errors = array(), $query, $offset, $pageSize, $filters, $total = 0;
	...
	protected function getBaseRequest($q = null)
	{
		...
		$q->from ( new \Search\Model\Orm\Index () )->asAlias ( 'A' )->where ( "MATCH(A.`title`, A.`indextext`) AGAINST('#query' IN BOOLEAN MODE)", array('query' => $this->getStemmedQuery () 
		) );
	...
	function search(\RS\Orm\Request $q = null)
	{
		...
		if ($this->total) {
			...
			if ($this->orderType == self::ORDER_RELEVANT) {
				$q->select ( "*, MATCH(A.`title`, A.`indextext`) AGAINST('" . \RS\Db\Adapter::escape ( $this->query ) . "' IN NATURAL LANGUAGE MODE WITH QUERY EXPANSION) as rank" )->orderby ( 'rank DESC' );
			} else {
	...
	function joinQuery(\RS\Orm\Request $q, $alias_product = 'A', $alias = 'B')
	{
		if ($this->config['searchtype'] == 'like') {
			$q->join ( new \Search\Model\Orm\Index (), "$alias.entity_id = $alias_product.id", $alias )->where ( "($alias.`title` like '#query' OR $alias.`indextext` like '#query')", array('query' => $this->getQueryForLike () 
			) );
		} else {
			$q->join ( new \Search\Model\Orm\Index (), "$alias.entity_id = $alias_product.id", $alias )->where ( "MATCH($alias.`title`, $alias.`indextext`) AGAINST('#query' IN BOOLEAN MODE)", array('query' => $this->getStemmedQuery () 
			) );
		}
		
		if ($this->orderType == self::ORDER_RELEVANT && $this->config['searchtype'] != 'like') {
			$q->select ( "MATCH($alias.`title`, $alias.`indextext`) AGAINST('" . \RS\Db\Adapter::escape ( $this->query ) . "' IN NATURAL LANGUAGE MODE WITH QUERY EXPANSION) as rank" )->orderby ( 'rank DESC' );
		}
