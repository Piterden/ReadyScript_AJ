...
use \RS\Html\Table\Type as TableType, \RS\Html\Toolbar\Button as ToolbarButton, \RS\Html\Toolbar;
...
class Wizard extends \RS\Controller\Admin\Front
{
	...
	function init()
	{
		...
		$this->helper->viewAsAny ()->setTopTitle ( t ( 'Центр обновления' ) );
		
		$this->view->assign ( array('elements' => $this->helper 
		) );
	}
	function actionIndex()
	{
		...
		$this->view->assign ( array('canUpdate' => $can_update,'errors' => $this->api->getDisplayErrors (),'currentStep' => '1' 
		) );
	...
	function actionSelectProduct()
	{
		...
		$this->view->assign ( array('currentStep' => '2','data' => $this->api->getPrepearedData () 
		) );
	...
	function actionUpdate()
	{
		...
		$table = new \RS\Html\Table\Element ( array('Columns' => array(new TableType\Checkbox ( 'module', array('cellAttrParam' => 'checkbox_attr' 
		) ),new TableType\Usertpl ( 'title', t ( 'Название модуля' ), '%siteupdate%/module_col.tpl' ),new TableType\String ( 'my_version', t ( 'Текущая версия' ) ),new TableType\String ( 'new_version', t ( 'Доступная версия' ) ),new TableType\Usertpl ( 'module', '', '%siteupdate%/changelog_col.tpl' ) 
		) 
		) );
		...
		$button = ! count ( $data['updateData'] ) ? new ToolbarButton\Cancel ( $this->router->getAdminUrl ( false ), t ( 'назад' ) ) : new ToolbarButton\SaveForm ( $this->router->getAdminUrl ( 'update' ), t ( 'установить выбранные обновления' ), array('noajax' => true 
		) );
		
		$this->helper->setBottomToolbar ( new Toolbar\Element ( array('Items' => array($button 
		) 
		) ) );
		
		$this->view->assign ( array('success_text' => isset ( $_SESSION['SUCCESS_INSTALL_TEXT'] ) ? $_SESSION['SUCCESS_INSTALL_TEXT'] : false,'table' => $table,'data' => $data,'currentStep' => '3' 
		) );
	...
	function actionViewChangelog()
	{
		...
		$this->helper->viewAsForm ()->setTopTitle ( t ( 'Изменения в модуле {name}' ), array('name' => $module_name 
		) )->setBottomToolbar ( new Toolbar\Element ( array('Items' => array(new ToolbarButton\Cancel ( $this->router->getAdminUrl ( 'update' ) ) 
		) 
		) ) );
