...
use \RS\Cache\Manager as CacheManager, \RS\HashStore\Api as HashStoreApi;
...
{
	const REQUEST_TIMEOUT = 50, CHECK_FOR_UPDATES_INTERVAL = 14400, // 4 часа
SKIPPED_MODULE_STORE_KEY = 'SKIPPED_MODULES', SECTION_PRODUCTS_FOR_UPDATE = 'products';
	protected $time_marker = 0, $max_execution_time = 25, // сек
$config, $write_data_to_file = true, $update_tmp_folder = '/siteupdate', $update_tmp_folder_zip, $update_upacked_folder, $backup_folder, $data, $data_file = 'data.srz', $copy_files = array('/package.inc.php' 
	), $module_folders = array('@core' => array('/core/','/resource/','/templates/system/' 
	) 
	);
	...
	protected function getRequestVars()
	{
		...
		return array('licenses' => $licenses,'channel' => \Setup::$UPDATE_CHANNEL,'product' => \Setup::$SCRIPT_TYPE,'copy_id' => COPY_ID,'install_id' => INSTALL_ID,'lang' => \RS\Language\Core::getCurrentLang () 
		);
	}
	...
	function prepareProductsForUpdate(&$count)
	{
		...
		$this->setData ( null );
		$params = array('do' => 'getProductsForUpdate','modules' => implode ( ',', get_loaded_extensions () ) 
		);
	...
	{
		$versions = array('@core' => \Setup::$VERSION 
		);
	...
	{
		$params = array('do' => 'getServerVersions','need_product' => $product,'client_versions' => $this->getMyVersions () 
		);
		$response = $this->requester ( $params );
		...
		$this->setData ( array('updateProduct' => $product,'serverVersions' => $response['versions'] 
		) );
	...
	function compareVersions($skipped_modules = array())
	{
		...
		foreach($this->data['serverVersions'] as $module => $data) {
			...
			$item = array('new_version' => $data['version'],'my_version' => isset ( $my_versions[$module] ) ? $my_versions[$module] : t ( 'новый' ),'module' => $module,'title' => $data['name'],'description' => $data['description'],'changelog_exists' => $data['changelog_exists'] 
			);
	...
	function requester($params, $json = true)
	{
		...
		$context = stream_context_create ( array('http' => array('method' => 'POST','ignore_errors' => true,'timeout' => self::REQUEST_TIMEOUT,'header' => 'Content-Type: application/x-www-form-urlencoded' . PHP_EOL,'content' => http_build_query ( $params ) 
		) 
		) );
	...
	function prepareInstallUpdate(array $modules)
	{
		...
		$install_map = array(array('callback' => 'backupSystem','status' => t ( 'Создание точки восстановления системы' ) 
		),array('callback' => 'getUpdateFilename','status' => t ( 'Получение сведений от сервера обновлений' ) 
		),array('callback' => 'downloadUpdatePackage','status' => t ( 'Загрузка обновлений с удаленного сервера' ) 
		),array('callback' => 'unpackUpdatePackage','status' => t ( 'Распаковка файла обновлений' ) 
		) 
		);
		...
		foreach($modules as $module) {
			if ($module == '@core') {
				$install_map[] = array('callback' => 'updateCore','status' => t ( "Обновление ядра системы" ) 
				);
			} elseif ($module{0} == '#') {
				$template = substr ( $module, 1 );
				$install_map[] = array('callback' => 'updateTemplate','callback_params' => array($template 
				),'status' => t ( "Обновление шаблона %0", array($template 
				) ) 
				);
			} else {
				$install_map[] = array('callback' => 'updateModule','callback_params' => array($module 
				),'moduleName' => $update_data['serverVersions'][$module]['name'],'status' => t ( "Обновление модуля %0", array($update_data['serverVersions'][$module]['name'] 
				) ) 
				);
			}
		}
		
		$this->setData ( array('selectedModules' => $modules,'installMap' => $install_map,'position' => 0,'percent' => 0 
		) );
	...
	function doUpdate()
	{
		...
		if ($pos < count ( $this->data['installMap'] )) {
			...
			if (($update_result = call_user_func_array ( array($this,$method 
			), $params )) !== true) {
	...
	{
		$response = $this->requester ( array('do' => 'getUpdateFile','need_product' => $this->data['updateProduct'],'items' => $this->data['selectedModules'] 
		) );
		...
		$core_file_data = $response['size'] > 0 ? array(array('name' => 'core','type' => 'core','size' => $response['size'],'file_id' => $response['file_id'] 
		) 
		) : array();
		
		$this->setData ( array('download_files' => array_merge ( $core_file_data, $response['addons'] ),'download_state_index' => 0,'download_state_offset' => 0 
		) );
	...
	{
		if ($this->data['download_state_index'] == 0 && $this->data['download_state_offset'] == 0) {
		...
		$params = array('do' => 'downloadUpdatePackage','file_id' => $download_file['file_id'],'type' => $download_file['type'],'offset' => $this->data['download_state_offset'],'length' => $this->config->file_download_part_size_mb * 1024 * 1024 
		);
		
		$content = $this->requester ( $params, false );
		
		if ($content === false) return t ( 'Не удалось загрузить файл с сервера обновлений' );
		...
		if ($this->data['download_state_index'] < count ( $this->data['download_files'] ) - 1) {
			$this->setData ( array('download_state_index' => $this->data['download_state_index'] + 1,'download_state_offset' => 0 
			) );
	...
	private function finishUpdate()
	{
		...
		return array('next' => t ( 'Обновление завершено' ),'complete' => true,'percent' => 100 
		);
	}
	...
	protected function makeError($module_title, $message)
	{
		...
		foreach($message as $text) {
			$return['errors'][] = array('class' => 'field','fieldname' => $module_title,'errors' => ( array ) $text 
			);
		}
	...
	{
		$response = $this->requester ( array('do' => 'getChangelog','module' => $module,'need_product' => $this->data['updateProduct'],'version' => isset ( $this->data['myVersions'][$module] ) ? $this->data['myVersions'][$module] : null 
		) );
