...
class Control extends \RS\Controller\Admin\Crud
{
	...
	function helperIndex()
	{
		...
		$helper->setTable ( new Table\Element ( array('Columns' => array(new TableType\Checkbox ( 'id'/*, array('showSelectAll' => true)*/), // Отображаем флажок "выделить элементы на всех страницах"
new TableType\String ( 'user_id', 'ID пользователя', array('Sortable' => SORTABLE_BOTH 
		...
		) ),new TableType\String ( 'product_id', 'ID товара', array('Sortable' => SORTABLE_BOTH 
		...
		$helper->setFilter ( new Filter\Control ( array('Container' => new Filter\Container ( array('Lines' => array(new Filter\Line ( array('items' => array(new Filter\Type\String ( 'user_name', 'Имя пользователя', array('SearchType' => '%like%' 
		) ) 
		) 
		) ),new Filter\Line ( array('items' => array(new Filter\Type\String ( 'product_name', 'Название товара', array('SearchType' => '%like%' 
		) ) 
		) 
		) ) 
		) 
