...
class Transport
{
	public static $HTTPS_LOGIN = ""; // Ваш логин для HTTPS-протокола
	public static $HTTPS_PASSWORD = ""; // Ваш пароль для HTTPS-протокола
	public static $HTTPS_ADDRESS = "https://lcab.sms-uslugi.ru/"; // HTTPS-Адрес, к которому будут обращаться скрипты. Со слэшем на конце.
	public static $HTTP_ADDRESS = "http://lcab.sms-uslugi.ru/"; // HTTP-Адрес, к которому будут обращаться скрипты. Со слэшем на конце.
	public static $HTTPS_METHOD = "curl"; // метод, которым отправляется запрос (curl)
	public static $USE_HTTPS = 1; // 1 - использовать HTTPS-адрес, 0 - HTTP
	...
	public static $HTTPS_CHARSET_AUTO_DETECT = false;
	public static $HTTPS_CHARSET = "utf-8"; // кодировка ваших скриптов. cp1251 - для Windows-1251, либо же utf-8 для, сообственно - utf-8 :)
	...
	function reports($start = "0000-00-00", $stop = "0000-00-00", $dop = array())
	{
		...
		$result = $this->request ( "report", array("start" => $start,"stop" => $stop,"source" => $dop["source"],"number" => $dop["number"] 
		) );
	...
	function detailReport($smsid)
	{
		...
		if ($this->get ( $result, "code" ) != 1) {
			...
			$return = array("code" => $this->get ( $result, "code" ),"descr" => $this->get ( $result, "descr" ),"delivered" => $detail['delivered'],"notDelivered" => $detail['notDelivered'],"waiting" => $detail['waiting'],"enqueued" => $detail['enqueued'],"cancel" => $detail['cancel'],"onModer" => $detail['onModer'] 
			);
			if (isset ( $result['sms'] )) $return["sms"] = $result['sms'];
		}
		return $return;
	...
	function send($params = array(), $phones = array())
	{
		...
		if ($this->get ( $result, "code" ) != 1) {
			...
			$return = array("code" => 1,"descr" => $this->get ( $result, "descr" ),"datetime" => $this->get ( $result, "datetime" ),"action" => $this->get ( $result, "action" ),"allRecivers" => $this->get ( $result, "allRecivers" ),"colSendAbonent" => $this->get ( $result, "colSendAbonent" ),"colNonSendAbonent" => $this->get ( $result, "colNonSendAbonent" ),"priceOfSending" => $this->get ( $result, "priceOfSending" ),"colsmsOfSending" => $this->get ( $result, "colsmsOfSending" ),"price" => $this->get ( $result, "price" ),"smsid" => $this->get ( $result, "smsid" ) 
			);
		}
		return $return;
	}
	...
	function request_curl($action, $xml)
	{
		...
		else $address = self::$HTTP_ADDRESS . "API/XML/" . $action . ".php";
	...
	function getConvertedString($value, $from = false)
	{
		...
		if (strtolower ( $this->charset ) != "utf-8") {
			if (function_exists ( "iconv" )) {
				...
				else return iconv ( "utf-8", $this->charset, $value );
			} else
