...
class Api extends \RS\Module\AbstractModel\EntityList
{
	...
	{
		parent::__construct ( new \Alerts\Model\Orm\NoticeConfig (), array('multisite' => true 
		) );
	}
	...
	static public function getClassNameByPath($file_path)
	{
		...
		$file_path = str_replace ( array('.' . \Setup::$CUSTOM_CLASS_EXT,'.' . \Setup::$CLASS_EXT 
		), '', $file_path );
	...
	static public function getNoticeConfig($class_name)
	{
		...
		$notice_config = \Alerts\Model\Orm\NoticeConfig::loadByWhere ( array('site_id' => \RS\Site\Manager::getSiteId (),'class' => $class_name 
		) );
		if (! $notice_config['id']) {
