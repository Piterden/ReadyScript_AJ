...
class Ctrl extends \RS\Controller\Admin\Crud
{
	...
	function helperIndex()
	{
		...
		if ($this->url->isPost ()) {
			...
			$this->result->setSuccessText ( t ( 'Изменения успешно сохранены' ) )->setSuccess ( true );
		}
		
		$helper->viewAsAny ();
		$this->view->assign ( array('cfg' => \RS\Config\Loader::byModule ( $this ),'alerts' => $this->api->getList (),'tfolders' => \RS\Module\Item::getResourceFolders ( 'templates' ) 
		) );
		$helper['form'] = $this->view->fetch ( 'notice_list.tpl' );
		
		$helper->setTopToolbar ( new \RS\Html\Toolbar\Element ( array('Items' => array(new \RS\Html\Toolbar\Button\Button ( $this->router->getAdminUrl ( 'edit', array('mod' => 'alerts' 
		), 'modcontrol-control' ), t ( 'Настройка модуля «Уведомления»' ) ) 
		) 
		) ) );
