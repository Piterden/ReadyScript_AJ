    ...
    <tbody>
        ...
        <tr>
            ...
            <td>{$item.cartitem.title}
                <small>
                {if !empty($item.cartitem.model)}( {$item.cartitem.model} ){/if}
...
<br>
{static_call callback=['\RS\Helper\Tools','priceToString'] params=[$order_data.total_cost_noformat] var="totalcost_str"}
