...
class Api
{
	protected $obj_instance, $obj_link_instance;
	
	/**
	 * Разбивает строку на массив слов.
	 * Разбиваются слова с запятыми
	 * 
	...
	{
		return \RS\Orm\Request::make ()->select ( 'W.*, L.id as lid' )->from ( new Orm\Word () )->asAlias ( 'W' )->join ( new Orm\Link (), 'W.id = L.word_id', 'L' )->where ( array('type' => $type,'link_id' => $linkid 
		) )->orderby ( 'L.id' )->exec ()->fetchAll ();
	}
	function getList()
	{
		return \RS\Orm\Request::make ()->from ( new \Tags\Model\Orm\Link () )->objects ();
	}
	function getHelpList($beg_word)
	{
		$beg_word = $this->prepareWord ( $beg_word );
		return \RS\Orm\Request::make ()->from ( new Orm\Word () )->where ( "stemmed LIKE '#word%'", array('word' => $this->stemWord ( $beg_word ) 
		) )->exec ()->fetchAll ();
	}
	...
	{
		$q = \RS\Orm\Request::make ()->delete ()->from ( new Orm\Link () )->where ( array('id' => $id 
		) );
	...
	{
		$q = \RS\Orm\Request::make ()->from ( new \Tags\Model\Orm\Link () )->where ( array('link_id' => $link_id,'type' => $type 
		) )->delete ()->exec ();
	}
	...
	function getIdByTags($wordlist, $type = null)
	{
		...
		$q = \RS\Orm\Request::make ()->select ( 'link_id' )->from ( new Orm\Link () )->groupby ( 'link_id' )->whereIn ( 'word_id', $w_ids );
	...
	{
		$word = \RS\Orm\Request::make ()->from ( new \Tags\Model\Orm\Word () )->where ( array('alias' => $alias 
		) )->object ();
