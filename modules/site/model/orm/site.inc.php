...
use \RS\Orm\Type, \RS\Cache\Cleaner as CacheCleaner;
...
class Site extends \RS\Orm\OrmObject
{
	...
	function _init()
	{
		...
		$this->getPropertyIterator ()->append ( array('title' => new Type\String ( array('checker' => array('chkEmpty',t ( 'Укажите название' ) 
		),'description' => t ( 'Краткое название сайта' ) 
		) ),'full_title' => new Type\String ( array('description' => t ( 'Полное название сайта' ),'hint' => t ( 'Будет использовано в подписи почтовых уведомлений. Например: "Интернет-магазин модной женской одежды"' ) 
		) ),'domains' => new Type\Text ( array('description' => t ( 'Доменные имена (через запятую)' ),'hint' => t ( 'Первый домен в списке - является основным. Именно он используется для построения абсолютных ссылок' ) 
		) ),

		'folder' => new Type\String ( array('description' => t ( 'Папка сайта' ),'hint' => t ( 'Например: en или version/english' ),'attr' => array(array('size' => 30 
		) 
		) 
		) ),'language' => new Type\String ( array('description' => t ( 'Язык' ),'hint' => t ( '2 английские буквы. Например: en или ru' ),'attr' => array(array('size' => 2 
		) 
		) 
		) ),'default' => new Type\Integer ( array('description' => t ( 'По умолчанию' ),'hint' => t ( 'Этот сайт будет открыт, даже если домен не соответствует заявленным' ),'checkboxview' => array(1,0 
		) 
		) ),'update_robots_txt' => new Type\Integer ( array('description' => t ( 'Обновить robots.txt' ),'hint' => t ( 'При установке данного флага, файл robots.txt будет перезаписан, а в .htaccess будут внесены необходимые изменения (при использовании мультисайтовости)' ),'runtime' => true,'checkboxView' => array(1,0 
		) 
		) ),'redirect_to_main_domain' => new Type\Integer ( array('description' => t ( 'Перенаправлять на основной домен' ),'allowEmpty' => false,'checkboxView' => array(1,0 
		),'hint' => t ( 'Если включено, то при обращении к НЕ основному домену будт происходить 301 редирект на основной домен' ) 
		) ),'sortn' => new Type\Integer ( array('description' => t ( 'Сортировка' ),'visible' => false 
		) ),'theme' => new Type\String ( array('attr' => array(array('readonly' => 'readonly' 
		) 
		),'description' => t ( 'Тема' ),'runtime' => true,'template' => '%site%/form/site/theme.tpl' 
		) ) 
		) );
	}
	function beforeWrite($flag)
	{
		...
		if ($this['default'] == 1) {
			...
			\RS\Orm\Request::make ()->update ( $this )->set ( array('default' => 0 
			) )->exec ();
		}
	}
