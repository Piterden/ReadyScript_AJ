...
class Control extends \RS\Controller\Admin\Crud
{
	...
	function helperIndex()
	{
		...
		) ),new TableType\String ( 'title', t ( 'Название' ), array('Sortable' => SORTABLE_BOTH,'LinkAttr' => array('class' => 'crud-edit' 
		),'href' => $this->router->getAdminPattern ( 'edit', array(':id' => '@id' 
		) ) 
		) ),new TableType\String ( 'domains', t ( 'Доменные имена' ), array('Sortable' => SORTABLE_BOTH 
		) ),new TableType\String ( 'language', t ( 'Язык' ) ),new TableType\StrYesno ( 'default', t ( 'По умолчанию' ) ),new TableType\Actions ( 'id', array(new TableType\Action\Edit ( $this->router->getAdminPattern ( 'edit', array(':id' => '~field~' 
		) ) ),new TableType\Action\DropDown ( array(array('title' => 'Редактировать файл robots.txt','attr' => array('class' => 'crud-edit','@href' => $this->router->getAdminPattern ( 'editRobotsTxt', array(':site_id' => '@id' 
		) ) 
		) 
		) 
		)
	...
	{
		return new \RS\Controller\Admin\Helper\CrudCollection ( $this, $this->api, $this->url, array('bottomToolbar' => $this->buttons ( array('save','cancel' 
		) ),'viewAs' => 'form','formTitle' => t ( 'Редактировать robots.txt' ) 
		) );
	}
	function actionEditRobotsTxt()
	{
		...
		if ($this->url->isPost ()) {
			...
			return $this->result->setSuccess ( true )->setSuccessText ( t ( 'Изменения успешно сохранены' ) );
		}
		
		$this->view->assign ( array('elements' => $helper->active (),'robots_content' => $robots_content 
		) );
