...
class View extends \RS\Controller\Front
{
	...
	function actionIndex()
	{
		...
		if (! $article) return $this->e404 ( t ( 'Статья не найдена' ) );
		...
		if ($debug_group = $this->getDebugGroup ()) {
			$edit_href = $this->router->getAdminUrl ( 'edit', array('id' => $article['id'] 
			), 'article-ctrl' );
		...
		$this->app->breadcrumbs->addBreadCrumb ( $last_dir['title'], $this->router->getUrl ( 'article-front-previewlist', array('category' => $category 
		) + ($last_page > 1 ? array('p' => $last_page 
		) : array()) ) );
		
		$this->app->title->addSection ( $article['meta_title'] ?  : $article['title'] );
		
		$this->app->meta->addKeywords ( $article['meta_keywords'] )->addDescriptions ( $article['meta_description'] );
		
		$this->view->assign ( array('article' => $article 
		) );
