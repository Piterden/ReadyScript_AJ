...
{
	protected static $controller_title = 'Список свежих новостей', $controller_description = 'Отображает N последних новостей и ссылку на все новости';
	protected $default_params = array('indexTemplate' => 'blocks/lastnews/lastnews.tpl','pageSize' => 5 
	);
	public $article_api, $category_api;
	...
	{
		return parent::getParamObject ()->appendProperty ( array('category' => new Type\String ( array('description' => t ( 'Новости из какой рубрики отображать?' ),'list' => array(array('\Article\Model\CatApi','selectList' 
		),false 
		) 
		) ),'pageSize' => new Type\Integer ( array('description' => t ( 'Количество новостей на страницу' ) 
		) ) 
		) );
	}
		...
		if (! $this->view->cacheOn ( $page . $category )->isCached ( $this->getParam ( 'indexTemplate' ) )) {
			...
			if ($dir) {
				...
				if ($debug_group = $this->getDebugGroup ()) {
					$create_href = $this->router->getAdminUrl ( 'add', array('dir' => $dir['id'] 
					), 'article-ctrl' );
				...
				$this->view->assign ( array('news' => $news,'category_id' => $dir['id'],'category' => $dir 
				) );
			}
		}
