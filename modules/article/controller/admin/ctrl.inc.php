...
use \RS\Html\Table\Type as TableType, \RS\Html\Toolbar\Button as ToolbarButton, \RS\Html\Toolbar, \RS\Html\Tree, \RS\Html\Filter, \RS\Html\Table;

class Ctrl extends \RS\Controller\Admin\Crud
{
	protected $dir, $dirapi;
	...
	function helperIndex()
	{
		...
		// Параметры таблицы
		$collection->setTopToolbar ( new Toolbar\Element ( array('Items' => array(new ToolbarButton\Dropdown ( array(array('title' => t ( 'добавить статью' ),'attr' => array('href' => $this->router->getAdminUrl ( 'add', array('dir' => $this->dir 
		) ),'class' => 'button add crud-add' 
		) 
		),array('title' => t ( 'добавить категорию статей' ),'attr' => array('href' => $this->router->getAdminUrl ( 'add_dir' ),'class' => 'crud-add' 
		) 
		) 
		...
		$collection->setTable ( new Table\Element ( array('Columns' => array(new TableType\Checkbox ( 'id', array('showSelectAll' => true 
		) ),new TableType\String ( 'title', 'Название', array('href' => $this->router->getAdminPattern ( 'edit', array(':id' => '@id' 
		...
		) ),new TableType\String ( 'id', '№', array('ThAttr' => array('width' => '50' 
		),'TdAttr' => array('class' => 'cell-sgray' 
		),'Sortable' => SORTABLE_BOTH,'CurrentSort' => SORTABLE_DESC 
		...
		) ),new TableType\Action\DropDown ( array(array('title' => t ( 'клонировать' ),'attr' => array('class' => 'crud-add','@href' => $this->router->getAdminPattern ( 'clone', array(':id' => '~field~' 
		) ) 
		) 
		),array('title' => t ( 'показать статью на сайте' ),'attr' => array('target' => '_blank','@href' => function ($row) {
			return $row->getUrl ();
		} 
		) 
		) 
		...
		// Параметры фильтра
		$collection->setFilter ( new Filter\Control ( array('Container' => new Filter\Container ( array('Lines' => array(new Filter\Line ( array('items' => array(new Filter\Type\String ( 'id', '№', array('Attr' => array('size' => 4 
		) 
		...
		),'SecContainers' => array(new Filter\Seccontainer ( array('Lines' => array(new Filter\Line ( array('items' => array(new Filter\Type\String ( 'alias', 'Псевдоним', array('SearchType' => 'like%' 
		) ) 
		) 
		) ) 
		) 
		...
		$collection->setTree ( new Tree\Element ( array('sortIdField' => 'id','activeField' => 'id','activeValue' => $this->dir,'pathToFirst' => $path_to_first,'sortable' => true,'mainColumn' => new TableType\String ( 'title', 'Название', array('href' => $this->router->getAdminPattern ( false, array(':dir' => '@id' 
		) ) 
		...
		),array('title' => t ( 'клонировать' ),'attr' => array('class' => 'crud-add','@href' => $this->router->getAdminPattern ( 'clonedir', array(':id' => '~field~',':pid' => '@parent' 
		) ) 
		) 
		),array('title' => t ( 'показать на сайте' ),'attr' => array('target' => '_blank','@href' => function ($row) {
			return $row->getUrl ();
		} 
		) 
		...
		) ),'inlineButtons' => array('add' => array('attr' => array('title' => t ( 'Создать дочернюю категорию' ),'href' => $this->router->getAdminUrl ( 'add_dir', array('pid' => $this->dir 
		) ),'class' => 'add crud-add' 
		) 
		),'edit' => array('attr' => array('title' => t ( 'редактировать' ),'href' => $this->router->getAdminUrl ( 'edit_dir', array('id' => $this->dir 
		) ),'class' => 'edit crud-edit' 
		) 
		),'delete' => array('attr' => array('title' => t ( 'удалить' ),'href' => $this->router->getAdminUrl ( 'del_dir', array('chk[]' => $this->dir 
		) ),'class' => 'delete crud-remove-one' 
		) 
		) 
		),'headButtons' => array(array('attr' => array('title' => 'Создать категорию','href' => $this->router->getAdminUrl ( 'add_dir' ),'class' => 'add crud-add' 
		) 
		) 
		) 
		) ), $this->dirapi );
		
		$collection->setTreeBottomToolbar ( new Toolbar\Element ( array('Items' => array(new ToolbarButton\DropUp ( array(array('title' => t ( 'редактировать' ),'attr' => array('data-url' => $this->router->getAdminUrl ( 'multiedit_dir' ),'class' => 'crud-multiedit' 
		) 
		) 
