...
IMAGES_TYPE = 'article', TAG_TYPE = 'article';
	protected $attach_products = null, // Прикреплённые товары
$fast_mark_attached_products_use = null; // Флаг используются ли прикреплённые товары
	protected static $table = "article";
	protected function _init()
	{
		parent::_init ()->append ( array(t ( 'Основные' ),'site_id' => new Type\CurrentSite (),'title' => new Type\String ( array('maxLength' => '150','description' => 'Название','Checker' => array('chkEmpty','Необходимо заполнить поле название' 
		),'attr' => array(array('data-autotranslit' => 'alias' 
		) 
		) 
		) ),'alias' => new Type\String ( array('maxLength' => '150','index' => true,'description' => 'Псевдоним(Ан.яз)','meVisible' => false 
		) ),'content' => new Type\Richtext ( array('description' => 'Содержимое' 
		) ),'parent' => new Type\Integer ( array('index' => true,'description' => 'Рубрика','List' => array(array(new \Article\Model\Catapi (),'selectList' 
		) 
		),'Attr' => array(array('size' => 0 
		) 
		) 
		) ),'dateof' => new Type\Datetime ( array('maxLength' => '19','description' => 'Дата и время','index' => true 
		) ),'image' => new Type\Image ( array('maxLength' => '255','max_file_size' => 10000000,'allow_file_types' => array('image/pjpeg','image/jpeg','image/png','image/gif' 
		),'description' => 'Картинка' 
		) ),'user_id' => new Type\User ( array('description' => t ( 'Автор' ) 
		) ),'rating' => new Type\Decimal ( array('maxLength' => '3','decimal' => '1','default' => 0,'visible' => false,'description' => t ( 'Средний балл(рейтинг)' ),'hint' => t ( 'Расчитывается автоматически, исходя из поставленных оценок, если установлен блок комментариев на странице товара.' ) 
		) ),'comments' => new Type\Integer ( array('maxLength' => '11','description' => t ( 'Кол-во комментариев к статье' ),'default' => 0,'visible' => false 
		) ),'attached_products' => new Type\String ( array('maxLength' => 4000,'description' => t ( 'Прикреплённые товары' ),'visible' => false 
		) ),'attached_products_arr' => new Type\ArrayList ( array('visible' => false 
		) ),t ( 'Расширенные' ),'short_content' => new Type\Richtext ( array('description' => 'Краткий текст' 
		) ),

		t ( 'Мета тэги' ),'meta_title' => new Type\String ( array('maxLength' => 1000,'description' => 'Заголовок' 
		) ),'meta_keywords' => new Type\String ( array('maxLength' => 1000,'description' => 'Ключевые слова' 
		) ),'meta_description' => new Type\String ( array('maxLength' => 1000,'viewAsTextarea' => true,'description' => 'Описание' 
		) ),t ( 'Фото' ),'_photo_' => new Type\UserTemplate ( '%article%/form/article/photos.tpl' ),'_tmpid' => new Type\Mixed () 
		)
		 );
		
		if (\RS\Module\Manager::staticModuleExists ( 'catalog' )) {
			$this->getPropertyIterator ()->append ( array(t ( 'Прикреплённые товары' ),'_attached_products_' => new Type\UserTemplate ( '%article%/form/products/attachedproducts.tpl' ) 
			) );
		}
	...
	function getDebugActions()
	{
		...
		), 'article-ctrl' ) ),new \RS\Debug\Action\Delete ( \RS\Router\Manager::obj ()->getAdminPattern ( 'del', array(':chk[]' => '{id}' 
		), 'article-ctrl' ) ) 
		);
	}
	...
	function afterWrite($flag)
	{
		...
		if ($this['_tmpid'] < 0) {
			...
			$tags = \RS\Orm\Request::make ()->from ( new \Tags\Model\Orm\Link () )->where ( array('type' => self::TAG_TYPE,'link_id' => $this['_tmpid'] 
			) )->exec ()->fetchSelected ( 'word_id', 'word_id' );
			
			// Удалим предварительные денные по тегам и слову
			if ($tags) {
				\RS\Orm\Request::make ()->from ( new \Tags\Model\Orm\Link () )->where ( array('type' => self::TAG_TYPE,'link_id' => $this['id'] 
				) )->whereIn ( 'word_id', $tags )->delete ()->exec ();
			}
			
			\RS\Orm\Request::make ()->update ( new \Tags\Model\Orm\Link () )->set ( array('link_id' => $this['id'] 
			) )->where ( array('type' => self::TAG_TYPE,'link_id' => $this['_tmpid'] 
			) )->exec ();
	...
	function getUrl($absolute = false)
	{
		...
		return \RS\Router\Manager::obj ()->getUrl ( 'article-front-view', array('category' => $dir_alias,'id' => $id 
		), $absolute );
	}
		...
		if ($this->attach_products === null) {
			...
			if (isset ( $this['attached_products_arr']['product'] )) {
				foreach($this['attached_products_arr']['product'] as $id) {
					...
					if (isset ( $product['id'] )) { // Если товар существует
	...
	{
		// Получим id слов, которые прикреплены
		$tags = \RS\Orm\Request::make ()->from ( new \Tags\Model\Orm\Link () )->where ( array('type' => self::TAG_TYPE,'link_id' => $this['id'] 
		) )->objects ();
