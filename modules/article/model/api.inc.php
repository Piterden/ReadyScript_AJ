...
class Api extends \RS\Module\AbstractModel\EntityList
{
	...
	{
		parent::__construct ( new \Article\Model\Orm\Article (), array('aliasField' => 'alias','nameField' => 'title','multisite' => true,'defaultOrder' => 'dateof DESC' 
		) );
	}
	
	/**
	 * Возвращает ленту статей в категории.
	 * Используется для новостей
	 */
	public function getLineByCat($cat, $limit, $order = '')
	{
		$q = new \RS\Orm\Request ();
		$q->select ( "*" )->from ( $this->obj_instance )->where ( array('parent' => $cat 
		) )->limit ( 0, $limit );
