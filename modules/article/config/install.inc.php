...
class Install extends \RS\Module\AbstractInstall
{
	...
	{
		return $this->importCsvFiles ( array(array('\Article\Model\CsvSchema\Article','articles' 
		) 
		), 'utf-8', $params );
	}
	...
	function deferredAfterInstall($options)
	{
		if ($options['set_demo_data']) {
			...
			if ($site_config->getThemeName () == 'default') {
				...
				$news_category = $cat_api->setFilter ( 'title', 'Новости' )->getFirst ();
				
				// Настраиваем блок новости в дефолтном шаблоне
				if ($news_category) {
					\Templates\Model\PageApi::setupModule ( null, 'article\controller\block\lastnews', array('category' => $news_category['id'],'pageSize' => 5 
					) );
				}
			}
		}
