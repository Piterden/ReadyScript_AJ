...
class Api
{
	protected $site_id, $folder = '/storage/sitemap', $filename = 'sitemap-{site_id}.xml', $full_filename;
	...
	function sitemapToOutput()
	{
		...
		if (file_exists ( $this->full_filename )) {
			$app->headers->addHeader ( 'Content-Type', 'text/xml' )->sendHeaders ();
	...
	function generateSitemap()
	{
		...
		foreach(explode ( "\n", $additional_urls ) as $url) {
			if ($url = trim ( $url )) {
				$pagelist[] = array('loc' => $url 
				);
			}
		}
		
		$default_page = array('priority' => $config['priority'] 
		);
		...
		foreach($pagelist as $page_data) {
			$page = array_intersect_key ( $page_data, array_flip ( array('loc','lastmod','changefreq','priority' 
			) ) ) + $default_page;
