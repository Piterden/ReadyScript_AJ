...
{
	const CHARSET = 'utf-8';
	private static $offerTypes = null;
	private $cacheSelectedProductIds = null;
	function _init()
	{
		return parent::_init ()->append ( array('products' => new Type\ArrayList ( array('description' => t ( 'Список товаров' ),'template' => '%export%/form/profile/products.tpl' 
		) ),t ( 'Поля данных' ),'offer_type' => new Type\String ( array('description' => t ( 'Тип описания' ),'List' => array(array('\Export\Model\ExportType\Yandex\Yandex','getOfferTypeNames' 
		) 
		) 
		) ),'fieldmap' => new Type\Mixed ( array('description' => t ( '' ),'visible' => true,'template' => '%export%/form/profile/fieldmap.tpl' 
		) ) 
		) );
	}
	...
	/**
	 * Возвращает описание типа экспорта для администратора.
	 * Возможен HTML
	 * 
	 * @return string
	 */
	...
	/**
	 * Возвращает идентификатор данного типа экспорта.
	 * (только англ. буквы)
	 * 
	 * @return string
	 */
	...
	private function exportOffers(\Export\Model\Orm\ExportProfile $profile, \XMLWriter $writer)
	{
		...
		$query = \RS\Orm\Request::make ()->from ( new \Catalog\Model\Orm\Product () )->where ( array('public' => 1 
		) );
	...
	private function getSelectedProductIds(\Export\Model\Orm\ExportProfile $profile)
	{
		...
		if (! empty ( $group_ids )) {
			...
			while ( true ) {
				$subgroups_ids = \RS\Orm\Request::make ()->select ( 'id' )->from ( new \Catalog\Model\Orm\Dir () )->whereIn ( 'parent', $group_ids )->exec ()->fetchSelected ( null, 'id' );
			...
			$ids = \RS\Orm\Request::make ()->select ( 'product_id' )->from ( new \Catalog\Model\Orm\Xdir () )->whereIn ( 'dir_id', $group_ids )->exec ()->fetchSelected ( null, 'product_id' );
	...
	private function getSelectedProductGroupIds(\Export\Model\Orm\ExportProfile $profile)
	{
		...
		$groups_ids = \RS\Orm\Request::make ()->select ( 'maindir' )->from ( new \Catalog\Model\Orm\Product () )->where ( array('public' => 1 
		) )->whereIn ( 'id', $selected_product_ids )->exec ()->fetchSelected ( null, 'maindir' );
	...
	private function getSelectedProductGroups(\Export\Model\Orm\ExportProfile $profile)
	{
		...
		return \RS\Orm\Request::make ()->from ( new \Catalog\Model\Orm\Dir () )->whereIn ( 'id', $selected_product_group_ids )->objects ();
	}
	...
	{
		return array('simple' => t ( 'Упрощенное' ),'vendor.model' => t ( 'Произвольный товар (vendor.model)' ),'book' => t ( 'Книги (book)' ),'audiobook' => t ( 'Аудиокниги (audiobook)' ),'artist.title' => t ( 'Музыкальная и видео продукция (artist.title)' ) 
		);
	}
	...
	static public function getOfferTypes()
	{
		if (self::$offerTypes == null) {
			self::$offerTypes = array('simple' => OfferType\Simple::getEspecialTags (),'vendor.model' => OfferType\VendorModel::getEspecialTags (),'book' => OfferType\Book::getEspecialTags (),'audiobook' => OfferType\AudioBook::getEspecialTags (),'artist.title' => OfferType\ArtistTitle::getEspecialTags () 
			);
		}
