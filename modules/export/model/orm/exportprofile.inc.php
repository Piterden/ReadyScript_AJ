...
class ExportProfile extends \RS\Orm\OrmObject
{
	...
	function _init()
	{
		...
		) ),'__gate_url__' => new Type\Mixed ( array('visible' => true,'description' => t ( 'URL для экспорта' ),'template' => '%export%/form/profile/url.tpl' 
		) ),'class' => new Type\String ( array('maxLength' => '255','description' => t ( 'Класс экспорта' ),'list' => array(array('\Export\Model\Api','getTypesAssoc' 
		) 
		),'visible' => false 
		) ),'life_time' => new Type\Integer ( array('description' => t ( 'Период экспорта' ),'listfromarray' => array(array(0 => t ( 'При каждом обращении' ),1 => t ( '1 день' ),5 => t ( '5 дней' ),10 => t ( '10 дней' ),20 => t ( '20 дней' ) 
		) 
		) 
		)
	...
	function initClass()
	{
		...
		$type = new $class_name ();
		$this->getPropertyIterator ()->appendPropertyIterator ( $type->getPropertyIterator ()->arrayWrap ( 'data' )->setPropertyOptions ( array('runtime' => true 
		) ) );
	...
	public function getExportUrl()
	{
		...
		return $router->getUrl ( 'export-front-gate', array('site_id' => \RS\Site\Manager::getSiteId (),'export_id' => $this->id,'export_type' => $this->class 
		), true );
	}
