...
class Api extends \RS\Module\AbstractModel\EntityList
{
	...
	{
		parent::__construct ( new \Export\Model\Orm\ExportProfile (), array('multisite' => true 
		) );
	...
	function printExportedData(Orm\ExportProfile $profile)
	{
		...
		if ($profile->life_time > 0) {
			...
			if (time () < filemtime ( $profile->getCacheFilePath () ) + $life_time_in_sec) {
				readfile ( $profile->getCacheFilePath () );
				return;
			}
		}
