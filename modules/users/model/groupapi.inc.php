...
class GroupApi extends \RS\Module\AbstractModel\EntityList
{
	...
	{
		parent::__construct ( new \Users\Model\Orm\UserGroup (), array('nameField' => 'name' 
		) );
	}
	...
	/**
	 * Подготавливает данные по правам к модулю.
	 * Конвертирует совокупность отмеченных битов в число.
	 */
	...
	function prepareModuleAccessData($module_access)
	{
		...
		foreach($modlist as $modclass => $modconfig) {
			$linedata = array('class' => $modclass,'name' => $modconfig['name'],'description' => $modconfig['description'],'access' => isset ( $module_access[$modclass] ) ? $module_access[$modclass] : 0,'accessbit' => $modconfig->access_bit, // Подписи для битов
'bits' => array() 
			);
