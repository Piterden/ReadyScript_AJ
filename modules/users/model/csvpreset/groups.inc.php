...
class Groups extends \RS\Csv\Preset\AbstractPreset
{
	...
	protected $delimiter = ",", $value_delimiter = ",", $id_field = 'id', $array_field = 'groups', $link_id_field, $link_preset_id, $many_link_ormobject, $loaded_groups = array(), // Группы пользователей
$loaded_groups_by_name = array(), // Группы пользователей к ключом по name
$title;
	function __construct($options)
	{
		$defaults = array('ormObject' => new \Users\Model\Orm\UserGroup (),'manyLinkOrmObject' => new \Users\Model\Orm\UserInGroup () 
		);
	...
	function loadUserGroups()
	{
		if (empty ( $this->loaded_groups )) {
			$this->loaded_groups = \RS\Orm\Request::make ()->from ( $this->getOrmObject () )->objects ( null, 'alias' );
		}
	...
	function loadUserGroupsByName()
	{
		if (empty ( $this->loaded_groups_by_name )) {
			...
			$items = \RS\Orm\Request::make ()->from ( $this->getOrmObject () )->objects ( null, 'name' );
	...
	function loadData()
	{
		$this->row = array();
		if ($this->schema->ids) {
			$this->rows = \RS\Orm\Request::make ()->from ( $this->getManyLinkOrmObject () )->whereIn ( $this->link_id_field, $this->schema->ids )->objects ( null, $this->link_id_field, true );
		}
	}
	...
	function getColumns()
	{
		$colums_propety = array($this->id . '-usergroups' => array('key' => 'usergroups','title' => 'Группы пользователя' 
		) 
		);
