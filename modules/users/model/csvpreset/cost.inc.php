...
class Cost extends \RS\Csv\Preset\AbstractPreset
{
	...
	protected $id_field = 'id', $link_id_field, $link_preset_id, $array_field, // Поле для объекта пользователя
...
$title;
	...
	{
		$defaults = array('ormObject' => new \Catalog\Model\Orm\Typecost (),'userObject' => new \Users\Model\Orm\User () 
		);
	...
	{
		if (empty ( $this->costs )) {
			$this->costs = \RS\Orm\Request::make ()->from ( $this->getOrmObject () )->where ( array('site_id' => $this->current_site 
			) )->objects ( null, 'id' );
		}
		return $this->costs;
	}
	...
	function loadCostsByTitle()
	{
		if (empty ( $this->costs_by_title )) {
			$this->costs_by_title = \RS\Orm\Request::make ()->from ( $this->getOrmObject () )->where ( array('site_id' => $this->current_site 
			) )->objects ( null, 'title' );
		}
	...
	function loadData()
	{
		$this->row = array();
		if ($this->schema->ids) {
			$items = \RS\Orm\Request::make ()->from ( $this->getUserObject () )->whereIn ( $this->link_id_field, $this->schema->ids )->objects ( null, $this->link_id_field );
			$users = array();
			if (! empty ( $items )) {
				foreach($items as $id => $user) {
					...
					if (is_array ( $costs ) && ! empty ( $this->sites )) {
						foreach($this->sites as $site) {
							...
							$users[$id][$site['id']]['cost_title'] = 'По умолчанию';
	...
	function getColumns()
	{
		...
		if (! empty ( $this->sites )) {
			foreach($this->sites as $site) {
				$colums_propety[$this->id . '-usercosts' . $site['id']] = array('key' => 'usercosts' . $site['id'],'title' => 'Тип цены(' . $site['title'] . ')','site_id' => $site['id'] 
				);
			}
		}
