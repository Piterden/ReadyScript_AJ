...
class Api extends \RS\Module\AbstractModel\EntityList
{
	...
	public function getLike($term, array $fields)
	{
		...
		foreach($fields as $field) {
			$q->where ( "$field like '%#term%'", array('term' => $term 
			), 'OR' );
		}
	...
	{
		$user = \RS\Orm\Request::make ()->from ( new Orm\User () )->where ( array('login' => $login 
		) )->object ();
		
		if ($user) {
			$uniq = $user['hash'];
			if ($admin) {
				$recover_href = \RS\Router\Manager::obj ()->getUrl ( 'main.admin', array('Act' => 'changePassword','uniq' => $uniq 
				), true );
			} else {
				$recover_href = \RS\Router\Manager::obj ()->getUrl ( 'users-front-auth', array('Act' => 'changePassword','uniq' => $uniq 
				), true );
			}
			$host = \RS\http\Request::commonInstance ()->getDomainStr ();
			$tpl = new \RS\View\Engine ();
			$data = array('host' => $host,'user' => $user,'recover_href' => $recover_href 
			);
	...
	function beforeSqlWhereCallback($items, $filter_control)
	{
		...
		if ($dateofreg_from) {
			$this->setFilter ( array('dateofreg:>=' => $dateofreg_from 
			) );
		}
		...
		if ($dateofreg_to) {
			$this->setFilter ( array('dateofreg:<=' => $dateofreg_to . ' 23:59:59' 
			) );
		}
