...
{
	const PASSWORD_LEN = 4, SUPERVISOR_GROUP = 'supervisor';
...
$access_menu_table, $access_module_table, 

	$cache_cost_id, $cache_mod_access, $cache_menu_access, $cache_admin_menu_access, $cache_allow_sites;
	protected $groups;
	...
	protected function _init()
	{
		...
		$this->getPropertyIterator ()->append ( array(t ( 'Основные' ),'name' => new Type\String ( array('maxLength' => '100','description' => 'Имя','Checker' => array('chkEmpty','Укажите, пожалуйста, имя' 
		) 
		) ),'surname' => new Type\String ( array('maxLength' => '100','description' => 'Фамилия','Checker' => array('chkEmpty','Укажите, пожалуйста, фамилию' 
		) 
		) ),'midname' => new Type\String ( array('maxLength' => '100','description' => 'Отчество' 
		) ),'e_mail' => new Type\String ( array('maxLength' => '150','description' => 'E-mail','unique' => true,'Checker' => array('chkEmail','Неверно заполнен e-mail' 
		) 
		) ),'login' => new Type\String ( array('maxLength' => '64','unique' => true,'description' => 'Логин' 
		) ),'openpass' => new Type\String ( array('template' => '%users%/form/user/chpass.tpl','maxLength' => '100','description' => 'Новый пароль','runtime' => true,'Attr' => array(array('size' => '20','type' => 'password' 
		) 
		),'Checker' => array(array(__CLASS__,'checkOpenPassword' 
		),'' 
		) 
		) ),'pass' => new Type\String ( array('maxLength' => '32','description' => 'Пароль','Attr' => array(array('size' => '20','type' => 'password' 
		) 
		),'listenPost' => false,'visible' => false 
		) ),'phone' => new Type\String ( array('maxLength' => '50','description' => 'Телефон','Checker' => array('chkPattern','Неверно указан телефон','/^[0-9()\-\s+,]+$/' 
		) 
		) ),'sex' => new Type\String ( array('maxLength' => '1','description' => 'Пол','ListFromArray' => array(array('' => 'Не выбрано','M' => 'Мужской','F' => 'Женский' 
		) 
		),'Attr' => array(array() 
		) 
		) ),'hash' => new Type\String ( array('maxLength' => '64','index' => true,'description' => 'Ключ','visible' => false 
		) ),'subscribe_on' => new Type\Integer ( array('maxLength' => '1','description' => 'Получать рассылку','CheckBoxView' => array(1,0 
		) 
		) ),'dateofreg' => new Type\Datetime ( array('description' => t ( 'Дата регистрации' ) 
		) ),'balance' => new Type\Decimal ( array('allowEmpty' => false,'visible' => false,'maxLength' => '15','decimal' => 2,'description' => t ( 'Баланс' ) 
		) ),'balance_sign' => new Type\String ( array('visible' => false,'description' => t ( 'Подпись баланса' ) 
		) ),'ban_expire' => new Type\Datetime ( array('description' => t ( 'Заблокировать до ...' ),'template' => '%users%/form/user/ban_expire.tpl' 
		) ),'ban_reason' => new Type\String ( array('description' => t ( 'Причина блокировки' ),'visible' => false 
		) ),t ( 'Организация' ),'is_company' => new Type\Integer ( array('maxLength' => '1','description' => 'Это юридическое лицо?','ListFromArray' => array(array('0' => 'Нет','1' => 'Да' 
		) 
		) 
		) ),'company' => new Type\String ( array('maxLength' => '255','description' => 'Название организации','condition' => $for_company,'Checker' => array($chk_depend,'Не указано название организации','chkEmpty',$for_company 
		) 
		) ),'company_inn' => new Type\String ( array('maxLength' => '12','description' => 'ИНН организации','condition' => array('is_company' => 1 
		),'Checker' => array($chk_depend,'ИНН должен состоять из 10 или 12 цифр','chkPattern',$for_company,array('/^(\d{10}|\d{12})$/' 
		) 
		),'attr' => array(array('size' => 20 
		) 
		) 
		) ),t ( 'Группы' ),'__groups__' => new Type\UserTemplate ( '%users%/form/user/groups.tpl' ),'groups' => new Type\ArrayList ( array('description' => 'Группы' 
		) ),

		t ( 'Дополнительные сведения' ),'data' => new Type\ArrayList ( array('description' => '','template' => '%users%/form/user/userfields.tpl' 
		) ),'changepass' => new Type\Integer ( array('runtime' => true,'CheckBoxView' => array(1,0 
		),'visible' => false,'Attr' => array(array('id' => 'chpass' 
		) 
		) 
		) ),'_serialized' => new Type\Text ( array('visible' => false 
		) ),'captcha' => new Type\Captcha ( array('enable' => false 
		) ) 
		) );
	}
	...
	function beforeWrite($flag)
	{
		...
		$q = \RS\Orm\Request::make ()->from ( $this )->where ( array('e_mail' => $this['e_mail'] 
		) );
		...
		if ($q->count ()) {
			$this->addError ( 'такой E-mail уже занят', 'e_mail' );
			$ret = false;
		}
		...
		if (! isset ( $this->groups )) // Не запрашиваем дважды группы у юзера
{
			...
			if ($this['id'] > 0) {
				...
				$this->groups += \RS\Orm\Request::make ()->select ( 'G.*' )->from ( new \Users\Model\Orm\UserGroup () )->asAlias ( 'G' )->from ( new \Users\Model\Orm\UserInGroup () )->asAlias ( 'I' )->where ( "I.group = G.alias AND I.user='#user'", array('user' => $this['id'] 
	...
	function addGroup($groupid)
	{
		...
		$user_groups = $q->from ( new \Users\Model\Orm\UserInGroup () )->where ( array('user' => $this['id'] 
		) )->objects ();
	...
	{
		$q = new \RS\Orm\Request ();
		$q->delete ()->from ( new \Users\Model\Orm\UserInGroup () )->where ( array('user' => $this['id'] 
		) + ($group_id ? array('group' => $group_id 
		) : array()) )->exec ();
	}
	...
	function getModuleAccess()
	{
		...
		if (in_array ( self::SUPERVISOR_GROUP, $groups )) {
			...
			$q = \RS\Orm\Request::make ()->select ( 'module, BIT_OR(access) as access' )->from ( $this->access_module_table )->where ( empty ( $this['id'] ) ? null : "user_id={$this['id']}" )->groupby ( 'module' );
	...
	function getMenuAccess()
	{
		...
		if (in_array ( self::SUPERVISOR_GROUP, $groups )) {
			$menu_access = array(AccessMenu::FULL_USER_ACCESS 
			);
		} else {
			$q = \RS\Orm\Request::make ()->select ( 'menu_id' )->from ( $this->access_menu_table )->where ( array('menu_type' => 'user' 
			) )->openWGroup ()->where ( empty ( $this['id'] ) ? null : "user_id={$this['id']}" );
			
			if ($groups) {
				$q->whereIn ( 'group_alias', $groups, "OR" );
			}
			
			$q->closeWGroup ()->groupby ( 'menu_id' );
	...
	function getAdminMenuAccess()
	{
		...
		if (in_array ( self::SUPERVISOR_GROUP, $groups )) {
			$menu_access = array(AccessMenu::FULL_ADMIN_ACCESS 
			);
		} else {
			$q = \RS\Orm\Request::make ()->select ( 'menu_id' )->from ( $this->access_menu_table )->where ( array('menu_type' => 'admin' 
			) )->openWGroup ()->where ( empty ( $this['id'] ) ? null : "user_id={$this['id']}" );
			if ($groups) {
				$q->whereIn ( 'group_alias', $groups, "OR" );
			}
	 ...
	 * Вызовите $this->getLastError(), чтобы увидеть текст ошибки
	...
	{
		$q = new \RS\Orm\Request ();
		$res = $q->select ( '*' )->from ( $this )->where ( array('login' => $this['login'],'e_mail' => $this['e_mail'] 
		), null, 'OR' )->exec ();
		
		if ($res->rowCount ()) {
			$user_row = $res->fetchRow ();
			
			if (! empty ( $this['login'] ) && stristr ( $user_row['login'], $this['login'] ) !== false) $this->addError ( 'Пользователь с таким логином уже существует', 'login' );
			
			if (! empty ( $this['e_mail'] ) && stristr ( $user_row['e_mail'], $this['e_mail'] ) !== false) $this->addError ( 'Пользователь с таким E-mail`ом уже существует', 'e_mail' );
	...
	{
		return \RS\Config\Loader::byModule ( $this )->getUserFieldsManager ()->setErrorPrefix ( 'userfield_' )->setArrayWrapper ( 'data' )->setValues ( ( array ) $this['data'] );
	}
	...
	/**
	 * Обновляет секретный хэш пользователя.
	 * Хэш используется для восстановления пароля
	 * 
	...
	function updateHash($commit = true)
	{
		...
		if ($commit) {
			$q = new \RS\Orm\Request ();
			$q->update ( $this )->set ( array('hash' => $this['hash'] 
	...
	public function getAllowSites()
	{
		if (! isset ( $this->cache_allow_sites )) {
			...
			if ($this->isSupervisor ()) {
				$q = \RS\Orm\Request::make ()->from ( new \Site\Model\Orm\Site () );
			} else {
				$q = \RS\Orm\Request::make ()->select ( 'S.*' )->from ( new \Users\Model\Orm\AccessSite () )->asAlias ( 'A' )->join ( new \Site\Model\Orm\Site (), 'A.site_id = S.id', 'S' )->where ( empty ( $this['id'] ) ? null : "user_id={$this['id']}" );
				if ($groups) {
					$q->whereIn ( 'group_alias', $groups, "OR" );
				}
			}
