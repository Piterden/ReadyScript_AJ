...
use \RS\Csv\Preset, \Users\Model\CsvPreset as UsersPreset, \Users\Model\Orm;
...
class Users extends \RS\Csv\AbstractSchema
{
	function __construct()
	{
		...
		parent::__construct ( new Preset\Base ( array('ormObject' => new Orm\User (),'excludeFields' => array('id','pass','hash','balance','balance_sign','data','changepass','_serialized','captcha','user_cost','cost_id' 
...
'searchFields' => $config['csv_id_fields'] 
		) ), array(new UsersPreset\Groups ( array('title' => t ( 'Группы пользователя' ),'linkPresetId' => 0,'delimeter' => ',','arrayField' => 'groups','linkIdField' => 'user' 
		) ),new UsersPreset\Cost ( array('title' => t ( 'Тип цены' ),'linkPresetId' => 0,'arrayField' => 'user_cost','linkIdField' => 'id' 
		) ) 
		) );
	}
