...
use \RS\Html\Table\Type as TableType, \RS\Html\Toolbar\Button as ToolbarButton, \RS\Html\Toolbar, \RS\Html\Table;

class Ctrlgroup extends \RS\Controller\Admin\Crud
{
	protected $module_access = array(), $menu_access = array(), $site_access, 
	...
	function helperIndex()
	{
		...
		$helper->setTable ( new Table\Element ( array('Columns' => array(new TableType\Checkbox ( 'alias', array('ThAttr' => array('width' => 20 
		) 
		) ),new TableType\String ( 'name', 'Название', array('href' => $this->router->getAdminPattern ( 'edit', array(':id' => '@alias' 
		) ),'Sortable' => SORTABLE_BOTH 
		) ),new TableType\String ( 'description', 'Описание' ),new TableType\String ( 'alias', 'Псевдоним', array('ThAttr' => array('width' => '50' 
		),'Sortable' => SORTABLE_BOTH,'CurrentSort' => SORTABLE_ASC 
		) ),new TableType\StrYesno ( 'is_admin', 'Админ' ),new TableType\Actions ( 'alias', array(new TableType\Action\Edit ( $this->router->getAdminPattern ( 'edit', array(':id' => '~field~' 
		) ), null, array('disableAjax' => true 
		) ) 
		...
		),array('title' => t ( 'Экспорт групп пользователей в CSV' ),'attr' => array('href' => \RS\Router\Manager::obj ()->getAdminUrl ( 'exportCsv', array('schema' => 'users-usersgroup','referer' => $this->url->selfUri () 
		), 'main-csv' ),'class' => 'crud-add' 
		) 
		),array('title' => t ( 'Импорт групп пользователей из CSV' ),'attr' => array('href' => \RS\Router\Manager::obj ()->getAdminUrl ( 'importCsv', array('schema' => 'users-usersgroup','referer' => $this->url->selfUri () 
		), 'main-csv' ),'class' => 'crud-add' 
		) 
		) 
		) ) 
	...
	function actionAdd($primaryKeyValue = null, $returnOnSuccess = false, $helper = null)
	{
		...
		$table_structure = array('Columns' => array(new TableType\String ( 'class', 'Модуль', array('TdAttr' => array('align' => 'center' 
		) 
		) ),new TableType\String ( 'name', 'Название' ),new TableType\String ( 'description', 'Описание' ),new TableType\Usertpl ( 'access', 'Уровень доступа', '%users%/modright.tpl', array('TdAttr' => array('style' => 'white-space:nowrap' 
		),'bitcount' => \RS\Orm\ConfigObject::BIT_COUNT 
		) ) 
		) 
		);
		...
		$admin_tree = new \RS\Html\Tree\Element ( array('checked' => $this->menu_access,'uniq' => $admin_menu_api->uniq,'sortIdField' => 'alias','activeField' => 'alias','mainColumn' => new TableType\String ( 'title', 'Название', array() ),'checkboxName' => 'menu_admin_access[]' 
		) );
		...
		$this->view->assign ( array('admin_tree' => $admin_tree,'user_tree' => $user_tree,'elem' => $this->api->getElement (),'site_access' => $this->site_access,'menu_access' => $this->menu_access,'module_access' => $this->module_access,'elements' => $helper 
		) );
		$helper->viewAsForm ();
