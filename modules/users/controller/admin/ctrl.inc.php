...
use \RS\Html\Table\Type as TableType, \RS\Html\Toolbar\Button as ToolbarButton, \RS\Html\Toolbar, \RS\Html\Filter, \RS\Html\Table;
...
class Ctrl extends \RS\Controller\Admin\Crud
{
	...
	function helperIndex()
	{
		...
		) ),new TableType\Usertpl ( 'group', t ( 'Группа' ), '%users%/form/filter/group.tpl' ),new TableType\Actions ( 'id', array(new TableType\Action\Edit ( $this->router->getAdminPattern ( 'edit', array(':id' => '~field~' 
		) ) ),new TableType\Action\DropDown ( array(array('title' => t ( 'клонировать пользователя' ),'attr' => array('class' => 'crud-add','@href' => $this->router->getAdminPattern ( 'clone', array(':id' => '~field~' 
		) ) 
		) 
		) 
		) ) 
		...
		$group_list = array('' => 'Любой' 
		) + array('NULL' => 'Без группы' 
		) + array_diff_key ( $group_api->getSelectList (), array('guest','clients' 
		) );
		
		$helper->setFilter ( new Filter\Control ( array('Container' => new Filter\Container ( array('Lines' => array(new Filter\Line ( array('items' => array(new Filter\Type\String ( 'id', t ( '№' ), array('attr' => array('class' => 'w50' 
		) 
		) ),new Filter\Type\String ( 'login', t ( 'Логин' ) ),new Filter\Type\String ( 'e_mail', t ( 'E-mail' ), array('SearchType' => 'like%' 
		) ),new Filter\Type\String ( 'dispname', t ( 'Ник' ), array('SearchType' => 'like%' 
		) ),new Filter\Type\Select ( 'group', t ( 'Группа' ), $group_list ) 
		)
		 
		) ) 
		),'SecContainers' => array(new Filter\Seccontainer ( array('Lines' => array(new Filter\Line ( array('items' => array(new Filter\Type\String ( 'name', t ( 'Имя' ), array('SearchType' => 'like%' 
		...
		) ),new Filter\Type\Date ( 'dateofreg_from', t ( 'Дата регистрации, от' ) ),new Filter\Type\Date ( 'dateofreg_to', t ( 'Дата регистрации, до' ) ) 
		) 
		) ) 
		) 
		) ) 
		...
		$helper->setTopToolbar ( new Toolbar\Element ( array('Items' => array(new ToolbarButton\Add ( $this->router->getAdminUrl ( 'add' ), t ( 'добавить' ), array(array('attr' => array('class' => 'button add crud-add' 
		) 
		) 
		) ),new ToolbarButton\Dropdown ( array(array('title' => t ( 'Импорт/Экспорт' ),'attr' => array('class' => 'button','onclick' => "JavaScript:\$(this).parent().rsDropdownButton('toggle')" 
		) 
		),array('title' => t ( 'Экспорт пользователей в CSV' ),'attr' => array('href' => \RS\Router\Manager::obj ()->getAdminUrl ( 'exportCsv', array('schema' => 'users-users','referer' => $this->url->selfUri () 
		), 'main-csv' ),'class' => 'crud-add' 
		) 
		),array('title' => t ( 'Импорт пользователей из CSV' ),'attr' => array('href' => \RS\Router\Manager::obj ()->getAdminUrl ( 'importCsv', array('schema' => 'users-users','referer' => $this->url->selfUri () 
		), 'main-csv' ),'class' => 'crud-add' 
		) 
		) 
		) ) 
		...
		$helper->setBottomToolbar ( new Toolbar\Element ( array('Items' => array(new Toolbar\Button\Button ( null, t ( 'Сбросить пароли' ), array('attr' => array('class' => 'crud-post','data-url' => $this->router->getAdminUrl ( 'generatePassword' ),'data-confirm-text' => t ( 'Вы действительно хотите сгенерировать новые пароли для выбранных пользователей и отправить их пользователям на почту?' ) 
		) 
		) ),$this->buttons ( 'delete' ) 
		) 
		) ) );
