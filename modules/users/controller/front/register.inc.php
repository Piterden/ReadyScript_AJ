...
class Register extends \RS\Controller\Front
{
	function actionIndex()
	{
		...
		$user->usePostKeys ( array('is_company','company','company_inn','name','surname','midname','phone','e_mail','openpass','captcha','data' 
		) );
		
		$referer = \RS\Helper\Tools::cleanOpenRedirect ( urldecode ( $this->url->request ( 'referer', TYPE_STRING ) ) );
		
		$conf_userfields = $this->getModuleConfig ()->getUserFieldsManager ()->setErrorPrefix ( 'userfield_' )->setArrayWrapper ( 'data' );
		...
		$this->view->assign ( array('conf_userfields' => $conf_userfields,'user' => $user,'referer' => urlencode ( $referer ) 
		) );
