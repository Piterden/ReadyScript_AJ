...
class Profile extends \RS\Controller\AuthorizedFront
{
	function actionIndex()
	{
		...
		$user->usePostKeys ( array('is_company','company','company_inn','name','surname','midname','phone','e_mail','openpass','captcha','data','changepass' 
		) );
		...
		if ($this->isMyPost ()) {
			...
			if (($theme->getName () == 'default' && \RS\Helper\Tools::compareVersion ( '3.0.0.42', $version )) || ($theme->getName () != 'default' && \RS\Helper\Tools::compareVersion ( '1.0.0.16', $version ))) {
				$this->url->checkCsrf ();
			}
		...
		$this->view->assign ( array('conf_userfields' => $conf_userfields,'user' => $user 
		) );
