...
class Auth extends \RS\Controller\Front
{
	...
	{
		$this->app->breadcrumbs->addBreadCrumb ( 'Авторизация' );
		...
		$data = array('login' => $this->url->request ( 'login', TYPE_STRING ),'pass' => $this->url->request ( 'pass', TYPE_STRING ),'referer' => urlencode ( $referer ),'remember' => $this->url->request ( 'remember', TYPE_BOOLEAN ) 
		);
		
		if ($this->isMyPost ()) {
			$this->result->setSuccess ( AppAuth::login ( $data['login'], $data['pass'], $data['remember'] ) );
			if ($this->result->isSuccess ()) {
				return $this->result->setNoAjaxRedirect ( $referer )->addSection ( 'reloadPage', true );
			} else {
				$error = AppAuth::getError ();
			}
		}
		
		$this->view->assign ( array('status_message' => isset ( $_SESSION['auth_access_error'] ) ? $_SESSION['auth_access_error'] : '','error' => $error,'referrer' => $referer,'data' => $data 
		) );
	...
	{
		$this->app->breadcrumbs->addBreadCrumb ( t ( 'Авторизация' ), $this->router->getUrl ( 'users-front-auth' ) )->addBreadCrumb ( t ( 'Восстановление пароля' ) );
		
		$data = array('login' => $this->url->request ( 'login', TYPE_STRING ) 
		);
		$error = false;
		if ($this->isMyPost ()) {
		...
		$this->view->assign ( array('error' => $error,'data' => $data 
		) );
	...
	function actionChangePassword()
	{
		...
		$this->view->assign ( array('uniq' => $hash,'user' => $user,'error' => $error 
		) );
